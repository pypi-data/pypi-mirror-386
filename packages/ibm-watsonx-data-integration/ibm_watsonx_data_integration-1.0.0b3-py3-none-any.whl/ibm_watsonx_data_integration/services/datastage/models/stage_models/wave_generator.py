...
class wave_generator(BaseStage):
    ...
    combinability: WAVE_GENERATOR.Combinability | None = Field(
        WAVE_GENERATOR.Combinability.auto, alias="combinability"
    )
    ...
    def validate_parameters(self) -> tuple[set, set]:
        ...
        (
            include.add("input_count")
            if (((self.input_count)) and (not self.input_count))
            else exclude.add("input_count")
        )
        (
        ...
        (
            ...
            if (
                self.mode and ((hasattr(self.mode, "value") and self.mode.value == "count") or (self.mode == "count"))
            )
