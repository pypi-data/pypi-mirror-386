...
class CFDI:
    ...
    def _timbrar_detecno(self, registrar=True):
        ...
        if "El CFDI contiene un timbre previo" in respuesta_texto:
            respuesta_texto = unescape(self.recuperar_tfd_detecno(respuesta_texto))
    ...
    def recuperar_tfd_detecno(self, respuesta_texto):
        from xml.sax.saxutils import escape, unescape

        docid = respuesta_texto.split("DocId = ")[1].split(" UUID")[0]
        uuid = respuesta_texto.split("UUID = ")[1].split("<")[0]
        ...
        payload = '''<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:tem="http://tempuri.org/">
        <soap:Header xmlns:wsa="http://www.w3.org/2005/08/addressing">
            <wsa:Action>http://tempuri.org/IDetecnoPac/RecuperarTfd</wsa:Action>
            <wsa:To>{}</wsa:To>
        </soap:Header>
        <soap:Body>
            <tem:RecuperarTfd>
    ...
    </soap:Envelope>'''.format(url, usuario, password, docid, uuid)
        headers = {
            'Content-Type': "application/soap+xml; charset=utf-8",
            'SOAPAction': '"http://tempuri.org/IDetecnoPac/RecuperarTfd"',
            'Host': host,
            'Connection': "keep-alive",
            }
