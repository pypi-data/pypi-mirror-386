...
# Phase 2: 遅延ロード - ツールクラスはハンドラー関数内で動的インポート
# ツールクラスキャッシュ（初回アクセス時にロード）
_TOOL_CLASS_CACHE: dict[str, Any] = {}

def _get_tool_class(tool_name: str) -> Any:
    """遅延ロード: ツールクラス取得（キャッシュ付き）"""
    if tool_name not in _TOOL_CLASS_CACHE:
        module_map = {
        ...
        if tool_name in module_map:
            ...
            _TOOL_CLASS_CACHE[tool_name] = getattr(module, tool_name)
        else:
            raise ValueError(f"Unknown tool class: {tool_name}")
    return _TOOL_CLASS_CACHE[tool_name]

# Phase 2: 後方互換 - テストがmonkeypatchできるよう明示的にNoneで初期化
# ハンドラー関数はglobals()経由でアクセスし、遅延ロードする
ListQualityPresetsTool = None  # type: ignore[assignment]
GetQualitySchemaTool = None  # type: ignore[assignment]
CheckReadabilityTool = None  # type: ignore[assignment]
CheckGrammarTool = None  # type: ignore[assignment]
CheckStyleTool = None  # type: ignore[assignment]
CheckRhythmTool = None  # type: ignore[assignment]
ImproveQualityUntilTool = None  # type: ignore[assignment]
RunQualityChecksTool = None  # type: ignore[assignment]
ResultAnalysisTool = None  # type: ignore[assignment]
BackupTool = None  # type: ignore[assignment]
FixQualityIssuesTool = None  # type: ignore[assignment]
GetIssueContextTool = None  # type: ignore[assignment]
ExportQualityReportTool = None  # type: ignore[assignment]
PolishManuscriptTool = None  # type: ignore[assignment]
PolishManuscriptApplyTool = None  # type: ignore[assignment]
RestoreManuscriptFromArtifactTool = None  # type: ignore[assignment]
PolishTool = None  # type: ignore[assignment]

def _get_tool_class_with_fallback(tool_name: str) -> Any:
    """Get tool class with fallback to globals (for test monkeypatching)."""
    # テストがmonkeypatchしている場合はそちらを優先
    global_cls = globals().get(tool_name)
    if global_cls is not None:
        return global_cls
    # 通常の遅延ロード
    return _get_tool_class(tool_name)
...
def _resolve_path_service(project_root: str | None) -> Any | None:
    """Infrastructure-compliant path service resolution.

    Directly calls Infrastructure layer factory instead of going through Presentation.
    Returns None defensively if path service factory is unavailable.
    """
    try:
...
async def list_quality_presets(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('ListQualityPresetsTool')()
...
async def get_quality_schema(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    request = ToolRequest(episode_number=arguments.get("episode_number", 1), additional_params=arguments)
    tool = _get_tool_class_with_fallback('GetQualitySchemaTool')()
    response = tool.execute(request)
    return {
        "success": response.success,
        "score": response.score,
...
async def check_readability(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('CheckReadabilityTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def check_grammar(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('CheckGrammarTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def check_style(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('CheckStyleTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def run_quality_checks(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('RunQualityChecksTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def improve_quality_until(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('ImproveQualityUntilTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def fix_quality_issues(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('FixQualityIssuesTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def get_issue_context(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('GetIssueContextTool')()
    response = tool.execute(request)
    return {
        "success": response.success,
        "score": response.score,
...
async def export_quality_report(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('ExportQualityReportTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def analyze_test_results(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('ResultAnalysisTool')()
    response = tool.execute(request)
    return {
        "success": response.success,
        "score": response.score,
...
async def backup_management(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('BackupTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def polish_manuscript(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('PolishManuscriptTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def polish_manuscript_apply(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('PolishManuscriptApplyTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def restore_manuscript_from_artifact(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('RestoreManuscriptFromArtifactTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def polish(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('PolishTool')()
    response = tool.execute(request)
    result = {
        "success": response.success,
        "score": response.score,
...
async def fetch_artifact(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    ps = _resolve_path_service(arguments.get("project_root"))
...
async def write_file(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if project_root:
        ps = _resolve_path_service(project_root)
...
async def status(arguments: dict[str, Any]) -> dict[str, Any]:
    """Show project status including written episodes.
    
    Args:
        arguments: Raw MCP payload containing optional project_root
        
    Returns:
        dict[str, Any]: Status information with episodes list and metadata
    """
    project_root = arguments.get("project_root")
    
    try:
        # Try to use Path Service
        ps = _resolve_path_service(project_root)
        if ps:
            base_path = Path(getattr(ps, "project_root", Path.cwd()))
            try:
                manuscripts_dir = Path(ps.get_manuscript_dir(create=False))
            except TypeError:
                manuscripts_dir = Path(ps.get_manuscript_dir())
            if not manuscripts_dir.is_absolute():
                manuscripts_dir = base_path / manuscripts_dir
        elif project_root:
            base_path = Path(project_root).absolute()
            manuscripts_dir = base_path / DEFAULT_PATH_CONFIG.manuscripts
            if not manuscripts_dir.exists():
                manuscripts_dir = base_path / "manuscripts"
        else:
            base_path = Path.cwd()
            manuscripts_dir = base_path / DEFAULT_PATH_CONFIG.manuscripts
            if not manuscripts_dir.exists():
        ...
        if manuscripts_dir.exists():
            for txt_file in sorted(manuscripts_dir.glob("第*話*.txt")):
                try:
                    # Parse episode number from filename
                    import re
                    match = re.search(r'第(\d+)話', txt_file.name)
                    if match:
        ...
        return {
            ...
            "episodes": episodes,
            "message": f"プロジェクト状況: {len(episodes)}話が執筆済み"
        }
        
    except Exception as e:
        return {"success": False, "error": f"ステータス取得エラー: {e}"}
...
async def check_rhythm(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    tool = _get_tool_class_with_fallback('CheckRhythmTool')()
    response = tool.execute(request)
