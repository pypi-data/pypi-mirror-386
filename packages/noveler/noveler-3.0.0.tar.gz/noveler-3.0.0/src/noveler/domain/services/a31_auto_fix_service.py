...
SPEC-QUALITY-001に基づくA31チェックリスト項目の自動修正。
安全性レベルに応じた段階的修正を提供。
"""
                ...
                for variant in variants:
                    if variant in fixed_content and variant != canonical:
                        ...
                        if count > 0:
                            ...
                            changes.append(f"{category}統一: {variant} → {canonical} ({count}箇所)")
        else:
        ...
        for canonical, variants in basic_terminology_patterns:
            for variant in variants:
                if variant in fixed_content:
                    ...
                    if count > 0:
                        fixed_content = fixed_content.replace(variant, canonical)
                        changes.append(f"用語統一: {variant} → {canonical} ({count}箇所)")
