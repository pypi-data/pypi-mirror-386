...
class PerformanceMonitor:
    ...
    def print_summary(self) -> None:
        ...
        if not summary:
            self.console_service.print_("📊 パフォーマンスデータなし")
            return

        self.console_service.print_("\n" + "=" * 60)
        self.console_service.print_("📊 パフォーマンスサマリー")
        self.console_service.print_("=" * 60)

        self.console_service.print_(f"総呼び出し回数: {summary['total_calls']}")
        self.console_service.print_(f"総実行時間: {summary['total_duration']:.2f}秒")
        self.console_service.print_(f"総メモリ変化: {summary['total_memory_change']:.1f}MB")
        ...
        for func, stats in sorted_funcs[:10]:  # 上位10件
            ...
            self.console_service.print_(f"\n{func}:")
            self.console_service.print_(f"  呼び出し回数: {stats['count']}回")
            self.console_service.print_(f"  総実行時間: {stats['total_duration']:.2f}秒")
            self.console_service.print_(f"  平均実行時間: {avg_duration:.3f}秒")
            self.console_service.print_(f"  最大実行時間: {stats['max_duration']:.3f}秒")
            self.console_service.print_(f"  総メモリ変化: {stats['total_memory_change']:.2f}MB")
