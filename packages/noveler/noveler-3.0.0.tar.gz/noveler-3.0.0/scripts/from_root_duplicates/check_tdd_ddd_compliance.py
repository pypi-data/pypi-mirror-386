...
from noveler.presentation.shared.shared_utilities import console, get_unified_logger
...
def simple_performance_monitor(operation_name: str):
    """簡易パフォーマンス監視（monitor_performanceの代替）"""
    ...
    try:
        ...
        logger.info(f"完了: {operation_name} ({duration:.2f}秒)")
...
class DDDComplianceChecker:
    ...
    def __init__(
        ...
        # B20準拠：共有Consoleサービス使用（Console直接インスタンス化禁止）
    ...
    async def check_compliance(self, quick_mode: bool = False, verbose: bool = False) -> int:
        ...
        Returns:
            エラー数（0=成功、>0=違反検出）
        """
        with simple_performance_monitor("ddd_compliance_check"):
            try:
