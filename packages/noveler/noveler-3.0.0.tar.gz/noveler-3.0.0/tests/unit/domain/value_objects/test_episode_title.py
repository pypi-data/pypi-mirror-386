...
class TestEpisodeTitle:
    ...
    def test_forbidden_characters_complete(self) -> None:
        """全ての禁止文字でエラー（コロンと疑問符は除く）"""
        # コロン（:）は「第X話:タイトル」形式をサポートするため許可
        # 疑問符（?）は強調表現(!?, ?!)をサポートするため許可
        forbidden_chars = ["\\", "/", "*", '"', "<", ">", "|"]
        ...
        # コロンと疑問符は許可されている
        title_with_colon = EpisodeTitle("第1話:冒険の始まり")
        assert str(title_with_colon) == "第1話:冒険の始まり"

        title_with_question = EpisodeTitle("なぜ?どうして?")
        assert str(title_with_question) == "なぜ?どうして?"

    @pytest.mark.spec("SPEC-EPISODE-009")
    @pytest.mark.spec("SPEC-EPISODE-005")
    def test_emphasis_sequences_preserved(self) -> None:
        """強調表現(!?と?!)がタイトル値に保持されることを確認"""
        # !? パターン
        title1 = EpisodeTitle("第1話 特別な一日!?")
        assert title1.value == "第1話 特別な一日!?"  # 値は保持される
        assert title1.to_filename_safe() == "第1話_特別な一日"  # ファイル名では削除

        # ?! パターン
        title2 = EpisodeTitle("なんと?! 驚きの展開")
        assert title2.value == "なんと?! 驚きの展開"  # 値は保持される
        assert title2.to_filename_safe() == "なんと_驚きの展開"  # ファイル名では削除

        # 両方含む
        title3 = EpisodeTitle("まさか!? それとも?!")
        assert title3.value == "まさか!? それとも?!"  # 値は保持される
        assert title3.to_filename_safe() == "まさか_それとも"  # ファイル名では削除
    ...
    def test_filename_safe_conversion_complete(self) -> None:
        ...
        assert title4.value == "第1話 特別な一日!?。、"  # タイトル値は保持される
