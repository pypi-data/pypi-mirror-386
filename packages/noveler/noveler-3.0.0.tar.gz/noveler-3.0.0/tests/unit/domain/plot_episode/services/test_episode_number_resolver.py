...
class TestEpisodeNumberResolver:
    ...
    def mock_project_root(self, tmp_path: Path) -> Path:
        """テスト用プロジェクトルート"""
        # テスト用の最小ディレクトリ構造を作成
        management_dir = tmp_path / "50_管理資料"
    ...
    def mock_management_file(self, mock_project_root: Path, sample_management_data: dict[str, Any]) -> Path:
        """話数管理.yamlを作成"""
        management_dir = mock_project_root / "50_管理資料"
        management_dir.mkdir(parents=True, exist_ok=True)
        ...
        # 対応する原稿ファイルも作成
        manuscript_dir = mock_project_root / "40_原稿"
        manuscript_dir.mkdir(parents=True, exist_ok=True)
    ...
    def test_resolve_filepath_from_episode_number_success(
        ...
        _ = mock_management_file  # Fixture ensures 話数管理.yaml is present
        # Arrange
