...
class TestEpisodePreviewGenerationService:
    ...
    def test_generate_preview_includes_metadata(
        ...
        assert isinstance(metadata["quality"]["passed"], bool)
        assert metadata["quality"]["minimum_required"] == pytest.approx(default_config.get_minimum_quality_score())
        assert metadata["quality"]["score"] == pytest.approx(result.quality_score)
