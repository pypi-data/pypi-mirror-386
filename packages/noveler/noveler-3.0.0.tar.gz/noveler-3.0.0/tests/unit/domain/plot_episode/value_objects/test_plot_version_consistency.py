...
from noveler.application.use_cases.consistency_update_orchestrator import (
...
class TestConsistencyUpdateOrchestrator(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.mock_foreshadow_analyzer.analyze_foreshadowing_validity.return_value = ForeshadowImpact(
            potentially_invalidated=["foreshadow_001"],
            review_recommendations=["レビュー推奨"],
        )
    ...
    def test_execute_major_version_consistency_update(self) -> None:
        ...
        # Mock ForeshadowImpact
        from noveler.application.use_cases.consistency_update_orchestrator import ForeshadowImpact
        mock_impact = ForeshadowImpact(
            potentially_invalidated=["伏線ID1", "伏線ID2"],
            review_recommendations=["レビュー推奨1", "レビュー推奨2"]
        )
        self.mock_foreshadow_analyzer.analyze_foreshadowing_validity.return_value = mock_impact
