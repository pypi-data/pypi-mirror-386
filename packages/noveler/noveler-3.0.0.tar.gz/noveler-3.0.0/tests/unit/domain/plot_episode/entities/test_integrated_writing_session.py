...
class TestIntegratedWritingSession:
  ...
  word_count: '3500'"""
    ...
    def test_create_session_with_nonexistent_project_root(self, valid_episode_number: EpisodeNumber) -> None:
        ...
        Note: IntegratedWritingSession はプロジェクトルートの存在を検証するため、
        存在しないパスを渡して ValueError が発生することを確認する。
        Domain Entity レベルでのバリデーションテスト。
        """
        nonexistent_path = Path("/nonexistent/path")  # 負のテストケース
