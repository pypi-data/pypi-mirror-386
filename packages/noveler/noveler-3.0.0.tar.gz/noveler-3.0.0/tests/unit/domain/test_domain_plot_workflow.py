...
class TestPrerequisiteRule(unittest.TestCase):
    ...
    def test_parameterized_paths_can_be_expanded(self) -> None:
        ...
        rule = PrerequisiteRule(
            file_path="20_プロット/章別プロット/第{chapter:02d}章.yaml",
            required=True,
            description="章別プロット",
        )
...
class TestWorkflowStage(unittest.TestCase):
    ...
    def test_prerequisite_check_with_parameters_is_possible(self) -> None:
        ...
        prerequisites = [
            PrerequisiteRule("20_プロット/章別プロット/第{chapter:02d}章.yaml", True, "章別プロット"),
        ]
...
class TestPlotCreationService(unittest.TestCase):
    ...
    def test_customized_file_creation_using_template(self) -> None:
        ...
        # モック設定
        self.file_repo.exists.return_value = True
        self.file_repo.load_project_config.return_value = {"title": "テストプロジェクト"}
        self.template_repo.load_template.return_value = {"project": "{title}"}
        ...
        assert result.success
        saved_content = self.file_repo.save_file.call_args[0][1]
