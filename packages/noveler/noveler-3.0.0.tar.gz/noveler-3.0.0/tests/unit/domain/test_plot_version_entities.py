...
class TestPlotChangeSet:
    ...
    def test_get_affected_episode_numbers(self, changeset: PlotChangeSet) -> None:
        ...
        # affected_chapters=[2, 3] → エピソード011-020, 021-030
        expected_episodes = []
        # ch02: エピソード011-020
        for ep in range(11, 21):
            expected_episodes.append(f"{ep:03d}")
        # ch03: エピソード021-030
        for ep in range(21, 31):
            expected_episodes.append(f"{ep:03d}")

        assert set(affected_episodes) == set(expected_episodes)
    ...
    def test_get_affected_episode_numbers_multiple_chapters(self) -> None:
        ...
        # ch01: 001-010, ch05: 041-050, ch10: 091-100
        expected_count = 10 + 10 + 10  # 30エピソード
        assert len(affected_episodes) == expected_count
        assert all(len(ep) == 3 and ep.isdigit() for ep in affected_episodes)

        # 代表的なエピソードが含まれているかチェック
        assert "001" in affected_episodes  # ch01
        assert "005" in affected_episodes  # ch01
        assert "041" in affected_episodes  # ch05
        assert "050" in affected_episodes  # ch05
        assert "091" in affected_episodes  # ch10
        assert "100" in affected_episodes  # ch10
