...
class MockFileSystemRepository(FileSystemRepository):
    ...
    def list_files(self, directory: str, extensions: set[str]) -> list[str]:
        ...
        Note: 実際のFileSystemRepositoryインターフェースはstr -> list[str]
        """
        # パスを正規化して比較（Windows/Unix互換）
        dir_normalized = Path(directory).as_posix()

        if dir_normalized not in self.directories and directory not in self.directories:
            return []

        files = []
        for file_path in self.files:
            path_obj = Path(file_path)
            parent_normalized = path_obj.parent.as_posix()

            if parent_normalized == dir_normalized and path_obj.suffix.lower() in extensions:
...
class TestSettingsFileWatcherRefactored:
    ...
    def test_initialize_with_no_project_root(self) -> None:
        ...
        Note: MockFileSystemRepositoryを使用 - 実ファイルシステムへのアクセスなし。
