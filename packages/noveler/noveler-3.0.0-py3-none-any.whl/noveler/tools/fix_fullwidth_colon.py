...
Phase 2: 全角コロン : を半角コロン : に一括置換
RUF001-003エラーの体系的修正ツール
"""
from pathlib import Path

def fix_fullwidth_colon(file_path: Path) -> tuple[bool, int]:
    """
    ファイル内の全角コロンを半角コロンに修正
    ...
    try:
        ...
        if content != original_content:
            ...
            # 修正箇所数をカウント
            colon_count = original_content.count("：")

            return True, colon_count
...
def scan_and_fix_directory(directory: Path) -> dict[str, int]:
    ...
    for file_path in python_files:
        ...
        modified, fix_count = fix_fullwidth_colon(file_path)
...
def main():
    ...
    if stats["files_modified"] > 0:
        print("\n✅ Phase 2 完了: 全角コロン修正成功")
    else:
        print("\nℹ️  修正対象の全角コロンが見つかりませんでした")
