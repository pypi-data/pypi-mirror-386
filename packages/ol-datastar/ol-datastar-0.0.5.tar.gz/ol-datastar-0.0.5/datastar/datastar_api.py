...
class DatastarAPI:
    ...
    def __init__(self):
        ...
        if DatastarAPI.app_key is None:
            # 1) Try environment (platforms may auto-provide)
            DatastarAPI.app_key = os.getenv("OPTILOGIC_APPKEY")

        if not DatastarAPI.app_key:
            # 2) Fallback to local app.key file (desktop usage)
            try:
                key_path = os.path.join(os.getcwd(), "app.key")
                with open(key_path, "r", encoding="utf-8") as f:
                    # Read first non-empty line
                    for line in f:
                        candidate = line.strip()
                        if candidate:
                            DatastarAPI.app_key = candidate
                            break
        ...
        if not DatastarAPI.app_key:
            raise ValueError(
                "API key is required. Provide an app.key file next to your script, or run on the Optilogic platform where the key is provisioned. "
                "You can manage keys at: https://optilogic.app/#/user-account?tab=appkey"
            )
