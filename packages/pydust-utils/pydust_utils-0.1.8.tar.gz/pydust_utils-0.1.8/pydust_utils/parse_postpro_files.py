...
class SectionalData:
    ...
    Attributes:
        ...
        y_span: Spanwise span locations array (n_sections,)
        chord: Chord lengths array (n_sections,)
        sec: Sectional data array (n_time, n_sections)
    """

    time: NDArray[np.floating]
    y_cen: NDArray[np.floating]
    y_span: NDArray[np.floating]
    chord: NDArray[np.floating]
    sec: NDArray[np.floating]

    @classmethod
    def from_file(cls, filename: str) -> "SectionalData":
        ...
        Example:
            >>> data = SectionalData.from_file('sectional_loads.dat')
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Sectional loads file not found: {filename}"
            ) from e
        except Exception as e:
            raise ValueError(f"Error reading sectional loads file: {e}") from e
        ...
        if n_sec is None or n_time is None:
            raise ValueError(
                f"Could not find n_sec and n_time in file header: {filename}"
            )

        # Find data start (after y_cen, y_span, chord line)
        data_start_index = next(i for i, l in enumerate(lines) if "y_cen" in l) + 1
        data_lines = [
            line for line in lines[data_start_index:] if not line.startswith("#")
        ]

        # Parse first 3 lines: y_cen, y_span, chord
        y_cen = np.array(data_lines[0].split(), dtype=np.float64)
        y_span = np.array(data_lines[1].split(), dtype=np.float64)
        chord = np.array(data_lines[2].split(), dtype=np.float64)
        ...
        return cls(
            ...
            y_span=np.asarray(y_span, dtype=np.float64),
...
class ProbesData:
    ...
    @classmethod
    def from_file(cls, filename: str) -> "ProbesData":
        ...
        Example:
            >>> data = ProbesData.from_file('probes_velocity.dat')
        ...
        try:
            ...
            raise FileNotFoundError(f"Probes file not found: {filename}") from e
        except Exception as e:
            raise ValueError(f"Error reading probes file: {e}") from e
        ...
        if (
            n_probes is None
            or n_time is None
            or line_probe is None
            or line_time is None
        ):
            raise ValueError(
                f"Could not find n_probes and n_time in file header: {filename}"
            )
...
class ChordwiseData:
    ...
    Attributes:
        time: Time values array (n_time,)
        y_span: Spanwise center locations array (n_sections,)
        x_cen: Chordwise center locations array (n_chord,)
        z_chord: Vertical center locations array (n_chord,)
        data_chord: Chordwise data array (n_time, n_chord)
    """

    time: NDArray[np.floating]
    y_span: NDArray[np.floating]
    x_cen: NDArray[np.floating]
    z_cen: NDArray[np.floating]
    data_chord: NDArray[np.floating]

    @classmethod
    def from_file(cls, filename: str) -> "ChordwiseData":
        ...
        Example:
            >>> data = ChordwiseData.from_file('chordwise_cp.dat')
            >>> print(f"Sections: {len(data.y_span)}")
        ...
        try:
            ...
            raise FileNotFoundError(f"Chordwise data file not found: {filename}") from e
        except Exception as e:
            raise ValueError(f"Error reading chordwise data file: {e}") from e
        ...
        for line in lines:
            if line.startswith("# n_chord"):
                match = re.search(
                    r"# n_chord\s*:\s*(\d+)\s*;\s*n_time\s*:\s*(\d+)", line
                )
                if match:
        ...
        if n_chord is None or n_time is None:
            raise ValueError(
                f"Could not find n_chord and n_time in file header: {filename}"
            )

        # Find data start (after x_chord, z_chord line)
        data_start_index = (
            next(i for i, line in enumerate(lines) if "x_chord" in line) + 1
        )
        data_lines = [
            line for line in lines[data_start_index:] if not line.startswith("#")
        ]
        ...
        z_cen = np.array(data_lines[1].split(), dtype=np.float64)

        # Parse time series data: time + Cp values
        time = np.zeros(n_time)
        data_chord = np.zeros((n_time, n_chord))

        for i in range(n_time):
            time_line = data_lines[2 + i]
            values = np.array(time_line.split(), dtype=np.float64)
            time[i] = values[0]
            data_chord[i, :] = values[1 : n_chord + 1]

        # For single spanwise location, y_cen is extracted from header
        y_span = np.array([0.0])  # Placeholder, actual value in header
        for line in lines:
            if "spanwise_location" in line:
                match = re.search(r"spanwise_location:\s*([\d.]+)", line)
                if match:
                    y_span = np.array([float(match.group(1))])
                    break
        ...
        return cls(
            ...
            z_cen=np.asarray(z_cen, dtype=np.float64),
            data_chord=np.asarray(
                data_chord, dtype=np.float64
            ),  # Shape: (n_time, n_elems)
        )
...
class IntegralData:
    ...
    @classmethod
    def from_file(cls, filename: str) -> "IntegralData":
        ...
        Example:
            >>> data = IntegralData.from_file('integral_loads.dat')
        ...
        try:
            ...
            raise FileNotFoundError(f"Integral loads file not found: {filename}") from e
        except Exception as e:
            raise ValueError(f"Error reading integral loads file: {e}") from e
...
class HingeData:
    ...
    @classmethod
    def from_file(cls, filename: str) -> "HingeData":
        ...
        Returns:
            HingeData object containing time, forces, and moments arrays
        ...
        Example:
            >>> data = HingeData.from_file('hinge_loads.dat')
        ...
        try:
            ...
            raise FileNotFoundError(f"Hinge loads file not found: {filename}") from e
        except Exception as e:
            raise ValueError(f"Error reading hinge loads file: {e}") from e
