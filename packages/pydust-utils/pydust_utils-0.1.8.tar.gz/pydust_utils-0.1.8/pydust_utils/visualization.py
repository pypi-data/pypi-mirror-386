...
def plot_dustpre_pv(
    ...
    rendering_backend : str, optional
        ...
        by default 'trame'.
    show : bool, optional
        Whether to display the plot immediately. Set to False in testing environments
        to avoid rendering issues in headless systems, by default True.
    ...
    plotter.background_color = background  # type: ignore[assignment]
    ...
    if title:
        plotter.add_text(  # type: ignore[call-arg]
            title,
            font="times",
    ...
    # Show with specified backend if requested
    if show:
