...
def generate_airfoil_data(
    ...
    Returns:
        Airfoil object with computed aerodynamic coefficients.
    ...
    Examples:
        ...
        ...     n_mach=5
        ... )
        >>> airfoil.generate_c81_file()  # Creates DUST format with 3 Re tables
    """
    # Determine if single or multiple Reynolds numbers
    is_multi_re = isinstance(reynolds, (list, np.ndarray))

    if is_multi_re:
        # Convert to numpy array if list
        if isinstance(reynolds, list):
            reynolds_array: NDArray[np.float64] = np.array(reynolds, dtype=np.float64)
        else:
            # reynolds is already ndarray, ensure it's float64
            assert isinstance(reynolds, np.ndarray)
            reynolds_array = reynolds.astype(np.float64)

        # Validate multiple Re
        if len(reynolds_array) == 0:
            raise ValueError("Reynolds number array cannot be empty")
        if np.any(reynolds_array <= 0):
            raise ValueError(
                f"All Reynolds numbers must be positive, got {reynolds_array}"
            )
    else:
        # Validate single Re
    ...
    if is_multi_re:
        # Multiple Reynolds numbers: shape [n_re, n_mach, n_aoa]
        ...
        # Single Reynolds number: shape [n_mach, n_aoa]
        # Type narrowing: at this point reynolds must be float
        assert isinstance(reynolds, float)
        reynolds_single = reynolds

        aero_data_single: dict[str, list[NDArray[np.float64]]] = {
            "cl": [],
            "cd": [],
            "cm": [],
        }
        ...
        for m in mach:
            aero = af.get_aero_from_neuralfoil(
                alpha=alphas_nf, Re=reynolds_single, mach=m, model_size=model_size
            )
            aero_data_single["cl"].append(aero["CL"])
            aero_data_single["cd"].append(aero["CD"])
            aero_data_single["cm"].append(aero["CM"])

        # Convert to numpy arrays with shape [n_mach, n_aoa]
        aero_data_single_np = {k: np.array(v) for k, v in aero_data_single.items()}
        ...
        return Airfoil(
            ...
            mach=mach,
            re=reynolds_single,
            cl=aero_data_single_np["cl"],
            cd=aero_data_single_np["cd"],
            cm=aero_data_single_np["cm"],
        )
