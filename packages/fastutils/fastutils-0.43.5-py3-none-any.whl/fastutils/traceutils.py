...
def register_trace_modules(*module_names):
    for module_name in module_names:
        module = importutils.import_from_string(module_name)
        if module:
            filename = module.__file__
            if filename.endswith("__init__.py"):
                traced_folders.add(
                    pathlib.Path(os.path.abspath(os.path.dirname(filename)))
                )
            else:
...
def trace_calls(frame, event, arg):
    ...
    if frame.f_code:
        ...
        info["filename"] = None
        info["function"] = None
    msg = "{process}{trace_message_sep}{thread}{trace_message_sep}{id}{trace_message_sep}{pid}{trace_message_sep}{filename}{trace_message_sep}{function}{trace_message_sep}{ts}".format(
        trace_message_sep=trace_message_sep, **info
    )
