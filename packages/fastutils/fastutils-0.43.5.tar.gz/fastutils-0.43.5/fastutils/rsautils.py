...
def load_private_key(text, passphrase=None):
    ...
    if isinstance(text, sixutils.BASESTRING_TYPES):
        text = text.strip()
    try:
...
def load_public_key(text):
    ...
    if isinstance(text, sixutils.STR_TYPE):
        text = text.strip()
        if not text.startswith(sixutils.TEXT("-----BEGIN")):
            text = (
                sixutils.TEXT("-----BEGIN PUBLIC KEY-----\n")
                + text
                + sixutils.TEXT("\n-----END PUBLIC KEY-----\n")
            )
        text = text.strip()
    if isinstance(text, sixutils.BYTES_TYPE):
        text = text.strip()
        if not text.startswith(sixutils.BYTES("-----BEGIN")):
            text = (
                sixutils.BYTES("-----BEGIN PUBLIC KEY-----\n")
                + text
                + sixutils.BYTES("\n-----END PUBLIC KEY-----\n")
            )
        text = text.strip()
