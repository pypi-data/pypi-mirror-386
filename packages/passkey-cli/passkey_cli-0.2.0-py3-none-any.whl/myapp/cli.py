"""
passkey - Minimal Secure CLI Demo
...
app = typer.Typer(help="passkey — minimal secure CLI demo (test only)")
...
def _get_mongodb_connection():
    ...
    atlas_user = "dalilahmer1212_db_user"
    atlas_pass = "zqtrGsEV5ouT9tFf"
    atlas_cluster = "keypass.6cpus8e"          # MongoDB Atlas cluster name
...
try:
    ...
    DB_NAME = os.getenv("passkey_DB_NAME", "passkey_db")  # Default database name
...
def _ensure_db_connection():
    ...
    if DB is None:
        try:
            _client = _get_mongodb_connection()
            DB_NAME = os.getenv("passkey_DB_NAME", "passkey_db")
            DB = _client[DB_NAME]
...
KEYRING_SERVICE = "passkey"      # Service name for keyring storage
...
def _get_fernet():
    ...
    if not key:
        typer.echo("No master key found. Run: passkey init")
        raise typer.Exit(code=1)
