...
class UniversalEvaluationRequest(BaseModel):
    ...
    submodules_to_run: Optional[List[Literal["ti_question_qa", "answer_verification", "external_edubench", "reading_question_qc", "math_content_evaluator", "text_content_evaluator"]]] = None
    ...
    def _determine_submodules(self) -> List[str]:
        ...
        This routing logic is internal - users only need to specify subject, grade, and type.
        """
        modules = []
        ...
        if has_questions:
            # Core question evaluators - always run for questions
            ...
            # Reading QC - specialized for ELA MCQ questions with passages
            # Run if: ELA subject OR MCQ type OR if questions have passages
            if (self.subject == "ela" or
                self.type == "mcq" or
                any(hasattr(q, 'passage') and q.passage for q in self.generated_questions)):
                modules.append("reading_question_qc")

            # Math content evaluator - run for math questions
        ...
        if has_content or self.type in ["text-content", "passage"]:
            # Text content evaluator - for pedagogical text assessment
            modules.append("text_content_evaluator")

            # Math content evaluator - for math-related content rigor
            if self.subject == "math":
                modules.append("math_content_evaluator")

        # If no subject specified and we have both questions and content, include both evaluators
        if not self.subject and has_questions and has_content:
            modules.append("math_content_evaluator")
            modules.append("reading_question_qc")
