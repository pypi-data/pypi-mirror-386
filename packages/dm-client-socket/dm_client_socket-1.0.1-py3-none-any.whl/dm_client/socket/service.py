...
class SocketClient(object):
    ...
    def _api(self, service, id, method, url_path, *args, **kwargs):
        response, request = Requests(service, id, self.private_key)(method, '{}{}'.format(self.url, url_path), *args,
                                                                    **kwargs)
    ...
    def event_internal_notify(self, application, account_id, message) -> (Response, Request):
        data = {
            'application': application,
            'resource': account_id,
            'message': message
        }
        return self._api(self.target_service, 'event.internal.notify', json=data, request_auth=RequestAuthEnum.INTERNAL)