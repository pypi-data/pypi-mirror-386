...
class LuloManager:
    ...
    @staticmethod
    async def lulo_lend(agent: SolanaAgentKit, mint_address: str, amount: str) -> str:
        """
        ...
        Args:
            ...
            amount (str): Amount to lend.

        Returns:
            str: Transaction signature.
        """
        print(f"Lulo API URL1: {LuloManager.BASE_URL} {agent.flexland_api_key} {agent.wallet_address}")
        logger.info(f"Lulo API URL1: {LuloManager.BASE_URL} {agent.flexland_api_key} {agent.wallet_address}")
        try:
            ...
            payload = json.dumps({
                ...
                "regularAmount": amount_num,
                "protectedAmount": amount_num,
            })
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, data=payload) as response:
                    if response.status != 200:
                        ...
                        print(f"Lulo API Error Response: {error_text}")
                        raise Exception(f"Lulo API Error: {response.status} - {error_text}")
                    data = await response.json()
