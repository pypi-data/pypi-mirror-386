...
def test_edit_result_table_0(t_res):
    expected = [
        "id",
        "compound",
        "sample_type",
    ...
    is_ = ut.edit_result_table(t_res)
    assert all([is_.col_names[i] == expected[i] for i in range(len(expected))])


def test_edit_result_table_1(t_res):
    hided_columns = ["a", "b", "c"]
    t = ut.edit_result_table(t_res, hided_columns=hided_columns)
    name2format = dict(zip(t.col_names, t.col_formats))
    assert all([name2format[key] is None for key in hided_columns])


def test_edit_result_table_2(t_res):
    with pytest.raises(AssertionError):
        is_ = ut.edit_result_table(t_res, compound_col="")
