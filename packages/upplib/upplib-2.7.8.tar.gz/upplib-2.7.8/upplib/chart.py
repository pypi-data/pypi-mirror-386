...
def to_chart_segment(data_list: list[list[list[str | Any]]],
    ...
    for y_i in range(len(data_list)):
        ...
        for y_index in range(len(d_list)):
            ...
            d_o_data.append([start_index, y_i + 1, {'name': d_one[0], 'differ': end_index - start_index, 'from': d_one[1], 'to': d_one[2]}])
            d_o_data.append([end_index, y_i + 1, {'name': d_one[0], 'differ': end_index - start_index, 'from': d_one[1], 'to': d_one[2]}])
            series.append({
                'data': d_o_data,
