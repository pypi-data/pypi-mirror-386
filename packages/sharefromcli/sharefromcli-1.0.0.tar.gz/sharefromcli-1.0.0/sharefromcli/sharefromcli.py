"""
ShareFromCLI – Simple CLI for the remote File‑Sharing service.

Checkout the application to generate token
https://sharefromcli.xyz
...
The server endpoints are:
  POST   https://api.sharefromcli.xyz/api/createDoc   – create a new document
  GET    https://api.sharefromcli.xyz/api/getDoc/<id>  – retrieve a document

A valid authentication token must be available in the environment variable
`SHAREFROMCLI_TOKEN`.  The token is added as the `X-User-ID` header and
...
def get_token():
    ...
    Returns:
        str: The token value stored in the `SHAREFROMCLI_TOKEN` environment variable.
    ...
    """
    token = os.getenv("SHAREFROMCLI_TOKEN")
    if not token:
        print()
        print("\033[91m❌ Error: Environment variable SHAREFROMCLI_TOKEN not set.\033[0m")
        print("👉 Please set it using:")
        print("   export SHAREFROMCLI_TOKEN=your_token_here  (Linux/macOS)")
        print("   set SHAREFROMCLI_TOKEN=your_token_here    (Windows)")
        sys.exit(1)

    thread = spinner("🔑 Verifying token...")
    data = requests.get(
        f"https://api.sharefromcli.xyz/api/verifyToken/{token}",
    ).json()
    stop_spinner(thread)

    if(data['status']== False):
        print()
        print("\033[91m❌ Error: Environment variable SHAREFROMCLI_TOKEN is not valid.\033[0m")
        sys.exit(1)
...
def main():
    ...
    try:
        ...
        if action == "read":
            ...
            data = requests.get(
                f"https://api.sharefromcli.xyz/api/getDoc/{arg}",
            ...
            if data['status'] == "error":
                ...
                if choose == "download":
                    ...
                    with open(filename, "w") as f:
                        f.write(data['sharefromcliRes'])
                    print(f"{Fore.GREEN}File saved as {filename}{Style.RESET_ALL}")
                else:
                    print(data['sharefromcliRes'])
        else:
            # Create a new document on the server
            thread = spinner("📂 Creating file...")
            data = requests.post(
                "https://api.sharefromcli.xyz/api/createDoc",
                headers={
                    "X-User-ID": token,
            ...
            if data['valid']:
                ...
                print(f"> pip install sharefromcli\n> sharefromcli read {data['sharefromcliRes']}")
            else:
                
                thread = spinner("📂 Creating file error...")
                print()
                print(f"\033[91m❌ Error: {data['sharefromcliRes']}\033[0m")
                stop_spinner(thread)
