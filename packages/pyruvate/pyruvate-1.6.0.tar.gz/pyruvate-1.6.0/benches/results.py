...
class Parser(object):
    ...
    def process(self, directory):
        for fileName in sorted(os.listdir(directory)):
            with open(os.path.join(directory, fileName), 'r') as f:
                ...
                if parts['type'] == 'log':
                    self._logHandler(
                        directory, parts['server'], parts['connections'], f)
    ...
    def _logHandler(self, directory, server, connections, content):
        for line in content:
            ...
            if parts[0] == 'Latency':
                ...
                self.RESULTS[directory].LATENCIES[server][connections] = float(
                    digits) * self.LATENCY_MULTIPLIERS[unit]
            elif 'Requests' in parts[0]:
                self.RESULTS[directory].REQUESTS[server][connections] = float(
                    parts[1])
            elif 'Socket' == parts[0]:
    ...
    def _statsHandler(self, directory, server, connections, content):
        for line in content:
            parts = [part for part in line.split(' ') if part]
            if 'ID' not in parts[0]:
                cpu = float(parts[2].rstrip('%'))
                memory = float(parts[3].rstrip('MB'))
                if cpu > self.RESULTS[directory].CPU[server][connections]:
                    self.RESULTS[directory].CPU[server][connections] = cpu
                if memory > self.RESULTS[directory].MEMORY[
                        server][connections]:
                    self.RESULTS[directory].MEMORY[
                            server][connections] = memory
...
class Output(object):
    ...
    def analyze(self, results):
        ...
        self._servers = sorted(
            next(iter(results.values())).REQUESTS.keys()
        )
        self._counts = sorted(
            next(iter(next(iter(results.values())).REQUESTS.values())).keys()
        )
