...
class RsCMPX_LteMeas:
	"""1048 total commands, 6 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMX/CMP/CMW/PVT, SupportedIdnPatterns = CMX/CMP/CMW/PVT, SimulationIdnString = 'Rohde&Schwarz,CMP180,100001,6.0.61.0018'"
	...
	def __init__(self, resource_name: str, id_query: bool= True, reset: bool=False, options: str=None, direct_session: object=None):
		...
		self._core.driver_version = '6.0.61.0018'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '6.0.61.0018'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_LteMeas version failed. Current version: '6.0.61.0018', minimum required version: '{min_version}'")
