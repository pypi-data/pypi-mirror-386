import json, uuid, sys, os, datetime, mimetypes, types, io, cgi, threading, xmldict, jwt, tempfile
...
class Request(threading.local):
    ...
    def getRequestText(self):
        ...
        body = io.BytesIO() if maxread < MEMFILE_MAX else tempfile.TemporaryFile(mode='w+b')
        while maxread > 0:
    ...
    def bind(self, environ, url_map):
        ...
        if self.path_resource in url_map and self.path_operation in url_map[self.path_resource] \
                and url_map[self.path_resource][self.path_operation]["request_process"] == False:
            pass
        else:
    ...
    if path_resource in url_map and path_operation in url_map[path_resource]:
        if path_operation in url_map[path_resource]:
            ...
            if type(result) == dict:
                ...
                star_response('200 OK', [('Content-Type', 'text/event-stream'), ('Cache-Control', 'no-cache')])
                return result
            else:
                star_response('200 OK', [('Content-Type', 'application/json;charset=urf-8')])
                return [result, ]
...
def route(resource, operation, method=['POST'], request_process=True):
    def wrapper(handler):
