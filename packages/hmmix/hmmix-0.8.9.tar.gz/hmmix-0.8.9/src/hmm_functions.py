...
def Write_Decoded_output(outputprefix, segments, obs_file, admixpop_file, extrainfo):
    ...
    for ploidity, output in outfile_mapper.items():
        ...
        if admixpop_file is not None:
            if extrainfo:
                out.write('chrom\tstart\tend\tlength\tstate\tmean_prob\tsnps\tadmixpopvariants\t{}\tcalled_sequence\tmutationrate\tvariants\tDAV_variants\n'.format('\t'.join(admixpop_names)))
            else:
        ...
        if admixpop_file is not None:
            ...
            for snp_position in variants.split(','):
                ...
                if carriers != '':
                    ...
                    archaic_variants_by_position.append('none')
