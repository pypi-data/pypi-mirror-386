...
class TaskViewSet(GenericViewSet):
    ...
    def configs_delete(self, request, pk):
        ...
        if request.method == 'POST':
            task.delete()
            return redirect('plugins:allure_megauploader:task-list')
...
class UploaderViewSet(GenericViewSet):
    ...
    def parse_report_post(self, request):
        ...
        return redirect(reverse('plugins:allure_megauploader:task-status', kwargs={'task_id': task.task_id}))
    ...
    def upload_report(self, request):
        ...
        if request.POST.get('submit'):
            ...
            return redirect(
                reverse(
                    'plugins:allure_megauploader:task-status',
...
class ConfigViewSet(GenericViewSet):
    ...
    def configs_add_post(self, request):
        ...
        if not form.is_valid():
            return render(request, 'v2_uploaderconfig_form.html', context=context)
        form.save()
        return redirect('plugins:allure_megauploader:config-list')
...
def redirect_index(request):
    return redirect(reverse('plugins:allure_megauploader:config-list'))
