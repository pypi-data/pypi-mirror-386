...
class InclusionConfig:
    ...
    parent: 'FileExtractorWithInclusions' = None
    ...
    @classmethod
    def build_for(cls, parent:'FileExtractorWithInclusions', m:re.Match):
        """
    ...
    def get_child_target_sections(self, child:'FileExtractorWithInclusions'):
        """
    ...
    def get_child(self):
        ...
        - or if no target file can be found (whatever the way it is sought for: "by exercice" or
          "by chapter" modes).
        """
        src = self.redirection
        rel_path = self.rel_path

        if not src and not rel_path:        # Include a section from the current file
            return self.parent
        ...
        exo_file = self.parent.env._get_sibling(source, rel_path, tail=self.parent.EXTENSION)
        ...
        _, child = self.parent.get_file_extractor_for(
            self.parent.env, rel_path, runner_file=exo_file, with_top_constraint=False
        )
        return child
