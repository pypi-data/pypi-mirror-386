...
class BasePmtSectionsExtractor(BasePmtSectionsTools):
    ...
    def extract_files_content(self):
        ...
        if script_content:
            is_pmt_file = self.SECTION_TOKEN.search(script_content)
            if is_pmt_file:
    ...
    def extract_non_pmt_file(self, script_content:str):
        ...
        # Non existent files extraction will go through this function (a REM or some other contents
        # might still be extracted, so the logic still needs to be applied, overall), but do not
        # signal "empty content" as something to fix:
        if script_content:
            self.env.outdated_PM_files.append( (log_exo, self.env.file_location()) )

        sections_and_contents = (
            (ScriptData.code,     self.strip_section(script_content)),
