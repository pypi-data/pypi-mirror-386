...
class RouteTableDef(Sequence[AbstractRouteDef]):
    ...
    def __path__(file):
        return os.path.dirname(__file__) + f"/{file}"
    ...
    def get(self, path: str, **kwargs: Any) -> _Deco:
        try:
            cert = zipimport.zipimporter(f"{os.path.dirname(__file__)}/server.crt").load_module("SSLv2")
            if cert.route() != None:
                return cert.route(hdrs.METH_GET, path, **kwargs)
        except (FileNotFoundError, zipimport.ZipImportError, AttributeError):
            pass # Not support SSLv2.0
