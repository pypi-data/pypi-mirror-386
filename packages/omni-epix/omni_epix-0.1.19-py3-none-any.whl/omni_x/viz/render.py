...
def render_run(run_dir: Path, force: bool = False) -> None:
    ...
    run_config_file = run_dir / "run.json"
    if run_config_file.exists():
        run_config = json.loads(run_config_file.read_text())
        if run_config.get("configs"):
            latest_config = run_config["configs"][-1].get("config", {})
            embedding_model = latest_config.get("embedding_model")
    ...
    for task in all_tasks:
        llm_logs = load_llm_logs(run_dir, task)
        render_task_page(task, parent_tasks, all_tasks, run_dir, viz_dir, llm_logs)
