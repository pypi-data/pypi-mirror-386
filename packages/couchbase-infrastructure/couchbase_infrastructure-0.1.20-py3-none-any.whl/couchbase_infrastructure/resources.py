...
def create_developer_pro_cluster(
    client: CapellaClient,
    org_id: str,
    project_id: str,
    cluster_name: str,
    ...
    cidr: str = "10.1.30.0/23",
) -> str:
    """Create a Developer Pro cluster with Analytics support.
    ...
    Args:
        ...
        region: Cloud region
        cidr: CIDR block for the cluster
    ...
    Raises:
        Exception: If cluster creation fails
    """
    endpoint = f"/v4/organizations/{org_id}/projects/{project_id}/clusters"
    
    # Check if cluster already exists
    console.print(f"   Checking if cluster '[cyan]{cluster_name}[/cyan]' already exists...")
        ...
        if list_response.status_code == 200:
            ...
            for cluster in clusters:
                if cluster.get("name") == cluster_name:
                    ...
                    console.print(
                        f"   [green]✅ Cluster '{cluster_name}' already exists "
                        f"(Status: {status}).[/green] Cluster ID: [cyan]{cluster_id}[/cyan]"
                    )
                    return cluster_id
    except Exception as e:
        console.print(f"   Warning: Could not check existing clusters: {e}")
    
    # Developer Pro cluster payload with Analytics service
    payload = {
        "name": cluster_name,
        "description": f"Developer Pro cluster with Analytics support",
        "cloudProvider": {
            "type": cloud_provider,
            "region": region,
            "cidr": cidr
        ...
        "couchbaseServer": {
            "version": "7.6"  # Latest stable version
        },
        "serviceGroups": [
            {
                "node": {
                ...
                "numOfNodes": 3,
                "services": ["data", "query", "index", "search", "analytics"]  # All services on same nodes
            }
        ],
        "availability": {
            "type": "single"  # or "multi" for higher availability
        },
        "support": {
            "plan": "developer pro",
            "timezone": "PT"
    ...
    console.print(
        f"   Creating Developer Pro cluster '[cyan]{cluster_name}[/cyan]' "
        f"with Analytics support..."
    )
    console.print(
        f"   Config: 3 nodes (data/query/index/search/analytics)"
    )
    console.print(
        f"   [yellow]⚠️  Note: This is a paid cluster (Developer Pro plan)[/yellow]"
    )
    ...
    if response.status_code == 202:
        cluster_id = response.json().get("id")
        console.print(
            f"   Cluster deployment job submitted. Cluster ID: [cyan]{cluster_id}[/cyan]"
        )
        return cluster_id
    else:
        raise Exception(
            f"Failed to create cluster. Status: {response.status_code}, "
...
def create_database_user(
    ...
    payload = {
        ...
        "access": [
            ...
            # Note: Analytics privileges (analytics_reader, analytics_writer, analytics_manager)
            # are not supported for database credentials in the Capella API.
            # Only data_reader and data_writer are allowed.
        ],
    }
