...
def test_task_list_search_flow(monkeypatch):
    ...
    async def _scenario():
        ...
        assert await state.get_state() is None
        # 在 MarkdownV2 模式下 * 会被转义
        assert message.edits and ("*任务搜索结果*" in message.edits[-1]["text"] or "\\*任务搜索结果\\*" in message.edits[-1]["text"])
...
def test_task_list_search_cancel_restores_list(monkeypatch):
    ...
    async def _scenario():
        ...
        assert await state.get_state() is None
        # 在 MarkdownV2 模式下 * 会被转义
        assert message.edits and ("*任务列表*" in message.edits[-1]["text"] or "\\*任务列表\\*" in message.edits[-1]["text"])
