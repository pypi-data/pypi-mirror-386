...
def test_projects_overview_hides_create_button(repo: ProjectRepository, tmp_path: Path):
    ...
    model_buttons = [label for label in labels if label.startswith("🧠 模型")]
    assert model_buttons, "项目列表应提供单独的模型切换按钮"
    for label in run_buttons + stop_buttons + model_buttons:
