...
class Spheroid(Particle):
    ...
    def compute_t_matrix(self, vacuum_wavelength, n_medium):
        ...
        # This method is done automatically, so no other changes in the linear system
        # are necessary to efficiently use memory and save computation time. 
        return _compute_spheroid_t_matrix_nfmds(spheroid_refractive_index = self.refractive_index,
                                                        spheroid_semi_axis_c = self.semi_axis_c,
                                                        spheroid_semi_axis_a = self.semi_axis_a,
                                                        spheroid_l_max = self.l_max, 
                                                        spheroid_m_max = self.m_max, 
                                                        spheroid_n_rank = self.nrank,
                                                        vacuum_wavelength = vacuum_wavelength, 
                                                        medium_refractive_index = n_medium)

@memo.Memoize
def _compute_spheroid_t_matrix_nfmds(spheroid_refractive_index,
                            spheroid_semi_axis_c,
                            spheroid_semi_axis_a,
                            spheroid_l_max, 
                            spheroid_m_max, 
                            spheroid_n_rank,
                            vacuum_wavelength, 
                            medium_refractive_index):
    nrank = spheroid_n_rank if spheroid_n_rank is not None else spheroid_l_max + 5
    Nmax = nrank * (2 + nrank)
    surf = [spheroid_semi_axis_c, spheroid_semi_axis_a]
    tnfmds = nfmds.taxsym(surf, Nmax, typegeom=1, nparam=1, wavelength=vacuum_wavelength,
                          ind_refrel=spheroid_refractive_index / medium_refractive_index + 0j,
                          nrank=nrank, ind_refmed=medium_refractive_index, prnprogress=False)
    t = nfic.nfmds_to_smuthi_matrix(tnfmds, l_max=spheroid_l_max, m_max=spheroid_m_max)
    return t
...
class LayeredSpheroid(Particle):
    ...
    Args:
        ...
        polar_angle (float):        Polar angle of axis of revolution.
        azimuthal_angle (float):    Azimuthal angle of axis of revolution.
        layer_refractive_indices (complex array): Complex refractive index of particle layers (from outside to inside)
        layer_semi_axes_c (float array):    Spheroid half axes in direction of axis of revolution (z-axis of local coordinate system).
                                            Note: the semi axes should be given starting with the outer layer. 
                                            The full semi axis of the particle up to the i-th layer must be given (not the layer thickness).
        layer_semi_axes_a (float array):    Spheroid half axes in lateral direction (x- and y-axis of local coordinate system).
                                            Note: the semi axes should be given starting with the outer layer. 
                                            The full semi axis of the particle up to the i-th layer must be given (not the layer thickness).
        l_max (int):                Maximal multipole degree used for the spherical wave expansion of incoming and
                                    scattered field
    ...
    def compute_t_matrix(self, vacuum_wavelength, n_medium):
        ...
        # This method is done automatically, so no other changes in the linear system
        # are necessary to efficiently use memory and save computation time. 
        return _compute_layered_spheroid_t_matrix(spheroid_layer_refractive_indices = self.layer_refractive_indices,
                                                  spheroid_semi_axis_c = self.layer_semi_axes_c,
                                                  spheroid_semi_axis_a = self.layer_semi_axes_a,
                                                  spheroid_l_max = self.l_max, 
                                                  spheroid_m_max = self.m_max, 
                                                  spheroid_n_rank = self.n_rank,
                                                  vacuum_wavelength = vacuum_wavelength,
                                                  medium_refractive_index = n_medium)
    
@memo.Memoize
def _compute_layered_spheroid_t_matrix(spheroid_layer_refractive_indices,
                                       spheroid_semi_axis_c,
                                       spheroid_semi_axis_a,
                                       spheroid_l_max, 
                                       spheroid_m_max, 
                                       ...
                                       medium_refractive_index):
    nrank = spheroid_n_rank if spheroid_n_rank is not None else spheroid_l_max + 5
    spheroid_semi_axis_c = np.array(spheroid_semi_axis_c)
    spheroid_semi_axis_a = np.array(spheroid_semi_axis_a)
    surf = np.hstack((spheroid_semi_axis_c[:,np.newaxis], spheroid_semi_axis_a[:,np.newaxis]))
    k = 2 * np.pi / vacuum_wavelength * medium_refractive_index
    layer_indrel = [ri / medium_refractive_index for ri in spheroid_layer_refractive_indices]
    tnfmds = nfmds.tlay(k, layer_indrel, surf, nrank)
    t = nfic.nfmds_to_smuthi_matrix(tnfmds, l_max=spheroid_l_max, m_max=spheroid_m_max)
    return t
