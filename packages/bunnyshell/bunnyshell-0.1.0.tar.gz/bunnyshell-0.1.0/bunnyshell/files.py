...
class Files:
    ...
    def __init__(self, client: AgentHTTPClient, ws_client: Optional[Any] = None):
        """
        Initialize Files resource.
        
        Args:
            client: Shared agent HTTP client
            ws_client: Optional WebSocket client for file watching
        """
        self._client = client
    ...
    async def watch(
        ...
        *,
        ...
        async with await self._ws_client.connect("/files/watch", timeout=timeout) as ws:
            # Send watch request
