...
from lacbox.gtsdf.unix_time import from_unix
...
def _load_info(f):
    ...
    check_type(f)
    if 'name' not in info:
        info['name'] = os.path.splitext(os.path.basename(f.filename))[0].strip()
    if 'attribute_names' in f:
        info['attribute_names'] = [v.decode('latin1').strip() for v in f['attribute_names']]
    if 'attribute_units' in f:
        info['attribute_units'] = [v.decode('latin1').strip() for v in f['attribute_units']]
    if 'attribute_descriptions' in f:
        info['attribute_descriptions'] = [v.decode('latin1').strip() for v in f['attribute_descriptions']]
    try:
