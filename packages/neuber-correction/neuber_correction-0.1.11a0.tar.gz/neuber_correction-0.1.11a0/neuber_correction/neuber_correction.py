...
class NeuberCorrection:
    ...
    def plot_neuber_diagram(
        ...
        eps_u_for_n = self._calculate_eps_u_from_given_ramberg_osgood_n(n) # usef for checking 
        warning_message = ""
        ...
        if n_used == "calculated":
            n_calc_text = f"n_calculated (used): {n_calculated:.2f}"
            n_given_text = "n_given: -"
        else:
            n_calc_text = f"n_calculated: {n_calculated:.2f}"
            n_given_text = f"n_given (used): {self.material.hardening_exponent:.2f}"
