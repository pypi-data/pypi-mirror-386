...
class PrefectEventSubscriber:
    ...
    async def _reconnect(self) -> None:
        ...
        await self._websocket.send(json.dumps({"type": "auth", "token": token}))

        try:
            message: Dict[str, Any] = json.loads(await self._websocket.recv())
        ...
        await self._websocket.send(json.dumps(filter_message))
