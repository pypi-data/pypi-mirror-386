...
def detect_current_architecture() -> str:
    """Detect the current OS and architecture and return the appropriate architecture string."""
    ...
    if system == "Darwin":  # macOS
        ...
        elif machine_lower in ("x86_64", "amd64"):
            return "x86_64-unknown-linux-gnu"
        elif machine_lower in ("i386", "i686"):
            return "i686-unknown-linux-gnu"
        else:
            # Default to x86_64 Linux if unknown
            return "x86_64-unknown-linux-gnu"
    else:
        # Default to Linux x86_64 if unknown OS
        return "x86_64-unknown-linux-gnu"


def get_latest_python_version() -> str:
    """Fetch the latest stable (non-alpha/beta/rc) Python version from the latest GitHub release."""
    try:
        release_json = get_release("latest")
        
        # Parse all assets to find the highest stable Python version
        versions = set()
        for asset in release_json["assets"]:
            parsed = parse_filename(asset["name"])
            if parsed and "version" in parsed:
                ...
                # These contain 'a', 'b', or 'rc' in the version string
        ...
        if not versions:
            # Fallback to 3.13 if we can't detect
            return "3.13"
        ...
        # Fallback to 3.13 if anything goes wrong
        return "3.13"
...
def main():
    # Detect current platform and latest Python version
    default_architecture = detect_current_architecture()
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=f"""
Examples:
  getpybs                                           # Default: Python {default_python_version}, {default_architecture}, latest release, stripped
  getpybs --python-version 3.12                     # Python 3.12 instead
  getpybs --architecture x86_64-apple-darwin        # macOS Intel
  getpybs --build-config pgo+lto                    # Fastest build
  getpybs --content-type full                       # Full build with all artifacts
  getpybs --build-version 20250920 --architecture aarch64-apple-darwin --build-config pgo+lto
  getpybs --list-options                            # List all available options
        """,
    )
    ...
    parser.add_argument(
        "--build-version",
        default="latest",
        help="Release version: 'latest' or a release tag (e.g. 20250920) (default: latest)",
    )
    ...
    parser.add_argument(
        ...
        help=f"Python version (default: {default_python_version})"
    )
    ...
    parser.add_argument(
        ...
        help=f"Target architecture (default: {default_architecture})",
    )
    ...
    print(f"Fetching release information for {args.build_version}...")
    release_json = get_release(args.build_version)
