"""
Generate SSH and GPG keys for a profile with passkey protection.
"""
...
from ..core.passkey import PasskeyGenerator
...
def forge(ctx, profile_name: str):
    ...
    # Generate and display passkey first
    console.print("🔐 [bold yellow]Generating secure passkey for SSH and GPG key protection...[/bold yellow]")
    passkey = PasskeyGenerator.generate_passkey()
    
    # Display passkey securely and wait for confirmation
    if not PasskeyGenerator.display_passkey_securely(passkey, profile_name):
        console.print("❌ [red]Key generation cancelled. Please save the passkey first.[/red]")
        return
    ...
    with Progress(
        ...
        # Generate SSH key with passkey
        ssh_task = progress.add_task("Generating SSH key with passphrase...", total=None)
        try:
            private_key_path, public_key_path = KeyGenerator.generate_ssh_key(
                profile_name, profile.git_email, passkey
            )
        ...
        # Generate GPG key with passkey
        gpg_task = progress.add_task("Generating GPG key with passphrase...", total=None)
        try:
            fingerprint = KeyGenerator.generate_gpg_key(
                profile.git_name, profile.git_email, profile_name, passkey
            )
            if fingerprint:
        ...
        # Save profile updates with passkey information
        save_task = progress.add_task("Saving profile...", total=None)
        try:
            from datetime import datetime
            profile.updated_at = datetime.now().isoformat()
            profile.has_passkey = True
            profile.passkey_hint = PasskeyGenerator.get_passkey_hint(profile_name)
