...
class KeyGenerator:
    ...
    @staticmethod
    def generate_ssh_key(profile_name: str, email: str, passphrase: str, key_path: Optional[Path] = None) -> Tuple[Path, Path]:
        """Generate an Ed25519 SSH key pair with passphrase protection.
    ...
    @staticmethod
    def generate_gpg_key(name: str, email: str, profile_name: str, passphrase: str) -> Optional[str]:
        """Generate a GPG key using gpg command with passphrase protection.
        
        Args:
            name: Full name for the key
            email: Email for the key
            profile_name: Profile name for key comment
            passphrase: Passphrase to protect the GPG key
        ...
        # Generate batch file for GPG key generation with passphrase protection
        batch_content = f"""
Key-Type: RSA
Key-Length: 4096
Subkey-Type: RSA
