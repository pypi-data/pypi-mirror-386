...
from ..core.quantum_crypto import VaultLockedError
...
def verify(ctx, profile_name: str, verbose: bool):
    ...
    try:
        ...
        console.print("🔒 [red]Vault is locked![/red]")
        console.print("   Run [cyan]bapao vault unlock[/cyan] to unlock your quantum-safe vault.")
        return
    ...
    if all_passed:
        ...
        # Show passkey usage information if applicable
        if hasattr(profile, 'has_passkey') and profile.has_passkey:
            PasskeyGenerator.prompt_for_passkey_usage(profile_name)
    ...
    if verbose:
        ...
        # Show passkey information
        if hasattr(profile, 'has_passkey') and profile.has_passkey:
            console.print(f"   🔐 Passkey Protected: [green]Yes[/green]")
            console.print(f"   🔑 Passkey Hint: {profile.passkey_hint or 'No hint available'}")
        else:
            console.print(f"   🔐 Passkey Protected: [yellow]No (Legacy)[/yellow]")
