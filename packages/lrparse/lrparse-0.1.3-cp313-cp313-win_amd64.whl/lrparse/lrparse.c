                     ...
                     const char *right, size_t rlen)
{
    ...
    const char *pL = (llen == 0) ? s : memmem_custom(s, slen, left, llen);
    if (!pL) return out;

    const char *start = pL + llen;
    size_t remain = slen - (start - s);

    const char *pR = (rlen == 0) ? (s + slen) : memmem_custom(start, remain, right, rlen);
    if (!pR) return out;
                      ...
                      const char *right, size_t rlen)
{
    ...
    while (1) {
        const char *pL = (llen == 0) ? pos : memmem_custom(pos, remain, left, llen);
        if (!pL) break;

        const char *start = pL + llen;
        size_t remain2 = slen - (start - s);

        const char *pR = (rlen == 0) ? (s + slen) : memmem_custom(start, remain2, right, rlen);
        if (!pR) break;
