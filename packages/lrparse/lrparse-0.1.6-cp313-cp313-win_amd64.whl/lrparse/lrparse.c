                     ...
                     const char *right, size_t rlen)
{
    ...
    if (started && depth > 0)
    sl_add_slice(&out, s + start, slen - start);
                      ...
                      const char *right, size_t rlen)
{
    ...
    if (llen == 0 || rlen == 0) {
        lr(s, slen, left, llen, right, rlen);
        return out;
    }
    ...
    while (i + llen <= slen) {
        if (same && memcmp(s + i, left, llen) == 0) {
            if (depth == 0) start = i + llen, depth = 1;
            else sl_add_slice(&out, s + start, i - start), depth = 0;
            i += llen;
            continue;
        }
        ...
        if (!same && depth > 0 && memcmp(s + i, right, rlen) == 0) {
            depth--;
            if (depth == 0) sl_add_slice(&out, s + start, i - start);
            i += rlen;
            continue;
        }
