...
class StrapiClientAsync(StrapiClientBase):
    ...
    async def upsert_document(
        ...
        First calls get_documents to search for existing document by unique_field.
        If found, updates the document; otherwise creates a new one.
        If unique_value is None, always creates a new document.
        
        Args:
            plural_api_id: The API endpoint for the collection
            data: Document data to create or update
            unique_field: Field name to use for finding existing document (default: "slug")
            unique_value: Value to search for in unique_field. If None, always creates new document
            populate: Fields to populate in the response
            fields: Fields to include in the response
            
        Returns:
            DocumentResponse: The created or updated document
        """
        # If no unique_value provided, always create new document
        if unique_value is None:
            return await self.create_document(plural_api_id, data)
        
        # Extract unique_value from data if not explicitly provided
        if isinstance(data, dict) and unique_field in data:
            search_value = data[unique_field]
        else:
            search_value = unique_value
            
        # Search for existing document using get_documents
        try:
            filters = {unique_field: {"$eq": search_value}}
            existing_docs = await self.get_documents(
            ...
            if existing_docs.data and len(existing_docs.data) > 0:
                ...
                document_id = existing_doc["documentId"]
                return await self.update_document(plural_api_id, document_id, data)
            else:
                # Document doesn't exist, create new one
                return await self.create_document(plural_api_id, data=data)
                
        except Exception as search_error:
            # If search fails, try to create new document
            # This handles cases where the collection doesn't exist or search fails
            return await self.create_document(plural_api_id, data)
