...
def serialize_document_data(data: dict[str, Any] | BaseModel) -> dict[str, Any]:
    ...
    # Clean up the data recursively to remove None values and empty containers
    data_dict = _clean_data_recursive(data_dict)
...
def _clean_data_recursive(data: Any) -> Any:
    """Recursively clean data by removing None values and empty containers."""
    if isinstance(data, dict):
        cleaned = {}
        for key, value in data.items():
            cleaned_value = _clean_data_recursive(value)
            # Only include non-None values and non-empty containers
        ...
        cleaned = []
        for item in data:
            cleaned_item = _clean_data_recursive(item)
            # Only include non-None values and non-empty containers
