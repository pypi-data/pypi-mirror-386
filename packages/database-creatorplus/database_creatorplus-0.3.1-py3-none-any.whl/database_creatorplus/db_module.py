    ...
    class Table:
        ...
        def write(database: object, table: str, *args) -> None:
            ...
            def get_id_key(database: object, tableName: str):
                data = database.Table.get(database, tableName,f"MAX({database.Info.getColumns(database, table)[0]})")[0][0]
                return f"{int(data) + 1},"
        ...
        def update(database: object, table: str, request: str, *args):

            str_data = ""    
            columns = database.Info.getColumns(database, table)
            
            for i in range(len(args)):
                str_data += f"{columns[i+1]} = ?" 
                str_data += ", " if i != len(args) - 1 else ""

            database.cursor.execute(f"UPDATE {table} SET {str_data} WHERE {request}", args)
            database.db.commit()