...
def setup_deepface_environment():
    """
    Configure DeepFace to use local weights and avoid internet downloads.
    Call this BEFORE importing deepface.DeepFace.
    """
    # 1️⃣ Respect existing env var or use a fallback
    deepface_home = os.getenv("DEEPFACE_HOME")
    if not deepface_home:
        deepface_home = os.path.expanduser("~/.deepface")
        os.environ["DEEPFACE_HOME"] = deepface_home

    os.makedirs(os.path.join(deepface_home, "weights"), exist_ok=True)

    # 2️⃣ Try to disable DeepFace auto-downloads safely
    try:
        df_functions = importlib.import_module("deepface.commons.functions")
        df_functions.downloadModel = lambda *args, **kwargs: None
        print(f"[DeepFace] Downloads disabled. Using weights from {deepface_home}/weights")
    except ModuleNotFoundError:
        print("[DeepFace] Warning: deepface.commons.functions not found — skipping patch.")
    except Exception as e:
        print(f"[DeepFace] Warning: could not disable downloads ({e})")
