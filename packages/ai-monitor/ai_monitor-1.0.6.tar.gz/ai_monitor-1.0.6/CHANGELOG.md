...
### Added
- **traceloop-sdk**: Added to `ai-monitor[all]` extras for complete Traceloop integration
- **requests**: Added as core dependency for HTTP monitoring compatibility

### Fixed
- **Import Issues**: Removed problematic `token_fix_patch` import from `__init__.py`
- **HTTPX Monitoring**: Improved robustness of HTTPX client monitoring
- **Dependency Consistency**: Synchronized pyproject.toml and setup.py dependencies

### Changed
- **Package Structure**: Enhanced plug-and-play functions with better error handling
