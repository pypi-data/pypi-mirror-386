...
from django.db.models import ForeignKey, ManyToOneRel, Model, Q, Window
...
class ListRestViewMixin(ModelResponseMixin):
    ...
    def get_navigation_meta(self, obj):
        ...
        sql = f"""
            WITH annotated AS (
                {qs_window.values('id', 'prev_id', 'next_id').query}
            )
        ...
        with connection.cursor() as cursor:
            ...
            prev_id, next_id = cursor.fetchone() or (None, None)
            return {
                'previous_pk': prev_id,
                'next_pk': next_id}
