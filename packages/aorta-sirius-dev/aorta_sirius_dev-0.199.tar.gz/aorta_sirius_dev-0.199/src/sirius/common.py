...
from sirius.constants import EnvironmentSecretKey, EnvironmentVariableKey, SiriusEnvironmentSecretKey
...
async def get_environmental_secret(key: EnvironmentSecretKey, default_value: str | None = None) -> str:
    if not isinstance(key, EnvironmentSecretKey) or not isinstance(key, Enum):
        raise SDKClientException("The key has to be a subclass of EnvironmentSecretKey and an Enum")
