...
class StockMarketData(DataClass, ABC):
    ...
    async def get_latest(stock: Stock) -> "StockMarketData":
        from_timestamp: datetime.datetime = common.get_previous_business_day_adjusted_date(datetime.datetime.now()) - datetime.timedelta(days=1)
        return max(await StockMarketData.get(stock, from_timestamp), key=lambda s: s.timestamp)
...
class OptionPerformanceAnalysis(DataClass):
    ...
    async def get(option_contract: Option) -> "OptionPerformanceAnalysis":
        ...
        underlying_price: Decimal = (await StockMarketData.get_latest(option_contract.underlying_stock)).close
        itm_required_absolute_return: Decimal = (option_contract.strike_price - underlying_price) / underlying_price
        stock_performance_analysis: StockPerformanceAnalysis = await StockPerformanceAnalysis.get(option_contract.underlying_stock.ticker, number_of_days_invested, number_of_days_to_analyse)
