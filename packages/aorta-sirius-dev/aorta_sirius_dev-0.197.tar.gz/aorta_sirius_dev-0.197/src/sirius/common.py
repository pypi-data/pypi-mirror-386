...
from sirius.constants import EnvironmentSecretKey, EnvironmentVariableKey
...
async def get_environmental_secret(key: EnvironmentSecretKey, default_value: str | None = None) -> str:
...
def get_application_name() -> str:
    return get_environment_variable(EnvironmentVariableKey.APPLICATION_NAME) or Path.cwd().name.title()
...
async def get_central_finite_curve_authentication_headers() -> Dict[str, str]:
    return {"Authorization": f"Bearer {await get_environmental_secret(EnvironmentSecretKey.API_KEY)}"} if is_production_environment() else {"Authorization": f"Bearer NULL"}
...
async def verify_token(token: HTTPAuthorizationCredentials = Depends(HTTPBearer(auto_error=False))) -> None:
    ...
    if token.credentials != await get_environmental_secret(EnvironmentSecretKey.API_KEY):
