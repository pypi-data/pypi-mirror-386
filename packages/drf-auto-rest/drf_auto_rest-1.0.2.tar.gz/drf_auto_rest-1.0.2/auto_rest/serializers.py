    ...
    def _handle_hashid_fields(cls, model_class, attrs):
        ...
        for field in model_class._meta.get_fields():
            if isinstance(field, (HashidField, HashidAutoField)):
                # Explicitly declare the HashID field with proper source_field format
                source_field_path = f"{model_class._meta.app_label}.{model_class.__name__}.{field.name}"
                attrs[field.name] = HashidSerializerCharField(
                    source_field=source_field_path, read_only=True
                )
