...
<script lang="ts">
    ...
    function applyIncomingValue(obj: any): void {
        if (!obj || typeof obj !== "object" || Array.isArray(obj)) return;
        // Update corner labels/colors if provided
        if (typeof obj.upper_left_label === "string") upper_left_label = obj.upper_left_label;
        if (typeof obj.upper_right_label === "string") upper_right_label = obj.upper_right_label;
        if (typeof obj.lower_right_label === "string") lower_right_label = obj.lower_right_label;
        if (typeof obj.lower_left_label === "string") lower_left_label = obj.lower_left_label;
        if (typeof obj.color_upper_left === "string") color_upper_left = obj.color_upper_left;
        if (typeof obj.color_upper_right === "string") color_upper_right = obj.color_upper_right;
        if (typeof obj.color_lower_right === "string") color_lower_right = obj.color_lower_right;
        if (typeof obj.color_lower_left === "string") color_lower_left = obj.color_lower_left;
        // Optionally update point if x/y present and we're not dragging
        if (!isDraggingPoint && typeof obj.x === "number" && typeof obj.y === "number") {
            const nx = Math.max(x_min, Math.min(x_max, obj.x));
            const ny = Math.max(y_min, Math.min(y_max, obj.y));
            if (points.length === 0 || points[0][0] !== nx || points[0][1] !== ny) {
                points = [[nx, ny]];
            }
        }
        draw();
    ...
    // Apply incoming updates from Python or other components (labels/colors/x/y)
