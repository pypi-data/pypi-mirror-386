...
def generate_context_id() -> str:
    """
    Generate a context ID that always starts with a letter.
    
    :return: A context ID in format 'ctx-{10-char-string}' where string starts with a letter
    """
    import random
    import string
    
    # Generate a 10-character string that starts with a letter
    while True:
        # Generate 10 random characters (letters and digits)
        chars = ''.join(random.choices(string.ascii_lowercase + string.digits, k=10))
        # Check if it starts with a letter
        if chars[0].isalpha():
            return f"ctx-{chars}"
        # If not, modify the first character to a random letter
        first_char = random.choice(string.ascii_lowercase)
        chars = first_char + chars[1:]
        return f"ctx-{chars}"


def get_global_session_id() -> str:
    """
    Get or generate the global session ID.
    This ensures the same session ID is used throughout the application lifecycle.
    
    :return: A session ID that starts with 'a'
    """
    global global_session_id
    
    if global_session_id is None:
        from e2b_on_fc.simple_sandbox import generate_session_id
        global_session_id = generate_session_id()
        logger.info(f"Generated global session ID: {global_session_id}")
...
# Global session ID - shared between health check and E2B sandbox
...
async def handle_create_context(arguments: Any) -> Sequence[TextContent]:
    ...
    # Generate context_id (always starts with a letter)
    ...
    if E2B_AVAILABLE and e2b_sandbox:
        try:
            # Create E2B context with language parameter
            e2b_context = e2b_sandbox.create_code_context(language=args.language)
...
async def wait_for_sandbox_health(sandbox_url: str, max_retries: int = 300) -> bool:
    """
    Wait for sandbox health endpoint to return 200 status.
    
    Args:
        sandbox_url: Base URL of the sandbox service
        max_retries: Maximum number of retries (default: 300, i.e., 5 minutes)
    
    Returns:
        True if health check passes, False if max retries exceeded
    """
    health_url = f"{sandbox_url.rstrip('/')}/health"
    logger.info(f"Waiting for sandbox health check: {health_url}")
    
    # Use global session ID (consistent with E2B client)
    session_id = get_global_session_id()
    
    headers = {
        "X-CI-SESSION-ID": session_id,
        "Content-Type": "application/json",
        "User-Agent": "code-interpreter-mcp-server/0.0.8"
    }
            ...
            try:
                async with session.get(health_url, headers=headers) as response:
                    if response.status == 200:
                        logger.info(f"✅ Sandbox health check passed (attempt {attempt + 1})")
                        return True
                    else:
                        logger.warning(f"Health check returned status {response.status} (attempt {attempt + 1}/{max_retries})")
            except aiohttp.ClientError as e:
                logger.warning(f"Health check failed: {e} (attempt {attempt + 1}/{max_retries})")
            except Exception as e:
                logger.warning(f"Unexpected error during health check: {e} (attempt {attempt + 1}/{max_retries})")
            
            # Wait 1 second before next attempt
            if attempt < max_retries - 1:  # Don't wait after the last attempt
                await asyncio.sleep(1)
    
    logger.error(f"❌ Sandbox health check failed after {max_retries} attempts")
    return False
...
async def initialize_server(args=None):
    ...
    if E2B_AVAILABLE:
        try:
            ...
            # If sandbox_url is provided, wait for health check before creating sandbox
            if sandbox_url:
                logger.info(f"Using code interpreter endpoint: {sandbox_url}")
                
                # Wait for sandbox health check to pass
                health_check_passed = await wait_for_sandbox_health(sandbox_url)
                if not health_check_passed:
                    if disable_mock:
                        logger.error("Sandbox health check failed and mock mode is disabled")
                        raise RuntimeError("Sandbox health check failed and mock mode is disabled")
                    else:
                        logger.warning("Sandbox health check failed, but continuing with E2B Sandbox")
                
                # Create E2B Sandbox instance with sandbox_url and global session_id
                global_session_id = get_global_session_id()
                e2b_sandbox = Sandbox(envd_url=sandbox_url, session_id=global_session_id)
            else:
                # Create E2B Sandbox instance with global session_id
                global_session_id = get_global_session_id()
                e2b_sandbox = Sandbox(session_id=global_session_id)  # Will use SANDBOX_URL from environment
