...
Environment Variables:
  ...
  MCP_HOST      SSE server host (default: 0.0.0.0)
  MCP_PORT      SSE server port (default: 3000)
...
async def initialize_server(args=None):
    ...
    # Get SANDBOX_URL from environment or args
    sandbox_url = None
    if args:
    ...
    if E2B_AVAILABLE:
        try:
            ...
            if e2b_sandbox.is_running():
                ...
                logger.warning("Code interpreter is not responding")
                e2b_sandbox = None
...
async def main():
    ...
    port = int(getattr(args, 'port', None) or os.getenv('MCP_PORT', '3000'))
