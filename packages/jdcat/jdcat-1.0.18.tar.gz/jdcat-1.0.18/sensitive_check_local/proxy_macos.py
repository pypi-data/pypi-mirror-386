...
def _perform_installation() -> None:
    ...
    if _rhc and hasattr(_rhc, "wait_for_helper"):
        ...
        wait_ok = bool(_rhc.wait_for_helper(timeout=60.0, interval=5.0))
    else:
        # 实现轮询健康检查：每5秒检查一次，1分钟后失败
        _logger.info("[helper-install] 开始健康检查轮询（每5秒检查一次，最多1分钟）...")
        max_checks = 12  # 12次 * 5秒 = 60秒
        for i in range(max_checks):
            ...
            if i < max_checks - 1:  # 不是最后一次检查
                _logger.info("[helper-install] ⏳ Helper服务尚未就绪，5秒后重试...")
                time.sleep(5)
            else:
                _logger.warning("[helper-install] ❌ 健康检查超时，已尝试%d次（共%d秒）", max_checks, max_checks * 5)

    if not wait_ok:
        _logger.error("[helper-install] Helper服务60秒内未能启动，服务可能存在问题")
        ...
        raise RuntimeError("Root Helper 安装后健康检查失败（/health 不可达）- 服务在60秒内未能启动，可能存在配置或权限问题")
