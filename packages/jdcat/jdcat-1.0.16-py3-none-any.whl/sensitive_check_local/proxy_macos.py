...
def install_if_needed() -> None:
    ...
    # 安装完成后：等待 Helper 启动并健康就绪（增加等待时间，因为LaunchDaemon首次启动较慢）
    _logger.info("[helper-install] 安装完成，等待Helper服务启动...")
    ...
    if _rhc and hasattr(_rhc, "wait_for_helper"):
        # 增加超时时间到20秒，给LaunchDaemon足够启动时间
        wait_ok = bool(_rhc.wait_for_helper(timeout=20.0, interval=0.5))
    else:
        # 兼容：本地简单轮询，增加到40次检查（20秒）
        _logger.info("[helper-install] 开始健康检查轮询...")
        for i in range(40):
            if _helper_healthcheck(timeout=0.6):
                _logger.info("[helper-install] 健康检查成功，Helper在第%d次尝试后就绪", i + 1)
                wait_ok = True
                break
            if i % 10 == 9:  # 每5秒记录一次进度
                _logger.info("[helper-install] 健康检查进行中... (%d/40)", i + 1)
            time.sleep(0.5)

    if not wait_ok:
        _logger.error("[helper-install] Helper服务20秒内未能启动，可能需要更多时间")
        _logger.error("[helper-install] 建议：稍后重试，或检查系统日志")
        raise RuntimeError("Root Helper 安装后健康检查失败（/health 不可达）- LaunchDaemon可能需要更多启动时间")
