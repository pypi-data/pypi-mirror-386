...
class HybridStorage:
        ...
        try:
            ...
            for si, src_root in enumerate(sources, start=1):
                ...
                try:
                    ...
                    con.execute("BEGIN;")  # use BEGIN, not IMMEDIATE (safer lock behavior)

                    # Stage candidate rows (dedup via hash if requested)
                    ...
                    # Fetch staged rows
                    rows = con.execute(
                        "SELECT src_oid, energy, natoms, formula, meta_json, h FROM t_src_new ORDER BY src_oid ASC;"
                    ).fetchall()
                    ...
                    for src_oid, energy, natoms, formula, meta_json, h in rows:
                        # Ensure hash present in meta_json
                        mjson = meta_json
                        try:
                        ...
                        if comp_rows:
                            con.executemany(
                                ...
                                [(mapping[int(oid)], sym2id[str(sym)], float(ct))
                                 for (oid, sym, ct) in comp_rows if int(oid) in mapping]
                            )
                    ...
                    if sc_rows:
                        con.executemany(
                            ...
                            [(mapping[int(oid)], str(k), float(v) if v is not None else None)
                             for (oid, k, v) in sc_rows if int(oid) in mapping]
                        )

                    # --- commit before detaching (important!) ---
                    con.commit()
                    con.execute("DETACH DATABASE src;")

                    # --- HDF5 copy (outside SQL transaction) ---
                    dst_grp = dst._h5.require_group("objs")
                    for soid, did in mapping.items():
                        ...
                        try:
                            ...
                            if not quiet:
                                print(f"[merge-attach] Warning: missing dataset {sname} in {src_root}")
                            continue
                    dst._h5.flush()
                    ...
                    print(f"[merge-attach] Warning: merge failed for {src_root}: {e}")
                    con.rollback()
                    try:
                        con.execute("DETACH DATABASE src;")
                    except Exception:
            ...
            # --- optional compact and integrity check ---
            if compact:
                try:
                    dst.compact_hdf5()
                    if not quiet:
            ...
            try:
                res = con.execute("PRAGMA integrity_check;").fetchone()
                if res and res[0] != "ok":
                    print(f"[merge-attach] WARNING: integrity check: {res}")
            except Exception as e:
                if not quiet:
                    print(f"[merge-attach] integrity_check failed: {e}")
    ...
    def merge_roots_recursive_attach(
        ...
        *,
        ...
        Improvements:
          - Progress printed AFTER each source DB with per-source and total elapsed time.
          - Final statistics summary (totals, elapsed, throughput, successes/failures).
          - Chunked SELECT from sources to bound memory.
          - In-memory dedup set for object hashes (when dedup="hash").
          - Bulk insert via TEMP staging table + INSERT ... SELECT ... RETURNING id (ordered) to rebuild src->dst mapping.
          - Drop secondary indices before bulk load; recreate after.
          - HDF5 copy batched; optionally threaded only if the HDF5 library is thread-safe.

        Returns a dict summary with counts, timings, and destination paths.
        """
                ...
                try:
                    ...
                    try:
                        ...
                        if src_max_id is None:
                            ...
                            if not quiet:
                                print(f"[{si}/{len(sources)}] Empty DB, skipped. Total elapsed {_fmt_t(time.perf_counter()-overall_t0)}")
                            continue
                        ...
                        while last_id < src_max_id:
                            ...
                            # Insert into main table in src_oid order, get ids back
                            use_returning = True
                            try:
                                cur = con.execute("""
                            ...
                            def _copy_one(soid: int, did: int):
                                ...
                                try:
                                    if dname in dst_grp:
                                        del dst_grp[dname]
                                    dst_grp.copy(src._h5["objs"][sname], dname)
                                except KeyError:
                                    if not quiet:
                    ...
                    src_elapsed = time.perf_counter() - src_t0
