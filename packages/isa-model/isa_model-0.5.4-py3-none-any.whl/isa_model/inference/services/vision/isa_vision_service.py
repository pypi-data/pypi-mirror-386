...
class ISAVisionService(BaseVisionService):
        ...
        try:
            ...
            if result and result.get('success', False):
                ...
                if self._current_user_id:
                    try:
                        # Calculate units for billing (1 request = 1 unit for vision)
                        await self._publish_billing_event(
                            user_id=self._current_user_id,
                            service_type="vision",
                            operation="ui_detection",
                            input_units=1.0,  # 1 image processed
                            metadata={
    ...
    async def _call_modal_sdk_api(self, image_b64: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 使用超时控制Modal调用 - .remote() 在新版Modal SDK中是同步的，需要用 asyncio.to_thread 包装
            result = await asyncio.wait_for(
                asyncio.to_thread(
                    instance.detect_ui_elements_fast.remote,
                    image_b64,
                ...
                if self._current_user_id:
                    try:
                        await self._publish_billing_event(
                            ...
                            input_units=1.0,  # 1 image processed
                            metadata={
                                "text_blocks_extracted": len(result.get('text_blocks', [])),
                                "languages": languages,
                                "processing_time": result.get('processing_time', 0),
                                "estimated_cost_usd": cost
                            }
    ...
    async def _call_ocr_service(self, image_b64: str, languages: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 使用超时控制OCR调用 - .remote() 在新版Modal SDK中是同步的，需要用 asyncio.to_thread 包装
            result = await asyncio.wait_for(
                asyncio.to_thread(
                    instance.extract_text.remote,
                    image_b64,
                    languages
                ),
                timeout=self.timeout
    ...
    async def _publish_billing_event(
        ...
        """
        Publish billing event to NATS for ISA Vision services

        Args:
            user_id: User ID for billing
            service_type: Service type (vision)
            operation: Operation performed (ui_detection, ocr_extraction)
            input_units: Number of units consumed (images processed)
