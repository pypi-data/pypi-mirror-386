...
- **Proper Logging**: Uses Python's logging module for integration with monitoring tools
...
Change your profile type from `bigquery` to `rabbitbigquery` and add Rabbit configuration:
...
The adapter uses Python's `logging` module with the following log levels:

- **INFO**: Key events like optimization enabled and successful optimizations
- **DEBUG**: Detailed operation steps (optimization in progress)
- **WARNING**: Non-critical errors (optimization failures with fallback to original config)

### Viewing Logs

The adapter logs to the `dbt.adapters.rabbitbigquery` logger. By default, dbt may not display these logs in the console output. To view them:

1. **Check dbt's log files**: Look in `logs/dbt.log` in your dbt project directory
2. **Use dbt's debug mode**: Run dbt with the `--debug` flag to see more detailed logs
3. **Verify in Rabbit UI**: The easiest way to confirm optimization is working is to check the Rabbit dashboard where you can see all optimized jobs
...
# Check log file
cat logs/dbt.log | grep "dbt.adapters.rabbitbigquery"
```
...
**Solution:** Verify your `profiles.yml` has `type: rabbitbigquery` (not `bigquery`)
