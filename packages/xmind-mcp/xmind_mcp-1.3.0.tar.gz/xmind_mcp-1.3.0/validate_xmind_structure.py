...
class XMindValidator:
    ...
    def _parse_topic_recursive(self, topic, level: int = 0):
        ...
        # JSON路径下的子主题兼容处理
        json_children = []
        if isinstance(topic.get('children'), list):
            json_children = topic.get('children')
        elif isinstance(topic.get('topics'), list):
            json_children = topic.get('topics')
        elif isinstance(topic.get('subtopics'), list):
            json_children = topic.get('subtopics')
