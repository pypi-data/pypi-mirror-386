...
class GcpEntityFilter(BaseModel, GcSerializer):
    model_config = ConfigDict(
        populate_by_name=True,
        from_attributes=True,
    )
    scope: Scope2
    values: List[str] = Field(..., examples=[['dev1-451915', 'DT-GC-1']])
...
class StatusRequest(BaseModel, GcSerializer):
    ...
    detailed_status: Optional[
        Union[AzureSubscriptionsFilter, AwsAccountsFilter, GcpEntityFilter]
...
class GcpOnboarding(BaseModel, GcSerializer):
    model_config = ConfigDict(
        populate_by_name=True,
        from_attributes=True,
    )
    entity: GcpEntityFilter
...
class Step(BaseModel, GcSerializer):
    ...
    name: Name
    action: Union[AzureOnboarding, AwsOnboarding, GcpOnboarding, Dict[str, Any]]
