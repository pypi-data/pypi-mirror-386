...
from .headers import parse_sec_ch_ua, sort_headers, ACCEPT_LANGUAGE_MAP
...
class Session:
    ...
    def set_headers_order(self, headers: Union[Dict[str, str], rnet.HeaderMap], is_fetch: Optional[bool] = None):
        """
        请求头排序
        ...
        if is_fetch is not None:            
            if isinstance(headers, rnet.HeaderMap):
                ...
                }, chrome_version=self._chrome_version, is_fetch=is_fetch)
            else:
                headers = sort_headers(headers=headers, chrome_version=self._chrome_version, is_fetch=is_fetch)
    ...
    def _process_get_arguments(self, **kwargs):
        ...
        # 请求头排序
        self.set_headers_order(headers, is_fetch=kwargs.get("fetch"))
