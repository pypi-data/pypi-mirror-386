            ...
            [Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "timeInForce": {
                            "type": "string",
                            "description": "Time in force strategy: 'GTC' (Good Till Cancelled), 'IOC' (Immediate or Cancel), 'FOK' (Fill or Kill), 'PostOnly'. Market orders default to IOC, Limit orders default to GTC.",
                            "enum": ["GTC", "IOC", "FOK", "PostOnly"],
                            "default": "GTC"
                            },
                        "takeProfit": {
                            "type": "string",
                            "description": "Take profit price for the order. Requires tpslMode to be set for derivatives.",
                            "examples": ["55000", "3200.5"]
                            },
                        "stopLoss": {
                            "type": "string",
                            "description": "Stop loss price for the order. Requires tpslMode to be set for derivatives.",
                            "examples": ["48000", "2800.5"]
                            },
                        "tpTriggerBy": {
                            "type": "string",
                            "description": "Price type to trigger take profit. 'LastPrice' (default), 'MarkPrice', 'IndexPrice'.",
                            "enum": ["LastPrice", "MarkPrice", "IndexPrice"],
                            "default": "LastPrice"
                            },
                        "slTriggerBy": {
                            "type": "string",
                            "description": "Price type to trigger stop loss. 'LastPrice' (default), 'MarkPrice', 'IndexPrice'.",
                            "enum": ["LastPrice", "MarkPrice", "IndexPrice"],
                            "default": "LastPrice"
                            },
                        "reduceOnly": {
                            "type": "boolean",
                            "description": "Reduce only flag: true means the order can only reduce your position size. Used for closing positions.",
                            "default": False
                            },
                        "closeOnTrigger": {
                            "type": "boolean",
                            "description": "Close on trigger flag: true ensures the order reduces position size, canceling other orders if needed for margin. For guaranteed stop-losses.",
                            "default": False
                            },
                        "positionIdx": {
                            "type": "integer",
                            "description": "Position index for hedge mode: 0 for one-way, 1 for hedge-mode Buy side, 2 for hedge-mode Sell side.",
                            "enum": ["0", "1", "2"],
                            "default": "0"
                            },
                        "tpslMode": {
                            "type": "string",
                            "description": "TP/SL mode: 'Full' for entire position, 'Partial' for partial position. Required for TP/SL on derivatives.",
                            "enum": ["Full", "Partial"]
                            },
                        "tpLimitPrice": {
                            "type": "string",
                            "description": "Limit price for take profit order when tpOrderType is 'Limit'.",
                            "examples": ["55500", "3250.5"]
                            },
                        "slLimitPrice": {
                            "type": "string",
                            "description": "Limit price for stop loss order when slOrderType is 'Limit'.",
                            "examples": ["47500", "2750.5"]
                            },
                        "tpOrderType": {
                            "type": "string",
                            "description": "Order type for take profit: 'Market' (default) or 'Limit'.",
                            "enum": ["Market", "Limit"],
                            "default": "Market"
                            },
                        "slOrderType": {
                            "type": "string",
                            "description": "Order type for stop loss: 'Market' (default) or 'Limit'.",
                            "enum": ["Market", "Limit"],
                            "default": "Market"
                            }
                    },
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "triggerPrice": {
                            "type": "string",
                            "description": "New trigger price for conditional orders. For spot, it's for TP/SL. For derivatives, it's the conditional trigger.",
                            "examples": ["51500", "3150.5"]
                            },
                        "qty": {
                                "type": "string",
                        ...
                        "tpslMode": {
                            "type": "string",
                            "description": "TP/SL mode: 'Full' for entire position, 'Partial' for partial position. Required if modifying TP/SL on derivatives.",
                            "enum": ["Full", "Partial"]
                            },
                        "takeProfit": {
                            "type": "string",
                            "description": "New take profit price. Pass '0' to cancel existing TP. Valid for spot(UTA), linear, inverse.",
                            "examples": ["55000", "3200.5", "0"]
                            },
                        "stopLoss": {
                            "type": "string",
                            "description": "New stop loss price. Pass '0' to cancel existing SL. Valid for spot(UTA), linear, inverse.",
                            "examples": ["48000", "2800.5", "0"]
                            },
                        "tpTriggerBy": {
                            "type": "string",
                            "description": "Price type to trigger take profit. Required if not set initially.",
                            "enum": ["LastPrice", "MarkPrice", "IndexPrice"]
                            },
                        "slTriggerBy": {
                            "type": "string",
                            "description": "Price type to trigger stop loss. Required if not set initially.",
                            "enum": ["LastPrice", "MarkPrice", "IndexPrice"]
                            },
                        "triggerBy": {
                            "type": "string",
                        ...
                        "tpLimitPrice": {
                            "type": "string",
                            "description": "Limit price for take profit order if triggered. For partial TP/SL.",
                            "examples": ["55500", "3250.5"]
                            },
                        "slLimitPrice": {
                            "type": "string",
                            "description": "Limit price for stop loss order if triggered. For partial TP/SL.",
                            "examples": ["47500", "2750.5"]
                            }
                    },
                ...
                Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            ...
                            "stopOrderType": {
                                "type": "string",
                                "description": "Only used for category=linear or inverse and orderFilter=StopOrder,you can cancel conditional orders except TP/SL order and Trailing stop orders with this param",
                                "enum": ["Stop", "TakeProfit", "StopLoss"]
                            }
                        },
                        "required": ["category"]
                    }
