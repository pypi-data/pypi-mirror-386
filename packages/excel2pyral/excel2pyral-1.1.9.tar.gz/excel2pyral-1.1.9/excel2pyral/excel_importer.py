...
class ExcelToSystemRDLImporter:
    ...
    def excel_to_systemrdl(
        ...
        for sheet in xls.sheet_names:
            ...
            # Forward fill register name, offset, and reset value if cells are empty to reduce repetition
            df['Register Name'] = df['Register Name'].replace("", pd.NA).ffill()
            df['Offset'] = df['Offset'].replace("", pd.NA).ffill()
            # Forward fill reset value to allow register-level reset inheritance
            if 'Reset Value' in df.columns:
                df['Reset Value'] = df['Reset Value'].replace("", pd.NA).ffill()
            # Forward fill width values to allow register-level width inheritance
            if 'Width' in df.columns:
                df['Width'] = df['Width'].replace("", pd.NA).ffill()
            ...
            for (reg_name, offset), group in grouped:
                ...
                for _, field_row in group.iterrows():
                    ...
                    # Use reset value (now forward-filled from register level)
                    if reset_value:
                        if sw_access == "r" and hw_access == "r":
                            # For constant fields (sw=r, hw=r), extract field value from register reset
                            try:
                                reg_reset_int = int(reset_value, 0)  # Support hex/dec
                            ...
                            # For non-constant fields, use the reset value directly
                            rdl_lines.append(f"            reset = {reset_value};")
