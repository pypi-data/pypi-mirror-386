...
class ComprehensiveQATest:
    ...
    async def test_update_ticket(self, adapter_name: str, ticket_id: str) -> bool:
        ...
        try:
            import subprocess
            cmd = [
                "mcp-ticketer", "update", ticket_id,
                "--description", f"Updated by QA test at {datetime.now().isoformat()}",
                "--adapter", adapter_name
            ]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=15)
    ...
    async def test_transition_ticket(self, adapter_name: str, ticket_id: str) -> bool:
        ...
        try:
            import subprocess
            cmd = ["mcp-ticketer", "transition", ticket_id, "in_progress", "--adapter", adapter_name]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=15)
    ...
    async def test_add_comment(self, adapter_name: str, ticket_id: str) -> bool:
        ...
        try:
            ...
            cmd = ["mcp-ticketer", "comment", ticket_id, comment_text, "--adapter", adapter_name]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=15)
    ...
    def print_summary(self):
        ...
        for adapter_name, results in self.test_results.items():
            ...
            test_names = ["create", "read", "update", "transition", "comment", "list", "search"]
            adapter_passed = 0
            ...
            print(f"  {'Score:':<12} {adapter_passed}/{adapter_total} ({adapter_passed/adapter_total*100:.1f}%)")
        ...
        if passed_tests == total_tests:
            print("🎉 ALL TESTS PASSED! MCP Ticketer is working perfectly across all adapters!")
