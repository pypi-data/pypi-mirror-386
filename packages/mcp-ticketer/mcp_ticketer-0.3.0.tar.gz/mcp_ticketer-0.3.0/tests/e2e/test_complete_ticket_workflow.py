...
class TestCompleteTicketWorkflow:
    ...
    async def test_epic_to_task_complete_workflow(self, adapter):
        ...
        updated_parent = await adapter.transition_state(created_parent.id, TicketState.IN_PROGRESS)
        assert updated_parent.state == TicketState.IN_PROGRESS
        
        updated_parent = await adapter.transition_state(created_parent.id, TicketState.DONE)
        assert updated_parent.state == TicketState.DONE
        
        # Step 6: Transition Epic when parent Issue is done
        updated_epic = await adapter.transition_state(created_epic.id, TicketState.IN_PROGRESS)
        ...
        # Search by priority
        high_priority_search = SearchQuery(
            priority=Priority.HIGH,
            limit=10
        )
    ...
    async def test_blocked_and_waiting_states(self, adapter):
        ...
        # Start work
        updated = await adapter.transition_state(created_task.id, TicketState.IN_PROGRESS)
        assert updated.state == TicketState.IN_PROGRESS
        ...
        # Resume work
        resumed = await adapter.transition_state(created_task.id, TicketState.IN_PROGRESS)
