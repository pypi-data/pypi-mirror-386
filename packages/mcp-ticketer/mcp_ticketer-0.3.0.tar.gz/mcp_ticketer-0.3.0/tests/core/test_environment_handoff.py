    ...
    def run_worker_environment_test(self, test_file: Path):
        ...
        try:
            ...
            if "WORKER_ENV_START" in output and "WORKER_ENV_END" in output:
                ...
                self.worker_env = {"error": "Environment markers not found", "stdout": result.stdout, "stderr": result.stderr}
    ...
    def save_detailed_report(self, differences):
        ...
        report = {
            ...
            "summary": {
                ...
                "has_env_var_issues": any(d.startswith("env_var_") for d in differences),
