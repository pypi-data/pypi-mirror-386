...
class UserAssignmentTester:
            ...
            try:
                if adapter_name == "linear":
                    ...
                    if hasattr(adapter, 'get_team_members'):
                        ...
                        if users:
                            self.test_users[adapter_name] = users[:3]  # Take first 3 users
                            print(f"    ✅ Found {len(users)} users, using first 3 for testing")
                            ...
                            if current_user:
                                ...
                                print(f"    ✅ Using current user: {current_user.get('name', 'Unknown')}")
                    else:
                    ...
                    if hasattr(adapter, 'get_collaborators'):
                        ...
                        if users:
                            ...
                            print(f"    ✅ Found {len(users)} collaborators, using first 3 for testing")
                            for user in self.test_users[adapter_name]:
                                print(f"       - {user.get('login', 'Unknown')} ({user.get('name', 'No name')})")
                        else:
                            # Fallback to current user
                            current_user = await adapter.get_current_user()
                            if current_user:
                                self.test_users[adapter_name] = [current_user]
                                print(f"    ✅ Using current user: {current_user.get('login', 'Unknown')}")
                    else:
                    ...
                    if hasattr(adapter, 'get_project_users'):
                        ...
                        if users:
                            self.test_users[adapter_name] = users[:3]
                            print(f"    ✅ Found {len(users)} project users, using first 3 for testing")
                            for user in self.test_users[adapter_name]:
                                print(f"       - {user.get('displayName', 'Unknown')} ({user.get('emailAddress', 'no-email')})")
                        else:
                            # Fallback to current user
                            current_user = await adapter.get_current_user()
                            if current_user:
                                self.test_users[adapter_name] = [current_user]
                                print(f"    ✅ Using current user: {current_user.get('displayName', 'Unknown')}")
                    else:
                    ...
                    self.test_users[adapter_name] = [
                        {"name": "Test User 1", "email": "test1@example.com", "id": "test-user-1"},
                        {"name": "Test User 2", "email": "test2@example.com", "id": "test-user-2"},
                        {"name": "Test User 3", "email": "test3@example.com", "id": "test-user-3"}
                    ]
                    print(f"    ✅ Using predefined test users for local adapter")
                    for user in self.test_users[adapter_name]:
                        print(f"       - {user['name']} ({user['email']})")
                
                if adapter_name not in self.test_users or not self.test_users[adapter_name]:
                    print(f"    ⚠️  No users available for {adapter_name.upper()} - will test with None assignee")
                    self.test_users[adapter_name] = []
    ...
    async def test_ticket_assignment(self):
        ...
        for adapter_name, adapter in self.adapters.items():
            ...
            try:
                ...
                if not users:
                    print(f"    ⚠️  No users available - testing unassigned ticket creation")
                    ...
                    print(f"       Assignee: {getattr(created_task, 'assignee', 'None')}")
                    ...
                    for i, user in enumerate(users[:2], 1):  # Test with first 2 users
                        ...
                        print(f"    📝 Creating task assigned to: {user_name} ({user_id})")
                        task = Task(**task_data)
                        created_task = await adapter.create(task)
    ...
    async def test_assignment_updates(self):
        ...
        for adapter_name, adapter in self.adapters.items():
            ...
            try:
                ...
                if "assigned" in tickets and len(tickets["assigned"]) > 0 and len(users) > 1:
                    ...
                    if adapter_name == "linear":
                        ...
                        new_assignee_name = users[1].get('name') or users[1].get('login', 'Unknown')
                    elif adapter_name == "jira":
                    ...
                    print(f"    🔄 Reassigning {test_ticket.id} to: {new_assignee_name} ({new_assignee})")
                    
                    # Update the ticket assignment
                    updated_ticket = await adapter.update(test_ticket.id, {"assignee": new_assignee})
                    
                    if updated_ticket:
                        print(f"    ✅ Ticket reassigned successfully")
                        print(f"       New assignee: {getattr(updated_ticket, 'assignee', 'None')}")
                        
                        # Test unassigning (setting to None)
                        print(f"    🔄 Unassigning {test_ticket.id}...")
                        unassigned_ticket = await adapter.update(test_ticket.id, {"assignee": None})
                        
                        if unassigned_ticket:
                            print(f"    ✅ Ticket unassigned successfully")
                            print(f"       Assignee: {getattr(unassigned_ticket, 'assignee', 'None')}")
                        else:
                            print(f"    ⚠️  Unassignment may not be reflected immediately")
                    else:
                    ...
                    if adapter_name == "linear":
                        ...
                        new_assignee_name = users[0].get('name') or users[0].get('login', 'Unknown')
                    elif adapter_name == "jira":
                    ...
                    print(f"    🔄 Assigning {test_ticket.id} to: {new_assignee_name} ({new_assignee})")
                    ...
                    print(f"    ⏭️  No suitable tickets or users for reassignment testing")
            ...
            try:
                ...
                if len(users) > 0:
                    ...
                    if adapter_name == "linear":
                        ...
                        search_name = users[0].get('name') or users[0].get('login', 'Unknown')
                    elif adapter_name == "jira":
                        search_assignee = users[0].get('accountId') or users[0].get('name')
                        search_name = users[0].get('displayName', 'Unknown')
                    else:  # aitrackdown
                        search_assignee = users[0].get('id')
                        search_name = users[0].get('name', 'Unknown')
                    
                    print(f"    🔍 Searching for tickets assigned to: {search_name} ({search_assignee})")
                    ...
                    print(f"    ✅ Found {len(search_results)} tickets assigned to {search_name}")
                    
                    # Show first few results
                    for i, ticket in enumerate(search_results[:3], 1):
                        assignee = getattr(ticket, 'assignee', 'None')
                        print(f"       {i}. {ticket.id}: {ticket.title[:50]}... (assignee: {assignee})")
    ...
    def generate_summary(self):
        ...
        for adapter_name in ["linear", "github", "jira", "aitrackdown"]:
            ...
            print(f"    {adapter_name.upper()}: {status} ({user_count} users available)")
        ...
        for adapter_name in self.adapters.keys():
            ...
            if result.get("success"):
                ...
                print(f"    {adapter_name.upper()}: ✅ {tickets_count} tickets created with {users_count} users available")
            else:
                print(f"    {adapter_name.upper()}: ❌ Failed - {result.get('error', 'Unknown error')}")
        ...
        successful_tests = len([k for k, v in self.test_results.items() if v.get("success")])
        ...
        if successful_tests == total_tests:
            ...
            print(f"    ✅ Most assignment features working well - minor issues to address")
        else:
