...
class CommentingTester:
    ...
    async def setup_test_tickets(self):
        ...
        for adapter_name in adapters_to_test:
            ...
            try:
                ...
                if missing_keys:
                    print(f"    ⏭️  Skipping {adapter_name} - missing keys: {missing_keys}")
                    continue
            ...
            try:
                ...
                for i, comment_data in enumerate(test_comments, 1):
                    try:
                        ...
                        print(f"    ✅ Added comment #{i} ({comment_data['description']})")
        ...
        for adapter_name, ticket_info in self.test_tickets.items():
            ...
            try:
                ...
                if adapter_name in self.test_results:
                    self.test_results[adapter_name]["comments_retrieved"] = len(comments)
                else:
                    self.test_results[adapter_name] = {"comments_retrieved": len(comments)}
    ...
    def generate_summary(self):
        ...
        print(f"  📈 Success rate: {(successful_adapters/total_adapters*100):.1f}%" if total_adapters > 0 else "  📈 Success rate: 0%")
