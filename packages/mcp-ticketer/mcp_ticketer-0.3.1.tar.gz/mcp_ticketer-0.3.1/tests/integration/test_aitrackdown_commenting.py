...
class AitrackdownCommentTester:
    ...
    async def test_comment_retrieval(self):
        ...
        for ticket in self.test_tickets:
            ...
            try:
                comments = await self.adapter.get_comments(ticket.id, limit=10, offset=0)
        ...
        for ticket in self.test_tickets:
            ...
            try:
                ...
                result = subprocess.run([
                    "mcp-ticketer", "comment", ticket.id, cli_comment,
                    "--adapter", "aitrackdown"
    ...
    def generate_summary(self):
        ...
        if addition_results:
            ...
            print(f"    Total comments added: {addition_results.get('total_comments_added', 0)}")
            print(f"    Status: {'✅ Working' if addition_results.get('success') else '❌ Failed'}")
        ...
        if retrieval_results:
            ...
            print(f"    With comments: {retrieval_results.get('tickets_with_comments', 0)}")
            print(f"    Total retrieved: {retrieval_results.get('total_comments_retrieved', 0)}")
            print(f"    Status: {'✅ Working' if retrieval_results.get('success') else '❌ Failed'}")
        ...
        if cli_results:
            ...
            print(f"    Status: {'✅ Working' if cli_results.get('success') else '❌ Failed'}")
