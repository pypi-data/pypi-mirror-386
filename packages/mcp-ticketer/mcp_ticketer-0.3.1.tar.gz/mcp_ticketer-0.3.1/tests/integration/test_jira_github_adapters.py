...
class AdapterTester:
    ...
    async def test_jira_adapter(self):
        ...
        try:
            ...
            try:
                ...
                print(f"  ✅ JIRA authentication successful - found {len(issues)} issues")
    ...
    async def test_github_adapter(self):
        ...
        try:
            ...
            try:
                issues = await adapter.list(limit=5, offset=0)
                print(f"  ✅ GitHub authentication successful - found {len(issues)} issues")
    ...
    def generate_summary(self):
        ...
        print(f"⚙️  Configuration: {'✅ Loaded successfully' if config_ok else '❌ Failed to load'}")
        
        # Adapter summaries
        jira_results = self.test_results.get("jira", {})
        github_results = self.test_results.get("github", {})
        
        print(f"🎫 JIRA Adapter: {'✅ Working' if jira_results.get('success') else '❌ Failed'}")
        if jira_results.get("created_issue_id"):
            print(f"  Created issue: {jira_results['created_issue_id']}")
            
        print(f"🐙 GitHub Adapter: {'✅ Working' if github_results.get('success') else '❌ Working'}")
