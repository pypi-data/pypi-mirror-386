...
from mcp_ticketer.core.models import Epic, Task, Priority, TicketState, SearchQuery, TicketType
        ...
        for adapter_name, adapter in self.adapters.items():
            ...
            try:
                ...
                if adapter_name == "github":
                    ...
                    print(f"    ℹ️  GitHub doesn't support Epics natively - creating issues only")
                    created_epic = None
                    ...
                    if adapter_name == "linear":
                        print(f"    📝 Creating Project (Epic equivalent) for Linear...")
                    else:
                ...
                if created_epic:
                    ...
                    print(f"       Priority: {getattr(created_epic, 'priority', 'N/A')}")
                ...
                for i, issue_title in enumerate(issue_titles, 1):
                    issue_data = {
                        ...
                        "parent_epic": created_epic.id if created_epic and adapter_name not in ["linear", "github"] else None
                    }
                ...
                if adapter_name in ["jira", "linear"]:  # Platforms that support subtasks
                    ...
                    print(f"    ℹ️  {adapter_name.upper()} doesn't support subtasks - skipping")
                ...
                self.test_results[f"{adapter_name}_hierarchy"] = {
                    ...
                    "subtasks_created": len(self.created_tickets[adapter_name].get("subtasks", []))
                }
            ...
            try:
                ...
                for target_state in transitions:
                    try:
                        ...
                        updated_ticket = await adapter.transition_state(test_issue.id, target_state)
                        
                        if updated_ticket and updated_ticket.state == target_state:
                            print(f"    ✅ Successfully transitioned to: {target_state}")
                            successful_transitions += 1
                        else:
                            print(f"    ⚠️  Transition to {target_state} may not be reflected immediately")
        ...
        for adapter_name, adapter in self.adapters.items():
            ...
            try:
                ...
                for priority, tag_suffix in priority_tests:
                    try:
                        ticket_data = {
                            ...
                            "priority": priority,
                            "tags": ["priority-test", tag_suffix, adapter_name, "workflow-test"]
                        }
                        ...
                        print(f"    ✅ {priority.value.title()} priority ticket: {created_ticket.id}")
                        print(f"       Tags: {created_ticket.tags}")
                        
                    except Exception as e:
                        print(f"    ❌ Failed to create {priority.value} priority ticket: {e}")
                ...
                try:
                    ...
                    print(f"    ✅ Found {len(search_results)} tickets with 'priority-test' tag")
            ...
            try:
                ...
                if epic and issues:
                    ...
                    if retrieved_epic:
                        ...
                        if adapter_name == "linear":
                            print(f"    ℹ️  Linear uses Projects - relationship handled differently")
                    ...
                    for issue in issues[:2]:  # Check first 2 issues
                        ...
                        if retrieved_issue:
                            ...
                            if parent_epic:
                                print(f"    ✅ Issue {issue.id} correctly references parent epic: {parent_epic}")
                            elif adapter_name == "linear":
                                print(f"    ℹ️  Linear issue {issue.id} - project relationship handled via API")
                            else:
                                print(f"    ⚠️  Issue {issue.id} doesn't show parent epic reference")
                ...
                if subtasks and issues:
                    ...
                    if retrieved_issue:
                        ...
                        print(f"    📊 Issue {parent_issue.id} shows {children_count} child subtasks")
                    
                    # Check subtasks for parent issue reference
                    for subtask in subtasks:
                        retrieved_subtask = await adapter.read(subtask.id)
                        if retrieved_subtask:
                            parent_issue_ref = getattr(retrieved_subtask, 'parent_issue', None)
                            if parent_issue_ref:
                                print(f"    ✅ Subtask {subtask.id} correctly references parent issue: {parent_issue_ref}")
                            else:
                                print(f"    ⚠️  Subtask {subtask.id} doesn't show parent issue reference")
    ...
    def generate_summary(self):
        ...
        for adapter_name in self.adapters.keys():
            ...
            if result.get("success"):
                ...
                print(f"    {adapter_name.upper()}: ✅ {successful}/{total} transitions successful")
            else:
                print(f"    {adapter_name.upper()}: ❌ Failed - {result.get('error', 'Unknown error')}")
        ...
        for adapter_name in self.adapters.keys():
            ...
            if result.get("success"):
                ...
                print(f"    {adapter_name.upper()}: ✅ {tickets_created} priority tickets, tested: {', '.join(priorities)}")
            else:
                print(f"    {adapter_name.upper()}: ❌ Failed - {result.get('error', 'Unknown error')}")
        ...
        for adapter_name in self.adapters.keys():
            ...
            if result.get("success"):
                ...
                print(f"    {adapter_name.upper()}: Epic→Issue: {epic_issue}, Issue→Subtask: {issue_subtask}")
            else:
                print(f"    {adapter_name.upper()}: ❌ Failed - {result.get('error', 'Unknown error')}")
