...
class TestCommentsAndAttachments:
    ...
    async def test_comment_pagination_and_limits(self, adapter):
        ...
        offset_comments = await adapter.get_comments(created_task.id, limit=10, offset=10)
    ...
    async def test_comment_updates_and_editing(self, adapter):
        ...
        assert created_comment.body == "This is the original comment text that will be edited."
    ...
    async def test_cross_ticket_comment_references(self, adapter):
        ...
        assert any(created_task1.id in body and created_task2.id in body for body in epic_bodies)
