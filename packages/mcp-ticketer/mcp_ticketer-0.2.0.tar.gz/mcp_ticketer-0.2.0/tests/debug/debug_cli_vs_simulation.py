...
async def compare_cli_vs_simulation():
    ...
    try:
        ...
        print(f"   Team ID (config): {getattr(cli_adapter, 'team_id_config', 'Not set')}")
        ...
        print(f"   Ticket prefix: {cli_result.id.split('-')[0] if '-' in cli_result.id else 'No prefix'}")
    ...
    try:
        ...
        direct_adapter = AdapterRegistry.get_adapter("linear", adapter_config, force_new=True)
        print(f"   ✅ Direct adapter created successfully!")
        print(f"   Team ID (config): {getattr(direct_adapter, 'team_id_config', 'Not set')}")
        ...
        print(f"   Ticket prefix: {direct_result.id.split('-')[0] if '-' in direct_result.id else 'No prefix'}")
    ...
    if cli_result and direct_result:
        ...
        direct_prefix = direct_result.id.split('-')[0] if '-' in direct_result.id else 'Unknown'
