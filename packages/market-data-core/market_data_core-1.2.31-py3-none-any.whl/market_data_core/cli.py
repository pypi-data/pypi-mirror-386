...
def validate(config_path: Path, profile: Optional[str], verbose: bool) -> None:
    ...
    try:
        ...
        click.echo(
            click.style("❌ Configuration validation failed", fg="red", bold=True), err=True
        )
...
def inspect(config_path: Path, profile: Optional[str], output_format: str) -> None:
