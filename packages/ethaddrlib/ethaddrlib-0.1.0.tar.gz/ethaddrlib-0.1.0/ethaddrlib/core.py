...
from bip_utils import (
    ...
    Bip39Languages,
    Bip39MnemonicGenerator, Bip39WordsNum, Bip39Languages
)
...
def generate_mnemonic(words_num: int = 12, lang: Bip39Languages = Bip39Languages.ENGLISH) -> str:
    """
...
def validate_mnemonic_keys(p, lang: Bip39Languages = Bip39Languages.ENGLISH):
    ...
    try:
        ...
        pass
def validate_mnemonic(mnemonic: str, lang: Bip39Languages = Bip39Languages.ENGLISH) -> bool:
    """
    ...
    try:
        ...
        is_valid = bool(validator.IsValid(mnemonic))
        return is_valid
    except Exception:
        return False
...
def address_from_mnemonic(mnemonic: str, index: int = 0) -> Tuple[str, str]:
    ...
    acct = bip44_mst.Purpose().Coin().Account(0).Change(Bip44Changes.CHAIN_EXT).AddressIndex(index)

    priv_key_hex = "0x" + acct.PrivateKey().Raw().ToHex()
    eth_address = to_checksum_address(acct.PublicKey().ToAddress())

    return eth_address, priv_key_hex
