...
@singleton
class BankerClient(object):

    def __init__(self):
        meta_client = MetaClient()
        self.service = settings.SERVICE
        self.target_service = 'banker'
    ...
    def wallet_cards_list(self, token) -> (Response, Request):
        return self._api(self.target_service, 'wallet.cards.list', request_auth=RequestAuthEnum.EXTERNAL, auth=token)

    def wallet_cards_create(self, token, card_number, cardholder_name, expiration_month, expiration_year, cvv, default=True, **kwargs) -> (Response, Request):
        data = {
        ...
        return self._api(self.target_service, 'wallet.cards.create', json=data, request_auth=RequestAuthEnum.EXTERNAL, auth=token)