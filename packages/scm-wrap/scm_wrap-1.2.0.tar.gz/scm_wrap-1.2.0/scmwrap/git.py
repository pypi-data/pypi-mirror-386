...
class GitRepo(Repo):
    ...
    def load_repo_info(self):
        ...
        if self.tag is None:
            ...
            self._fill_tags(_tags)

        if cleanup_path:
            shutil.rmtree(path, ignore_errors=True)

    def _parse_tags(self, tag: str) -> Optional[List[str]]:
        _tag = tag.split()
    ...
    def _fill_tags(self, tags: str):
        self.tags = self._parse_tags(tags)
        if self.get_latest_valid_tag_cb is not None:
            self.tag = self.get_latest_valid_tag_cb(self.tags)
        elif self.tags:
