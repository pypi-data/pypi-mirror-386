...
class DisplayManager:
    ...
    def _filter_xml_tags(self, content: str) -> str:
        """Remove XML tags but keep content for immediate streaming"""
        self._xml_buffer += content
        result = []
        i = 0

        while i < len(self._xml_buffer):
            # Check for opening thinking tag
            if self._xml_buffer[i:i+10] == '<thinking>':
            ...
            if self._xml_buffer[i:i+11] == '</thinking>':
                # Skip the closing tag
                i += 11
                self._in_thinking_tag = False
                continue

            # Check if we might be at the start of a tag but need more data
            if i + 11 >= len(self._xml_buffer) and self._xml_buffer[i] == '<':
                # Might be an incomplete tag, buffer it
                self._xml_buffer = self._xml_buffer[i:]
                return ''.join(result)

            # Regular content, add to result (including content inside thinking tags)
            result.append(self._xml_buffer[i])
            i += 1
    ...
    def update_streaming_content(self, delta_content: str) -> None:
        # Filter out XML tags first
        filtered_content = self._filter_xml_tags(delta_content)
