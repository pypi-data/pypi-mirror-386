...
from markitdown_pro.handlers.ipynb_handler import IpynbHandler
...
ALL_FILES = list_files(
    TEST_FILES_PATH, include_ext=IpynbHandler.SUPPORTED_EXTENSIONS, recursive=True
)
log.info(f"IpynbHandler: Found {len(ALL_FILES)} test files in {TEST_FILES_PATH}.")


@pytest.mark.asyncio
@pytest.mark.parametrize("file_path", ALL_FILES, ids=[pretty_id(p) for p in ALL_FILES])
async def test_files(file_path: Path, ipynb_handler: IpynbHandler):
    log.info(f"IpynbHandler: Testing file: {file_path}")

    markdown_text = await ipynb_handler.handle(str(file_path))
