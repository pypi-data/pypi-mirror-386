...
ALL_FILES = list_files(
    TEST_FILES_PATH, include_ext=EmailHandler.SUPPORTED_EXTENSIONS, recursive=True
)
log.info(f"EmailHandler: Found {len(ALL_FILES)} test files in {TEST_FILES_PATH}.")
...
async def test_files(file_path: Path, email_handler: EmailHandler):
    log.info(f"EmailHandler: Testing file: {file_path}")
