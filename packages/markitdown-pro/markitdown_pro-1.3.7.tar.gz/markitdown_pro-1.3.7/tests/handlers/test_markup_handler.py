...
ALL_FILES = list_files(
    TEST_FILES_PATH, include_ext=MarkupHandler.SUPPORTED_EXTENSIONS, recursive=True
)
log.info(f"MarkupHandler: Found {len(ALL_FILES)} test files in {TEST_FILES_PATH}.")
...
async def test_files(file_path: Path, markup_handler: MarkupHandler):
    log.info(f"MarkupHandler: Testing file {file_path}")
