...
ALL_FILES = list_files(
    TEST_FILES_PATH, include_ext=MarkItDownHandler.SUPPORTED_EXTENSIONS, recursive=True
)
log.info(f"MarkItDownHandler: Found {len(ALL_FILES)} test files in {TEST_FILES_PATH}.")


@pytest.mark.asyncio
@pytest.mark.parametrize("file_path", ALL_FILES, ids=[pretty_id(p) for p in ALL_FILES])
async def test_files(file_path: Path, markitdown_handler: MarkItDownHandler):
    log.info(f"MarkItDownHandler: Testing file {file_path}")

    markdown_text = await markitdown_handler.handle(str(file_path))
