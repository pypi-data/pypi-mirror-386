...
from markitdown_pro.handlers.tabular_handler import TabularHandler
...
ALL_FILES = list_files(
    TEST_FILES_PATH, include_ext=TabularHandler.SUPPORTED_EXTENSIONS, recursive=True
)
log.info(f"TabularHandler: Found {len(ALL_FILES)} test files in {TEST_FILES_PATH}.")


@pytest.mark.asyncio
@pytest.mark.parametrize("file_path", ALL_FILES, ids=[pretty_id(p) for p in ALL_FILES])
async def test_files(file_path: Path, tabular_handler: TabularHandler):
    log.info(f"TabularHandler: Testing file {file_path}")

    markdown_text = await tabular_handler.handle(str(file_path))
