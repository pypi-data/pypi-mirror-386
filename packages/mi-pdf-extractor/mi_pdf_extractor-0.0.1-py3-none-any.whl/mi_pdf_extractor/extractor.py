...
def clean_text_spacing(text):
    """
    Removes irregular multiple spaces, redundant newlines,
    and normalizes spacing between lines.
    """
    text = re.sub(r'[ \t]+', ' ', text)               # collapse multiple spaces/tabs
    text = re.sub(r' *\n *', '\n', text)              # trim spaces around newlines
    text = re.sub(r'\n{2,}', '\n\n', text)            # limit consecutive newlines to 1 blank line
    text = text.strip()
    return text
...
def extract_text_from_pdf(pdf_path):
    ...
    try:
        ...
        full_text = clean_text_spacing(full_text)
