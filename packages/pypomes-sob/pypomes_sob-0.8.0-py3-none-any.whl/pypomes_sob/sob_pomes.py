...
class PySob:
    ...
    def get_values(attrs: list[str],
                   alias: str = None,
                   joins: list[tuple[Sob | str, str, str]] |
                          list[tuple[Sob | str, str, str, Literal["inner", "full", "left", "right"]]] = None,
                   where_clause: str = None,
                   where_vals: tuple = None,
                   where_data: dict[str, Any] = None,
        ...
        if not errors:
            ...
            # establish main query table
            query_tables: str = sob_db_specs[cls_name][0]
            if alias:
                query_tables += " " + alias

            # build joins
            for join in joins or []:
                target: Sob | str = join[0] if isinstance(join[0], str) else join[0].Db.Table
                mode: str = join[3].upper() if len(join) > 3 else "INNER"
                query_tables += f" {mode} JOIN {target} AS {join[1]} ON {join[2]}"

            # retrieve the data
            result = db_select(sel_stmt=f"SELECT DISTINCT {''.join(attrs)} FROM {query_tables}",
                               where_clause=where_clause,
                               where_vals=where_vals,
                               where_data=where_data,
    ...
    def retrieve(__references: type[Sob | list[Sob]] | list[type[Sob | list[Sob]]] = None,
        ...
        if not errors:
            ...
            # retrieve the data
            sel_stmt: str = f"SELECT DISTINCT {', '.join(attrs)} FROM {query_tables}"
