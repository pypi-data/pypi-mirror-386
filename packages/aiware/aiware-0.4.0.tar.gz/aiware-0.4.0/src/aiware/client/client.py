...
from aiware.client._stdlib_generated.client import _StdlibGeneratedAiware
...
from aiware.client.utils import catch_not_found
...
class Aiware(_StdlibGeneratedAiware):
    ...
    def search_media(self, request: SearchRequest) -> SliceSearchResult:
        ...
        response = self.http_client.post(self._search_path(""), json=data)
    ...
    def search_sdos(self, request: SearchSDOsRequest) -> SdoSliceSearchResult:
        return (self.search_media(request=cast(SearchRequest, cast(object, request)))).as_model(SdoSliceSearchResult)

    def vector_search(self, request: VectorSearchRequest) -> VectorSearchResults:
        data = request.model_dump(mode='json', exclude_unset=True)
        response = self.http_client.post(self._search_path("vector"), json=data)
        response.raise_for_status()
