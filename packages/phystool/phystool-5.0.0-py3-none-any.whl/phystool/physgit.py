...
class GitFile(ABC):
        ...
        for suffix, path in self._files.items():
            if path:
                match self.status:
                    ...
                    case self.Status.MODIFIED:
                        yield f"git diff {path} | delta {config.DELTA_THEME}"
                    case self.Status.DELETED:
                        yield f"git show HEAD:{path} | bat --color always -l {suffix}"
                    case _:
    ...
    def get_diff(self) -> str:
        ...
        for cmd in self._commands():
            tmp = subprocess.run(
                cmd, capture_output=True, text=True, shell=True, cwd=config.db.DB_DIR
            )
            if tmp.returncode == 0:
                out += tmp.stdout
            else:
...
class GitPDBFile(GitFile):
    ...
    def _get_title(self, uuid: str) -> str:
        ...
        return self._pdb_record_map[uuid]
...
class PhysGit:
    def __init__(self) -> None:
        ...
        self._staged: dict[GitFile.Status, tuple[list[GitFile], int]] = {
            status: ([], 0) for status in GitFile.Status
        }
    ...
    def commit(self, for_terminal: bool) -> str:
        ...
        for git_pdb_file in self._staged[GitFile.Status.UNTRACKED][0]:
            self._repo.index.add(git_pdb_file.get_files())
        for git_pdb_file in self._staged[GitFile.Status.MODIFIED][0]:
            self._repo.index.add(git_pdb_file.get_files())
        for git_pdb_file in self._staged[GitFile.Status.DELETED][0]:
