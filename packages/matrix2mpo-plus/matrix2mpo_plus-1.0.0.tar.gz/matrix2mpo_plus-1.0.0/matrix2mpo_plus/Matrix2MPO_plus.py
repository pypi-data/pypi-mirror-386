...
def load_svd_module():
    """Load svd_module from package directory"""
    try:
        # Try to load from package directory first
        package_dir = os.path.dirname(os.path.abspath(__file__))
        svd_module_path = os.path.join(package_dir, "libs", "svd_module.cpython-310-x86_64-linux-gnu.so")
        
        if os.path.exists(svd_module_path):
            spec = importlib.util.spec_from_file_location("svd_module", svd_module_path)
            svd_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(svd_module)
            return svd_module, True
        else:
            # Fallback to system path
            return None, False
    except Exception as e:
        print(f"Warning: Could not load svd_module: {e}")
        print("Using numpy.linalg.svd as fallback")
        return None, False

# Load svd_module
svd_module, HAS_SVD_MODULE = load_svd_module()

def compute_svd(matrix):
    """
    Compute SVD using available method (svd_module or numpy fallback)
    """
    if HAS_SVD_MODULE and svd_module is not None:
        return svd_module.computeSVD(matrix)
    else:
