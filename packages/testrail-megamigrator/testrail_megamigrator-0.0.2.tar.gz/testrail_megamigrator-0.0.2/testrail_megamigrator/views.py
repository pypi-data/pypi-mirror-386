...
class TestrailBackupDeleteView(DeleteView):
    ...
    success_url = reverse_lazy('plugins:testrail_megamigrator:backup-list')
...
class TestrailSettingsCreateView(CreateView):
    ...
    success_url = reverse_lazy('plugins:testrail_megamigrator:settings-list')
...
def redirect_index(request):
    return redirect(reverse('plugins:testrail_megamigrator:settings-list'))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            task = download_suites_task.delay(project_id, config_dict, download_attachments, backup_filename,
                                              testrail_suite_ids)
            return redirect(reverse('plugins:testrail_megamigrator:task_status', kwargs={'task_id': task.task_id}))
    return render(
