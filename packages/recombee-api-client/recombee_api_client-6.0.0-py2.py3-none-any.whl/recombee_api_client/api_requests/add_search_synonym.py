...
class AddSearchSynonym(Request):
    ...
    def __init__(self, term: str, synonym: str, one_way: bool = DEFAULT):
        super().__init__(
            path="/synonyms/items/", method="post", timeout=10000, ensure_https=False
        )
        self.term = term
    ...
    def get_body_parameters(self) -> dict:
        ...
        """
        p = dict()
        p["term"] = serialize_to_json(self.term)
        p["synonym"] = serialize_to_json(self.synonym)
        if self.one_way is not DEFAULT:
            p["oneWay"] = serialize_to_json(self.one_way)
