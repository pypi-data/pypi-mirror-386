...
class ListItems(Request):
    ...
    def __init__(
        ...
        super().__init__(
            path="/items/list/", method="get", timeout=100000, ensure_https=False
        )
        self.filter = filter
        self.count = count
        self.offset = offset
    ...
    def get_query_parameters(self) -> dict:
        ...
        if self.filter is not DEFAULT:
            params["filter"] = serialize_to_json(self.filter)
        ...
        if self.return_properties is not DEFAULT:
            params["returnProperties"] = serialize_to_json(self.return_properties)
        if self.included_properties is not DEFAULT:
            params["includedProperties"] = serialize_to_json(self.included_properties)
