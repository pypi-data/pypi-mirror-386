...
class RecommendItemsToItem(Request):
    ...
    * Allows the calculation of Actions and Conversions
    ...
    :param reql_expressions: A dictionary of [ReQL](https://docs.recombee.com/reql) expressions that will be executed for each recommended item.
    ...
    def __init__(
        ...
        super().__init__(
            path="/recomms/items/%s/items/" % (item_id),
            method="post",
            timeout=3000,
            ensure_https=False,
        )
    ...
    def get_body_parameters(self) -> dict:
        ...
        """
        p = dict()
        p["targetUserId"] = serialize_to_json(self.target_user_id)
        ...
        if self.user_impact is not DEFAULT:
            p["userImpact"] = serialize_to_json(self.user_impact)
        if self.diversity is not DEFAULT:
            p["diversity"] = serialize_to_json(self.diversity)
        if self.min_relevance is not DEFAULT:
            p["minRelevance"] = serialize_to_json(self.min_relevance)
        if self.rotation_rate is not DEFAULT:
            p["rotationRate"] = serialize_to_json(self.rotation_rate)
        if self.rotation_time is not DEFAULT:
            p["rotationTime"] = serialize_to_json(self.rotation_time)
