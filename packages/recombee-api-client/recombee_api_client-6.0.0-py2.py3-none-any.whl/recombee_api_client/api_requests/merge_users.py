...
class MergeUsers(Request):
    ...
    def __init__(
        self, target_user_id: str, source_user_id: str, cascade_create: bool = DEFAULT
    ):
        super().__init__(
            path="/users/%s/merge/%s" % (target_user_id, source_user_id),
            method="put",
            timeout=10000,
            ensure_https=False,
        )
    ...
    def get_query_parameters(self) -> dict:
        ...
        if self.cascade_create is not DEFAULT:
            params["cascadeCreate"] = serialize_to_json(self.cascade_create)
