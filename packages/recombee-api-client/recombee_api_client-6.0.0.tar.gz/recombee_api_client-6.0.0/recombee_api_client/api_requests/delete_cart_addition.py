...
class DeleteCartAddition(Request):
    ...
    def __init__(
        self, user_id: str, item_id: str, timestamp: Union[str, int, datetime] = DEFAULT
    ):
        super().__init__(
            path="/cartadditions/", method="delete", timeout=3000, ensure_https=False
        )
    ...
    def get_query_parameters(self) -> dict:
        ...
        if self.timestamp is not DEFAULT:
            params["timestamp"] = serialize_to_json(self.timestamp)
