...
### v0.6.1: Bug Fixes and API Restoration

**v0.6.1 restores missing APIs from v0.6.0** and includes important bug fixes:
...
**Migration**: If you encountered `AttributeError` for these functions in v0.6.0, upgrade to v0.6.1:
```bash
pip install --upgrade pixtreme>=0.6.1
```
...
| `imwrite(output_path, image, param, swap_rb)` | `output_path: str, image: cp.ndarray or np.ndarray, param: int = -1, swap_rb: bool = False` | `bool` | Write image with format-specific options |
