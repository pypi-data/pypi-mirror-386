...
class AuthModule:
    ...
    async def register(self, input_data: UserRegisterInput) -> User:
        ...
        user_data = cast(Dict[str, Any], response["register"])
    ...
    async def verify_user(self, verification_token: str) -> AuthResponse:
        ...
        verify_data = cast(Dict[str, Any], response["verifyUser"])

        # Set tokens after successful verification
        if verify_data["token"]:
            # Backend omits refresh token in this response
            self.client.set_tokens(
                access_token=verify_data["token"],
                refresh_token="",
            )

        user_data = cast(Dict[str, Any], verify_data["user"])
        user = User(
            id=user_data["id"],
            email=user_data["email"],
            name=user_data["name"],
    ...
    async def forgot_password(self, email: str) -> ForgotPasswordResponse:
        ...
        return ForgotPasswordResponse(
            success=response["forgotPassword"]["success"],
        )
    ...
    async def reset_password(
        ...
        return ResetPasswordResponse(
            success=response["resetPassword"]["success"],
        )
    ...
    async def change_password(self, id: str, new_password: str) -> User:
        ...
        user_data = cast(Dict[str, Any], response["changePassword"])
    ...
    async def switch_workspace(self, workspace_id: str) -> str:
        ...
        switch_workspace = response["switchWorkspace"]

        return cast(str, switch_workspace)
    ...
    async def switch_project(self, proj_id: str) -> str:
        ...
        switch_project = response["switchProject"]

        return cast(str, switch_project)
