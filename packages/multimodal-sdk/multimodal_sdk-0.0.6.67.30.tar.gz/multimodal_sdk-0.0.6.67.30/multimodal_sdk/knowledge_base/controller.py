...
async def update_ingestion_func(
    ...
    **kwargs
    ...
    form_data = aiohttp.FormData()
    for ns, rid, data in zip(namespaces, record_ids, updated_data):
        form_data.add_field("namespace", ns)
        form_data.add_field("record_id", rid)
        # If key is text - then Formadata Text field , key: text value: actual text
        # If key is data - then Formadata File field , key: data value: actual file
        for key, value in data.items():
            if key == "data":
                if os.path.isfile(value):
                    file = open(value, 'rb')
                    form_data.add_field('data', file, filename=os.path.basename(value))
                else:
                    raise ValueError(f"File not found: {value}")
            else:
                form_data.add_field(key, str(value))
    
    logger.info("Form data created successfully for update_ingestion_func")
