...
class Canvas:
    ...
    ) -> Iterable[Iterable[CanvasChar]]:
        return (self.get_row(y + dy) for dy in range(height))
    ...
    def set_area(
        self, x: int, y: int, width: int, height: int, value: CanvasChar, /
    ) -> None:  # TODO: optimize
    ...
    def draw_line_h(
        self, x: int, y: int, length: int, char: CanvasChar, /
    ) -> None:
    ...
    def draw_rect(
        self, x: int, y: int, width: int, height: int, char: CanvasChar, /
    ) -> None:
        # Bottom and top
