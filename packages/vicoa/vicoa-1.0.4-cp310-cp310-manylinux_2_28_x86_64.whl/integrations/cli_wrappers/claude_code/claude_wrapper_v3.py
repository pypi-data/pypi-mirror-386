    ...
    def _extract_permission_prompt(
        ...
        if is_ask_user_question:
            ...
            if enter_to_select_idx is None:
                ...
                for i in range(enter_to_select_idx - 1, max(0, enter_to_select_idx - 50), -1):
                ...
                if first_option_idx is None:
                    ...
                    for i in range(first_option_idx - 1, search_start - 1, -1):
                        ...
                        line_clean = line_clean.replace("\u2502", "")  # Remove box border
                        line_clean = line_clean.replace("\u276f", "")  # Remove arrow
                        line_clean = line_clean.replace("❯", "")  # Remove selection marker
                        line_clean = line_clean.strip()

                        # Skip empty lines, navigation hints, header UI elements, and option numbers
                        if (
                            line_clean
                            and not line_clean.startswith(("1.", "2.", "3.", "4.", "5.", ">", ">>"))
                            ...
                            and "Try " not in line_clean  # Skip suggestion lines like "Try ..."
                            and not re.match(r"^[←☐✔→\s]+$", line_clean)  # Skip pure UI elements
                            and not re.match(r"^[←☐✔→].*[←☐✔→]$", line_clean)  # Skip header lines with UI on both ends
                            and len(line_clean) > 10  # Skip very short lines
                            and "?" in line_clean  # Questions should contain a question mark
                            # Check that line contains mostly printable characters (not just escape sequences)
                            and len([c for c in line_clean if c.isprintable()]) > len(line_clean) * 0.7
                        ):
        ...
        if is_ask_user_question:
            ...
            if enter_to_select_idx is None:
                self.log("[WARNING] Could not find 'Enter to select' for options parsing")
            else:
                ...
                for i in range(search_start, enter_to_select_idx):
                    ...
                    clean_line = clean_line.replace("\u276f", "").strip()  # Remove arrow
                    clean_line = clean_line.replace("❯", "").strip()  # Remove selection marker
            ...
            if option_starts:
                ...
                for i in range(start_line, min(start_line + 20, len(lines))):
                    ...
                    if match:
                        ...
                        if "Enter to select" in clean_line or "Tab/Arrow keys" in clean_line:
                            break
                    ...
                    elif "Enter to select" in clean_line or "Tab/Arrow keys" in clean_line:
                        ...
                        if (
                            ...
                            if not hasattr(self, "_permission_handled"):
                                ...
                                with self.send_message_lock:
                                    ...
                                    has_valid_question = question and len(question) > 10 and "?" in question
                                    has_valid_options = (
                                        len(options) >= 2 and
                                        all(opt.strip() for opt in options)  # All options are non-empty
                                    )

                                    if not has_valid_question:
                                        self.log(f"[WARNING] Invalid question extracted, skipping prompt")
                                        question = "Waiting for your input..."
                                        options = []
                                    elif not has_valid_options:
                                        self.log(f"[WARNING] Invalid options extracted, skipping prompt")
                                        options = []
