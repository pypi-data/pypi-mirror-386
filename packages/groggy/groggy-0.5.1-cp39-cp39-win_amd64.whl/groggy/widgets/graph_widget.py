...
try:
    ...
    from traitlets import Unicode, Dict, List, Int, Bool, Float, observe, validate
...
from typing import Callable, Optional, Any, Dict as TypeDict, List as TypeList
import json
...
class GroggyGraphWidget(widgets.DOMWidget):
                ...
                for node_id in self.data_source.node_ids:
                    ...
                    try:
                        if hasattr(self.data_source, 'get_node_attr'):
                            attrs = {
                                'label': self.data_source.get_node_attr(node_id, 'label'),
                                'color': self.data_source.get_node_attr(node_id, 'color'), 
                    ...
                    try:
                        ...
                        try:
                            if hasattr(self.data_source, 'get_edge_attr'):
                                attrs = {
                                    'weight': self.data_source.get_edge_attr(edge_id, 'weight'),
                                    'label': self.data_source.get_edge_attr(edge_id, 'label'),
                                    'type': self.data_source.get_edge_attr(edge_id, 'type')
                                }
                                attrs = {k: v for k, v in attrs.items() if v is not None}
                        except:
                            pass
    ...
    def __repr__(self):
        ...
        return (f"GroggyGraphWidget(nodes={stats['node_count']}, "
