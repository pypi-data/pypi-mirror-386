...
class Asymmetry:
    ...
    def plot_asymmetry_diagnostics(self, method='conselice'):
        ...
        def plot_set(fig, axs, data, center, label):
            mask = self.segmentation.astype(bool)
    
            # Recenter image + mask to the chosen center
            ...
            # Build rotated + residual
            rotated = np.rot90(data_recentered, 2)
            residual = data_recentered - rotated
    
            # Panels data
            data_masked = np.where(mask_recentered & (data_recentered > 0), data_recentered, np.nan)  # for log
            rot_masked  = np.where(mask_recentered & (rotated > 0),        rotated,        np.nan)   # for log
            res_masked  = np.where(mask_recentered, residual, np.nan)                                # signed
    
            # Colormaps
            cmap_log = plt.get_cmap('gray_r').copy()
            cmap_log.set_bad('white', 1.0)
    
            rcmap = plt.get_cmap('RdBu_r').copy()
            rcmap.set_bad('white', 1.0)
    
            
            finite_res = np.isfinite(res_masked)
            # robust symmetric limits for residuals
            p95 = np.nanpercentile(np.abs(res_masked[finite_res]), 95)
            rnorm = TwoSlopeNorm(vcenter=0.0, vmin=-p95, vmax=+p95)
    
            # Plot
            m, s = np.nanmedian(data_masked), np.nanstd(data_masked)
            im0 = axs[0].imshow(data_masked, origin='lower', cmap=cmap_log, vmin = m-s, vmax = m+2*s)
            axs[0].set_title(f"{label}: Original", fontsize=20)
            axs[0].tick_params(labelsize=16, direction="in", length=7)
        
            im1 = axs[1].imshow(rot_masked, origin='lower', cmap=cmap_log, vmin = m-s, vmax = m+2*s)
            axs[1].set_title(f"{label}: Rotated", fontsize=20)
            axs[1].tick_params(labelsize=16, direction="in", length=7)
        
            im2 = axs[2].imshow(res_masked, origin='lower', cmap=rcmap, norm=rnorm)
            axs[2].set_title(f"{label}: Residual", fontsize=20)
            axs[2].tick_params(labelsize=16, direction="in", length=7)
        
            # Scatter (finite pixels only)
            orig_pixels = np.where(mask_recentered, data_recentered, np.nan)
            rot_pixels  = np.where(mask_recentered, rotated,        np.nan)
            good = np.isfinite(orig_pixels) & np.isfinite(rot_pixels)
    
            x = orig_pixels[good].ravel()
            y = rot_pixels[good].ravel()
            axs[3].scatter(x, y, s=8, alpha=0.5)
            # 1:1 line over valid range
            lo, hi = np.min([x.min(), y.min()]), np.max([x.max(), y.max()])
            axs[3].plot([lo, hi], [lo, hi], linestyle='--', linewidth=2)
            ...
            # Colorbars: one shared for panels 0 & 1, one for residual
            fig.colorbar(im0, ax=axs[:2], location='right', shrink = 0.8)
            fig.colorbar(im2, ax=axs[2],  location='right', shrink = 0.8)
    
        # Figure + axes
        fig, axs = plt.subplots(2, 2, figsize=(12, 12))
        axs = axs.flatten()
    
        plot_set(fig, axs, self.image, center_gal, "Galaxy")
...
class Smoothness:
    ...
    def plot_smoothness_diagnostics(self):
        ...
        # 1) Build masked arrays for plotting
        #    - For log-stretched panels, also drop nonpositive pixels
        data_masked     = np.where(mask, I,   np.nan)
        smoothed_masked = np.where(mask, I_s, np.nan)
        residual_masked = np.where(mask, I - I_s, np.nan)  # keep sign; no log here

        # 2) Colormaps (show NaNs as white)
        cmap_log = plt.get_cmap('gray_r').copy()
        cmap_log.set_bad('white', 1.0)

        cmap_div = plt.get_cmap('RdBu_r').copy()
        cmap_div.set_bad('white', 1.0)

        finite = np.isfinite(residual_masked)
        p95 = np.nanpercentile(np.abs(residual_masked[finite]), 95)
        norm_res = TwoSlopeNorm(vcenter=0.0, vmin=-p95, vmax=+p95)
        ...
        im1 = axs[1].imshow(smoothed_masked, origin='lower', cmap=cmap_log, vmin = m-s, vmax = m+2*s)
        axs[1].set_title("Galaxy: Smoothed", fontsize=20)
        axs[1].tick_params(labelsize=16, direction="in", length=7)
        
        im2 = axs[2].imshow(residual_masked, origin='lower', cmap=cmap_div, norm=norm_res)
        axs[2].set_title("Galaxy: Residual", fontsize=20)
        axs[2].tick_params(labelsize=16, direction="in", length=7)
        
        orig_pixels = data_masked[mask].ravel()
        smooth_pixels = smoothed_masked[mask].ravel()
        good = np.isfinite(orig_pixels) & np.isfinite(smooth_pixels)
        axs[3].scatter(orig_pixels[good], smooth_pixels[good], s=8, alpha=0.5, color="b")
        axs[3].plot([orig_pixels[good].min(), orig_pixels[good].max()],
                    [orig_pixels[good].min(), orig_pixels[good].max()],
        ...
        # 5) Colorbars
        #    One shared bar for Original+Smoothed (same norm), one for Residual
        fig.colorbar(im0, ax=axs[:2], location='right', shrink = 0.8)
        fig.colorbar(im2, ax=axs[2], location='right', shrink = 0.8)
