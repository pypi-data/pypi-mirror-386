...
@register_keras_serializable(package="Cb_FloodDy", name="ExtractLastMask")
class ExtractLastMask(Layer):
    """(B, T, H, W, 1) -> (B, H, W, 1)"""
    def call(self, t):
        return t[:, -1, ...]  # last timestep
    def get_config(self):
        return {**super().get_config()}
    

@register_keras_serializable(package="Cb_FloodDy", name="MergeStations")
class MergeStations(Layer):
    """(B, T, S) -> (B*S, T, 1), to run a shared per-station LSTM stack."""
    def call(self, wl):
        # wl: (B, T, S)
        wl_t = tf.transpose(wl, [0, 2, 1])      # (B, S, T)
        shape = tf.shape(wl_t)
        b, s, t = shape[0], shape[1], shape[2]
        return tf.reshape(wl_t, [b * s, t, 1])  # (B*S, T, 1)
    def get_config(self):
        return {**super().get_config()}


@register_keras_serializable(package="Cb_FloodDy", name="UnmergeStations")
class UnmergeStations(Layer):
    """(B*S, F) -> (B, S, F), inverts MergeStations after attention."""
    def __init__(self, num_stations, **kwargs):
        super().__init__(**kwargs)
        self.num_stations = int(num_stations)

    def get_config(self):
        return {**super().get_config(), "num_stations": self.num_stations}
    ...
    def call(self, x):
        ...
        s = self.num_stations
        b = bs // s
        return tf.reshape(x, [b, s, f])  # (B, S, F)
...
def build_model_with_cbam_weighted(
    ...
    last_mask = ExtractLastMask(name='Extract_Last_Mask')(mask_input)
    ...
    reshaped_water_level = MergeStations(name='Reshape_Water_Level')(water_level_input)
    ...
    # --- un-merge stations AFTER attention to (B, S, F) ---
    attention_output = UnmergeStations(num_stations, name='Reshape_Attention_Output')(attention_output)
    ...
    # no Cast_Output needed; you already set global policy to float32
    ...
    def on_train_end(self, logs=None):
        if self.best_epoch is not None:
            ...
            try:
                ...
                # Use the in-memory model which already holds best weights (EarlyStopping restore_best_weights=True)
                best_model = self.model
                
                # Build a submodel to output temporal attention weights
                attention_layer = best_model.get_layer('Temporal_Attention')   # returns [context, weights]
                attention_weights_output = attention_layer.output[1]
                temporal_attention_model = Model(inputs=best_model.inputs,
                ...
                # Reshape to (num_sequences, num_stations, sequence_length)
                ...
                temporal_attention_weights = temporal_attention_weights.reshape(
                    (num_sequences, num_stations, sequence_length)
                )
                
                ckpt_dir = os.path.dirname(self.filepath)
                os.makedirs(ckpt_dir, exist_ok=True)
                np.save(os.path.join(ckpt_dir, 'best_temporal_attention_weights.npy'),
...
def run_optimization(
    ...
    def objective(trial: optuna.Trial):
        ...
        if visualize:
            ...
            # Use the trained model in memory; EarlyStopping has restored best weights
            best_model = model
