    ...
    def on_change(path: Path):
        ...
        try:
            ...
            if p.endswith(".py") or p.endswith("__manifest__.py"):
                ...
                # 1) Hard check in container process list
                container = cfg.get("runtime", "container") or ""
                if _container_has_dev_all(container):
                    info("[opm] Detected --dev=all in container → skip quick_upgrade; safe reload only")
                    try:
                ...
                # 2) Soft check via RPC param (and handle 502 safely)
                try:
                    mode = rpc.call("ir.config_parameter", "get_param", "dev_mode")
                    if mode and "all" in str(mode).lower():
                        info("[opm] Detected dev_mode=all via RPC → skip quick_upgrade; safe reload only")
                        try:
                            broadcast(f"reload:{target_module}")
                        except Exception:
                            pass
                ...
                # 3) Normal path → perform quick upgrade, then reload
