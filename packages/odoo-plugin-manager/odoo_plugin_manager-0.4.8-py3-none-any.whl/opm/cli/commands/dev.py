...
def safe_rpc_call(rpc: OdooRPC, model: str, method: str, *args):
    """
    RPC call wrapper that gracefully handles transient 502/504 proxy windows.
    If a 502/504 happens (Odoo reloading behind nginx/proxy), we log and return None.
    """
    try:
...
class _WSBus:
    ...
    async def handler(self, ws):
        ...
        self.clients.add(ws)
        try:
            # Relay any inbound messages to all clients (incl. sender)
            async for msg in ws:
                try:
    ...
    async def broadcast(self, msg: str):
        """Send message to all clients (skip closed) and log count (throttled when empty)."""
        if not self.clients:
            now = time.time()
...
# 🧪 Helpers (optional)
# ----------------------------------------------------------------------
def _container_has_dev_all(container: str) -> bool:
    """
    Detect --dev=all from container process list (robust, no RPC).
    Currently unused, but handy if you want to branch behavior by dev-all.
    """
    if not container:
        return False
    try:
    ...
    def on_change(path: Path):
        ...
        try:
            ...
            if p.endswith(".py") or p.endswith("__manifest__.py"):
                ...
                # Quick health check: if Odoo is reloading, wait a bit then recheck
                try:
                    safe_rpc_call(rpc, "ir.config_parameter", "get_param", "web.base.url")
                except Exception:
                    info("[opm] Odoo seems to be reloading; waiting 3s...")
                    time.sleep(3)
                    try:
                        safe_rpc_call(rpc, "ir.config_parameter", "get_param", "web.base.url")
                    except Exception:
                        info("[opm] Odoo still unreachable → broadcasting WS reload only.")
                        try:
                ...
                # If reachable, attempt quick upgrade (safe_rpc_call handles 502/504)
                try:
                    info(f"[opm] Quick upgrade for {target_module}")
                    safe_rpc_call(rpc, "opm.dev.tools", "quick_upgrade", target_module)
                    try:
                        broadcast(f"reload:{target_module}")
                    except Exception:
