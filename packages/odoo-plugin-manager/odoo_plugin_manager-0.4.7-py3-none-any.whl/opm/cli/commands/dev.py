    ...
    def on_change(path: Path):
        ...
        try:
            ...
            if p.endswith(".py") or p.endswith("__manifest__.py"):
                ...
                # Hızlı RPC check — eğer Odoo reload ediyorsa, bekle
                try:
                    rpc.call("ir.config_parameter", "get_param", "web.base.url")
                except Exception:
                    info("[opm] Odoo reload sırasında, 3sn bekleniyor...")
                    time.sleep(3)
                    try:
                        rpc.call("ir.config_parameter", "get_param", "web.base.url")
                    except Exception:
                        info("[opm] Odoo hala erişilemez → sadece WS reload yapılıyor.")
                        broadcast(f"reload:{target_module}")
                        return

                # Erişilebiliyorsa upgrade yap
                try:
