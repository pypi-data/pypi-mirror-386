    ...
    def on_change(path: Path):
        ...
        try:
            ...
            if p.endswith(".py") or p.endswith("__manifest__.py"):
                ...
                # Always skip RPC when Odoo may reload (dev=all or not)
                info(f"[opm] Python/manifest changed: {p} → using CLI module upgrade")

                # Direct CLI upgrade instead of RPC (safe under dev=all)
                try:
                    code, out, err = run([
                        "bash", "-lc",
                        f"odoo-bin -u {shlex.quote(target_module)} --stop-after-init || true"
                    ])
                    info(f"[opm] CLI upgrade output: {out.strip() or err.strip() or code}")
                except Exception as e:
                    info(f"[opm] CLI upgrade failed: {e}")

                # Always reload frontend (safe)
                try:
