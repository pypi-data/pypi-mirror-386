    ...
    def on_change(path: Path):
        ...
        try:
            ...
            if p.endswith(".py") or p.endswith("__manifest__.py"):
                ...
                is_dev_all = False
                try:
                    # Try to detect if Odoo is running with --dev=all
                    ...
                    # If Odoo is reloading, skip RPC to avoid 502
                    info("[opm] Odoo likely reloading → skipping RPC, safe browser reload")
                    try:
                ...
                if is_dev_all:
                    info(f"[opm] Detected Odoo dev=all → skip quick_upgrade RPC")
                    try:
                        broadcast(f"reload:{target_module}")
                    except Exception:
                        pass
                ...
                # Normal mode → perform quick upgrade
