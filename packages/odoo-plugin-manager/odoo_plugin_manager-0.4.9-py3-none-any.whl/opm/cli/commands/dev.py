...
# ----------------------------------------------------------------------
# 🌡️ dev=all detection (no RPC; works during reload windows)
# ----------------------------------------------------------------------
def _detect_dev_all_in_container(container: str) -> bool:
    """
    Detect if Odoo inside a Docker container was started with --dev=all.
    This avoids RPC, so it also works while Odoo is reloading.
    """
    if not container:
        return False
    try:
        cmd = (
            ...
            f"\"ps -eo args | grep -E '(^| )odoo(-bin)?( |$).*--dev=all' | grep -v grep || true\""
        )
        code, out, _ = run(["bash", "-lc", cmd])
...
def _detect_dev_all_on_host() -> bool:
    """
    Detect if a host-run Odoo process has --dev=all in its command line.
    Linux/macOS via ps; harmless on Windows (returns False).
    """
    try:
        cmd = "ps -eo args | grep -E '(^| )odoo(-bin)?( |$).*--dev=all' | grep -v grep || true"
        code, out, _ = run(["bash", "-lc", cmd])
        return (code == 0) and ("--dev=all" in (out or ""))
    except Exception:
        return False
...
class _WSBus:
    ...
    async def handler(self, ws):
        ...
        try:
            ...
            async for msg in ws:
                try:
                    # Decode text or binary frames uniformly
    ...
    async def broadcast(self, msg: str):
        """Send a message to all clients (skip closed) and log the count."""
        if not self.clients:
            now = time.time()
...
def _start_ws_server(host: str, port: int, ping_interval: int = 30, ping_timeout: int = 60):
    """
    Start a lightweight WebSocket server in a background thread.
...
def dev(
    ...
    # Detect dev=all once at startup (Docker-aware, no RPC)
    ...
    info(f"[opm] Detected dev=all: {'YES' if DEV_ALL else 'NO'}")
    ...
    def on_change(path: Path):
        ...
        lower = p.lower()
        try:
            # XML: try smart heuristic; broadcast in all cases
            if p.endswith(".xml"):
                needs_upgrade = ("menu" in lower) or ("/data/" in lower)
                if needs_upgrade and not DEV_ALL:
            ...
            if p.endswith(".py") or p.endswith("__manifest__.py"):
                target_module = module or Path(p).parts[-2]
                if DEV_ALL:
                    # Under dev=all, let Odoo's own reloader do its job; avoid RPC to prevent 502.
                ...
                # Not dev=all: do a quick upgrade + reload
            ...
            # Defensive guard against unexpected watcher errors
