...
def dev(
    ...
    # on_change handler with XML menu/data heuristic +
    # - XML (menu/data): quick upgrade + broadcast
    # - XML (views): flush caches + broadcast
    # - assets (.scss/.js): flush caches + broadcast
    # - python/manifest: browser reload only (no RPC), to avoid 502 during crashes
    def on_change(path: Path):
        ...
        try:
            ...
            if p.endswith(".py") or p.endswith("__manifest__.py"):
                # Do NOT call RPC on Python/manifest changes — Odoo may be reloading/crashing.
                info(f"[opm] Python/manifest changed: {p} → browser reload only (no RPC)")
                try:
                    broadcast("reload")
                except Exception:
                    pass
