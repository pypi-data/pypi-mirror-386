...
class PublishCommand(Command):
    ...
    def _increment_version(self) -> None:
        ...
        if python_version_file.exists():
            ...
            if match:
                ...
                log_info(f"Updated Python version from {major}.{minor}.{patch} to {new_version}")
            else:
        ...
        if rust_version_file.exists():
            ...
            if match:
                ...
                if len(version_parts) == 3:
                    ...
                    log_info(f"Updated Rust version from {current_version} to {new_version}")
                else:
                    raise CommandError("Invalid version format in Cargo.toml")
            else:
                raise CommandError("Could not find version in Cargo.toml [package] section")
    ...
    def _upload_to_pypi(self, repository: str, dist_files: list[Path], username: str = None, password: str = None) -> None:
        ...
        if username and password:
            ...
            log_info(
                "No credentials provided, twine will prompt for them"
            )
