...
class Neo4jClient(BaseGRPCClient):
    ...
    def health_check(self) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"✅ [Neo4j] Healthy: {response.healthy}")
            print(f"   Version: {response.version}")
            print(f"   Edition: {response.edition}")
    ...
    def run_cypher(self, cypher: str, params: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Neo4j] Query returned {len(records)} records")
                return records
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def create_node(self, labels: List[str], properties: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Neo4j] Node created with ID: {node_id}")
    ...
    def get_node(self, node_id: int, database: str = 'neo4j') -> Optional[Dict]:
        ...
        try:
            ...
            if response.metadata.success and response.found:
                ...
                print(f"✅ [Neo4j] Node found: {node['labels']}")
                return node
            elif not response.found:
                print(f"ℹ️  [Neo4j] Node not found: {node_id}")
                return None
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def update_node(self, node_id: int, properties: Dict[str, Any],
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Neo4j] Node {node_id} updated")
                return True
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def delete_node(self, node_id: int, detach: bool = False,
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Neo4j] Node {node_id} deleted")
                return True
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def create_relationship(self, start_node_id: int, end_node_id: int, rel_type: str,
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Neo4j] Relationship created: ({start_node_id})-[:{rel_type}]->({end_node_id})")
                return rel_id
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def get_relationship(self, rel_id: int, database: str = 'neo4j') -> Optional[Dict]:
        ...
        try:
            ...
            if response.metadata.success and response.found:
                ...
                print(f"✅ [Neo4j] Relationship found: {rel['type']}")
                return rel
            elif not response.found:
                print(f"ℹ️  [Neo4j] Relationship not found: {rel_id}")
                return None
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def delete_relationship(self, rel_id: int, database: str = 'neo4j') -> Optional[bool]:
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Neo4j] Relationship {rel_id} deleted")
                return True
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def find_nodes(self, labels: Optional[List[str]] = None, properties: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Neo4j] Found {len(nodes)} nodes")
                return nodes
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def get_path(self, start_node_id: int, end_node_id: int, max_depth: int = 5,
        ...
        try:
            ...
            if response.metadata.success and response.found:
                ...
                print(f"✅ [Neo4j] Path found with {response.path.length} hops")
                return path
            elif not response.found:
                print(f"ℹ️  [Neo4j] No path found between nodes")
                return None
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def shortest_path(self, start_node_id: int, end_node_id: int, max_depth: int = 5,
        ...
        try:
            ...
            if response.metadata.success and response.found:
                ...
                print(f"✅ [Neo4j] Shortest path: {response.path.length} hops")
                return path
            elif not response.found:
                print(f"ℹ️  [Neo4j] No path found")
                return None
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def pagerank(self, database: str = 'neo4j') -> Optional[List[Dict]]:
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Neo4j] PageRank completed: {len(results)} nodes")
                return results
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
    ...
    def betweenness_centrality(self, database: str = 'neo4j') -> Optional[List[Dict]]:
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Neo4j] Betweenness Centrality completed: {len(results)} nodes")
                return results
            else:
                print(f"⚠️  [Neo4j] {response.metadata.message}")
                return None
