...
class PostgresClient(BaseGRPCClient):
    ...
    def health_check(self, detailed: bool = True) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"✅ [PostgreSQL] Status: {response.status}")
    ...
    def query(self, sql: str, params: Optional[List[Any]] = None, schema: str = 'public') -> Optional[List[Dict]]:
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [PostgreSQL] Query returned {response.row_count} rows")
                return rows
            else:
                print(f"⚠️  [PostgreSQL] {response.metadata.message}")
                return None
    ...
    def query_row(self, sql: str, params: Optional[List[Any]] = None, schema: str = 'public') -> Optional[Dict]:
        ...
        try:
            ...
            if response.metadata.success and response.found:
                ...
                print(f"ℹ️  [PostgreSQL] No row found")
                return None
            else:
                print(f"⚠️  [PostgreSQL] {response.metadata.message}")
                return None
    ...
    def execute(self, sql: str, params: Optional[List[Any]] = None, schema: str = 'public') -> Optional[int]:
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [PostgreSQL] Executed: {response.rows_affected} rows affected")
    ...
    def execute_batch(self, operations: List[Dict[str, Any]], schema: str = 'public') -> Optional[Dict]:
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [PostgreSQL] Batch executed: {response.total_rows_affected} total rows affected")
                return {
    ...
    def select_from(self, table: str, columns: Optional[List[str]] = None, where: Optional[List[Dict]] = None,
        ...
        try:
            ...
            if response.metadata.success:
                rows = [dict(row) for row in response.rows]
                print(f"✅ [PostgreSQL] SELECT returned {response.row_count} rows from {table}")
                return rows
            else:
                print(f"⚠️  [PostgreSQL] {response.metadata.message}")
                return None
    ...
    def insert_into(self, table: str, rows: List[Dict], returning: bool = False,
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [PostgreSQL] Inserted {response.rows_inserted} rows into {table}")
    ...
    def list_tables(self, schema: str = 'public') -> List[str]:
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [PostgreSQL] Found {len(response.tables)} tables in schema '{schema}'")
    ...
    def table_exists(self, table: str, schema: str = 'public') -> bool:
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"ℹ️  [PostgreSQL] Table '{table}' {status}")
                return exists
            else:
                print(f"⚠️  [PostgreSQL] {response.metadata.message}")
                return False
    ...
    def get_stats(self) -> Optional[Dict]:
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [PostgreSQL] Retrieved statistics")
                print(f"   Pool: {stats['pool']['active_connections']}/{stats['pool']['max_connections']} active")
                return stats
            else:
                print(f"⚠️  [PostgreSQL] {response.metadata.message}")
                return None
