...
class QdrantClient(BaseGRPCClient):
    ...
    def health_check(self) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"✅ [Qdrant] Healthy: {response.healthy}")
    ...
    def create_collection(self, collection_name: str, vector_size: int,
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Qdrant] Collection '{collection_name}' created (dim={vector_size})")
                return True
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def list_collections(self) -> List[str]:
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Qdrant] Found {len(collections)} collections")
                return collections
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return []
    ...
    def upsert_points(self, collection_name: str, points: List[Dict[str, Any]]) -> Optional[str]:
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Qdrant] Upserted {len(points)} points to '{collection_name}'")
    ...
    def delete_points(self, collection_name: str, ids: List[Any]) -> Optional[str]:
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Qdrant] Deleted {len(ids)} points from '{collection_name}'")
                return response.operation_id
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def count_points(self, collection_name: str) -> Optional[int]:
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Qdrant] Collection '{collection_name}' has {count} points")
                return count
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def get_collection_info(self, collection_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"   Points: {info['points_count']}, Segments: {info['segments_count']}")
                return info
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def search_with_filter(self, collection_name: str, vector: List[float],
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Qdrant] Search with filter returned {len(results)} results")
                return results
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def scroll(self, collection_name: str, filter_conditions: Optional[Dict] = None,
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Qdrant] Scrolled {len(points)} points")
    ...
    def recommend(self, collection_name: str, positive: List[Any], negative: List[Any] = None,
        ...
        try:
            ...
            if response.metadata.success:
                results = self._parse_scored_points(response.result, with_payload, with_vectors)
                print(f"✅ [Qdrant] Recommend returned {len(results)} results")
                return results
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def update_payload(self, collection_name: str, ids: List[Any],
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Qdrant] Updated payload for {len(ids)} points")
                return response.operation_id
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def delete_payload_fields(self, collection_name: str, ids: List[Any],
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Qdrant] Deleted {len(keys)} payload fields from {len(ids)} points")
                return response.operation_id
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def clear_payload(self, collection_name: str, ids: List[Any]) -> Optional[str]:
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Qdrant] Cleared payload from {len(ids)} points")
                return response.operation_id
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def create_field_index(self, collection_name: str, field_name: str,
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Qdrant] Created {field_type} index on '{field_name}'")
                return response.operation_id
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def delete_field_index(self, collection_name: str, field_name: str) -> Optional[str]:
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Qdrant] Deleted index on '{field_name}'")
                return response.operation_id
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def create_snapshot(self, collection_name: str) -> Optional[str]:
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Qdrant] Created snapshot: {response.snapshot_name}")
    ...
    def list_snapshots(self, collection_name: str) -> Optional[List[Dict]]:
        ...
        try:
            ...
            if response.metadata.success:
                ...
                print(f"✅ [Qdrant] Found {len(snapshots)} snapshots")
                return snapshots
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
    ...
    def delete_snapshot(self, collection_name: str, snapshot_name: str) -> Optional[bool]:
        ...
        try:
            ...
            if response.metadata.success:
                print(f"✅ [Qdrant] Deleted snapshot: {snapshot_name}")
                return True
            else:
                print(f"⚠️  [Qdrant] {response.metadata.message}")
                return None
