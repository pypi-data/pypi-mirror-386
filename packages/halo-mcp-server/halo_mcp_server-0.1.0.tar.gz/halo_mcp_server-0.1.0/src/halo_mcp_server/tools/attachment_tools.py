...
async def list_attachments(
    ...
    try:
        ...
        response = await client.get("/apis/api.console.halo.run/v1alpha1/attachments", params=params)
        return response
...
async def upload_attachment_tool(client: HaloClient, args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        result = await upload_attachment(file_path=file_path, policy_name=policy_name, group_name=group_name)
...
async def upload_attachment_from_url_tool(client: HaloClient, args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        result = await upload_attachment_from_url(url=url, policy_name=policy_name, group_name=group_name)
