...
def _validate_post_params(args: Dict[str, Any], required_fields: list = None) -> ToolResult:
    ...
    if slug and not re.match(r'^[a-z0-9-]+$', slug):
        return ToolResult.error_result("Error: Invalid slug format (only lowercase letters, numbers, and hyphens allowed)")
    
    # Validate cover URL format
    cover = args.get("cover")
    if cover and not cover.startswith(('http://', 'https://')):
        return ToolResult.error_result("Error: Cover must be a valid URL starting with http:// or https://")
    ...
    try:
        ...
        for item in posts:
            ...
            formatted_post = {
                ...
                "slug": spec.get("slug", ""),
                "excerpt": spec.get("excerpt", {}).get("raw", "") if isinstance(spec.get("excerpt"), dict) else spec.get("excerpt", ""),
                ...
                "is_published": metadata.get("labels", {}).get("content.halo.run/published") == "true"
            }
...
async def update_post_tool(client: HaloClient, args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if "content" in args:
            # raw 存储 Markdown 原文，content 存储转换后的 HTML
            post["metadata"]["annotations"]["content.halo.run/content-json"] = json.dumps(
                {
                    "raw": args["content"],
                    "content": markdown_to_html(args["content"]),
                    "rawType": "HTML",
                },
                ensure_ascii=False
            )

        logger.debug(f"Updating post: {name}")
...
async def publish_post_tool(client: HaloClient, args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        success_result = ToolResult.success_result(
            f"✓ Post '{name}' published successfully!",
            data={"post_name": name, "published": True}
        )
        return success_result.model_dump_json()
...
async def delete_post_tool(client: HaloClient, args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        success_result = ToolResult.success_result(
            f"✓ Post '{name}' deleted successfully!",
            data={"post_name": name, "deleted": True}
        )
        return success_result.model_dump_json()
...
async def update_post_draft_tool(client: HaloClient, args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        await client.update_post_draft(name, content)
