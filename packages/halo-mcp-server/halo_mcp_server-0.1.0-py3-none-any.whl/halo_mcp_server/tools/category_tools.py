...
async def create_category(
    ...
    try:
        ...
        response = await client.post("/apis/content.halo.run/v1alpha1/categories", json=category_data)
        return response
...
async def update_category(
    ...
    try:
        ...
        if prevent_parent_post_cascade_query is not None:
            existing_category["spec"]["preventParentPostCascadeQuery"] = prevent_parent_post_cascade_query
...
async def create_category_tool(client: HaloClient, args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if len(display_name) > 100:
            error_result = ToolResult.error_result("Error: Display name too long (max 100 characters)")
            return error_result.model_dump_json()
