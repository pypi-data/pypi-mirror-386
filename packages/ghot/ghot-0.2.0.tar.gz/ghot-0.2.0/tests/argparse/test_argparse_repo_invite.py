...
@pytest.mark.parametrize("args,expected_kwargs", [
    pytest.param(  # Default
        ["ghot", "repo", "invite", "org", "users.csv"],
        {"dry": False},
        id="default"
    ),
    pytest.param(  # Dry run
        ["ghot", "repo", "invite", "org", "users.csv", "--dry"],
        {"dry": True},
        id="dry"
    ),
])
...
def test_repo_invite(mock_load_users, mock_init_org_manager, args, expected_kwargs, monkeypatch):
    ...
    mock_org_manager.repo_invite.assert_called_once_with(
