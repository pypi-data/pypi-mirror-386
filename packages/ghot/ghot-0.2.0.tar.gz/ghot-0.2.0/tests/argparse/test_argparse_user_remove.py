...
@pytest.mark.parametrize("args, expected_kwargs", [
    pytest.param(  # Default
        ["ghot", "user", "remove", "org", "users.csv"],
        {"dry": False, "force": False},
        id="default"
    ),
    pytest.param(  # Dry
        ["ghot", "user", "remove", "--dry", "org", "users.csv"],
        {"dry": True, "force": False},
        id="dry"
    ),
    pytest.param(  # Force
        ["ghot", "user", "remove", "--force", "org", "users.csv"],
...
def test_user_remove(mock_load_users, mock_init_org_manager, args, expected_kwargs, monkeypatch):
    ...
    mock_org_manager.user_remove.assert_called_once_with(
