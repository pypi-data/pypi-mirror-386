...
from digital_life.models import MemoryCardGenerate,MemoryCardGenerateEasy, MemoryCard2, MemoryCard, MemoryCardScore, MemoryCards, Document, Chapter
...
class MemoryCardManager:
    ...
    async def agenerate_memory_card_by_text(
        ...
        time_dicts_time = await self.inters.intellect_remove_formats(
            input_datas=time_dicts.get("time"),
            prompt_id = "0078",
            version = None,
            inference_save_case=self.inference_save_case,
            OutputFormat = MemoryCardGenerate,
        )
        super_log(time_dicts,"time_dicts_before")

        for i,time_dict in enumerate(time_dicts):
            time_dict.update({"time":time_dicts_time[i].get("time")})
    ...
    async def agenerate_memory_card(self, chat_history_str: str, weight: int = 1000):
        ...
        # 将--到--岁转换到不惑
        time_dicts_time = await self.inters.intellect_remove_formats(
            input_datas=[time_dict.get('time') for time_dict in time_dicts],
            prompt_id = "0078",
            version = None,
            inference_save_case=self.inference_save_case,
            OutputFormat = MemoryCardGenerate,
