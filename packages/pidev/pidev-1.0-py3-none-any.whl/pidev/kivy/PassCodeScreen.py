...
class PassCodeScreen(Screen):
    ...
    def check_pass(self):
        ...
        if PASSWORD == USERPW:
            ...
            move_right = Animation(center_x = self.center_x + anim_dist, duration=anim_timing, t='linear')
            move_left = Animation(center_x = self.center_x - anim_dist, duration=(anim_timing*2), t='linear')
            return_back = Animation(center_x = self.center_x, duration=anim_timing, t='linear')
            full_anim= move_right + move_left + return_back
            full_anim.start(self.ids.pw)
            self.ids.pw.color = (1, 0, 0, 1)
            Clock.schedule_once(self.reset_color, anim_timing*8)

    def reset_color(self, dt=None):
        """
        Reset the color of the text
        :return: None
        """
        self.ids.pw.color = (0, 0, 0, 1)
