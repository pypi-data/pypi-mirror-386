...
class RsVideoTranscodeJobStatus(BaseModel):
    id: str = ""
    status: RsVideoTranscodeStatus = RsVideoTranscodeStatus.default()
    progress: float = 0.0

    class Config:
        # Fields are already lowercase, matching camelCase for single words
        populate_by_name = True
        # Enables serialization/deserialization with aliases if needed in future
