...
class LightwheelClient:
    ...
    def __init__(
        ...
        *,
        ...
        if self.host == "":
            env_host = os.environ.get("LW_API_ENDPOINT", None)
            if not env_host:
                self.host = "https://api.lightwheel.net"
            else:
                self.host = env_host
    ...
    def _clear_header(self, key: str):
    ...
    def _clear_account_cache(self):
        self.cache_path.unlink(missing_ok=True)
        self._clear_header(AUTH_KEY)
        self._clear_header(USERNAME_KEY)
    ...
    def post(self, path: str, *, data: Dict[str, Any] = None, timeout: int = None):
        ...
        try:
            ...
            try:
                ...
                self.logger.warning("Token refresh failed, clearing account cache")
                self._clear_account_cache()
            res = self._post(path, data=data, timeout=timeout)
        except (ApiException, UnknownException) as e:
            self.logger.error("API request failed for body: %s, error: %s", data, e)
            raise e
    ...
    def _post(self, path: str, *, data: Dict[str, Any] = None, timeout: int = None):
        ...
        if res.status_code == 401:
            raise AuthenticationFailedException(f"Authentication failed with host: {self.host} and path: {path}")
        if res.status_code != 200:
            raise ApiException(res, f"{self.host}/{path}")
