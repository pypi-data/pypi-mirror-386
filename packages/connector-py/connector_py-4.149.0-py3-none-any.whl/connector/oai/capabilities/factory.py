...
class CapabilityExecutorFactory(Generic[REQUEST, SETTINGS]):
    ...
    def __init__(
        ...
        *,
        ...
        self._instrument = Instrument.integrations.connectors.node(app_id)
    ...
    def create(self, name: str) -> CapabilityExecutor[REQUEST, SETTINGS]:
        ...
        """
        capability_instrument = self._instrument.capabilities.node(name)
        with capability_instrument.create.timer():
    ...
    def _create(
        ...
        if executor := self._cached_executors.get(name):
            capability_instrument.cache.hit.incr()
            return executor

        capability_instrument.cache.miss.incr()

        capability = self._capabilites.get(name)
        if capability is None:
            capability_instrument.not_implemented.incr()
        ...
        capability_instrument.created.incr()
