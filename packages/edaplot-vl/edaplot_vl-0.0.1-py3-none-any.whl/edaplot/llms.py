    ...
    if provider == "openai" or config.api_base_url is not None:
        ...
        if config.use_responses_api:
            extra_kwargs.update(
                ...
                reasoning=(
                    {"effort": config.reasoning_effort, "summary": "auto"} if is_reasoning_model(config.name) else None
                ),
            ...
            extra_kwargs.update(
                reasoning_effort=config.reasoning_effort if is_reasoning_model(name) else None,
                # The old API errors out if you provide a temperature for reasoning models
                temperature=config.temperature if not is_reasoning_model(name) else None,
            )
