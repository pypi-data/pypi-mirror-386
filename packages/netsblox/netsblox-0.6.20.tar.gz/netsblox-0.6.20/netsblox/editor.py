...
class OpenAI:
    ...
    def __getattr__(self, attr):
        if isinstance(attr, str) and not attr.startswith('_'):
            ...
            if res is not None: return res
        raise AttributeError(f'No attribute {attr}')
    def generate_image(self, prompt: str, size: Optional[str] = None) -> Image.Image:
        '''
        Generates an image given a prompt.
        
        :prompt: The prompt to provide to ChatGPT for completion.
        
        :size: The resolution of the generated image. Note that larger images are more expensive to generate.
        
        :returns: The generated image
        '''
        return self._client.call('OpenAI', 'generateImage', **{ 'prompt': prompt, 'size': size })
    def generate_text(self, prompt: Any) -> str:
        ...
        '''
        res = self._client.call('OpenAI', 'generateText', **{ 'prompt': prompt })
        return str(res)
    def set_key(self, key: str):
        ...
        :key: The new OpenAI API key to use for this account
        '''
        return self._client.call('OpenAI', 'setKey', **{ 'key': key })
