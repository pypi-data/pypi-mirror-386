...
async def _execute_agent(
    ...
    *,
    ...
    try:
        exit_task = asyncio.create_task(tui.wait_for_exit())
        async with ClaudeSDKClient(options=options) as client:
            while True:
                ...
                stream_active = await consume_task
...
def dict_to_permission_update(d: dict) -> PermissionUpdate:
    """Convert dictionary format to PermissionUpdate object."""
    update_type = d["type"]
    ...
    if "rules" in d:
        rules = [
            PermissionRuleValue(
                tool_name=rule["toolName"], rule_content=rule.get("ruleContent")
            )
            for rule in d["rules"]
        ]

    behavior = d.get("behavior", None)
    mode = d.get("mode", None)
    directories = d.get("directories", None)
    ...
    *,
    ...
    try:
        ...
        async def tool_use_callback(
            ...
            try:
                permission_updates = [
                    u.to_dict() if isinstance(u, PermissionUpdate) else u
                    for u in context.suggestions
                ]
                ...
                if tool_name == "ExitPlanMode":
                    ...
                    decision = await tui.prompt_permission_choice(
                        tool_name,
                        allow_updates=bool(permission_updates),
                    )
                ...
                match decision:
                    ...
                    case "allow_with_updates":
                        updated_permissions = [
                            dict_to_permission_update(u) for u in permission_updates
                        ]
                        await tui.log_text(
                            f"allowing {tool_name} with updates {updated_permissions}"
                        )
                        return PermissionResultAllow(
