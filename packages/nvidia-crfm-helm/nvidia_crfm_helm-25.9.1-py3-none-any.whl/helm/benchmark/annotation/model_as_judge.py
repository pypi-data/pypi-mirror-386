...
def _sanitize_error_message(error_message: str) -> str:
    ...
    # Pattern to match API tokens (common formats)
    # Matches patterns like: nvapi-..., sk-..., hf_..., etc.
    api_key_patterns = [
        r'nvapi-[a-zA-Z0-9_-]+',  # NVIDIA API keys
        r'sk-[a-zA-Z0-9_-]+',     # OpenAI API keys
        r'hf_[a-zA-Z0-9_-]+',     # HuggingFace API keys
        r'[a-zA-Z0-9_-]{20,}',    # Generic long tokens (20+ chars)
    ]
...
class LLMAsJuryAnnotator(Annotator):
    ...
    def _annotate_with_model(
        ...
        if not annotator_response.success:
            # Sanitize error message to remove API keys before logging
            sanitized_error = _sanitize_error_message(annotator_response.error)
            hwarn(f"Got an error response from {model_info.model_name}: {sanitized_error}")
            return None
