...
class OpenAIClient(CachingClient):
    ...
    def _make_chat_request(self, request: Request) -> RequestResult:
        ...
        try:
            ...
            # Try to create a new token and retry if this is an authentication error
            current_token = os.environ.get("OPENAI_API_KEY")
            new_token = create_token_on_error(str(e), service_name="openai")
            if new_token and new_token != current_token:
                # Set the new token in the environment and client, then retry once
                ...
                try:
                    ...
                    error2: str = f"OpenAI error after token refresh: {e2}"
                    return RequestResult(success=False, cached=False, error=error2, completions=[], embedding=[])
            else:
        ...
        for raw_completion in response["choices"]:
            ...
            if hasattr(self, "output_processor") and callable(self.output_processor):
        ...
        if response and isinstance(response, dict):
            if "request_time" in response:
                request_time = response["request_time"]
            if "request_datetime" in response:
                request_datetime = response["request_datetime"]
