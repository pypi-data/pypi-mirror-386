    ...
    header: {
        ...
        getInsulaAppsMenuLinks: async () => {
            ...
            try {
                 ...
                 * ;                      -> The semicolon
                 * */
                const match = textResponse.match(/window\.__APP_MENU_LINKS__\s*=\s*(\[[\s\S]*?\]);/);
                if (match && match[1]) {
        ...
        getExtraTopbarLinks: async () => {
            ...
            try {
                 ...
                 * ;                      -> The semicolon
                 * */
                const match = textResponse.match(/window\.__HEADER_LINKS__\s*=\s*(\[[\s\S]*?\]);/);
                if (match && match[1]) {
                    /* match[1] is the first capture group */
                    linksFromJupyterHubEnvVariables = JSON.parse(match[1]);
                }
    ...
    try {
        ...
        if (userData) {
            ...
            if (user && user.name) {
                ...
                if (_configuration__WEBPACK_IMPORTED_MODULE_0__.appConfig.header.getExtraTopbarLinks) {
                    const extraTopbarLinks = await _configuration__WEBPACK_IMPORTED_MODULE_0__.appConfig.header.getExtraTopbarLinks();
                    if (extraTopbarLinks) {
                        extraTopbarLinks.forEach(extraTopbarLink => {
                            ...
                            iconSpan.innerHTML = _icons__WEBPACK_IMPORTED_MODULE_1__.Icons.LinkIcon;
                ...
                if (_configuration__WEBPACK_IMPORTED_MODULE_0__.appConfig.header.getInsulaAppsMenuLinks) {
                    const insulaAppsMenuLinks = await _configuration__WEBPACK_IMPORTED_MODULE_0__.appConfig.header.getInsulaAppsMenuLinks();
                    if (insulaAppsMenuLinks) {
                        ...
                        insulaAppsMenuIcon.innerHTML = _icons__WEBPACK_IMPORTED_MODULE_1__.Icons.AppsIcon;
                        insulaAppsMenuIcon.id = 'insulaAppsMenuLinks';
                        insulaAppsMenuIcon.addEventListener('click', () => {
                            showHeaderMenu(insulaAppsMenuLinks, insulaAppsMenuIcon.id, true);
                        });
                ...
                if (_configuration__WEBPACK_IMPORTED_MODULE_0__.appConfig.header.otherInfoMenuLinks) {
                    ...
                    otherInfoMenuIcon.innerHTML = _icons__WEBPACK_IMPORTED_MODULE_1__.Icons.InfoIcon;
                    otherInfoMenuIcon.id = 'otherInfoMenuLinks';
                    otherInfoMenuIcon.addEventListener('click', () => {
                        showHeaderMenu(_configuration__WEBPACK_IMPORTED_MODULE_0__.appConfig.header.otherInfoMenuLinks, otherInfoMenuIcon.id, false);
                    });
...
const showHeaderMenu = (links, id, avatar) => {
    ...
    if (headerMenuContainerEl) {
        ...
        menuButtonEl === null || menuButtonEl === void 0 ? void 0 : menuButtonEl.appendChild(headerMenuContainerEl);
        currentOpenMenuId = headerMenuContainerId;
    }
};
...
//# sourceMappingURL=lib_index_js.c6373db979e1ba6fcd32.js.map