...
1. Publish to PyPI with `uv publish` (or `uv publish --index-url <your-repository-url>`
   for private repositories)

If you want to install this client into another project without publishing it (e.g. for
development) then:

1. If that project **is using uv**, you can add it with `uv add <path-to-this-client>`
   from that project
1. If that project is using a different package manager:
   1. Build a wheel with `uv build`
   1. Install that wheel from the other project `pip install dist/<wheel-file>.whl`
