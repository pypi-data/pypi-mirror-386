...
def TMPinvSolve(
    ...
    b_lim:         Sequence[float]           | None = None,
    ...
    *args, **kwargs
    ...
    This function may also receive internally generated keyword arguments
    (`b_lim`, `C_lim`) from `tmpinv()`. These arguments are ignored in the
    reduced model given its replacement of `S`.
    """
    ...
    *args, **kwargs : additional arguments
    ...
    for _ in range(iteration_limit):
        ...
        if len(M_idx) < n_cells:
            ...
            result = TMPinvSolve(*args, C_lim=(C_lim[finite_rows, :]),
                                        S=(S[finite_rows, :])[:,nonzero_cols],
                                        M=M,
                                        b_lim=(b_lim[finite_rows, :]),
                                        b_val=b_val, **kw)
        else:
            break
