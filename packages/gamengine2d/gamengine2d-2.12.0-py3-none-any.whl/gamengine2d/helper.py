...
class Context:
    ...
    def on_key_press(self, key_name, callback):
        self.key_pressed_callbacks[callback] = key_name

    def on_key_release(self, key_name, callback):
        self.key_released_callbacks[callback] = key_name

    def on_key_hold(self, key_name, callback):
        self.key_held_callbacks[callback] = key_name

    def update_key_callbacks(self):
        for callback, key in self.key_pressed_callbacks.items():
            if key in self.keys_pressed:
                callback()

        for callback, key in self.key_released_callbacks.items():
            if key in self.keys_released:
                callback()

        for callback, key in self.key_held_callbacks.items():
