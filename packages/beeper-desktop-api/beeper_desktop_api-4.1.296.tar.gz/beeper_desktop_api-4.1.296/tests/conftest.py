...
from beeper_desktop_api import BeeperDesktop, AsyncBeeperDesktop, DefaultAioHttpClient
from beeper_desktop_api._utils import is_dict
...
logging.getLogger("beeper_desktop_api").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[BeeperDesktop]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with BeeperDesktop(base_url=base_url, access_token=access_token, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncBeeperDesktop]:
