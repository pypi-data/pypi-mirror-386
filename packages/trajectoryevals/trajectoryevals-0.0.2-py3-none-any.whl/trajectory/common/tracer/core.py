...
from trajectory.common.constants import (
    TRAJECTORY_TRACING_LOCAL_ENV,
    TRAJECTORY_TRACING_LOCAL_DIR_ENV,
    TRAJECTORY_ONLY_LOCAL_TRACING_ENV,
)
from trajectory.common.logger import trajectory_logger, reconfigure_logger, configure_trajectory_logger
try:
...
class TraceClient:
    ...
    def async_evaluate(
        ...
        try:
            ...
            trajectory_logger.warning(f"Failed to load scorers: {str(e)}")
            return
    ...
    def save(self, final_save: bool = False) -> Tuple[str, dict]:
        ...
        """
        trajectory_logger.debug(f"Starting trace save | trace_id={self.trace_id} final_save={final_save}")
        
        if final_save:
            try:
                trajectory_logger.debug(f"Flushing pending spans for trace {self.trace_id}")
                self.otel_span_processor.flush_pending_spans()
                trajectory_logger.debug(f"Flushed pending spans | trace_id={self.trace_id}")
            except Exception as e:
                trajectory_logger.warning(
        ...
        trajectory_logger.debug(f"Trace summary | duration={total_duration:.3f}s spans={len(self.trace_spans)}")
        ...
        if self.conversation_id:
            ...
            trajectory_logger.debug(f"Added conversation_id to trace: {self.conversation_id}")
        ...
        if self.tracer.enable_local_tracing and self.tracer.local_trace_storage:
            try:
                trajectory_logger.info(f"Local tracing enabled - saving trace locally: {self.trace_id}")
                trajectory_logger.debug(f"Trace data size: {len(str(trace_data))} characters")
                trajectory_logger.debug(f"Number of spans in trace: {len(trace_data.get('trace_spans', []))}")
                
                self.tracer.local_trace_storage.save_trace(trace_data, self.trace_id, final_save=final_save)
                trajectory_logger.info(f"Saved trace locally | trace_id={self.trace_id}")
                
                # If not in only-local mode, also upsert remotely
                if not self.tracer.only_local_tracing:
                    trajectory_logger.debug("Local tracing enabled and only_local_tracing is false; upserting trace remotely as well")
                    ...
                    trajectory_logger.debug(f"Created local tracing response: {server_response}")
            except Exception as e:
                trajectory_logger.error(f"Failed to save trace locally: {e}")
                trajectory_logger.debug(f"Local tracing error details: {str(e)}")
                # Only fall back to remote tracing if not in only_local_tracing mode
                if not self.tracer.only_local_tracing:
                    trajectory_logger.info(f"Falling back to remote tracing for trace: {self.trace_id}")
                    server_response = self.trace_manager_client.upsert_trace(
                        trace_data,
                        offline_mode=self.tracer.offline_mode,
                        show_link=not final_save,
                    ...
                    trajectory_logger.warning(f"Only local tracing enabled but local save failed: {e}")
                    server_response = {
                        "ui_results_url": f"file://{self.tracer.local_tracing_dir}/trace_*_{self.trace_id[:8]}.json",
                        "trace_id": self.trace_id,
...
class Tracer:
    ...
    def __init__(
        ...
        enable_monitoring: bool = os.getenv("TRAJECTORY_MONITORING", "true").lower() == "true",
        enable_evaluations: bool = os.getenv("TRAJECTORY_EVALUATIONS", "true").lower() == "true",
        # S3 configuration
        use_s3: bool = False,
        ...
        local_tracing_dir: Optional[str] = None,  # None means use environment variable or default
        ...
        try:
            ...
            if not api_key or not organization_id:
                if is_only_local_tracing_enabled():
                    trajectory_logger.info("Only-local tracing enabled; bypassing API key/org validation")
            ...
            try:
                ...
                trajectory_logger.error(f"Issue verifying API key, disabling monitoring: {e}")
            ...
            if use_s3 and S3Storage is not None:
                ...
                trajectory_logger.warning("S3 storage requested but not available; disabling S3")
                self.use_s3 = False
            ...
            self.enable_local_tracing = self._determine_local_tracing_setting(enable_local_tracing)
            self.only_local_tracing = self._determine_only_local_tracing_setting(only_local_tracing)
            self.local_tracing_dir = local_tracing_dir or get_local_storage_dir()
            self.local_trace_storage = None
            
            trajectory_logger.debug(f"Local tracing settings - enabled: {self.enable_local_tracing}, only_local: {self.only_local_tracing}")
            trajectory_logger.debug(f"Local tracing directory: {self.local_tracing_dir}")
            
            if self.enable_local_tracing:
                try:
                    trajectory_logger.info(f"Initializing local trace storage at: {self.local_tracing_dir}")
                    self.local_trace_storage = LocalTraceStorage(self.local_tracing_dir)
                    if self.only_local_tracing:
                        trajectory_logger.info(f"Only local tracing enabled. Storage directory: {self.local_tracing_dir}")
                        trajectory_logger.info("Disabling remote monitoring and evaluations for local-only mode")
                        # Disable remote monitoring when only local tracing is enabled
                        self.enable_monitoring = False
                        self.enable_evaluations = False
                    else:
                        trajectory_logger.info(f"Local tracing enabled alongside remote tracing. Storage directory: {self.local_tracing_dir}")
                except Exception as e:
                    trajectory_logger.error(f"Failed to initialize local trace storage: {e}")
                    trajectory_logger.debug(f"Local trace storage error details: {str(e)}")
    ...
    def _determine_local_tracing_setting(self, enable_local_tracing: Optional[bool]) -> bool:
        """
    ...
    def _determine_only_local_tracing_setting(self, only_local_tracing: Optional[bool]) -> bool:
        """
        ...
        with trace.span(name or "unnamed_trace"):
            try:
                ...
                try:
                    ...
                    trajectory_logger.warning(f"Failed to finalize trace {trace.trace_id}: {e}")
        ...
        *,
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                try:
                    context_tracer = EVAL_TRACE_VAR.get() if EVAL_TRACE_VAR is not None else None
        ...
        async def wrapper(*args, **kwargs):
            ...
            with current_trace.span(span_name, span_type="llm") as span:
                ...
                try:
                    ...
                    trajectory_logger.error(f"LLM async call error in wrapped client: {e}")
                    _capture_exception_for_trace(current_trace, sys.exc_info())
                    raise e
...
def cost_per_token(*args, **kwargs):
    try:
        ...
        if (
            ...
            trajectory_logger.warning("LiteLLM returned a total of 0 for cost per token")
