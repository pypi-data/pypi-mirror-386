...
def check_experiment_type(
    ...
    try:
        ...
        raise JudgmentAPIError(f"Failed to check if experiment type exists: {e!s}")
...
def check_eval_run_name_exists(
    ...
    try:
        ...
        raise JudgmentAPIError(f"Failed to check if eval run name exists: {e!s}")
...
def send_trajectory_results_to_backend(
    trajectory_results: list[dict[str, Any]], judgment_api_key: str
) -> None:
...
def run_trace_eval(
    ...
    try:  # execute an EvaluationRun with just JudgmentScorers
        ...
        raise JudgmentAPIError(
            f"An error occurred while executing the Judgment API request: {e!s}"
        )
