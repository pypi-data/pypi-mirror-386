...
- **Historical data support** - Backtest portfolios using historical price data
- `HistoricalPriceData` model for historical price points with timestamp and volume
- `HistoricalSentimentData` model for historical sentiment analysis data
- `historical_data` field in `OptimizePortfolioRequest` (90+ days per asset)
- `historical_sentiment` field in `OptimizePortfolioRequest` (optional)
- Transparent API - same endpoint handles both live and backtest requests
- API automatically enters backtest mode when historical data is provided
- Full backward compatibility - all new fields are optional
- Comprehensive backtesting example (`examples/backtesting_example.py`)
