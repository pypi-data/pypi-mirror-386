...
class Re(Function):
    """
    Extract substring base on regular expression pattern
    """
    ...
    print(
        CTEFactory(
            """
                ...
                FROM (
                    SELECT id, nome FROM Vendedores WHERE status = 'ativo'
                    UNION
                    SELECT id, nome FROM Funcionario WHERE depto = 35 /* Vendedores */
                ) AS u001
                JOIN (
                    SELECT user_id, Sum(valor) as total from Pedidos group by user_id
                    UNION ALL
                    SELECT usuario, Sum(preco) as total FROM Ecommerce WHERE tipo = 'venda interna'
                ) as agg_vendas
