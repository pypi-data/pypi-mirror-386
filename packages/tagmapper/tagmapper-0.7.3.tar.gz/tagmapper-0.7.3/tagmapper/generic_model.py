...
class Model(ModelTemplate):
    ...
    def get_models(model_name: str, model_owner: str) -> List["Model"]:
        ...
        models = []
        mappings = get_mappings(model_owner=model_owner, model_name=model_name)
        ...
        for map in mappings:
            map_obj = {
                "object_name": map["unique_object_identifier"],
                "name": map["model_name"],
                "owner": map["model_owner"],
            }
            if map_obj not in mod_meta_data:
                mod_meta_data.append(map_obj)

        for meta_data in mod_meta_data:
            if validate_schema:
            ...
            # should not get all mappings again as they are already retrieved
