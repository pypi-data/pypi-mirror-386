...
try:
    ...
    FALLBACK_ICONS = {
        ...
        "file-code": '''<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/><path d="m10 13-2 2 2 2"/><path d="m14 17 2-2-2-2"/></svg>''',
        "file-json": '''<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>''',
        "image": '''<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>''',
...
def get_lucide_icon(icon_name: str, size: int = 16, color: str = "#000000") -> QIcon:
    ...
    if svg_string is None:
        ...
        elif "file" in icon_name or "folder" in icon_name:
            # Map unknown file/folder icons to basic fallbacks
            svg_string = FALLBACK_ICONS.get("file" if "file" in icon_name else "folder", FALLBACK_ICONS["file"])
        else:
            return QIcon()  
...
class GitignoreParser:
    """Parse and match .gitignore patterns with improved support"""
    ...
    def is_ignored(self, path: Path, root: Path) -> bool:
        ...
        # Check if explicitly negated (re-included)
        for pattern in self.negations:
            if self._match_pattern(pattern, rel_path, path):
                return False
    ...
    def _match_pattern(self, pattern: str, rel_path: str, path: Path) -> bool:
        """Match a single pattern against path"""
...
class SmartFileDetector:
    ...
    def should_exclude_dir(self, dir_path: Path) -> bool:
        ...
        # Check against DEFAULT_EXCLUDES patterns
    ...
    def should_exclude_file(self, file_path: Path, strategy: str = 'smart') -> bool:
        ...
        # For 'all' strategy, don't exclude binaries
        if strategy != 'all':
    ...
    def detect_files(self, strategy: str = 'smart', max_age_days: int = 30) -> List[Path]:
        ...
        for file_path in self.root_path.rglob('*'):
            ...
            if self.should_exclude_file(file_path, strategy):
                continue
...
class ModernFileSelector(QMainWindow):
    ...
    def create_smart_panel(self):
        ...
        # Initially disable days if not 'recent'
    ...
    def on_strategy_changed(self, index):
        """Enable/disable days spinbox based on strategy"""
        strategy_text = self.strategy_combo.currentText()
        strategy = strategy_text.split()[0]
        
        is_recent = strategy == 'recent'
        self.days_label.setEnabled(is_recent)
        self.days_spin.setEnabled(is_recent)
        ...
        def add_directory(parent_item, directory):
            ...
            for item_path in items:
                ...
                if item_path.is_dir():
                    ...
                    file_item.setText(2, item_path.suffix[1:].upper() if item_path.suffix else "File")
...
class VerCtrl:
    ...
    def create_backup(self):
        ...
        for filepath in self.config['files']:
            ...
            try:
                ...
                # Guard create_new_file with confirmation
                if self.config.get('create_new_file', False):
                    response = input(f"⚠ Truncate '{filepath}' to empty file? (y/N): ")
                    if response.lower() == 'y':
                        filepath.write_text('')
                        print_info(f"Created new empty file: {filepath}")
                    else:
                        print_info(f"Skipped truncation of: {filepath}")
    ...
    def cleanup_old_backups(self):
        """Remove old backups beyond keep_history limit using scheme-specific patterns"""
        ...
        for filepath in self.config['files']:
            ...
            # Use scheme-specific patterns for precise matching
            stem = re.escape(filepath.stem)
            suff = re.escape(filepath.suffix)
            
            if scheme == 'version':
                pattern = rf'^{stem}-v\d+{suff}$'
            elif scheme == 'timestamp':
                pattern = rf'^{stem}-\d{{8}}-\d{{6}}{suff}$'
            elif scheme == 'simple':
                pattern = rf'^{stem}-old{suff}$'
            else:
                pattern = rf'^{stem}-.*{suff}$'
            
            # Find matching backups
            backups = []
            for backup in backup_dir.iterdir():
                if backup.is_file() and re.match(pattern, backup.name):
                    backups.append(backup)
            
            # Sort by modification time (newest first) and keep only the most recent
    ...
    def list_backups(self):
        ...
        # Simple printing without complex alignment
        for backup in backups:
            if backup.is_file():
                ...
                print(f"{Colors.CYAN}{backup.name}{Colors.RESET}")
                print(f"  Size: {size_str} | Modified: {mtime}\n")

        print(f"{Colors.BOLD}Total: {len(backups)} backup(s){Colors.RESET}")

    def restore_backup(self, backup_name):
        """Restore a specific backup file with improved matching logic"""
        self.load_config()
        ...
        # Improved restore logic with strict pattern matching
        original_file = None
        scheme = self.config.get('naming_scheme', 'version')
        bn = backup_path.name

        for tracked_file in self.config['files']:
            tp = Path(tracked_file)
            stem = re.escape(tp.stem)
            suff = re.escape(tp.suffix)
            
            if scheme == 'version':
                pat = rf'^{stem}-v\d+{suff}$'
            elif scheme == 'timestamp':
                pat = rf'^{stem}-\d{{8}}-\d{{6}}{suff}$'
            elif scheme == 'simple':
                pat = rf'^{stem}-old{suff}$'
            else:
                # Fallback pattern
                pat = rf'^{stem}-.*{suff}$'
        ...
        if original_file is None:
            print_warning("Could not determine original filename from backup name.")
    ...
    def show_stats(self):
        ...
        print(f"  Naming Scheme: {self.config.get('naming_scheme', 'version')}")
