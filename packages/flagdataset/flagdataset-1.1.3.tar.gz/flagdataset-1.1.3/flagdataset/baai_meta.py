...
from .baai_downloder import save_remote_meta, read_remote_meta
...
def meta_download(cmd_args):
    ...
    while True:
        try:
            req_data = {"dataset_id": cmd_args.dataset, "prefix": cmd_args.dir_path}
            ...
            if resp_meta_.get("data").get("search_count") == 0:
                break
            print(f"meta_bin: {meta_part_path}")
            offset = offset + limit
