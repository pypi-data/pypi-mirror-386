...
def autotune(configs, key, prune_configs_by=None, reset_to_zero=None, warmup=25, rep=100):
    """
    Decorator for auto-tuning a :code:`ppl.jit`'d function.

    .. highlight:: python
    .. code-block:: python

        @ppl.autotune(configs=[
            ppl.Config(meta={'BLOCK_SIZE': 128}, num_warps=4),
            ppl.Config(meta={'BLOCK_SIZE': 1024}, num_warps=8),
          ],
    ...
    :param configs: a list of :code:`ppl.Config` objects
    :type configs: list[ppl.Config]
...
def heuristics(values):
    ...
    .. code-block:: python
        ...
        @ppl.heuristics(values={'BLOCK_SIZE': lambda args: 2 ** int(math.ceil(math.log2(args[1])))})
        @ppl.jit
        def kernel(x_ptr, x_size, **META):
