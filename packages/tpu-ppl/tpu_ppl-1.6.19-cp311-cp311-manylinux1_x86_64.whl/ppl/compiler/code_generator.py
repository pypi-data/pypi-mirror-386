...
class CodeGenerator(ast.NodeVisitor):
    def __init__(self, context, prototype, gscope, attributes, constants, function_name, arch,
                 ...
                 debug=False, noinline=False, file_name: Optional[str] = None, begin_line=0, function_ori_name="", v_names=[]):
    ...
    def set_value(self, name: str,
        ...
        if to_ir and isinstance(value, tensor) :
          self.builder.set_v_names(value.handle, name)
    ...
    def visit_FunctionDef(self, node):
        ...
        fn = self.builder.get_or_insert_function(self.module, self.function_name, self.prototype.to_ir(self.builder), visibility, self.noinline, self.is_kernel,self.function_ori_name, self.v_names)
...
def ast_to_ttir(fn, signature, specialization, constants, debug, arch):
    ...
    generator = CodeGenerator(context, prototype, gscope=gscope, constants=all_constants,
                              ...
                              arch=arch, function_ori_name = fn.__name__, v_names=arg_names)
    try:
        generator.visit(fn.parse())
