...
from mindgard.recon.command_fingerprint import GuardrailFingerprintCommand, OrchestratorSetupFingerprintRequest, OrchestratorSetupFingerprintResponse
...
from mindgard.utils import is_version_outdated, print_to_stderr, parse_toml_and_args_into_final_args, convert_test_to_cli_response, CliResponse, request_user_input
    ...
    if args.command == 'login':
        ...
        if passed_preflight:
            ...
            if args.command == 'recon-guardrail':
                ...
                @require_auth
                def guardrail_fingerprint(access_token: str, recon_id: str) -> None:
                    """
                    Manages a guardrail fingerprinting session from start to finish
                    Triggers the fingerprinting process, the prompt request/response loop, and fetches the final results

                    Args:
                        access_token (str): User access token
                        recon_id (str): overall session UUID for the user's recon process
                    """
                    guardrail_fingerprint_service = GuardrailService(
                        reconn_url=f"{API_BASE}/recon/guardrail/fingerprint",
                        get_events_url=f"{API_BASE}/events/prompt_request_response/pop",
                        push_events_url=f"{API_BASE}/events/prompt_request_response/push"
                    )
                    guardrail_command = GuardrailFingerprintCommand(call_system_under_test=model_wrapper.__call__,
                                                     service=guardrail_fingerprint_service)
                    orchestrator_setup_recon_request = OrchestratorSetupFingerprintRequest(recon_id=recon_id)
                    start_fingerprint_response: OrchestratorSetupFingerprintResponse = guardrail_command.start(
                        orchestrator_setup_recon_request,
                        access_token=access_token)
                    if start_fingerprint_response.recon_id != recon_id:
                ...
                def fingerprint_with_spinner() -> None:
                    ...
                    with Progress(
                        ...
                        with ThreadPoolExecutor(max_workers=1) as executor:
                            guardrail_future = executor.submit(guardrail_fingerprint, recon_id=result[0].id)
                ...
                if not detected:
                    ...
                    console.print(f"[underline]Recommendation[/underline]: Run the complete mindgard test suite for open-ended probing to uncover any additional risks. See [yellow]mindgard test --help[/yellow] to learn more.\n")
                    exit(CliResponse(0).code())
                ...
                if request_user_input(console, "(y/n): ", ["y", "n"]) == "n":
                    ...
                    console.print(f"[underline]Recommendation[/underline]: Shift from discovery to evasion. Run tests with different guardrail bypass domains using [yellow]mindgard test --domain bypass.\\[guardrail_name][yellow]\n")
                    exit(CliResponse(0).code())
                ...
                try:
                    ...
                    if highest_confidence == 0:
                        ...
                        console.print(f"[underline]Recommendation[/underline]: We recommend re-running the recon process to try different prompts.\n")
                    else:
                        ...
                        console.print(f"[underline]Recommendation[/underline]: Use the relevant Mindgard guardrail bypass domain to further validate and attempt evasion. Run [yellow]mindgard test --domain bypass.\\[guardrail_name][/yellow] with the selected domain.\n")
                    exit(CliResponse(0).code())
