...
class Sessions:
    ...
    def get_sql_queries(self, dataset_id: str, session_id: int, include_history: bool, include_metadata: bool,
        ...
        logger.debug(f"Generating SQL queries for dataset {dataset_id}, session {session_id}")
        
        try:
            result = self.sws_client.discoverable.post("session_api", url, data=body)
            logger.info(f"SQL queries generated successfully for dataset {dataset_id}, session {session_id}")
            
            # Return the result as-is since the API already provides the correct format
            return result
        except Exception as e:
            logger.error(f"Failed to generate SQL queries for dataset {dataset_id}, session {session_id}: {str(e)}")
