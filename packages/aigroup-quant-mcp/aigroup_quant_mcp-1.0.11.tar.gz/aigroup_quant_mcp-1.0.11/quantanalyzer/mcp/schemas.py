...
正确: 对生成的Alpha158因子进行CSZScoreNorm标准化
正确: 对因子进行鲁棒标准化（RobustZScoreNorm）
错误: 对原始OHLCV数据进行标准化
...
generate_alpha158 (生成因子)
    ↓
apply_processor_chain 当前步骤（标准化因子）
    ↓
train_model (训练模型)
            ...
            "examples": [
                {
                    ...
                    "input": {
                        ...
                        "result_id": "alpha158_norm",
                        "processors": [{"name": "CSZScoreNorm"}],
                        "export_path": "./exports/factors_norm.csv"
                    }
                }
            ]
        }
