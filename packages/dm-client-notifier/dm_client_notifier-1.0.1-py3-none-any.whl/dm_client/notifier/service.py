...
class NotifierClient(object):
    ...
    #################### Event ##########################
    def event_internal_lodge(self, event: str, channels: dict, variables: dict) -> (Response, Request):
        data = {
            'event': event,
            'channels': channels,
            'variables': variables
        }
        return self._api(self.target_service, 'event.internal.lodge', json=data, request_auth=RequestAuthEnum.INTERNAL)

    def event_internal_account_lodge(self, event: str, account: list[str], variables: dict) -> (Response, Request):
        data = {
            'event': event,
            'account': account,
            'variables': variables
        }
        return self._api(self.target_service, 'event.internal.lodge.account', json=data,
                         request_auth=RequestAuthEnum.INTERNAL)
