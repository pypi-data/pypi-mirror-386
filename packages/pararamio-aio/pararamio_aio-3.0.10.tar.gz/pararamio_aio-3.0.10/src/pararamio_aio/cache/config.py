"""Cache configuration management for async caches."""
...
class AsyncCacheConfig:
    """Configuration for async cache instances.

    This class provides a centralized way to configure async cache behavior,
    ...
    @classmethod
    def disabled(cls) -> AsyncCacheConfig:
        """Create a configuration with caching disabled.

        Returns:
            AsyncCacheConfig with enabled=False.
        """
        return cls(enabled=False)

    @classmethod
    def minimal(cls) -> AsyncCacheConfig:
        """Create a minimal configuration for testing or low-memory environments.

        Returns:
            AsyncCacheConfig with reduced limits.
        """
        return cls(
            max_size=100,
            max_memory_mb=10,
    ...
    @classmethod
    def aggressive(cls) -> AsyncCacheConfig:
        """Create an aggressive caching configuration.

        Returns:
            AsyncCacheConfig with increased limits and longer TTLs.
        """
        return cls(
            max_size=50000,
            max_memory_mb=500,
    ...
    def with_ttl_pattern(self, pattern: str, ttl: timedelta) -> AsyncCacheConfig:
        ...
        Returns:
            New AsyncCacheConfig with updated TTL patterns.
        """
