...
DEFAULT_BASE_URL = "https://agent-gateway.liveai.cloud/v1"
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            base_url (str, optional): LIVEAI_URL, if not provided, read from environment variable.
            api_key (str, optional): LIVEAI_API_KEY, if not provided, read from environment variable.
            api_secret (str, optional): LIVEAI_API_SECRET, if not provided, read from environment variable.
        ...
        la_base_url = (
            ...
            else os.environ.get("LIVEAI_INFERENCE_URL", DEFAULT_BASE_URL)
        )

        la_api_key = (
            api_key
            if is_given(api_key)
            else os.getenv("LIVEAI_INFERENCE_API_KEY", os.getenv("LIVEAI_API_KEY", ""))
        )
        if not la_api_key:
            raise ValueError(
                "api_key is required, either as argument or set LIVEAI_API_KEY environmental variable"
            )

        la_api_secret = (
            api_secret
            if is_given(api_secret)
            else os.getenv("LIVEAI_INFERENCE_API_SECRET", os.getenv("LIVEAI_API_SECRET", ""))
        )
        if not la_api_secret:
            raise ValueError(
                "api_secret is required, either as argument or set LIVEAI_API_SECRET environmental variable"
            )
    ...
    async def _recognize_impl(
        ...
        *,
        ...
        raise NotImplementedError(
            "LiveAI STT does not support batch recognition, use stream() instead"
        )
...
class SpeechStream(stt.SpeechStream):
            ...
            while True:
                ...
                if msg.type in (
                    ...
                    if closing_ws or self._session.closed:
                        return
                    raise APIStatusError(message="LiveAI STT connection closed unexpectedly")

                if msg.type != aiohttp.WSMsgType.TEXT:
                    logger.warning("unexpected LiveAI STT message type %s", msg.type)
                    continue
                ...
                if msg_type == "session.created":
                    ...
                    raise APIError(f"LiveAI STT returned error: {msg.data}")
                else:
                    logger.warning("received unexpected message from LiveAI STT: %s", data)
    ...
    async def _connect_ws(self) -> aiohttp.ClientWebSocketResponse:
        """Connect to the LiveAI STT WebSocket."""
        params: dict[str, Any] = {
            "settings": {
        ...
        try:
            ...
            if isinstance(e, aiohttp.ClientResponseError) and e.status == 429:
                raise APIStatusError("LiveAI STT quota exceeded", status_code=e.status) from e
            raise APIConnectionError("failed to connect to LiveAI STT") from e
        return ws
