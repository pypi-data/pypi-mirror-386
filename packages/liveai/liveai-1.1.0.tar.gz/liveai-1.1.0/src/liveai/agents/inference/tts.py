    ...
    async def _connect_ws(self, timeout: float) -> aiohttp.ClientWebSocketResponse:
        ...
        try:
            ...
            if isinstance(e, aiohttp.ClientResponseError) and e.status == 429:
                raise APIStatusError("LiveAI TTS quota exceeded", status_code=e.status) from e
            raise APIConnectionError("failed to connect to LiveAI TTS") from e
        ...
        try:
            ...
            await ws.close()
            raise APIConnectionError("failed to send session.create message to LiveAI TTS") from e
        ...
        async def _recv_task(ws: aiohttp.ClientWebSocketResponse) -> None:
            ...
            while True:
                ...
                if data.get("type") == "session.created":
                    ...
                    raise APIError(f"LiveAI TTS returned error: {msg.data}")
                else:
