...
la_dump_tts = int(os.getenv("LA_DUMP_TTS", 0))
...
class AudioEmitter:
    ...
    async def _main_task(self) -> None:
        ...
        def dump_segment() -> None:
            ...
            if not la_dump_tts or not debug_frames:
                return

            ts = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            fname = (
                f"la_dump/{self._label}_{self._request_id}_{segment_ctx.segment_id}_{ts}.wav"
                if self._streaming
                else f"la_dump/{self._label}_{self._request_id}_{ts}.wav"
            )
