...
from liveai import api, rtc
from liveai.protocol import agent, models
...
class WorkerOptions:
    ...
    max_retry: int = 16
    """Maximum number of times to retry connecting to LiveAI."""
    ws_url: str = "ws://localhost:7880"
    """URL to connect to the LiveAI server.

    By default it uses ``LIVEAI_URL`` from environment"""
    api_key: str | None = None
    """API key to authenticate with LiveAI.

    By default it uses ``LIVEAI_API_KEY`` from environment"""
    api_secret: str | None = None
    """API secret to authenticate with LiveAI.

    By default it uses ``LIVEAI_API_SECRET`` from environment"""
    ...
    """HTTP proxy used to connect to the LiveAI server.
...
class Worker(utils.EventEmitter[EventTypes]):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        opts.ws_url = opts.ws_url or os.environ.get("LIVEAI_URL", "")
        opts.api_key = opts.api_key or os.environ.get("LIVEAI_API_KEY", "")
        opts.api_secret = opts.api_secret or os.environ.get("LIVEAI_API_SECRET", "")
        opts._worker_token = os.environ.get("LIVEAI_WORKER_TOKEN", None)

        if not opts.ws_url:
            raise ValueError("ws_url is required, or add LIVEAI_URL in your environment")

        if not opts.api_key:
            raise ValueError("api_key is required, or add LIVEAI_API_KEY in your environment")

        if not opts.api_secret:
            raise ValueError(
                "api_secret is required, or add LIVEAI_API_SECRET in your environment"
            )
        ...
        os.environ["LIVEAI_URL"] = opts.ws_url
        os.environ["LIVEAI_API_KEY"] = opts.api_key
        os.environ["LIVEAI_API_SECRET"] = opts.api_secret
        ...
        self._api: api.LiveAIAPI | None = None
    ...
    async def simulate_job(
        ...
        from liveai.protocol.models import Room
        ...
        while not self._closed:
            try:
                ...
                if retry_count >= self._opts.max_retry:
                    raise RuntimeError(
                        f"failed to connect to liveai after {retry_count} attempts",
                    ) from None
                ...
                logger.warning(
                    f"failed to connect to liveai, retrying in {retry_delay}s", exc_info=e
                )
