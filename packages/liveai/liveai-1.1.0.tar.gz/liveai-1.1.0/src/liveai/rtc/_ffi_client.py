...
def get_ffi_lib():
    ...
    libpath = os.environ.get("LIVEAI_LIB_PATH", "").strip() or os.environ.get("LIVEKIT_LIB_PATH", "").strip()
    if libpath:
        return ctypes.CDLL(libpath)

    if platform.system() == "Linux":
        libname = "libliveai_ffi.so"
    elif platform.system() == "Darwin":
        libname = "libliveai_ffi.dylib"
    elif platform.system() == "Windows":
        libname = "liveai_ffi.dll"
    else:
        raise Exception(
            f"no ffi library found for platform {platform.system()}. \
                Set LIVEAI_LIB_PATH to specify a the lib path"
        )

    res = importlib.resources.files("liveai.rtc.resources") / libname
...
ffi_lib = get_ffi_lib()
_SYMBOL_PREFIXES = ("liveai", "livekit")


def _resolve_ffi_symbol(name: str):
    for prefix in _SYMBOL_PREFIXES:
        symbol = f"{prefix}_ffi_{name}"
        try:
            return getattr(ffi_lib, symbol)
        except AttributeError:
            continue
    raise AttributeError(f"FFI symbol not found for {name}. Tried prefixes {', '.join(_SYMBOL_PREFIXES)}")


ffi_initialize = _resolve_ffi_symbol("initialize")
ffi_request = _resolve_ffi_symbol("request")
ffi_drop_handle = _resolve_ffi_symbol("drop_handle")
ffi_dispose = _resolve_ffi_symbol("dispose")
ffi_cb_fnc = ctypes.CFUNCTYPE(None, ctypes.POINTER(ctypes.c_uint8), ctypes.c_size_t)

# C function types
ffi_initialize.argtypes = [
    ffi_cb_fnc,
    ctypes.c_bool,
    ctypes.c_char_p,
    ctypes.c_char_p,
...
ffi_request.argtypes = [
    ctypes.POINTER(ctypes.c_ubyte),
    ctypes.c_size_t,
    ctypes.POINTER(ctypes.POINTER(ctypes.c_ubyte)),
    ctypes.POINTER(ctypes.c_size_t),
]
ffi_request.restype = ctypes.c_uint64

ffi_drop_handle.argtypes = [ctypes.c_uint64]
ffi_drop_handle.restype = ctypes.c_bool


ffi_dispose.argtypes = []
ffi_dispose.restype = None
...
class FfiHandle:
    ...
    def dispose(self) -> None:
        if self.handle != INVALID_HANDLE and not self._disposed:
            self._disposed = True
            assert ffi_drop_handle(ctypes.c_uint64(self.handle))
...
def ffi_event_callback(
    ...
    if which == "logs":
        for record in event.logs.records:
            ...
            debug_env = os.environ.get("LIVEAI_RTC_DEBUG", "").strip().lower()
            rtc_debug = debug_env in ("true", "1")

            if level == logging.DEBUG and not rtc_debug:
                # ignore the rtc debug logs by default
                if record.target == "libwebrtc" or record.target.startswith("liveai"):
                    continue
