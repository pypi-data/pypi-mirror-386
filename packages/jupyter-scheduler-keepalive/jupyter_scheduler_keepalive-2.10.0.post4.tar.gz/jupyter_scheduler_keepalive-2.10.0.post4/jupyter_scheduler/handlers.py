...
class LastActivityHandler(ExtensionHandlerMixin, APIHandler):
    ...
    def get(self):
        ...
        last_activity: datetime | None = last_activity_times.get("jupyter_scheduler")

        # Convert datetime object to ISO 8601 string for JSON serialization
        if last_activity:
            return_value = last_activity.isoformat()
        else:
            return_value = None # Or an empty string, depending on desired explicit null/empty representation

        self.finish(json.dumps({"last_activity": return_value}))

    @authenticated
    def post(self):
        """
        Updates the last activity timestamp for jupyter_scheduler.
        This helps prevent the server from being culled due to inactivity.
        """
        self.serverapp._update_last_activity()
