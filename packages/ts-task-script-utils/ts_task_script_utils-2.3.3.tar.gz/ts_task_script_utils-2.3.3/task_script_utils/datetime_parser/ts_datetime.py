...
class TSDatetime:
    ...
    def change_fold(self, new_fold: Optional[int]):
        """Replace the fold value of stored datetime object
        if needed.
        """
        if new_fold is None:
            if self._is_fold_required:
                raise AmbiguousFoldError(
                    "DatetimeConfig.fold must not be None to parse datetime "
                    f"{self.tsformat()} without ambiguity."
                )
            # Setting fold=None has no effect. Return early.
            return
