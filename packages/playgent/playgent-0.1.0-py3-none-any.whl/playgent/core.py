...
def event_sender_worker():
    ...
    while not state.stop_sender.is_set():
        ...
        # Try to collect up to BATCH_SIZE events
    ...
    if remaining:
        try:
            ...
            logger.error(f"Failed to send remaining events: {str(e)}")
    ...
    if not session_id:
        ...
        if not session_id:
            try:
                ...
                logger.warning(f"Failed to create session via API: {e}, falling back to UUID")
                session_id = str(uuid.uuid4())
