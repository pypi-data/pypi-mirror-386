...
def test_runtime_measuring(dev_name, n_layers):
    ...
    assert all(
        x in recs for x in mod_names
    ), "failed to measure the runtime of some modules in basic mode"


@pytest.mark.parametrize("dev_name", ("cpu", "cuda"))
@pytest.mark.parametrize("n_layers", (1, 3, 5))
@pytest.mark.parametrize("config", ("baseline", "basic"))
def test_runtime_measuring_across_gms(dev_name, n_layers, config):
    if dev_name == "cuda" and not torch.cuda.is_available():
        dev_name = "cpu"
    device = torch.device(dev_name)
    ...
    assert all(
        x in recs for x in mod_names
    ), f"measure_runtimes failed to measure the runtime of some submodules in {config} mode"
