...
def mask_secret_value(
    ...
    Args:
        ...
        force_redact: When True, secrets are ALWAYS redacted regardless of include_secrets.
                      This is a configuration-level security override.
    ...
    # CRITICAL: If force_redact is enabled, ALWAYS redact secrets
    if is_secret and force_redact:
        logger.info(
            "SECRET_ACCESS_DENIED",
            extra={
                "timestamp": datetime.utcnow().isoformat(),
                "action": "secret_access_blocked_by_force_redact",
                "reason": "force_redact_secrets=true in configuration",
            },
        )
        return REDACTED
...
def format_parameter_output(
    ...
    masked_value = mask_secret_value(value, secret, include_secrets, force_redact)
