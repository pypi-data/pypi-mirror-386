...
class TestMaskSecretValue:
    ...
    def test_force_redact_always_masks_secrets(self):
        """Test force_redact=True always masks secrets even when include_secrets=True"""
        result = mask_secret_value(
            "secret-password", is_secret=True, include_secrets=True, force_redact=True
        )
        assert result == REDACTED
        assert result == "***REDACTED***"

    def test_force_redact_with_include_secrets_false(self):
        """Test force_redact=True masks secrets when include_secrets=False"""
        result = mask_secret_value(
            "secret-password", is_secret=True, include_secrets=False, force_redact=True
        )
        assert result == REDACTED

    def test_force_redact_does_not_affect_non_secrets(self):
        """Test force_redact=True does not affect non-secret values"""
        result = mask_secret_value(
            "normal-value", is_secret=False, include_secrets=False, force_redact=True
        )
        assert result == "normal-value"

    def test_force_redact_false_allows_secret_access(self):
        """Test force_redact=False allows secrets when include_secrets=True"""
        result = mask_secret_value(
            "secret-password", is_secret=True, include_secrets=True, force_redact=False
        )
        assert result == "secret-password"

    def test_force_redact_with_null_secret(self):
        """Test force_redact=True with null secret value"""
        result = mask_secret_value(None, is_secret=True, include_secrets=True, force_redact=True)
        assert result == "(not set)"
