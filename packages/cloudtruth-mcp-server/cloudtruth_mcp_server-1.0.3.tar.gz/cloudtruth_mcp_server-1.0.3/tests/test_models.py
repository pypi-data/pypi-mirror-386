...
class TestServerConfig:
    ...
    def test_force_redact_secrets_default(self):
        """Test force_redact_secrets defaults to False"""
        config = ServerConfig(api_key="test1234567890.ABC")
        assert config.force_redact_secrets is False

    def test_force_redact_secrets_true(self):
        """Test force_redact_secrets can be set to True"""
        config = ServerConfig(api_key="test1234567890.ABC", force_redact_secrets=True)
        assert config.force_redact_secrets is True

    def test_force_redact_secrets_false(self):
        """Test force_redact_secrets can be explicitly set to False"""
        config = ServerConfig(api_key="test1234567890.ABC", force_redact_secrets=False)
        assert config.force_redact_secrets is False
