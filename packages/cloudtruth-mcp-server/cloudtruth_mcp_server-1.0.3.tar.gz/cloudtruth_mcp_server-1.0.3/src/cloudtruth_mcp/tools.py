...
class ToolsHandler:
    ...
    async def get_parameter(
        ...
        try:
            ...
            return format_parameter_output(
                ...
                force_redact=self.config.force_redact_secrets,
            )
    ...
    async def get_parameters(
        ...
        try:
            ...
            # Get all parameters with values for the specified environment
            ...
            for param in parameters:
                # Check if values are already included in the parameter response
                # When list_parameters is called with environment_id, values are included
                if param.values and len(param.values) > 0:
                    # Values are already present, use them directly
                    # The values dict has environment URLs as keys
                    # We need to find the value for our target environment
                    env_url = f"{self.client.base_url}environments/{env.id}/"
                    value_data = param.values.get(env_url)
                        ...
                        params_data.append(
                            {
                                ...
                                "secret": value_data.get("secret", param.secret),
                                "type": str(param.type.value),
                                "description": param.description,
                            }
                        )
                    else:
                        # Value exists but not for this environment, show as not set
                        params_data.append(
                            {
                                "name": param.name,
                                "value": None,
                    ...
                    # Values not included, fall back to API call
                    # This happens when as_of (tag) is specified
            ...
            for param in parameters:
                ...
                if param.values and len(param.values) > 0:
                    ...
                    if value_data:
                        ...
                        is_secret = value_data.get("secret", param.secret)
                        masked_value = mask_secret_value(
                            value_str,
                            is_secret,
                            include_secrets,
                            self.config.force_redact_secrets,
                        )
