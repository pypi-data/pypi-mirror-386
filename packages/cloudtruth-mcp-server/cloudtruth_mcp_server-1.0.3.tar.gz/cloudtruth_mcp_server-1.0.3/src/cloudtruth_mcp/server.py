...
class CloudTruthMCPServer:
    ...
    def _register_handlers(self) -> None:
        ...
        async def list_tools() -> list[Tool]:
            ...
            return [
                ...
                Tool(
                    ...
                    description="Create a new parameter definition. Use 'project' with name or ID.",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "project": {
                                "type": "string",
                                "description": "Project name or ID (use this parameter, not project_id)",
                            },
                            "project_id": {
                                "type": "string",
                                "description": "DEPRECATED: Use 'project' instead. Project name or ID.",
                            },
                            "parameter_name": {
                                "type": "string",
                ...
                Tool(
                    name="get_version",
                    description="Get the CloudTruth MCP server version information",
                    inputSchema={
                        "type": "object",
                        "properties": {},
                    },
        ...
        async def call_tool(name: str, arguments: dict[str, Any]) -> Sequence[TextContent]:
            ...
            try:
                ...
                if name == "list_projects":
                    ...
                    # Handle both 'project' and 'project_id' parameter names
                    if "project_id" in arguments and "project" not in arguments:
                        arguments["project"] = arguments.pop("project_id")
                    elif "project_id" in arguments and "project" in arguments:
                        # Remove project_id if both provided
                        arguments.pop("project_id")
                    elif "project" not in arguments and "project_id" not in arguments:
                        return [
                            TextContent(
                                type="text",
                                text="Error: Either 'project' or 'project_id' parameter is required",
                            )
                        ]
                    ...
                    from cloudtruth_mcp import __version__

                    result = f"CloudTruth MCP Server version: {__version__}"
                else:
        ...
        async def list_prompts() -> list[Prompt]:
            ...
            prompts_data = self.prompts_handler.list_prompts()
            return [
                Prompt(
                    name=p["name"],
        ...
        async def get_prompt(
            ...
            try:
                prompt_data = self.prompts_handler.get_prompt(name, arguments or {})
                return [
                    PromptMessage(
                        role=msg["role"],
                ...
                return [
                    PromptMessage(
                        ...
                        content=TextContent(
                            type="text",
                            text=f"Error retrieving prompt: {sanitize_error_message(str(e))}",
                        ),
                    )
                ]
