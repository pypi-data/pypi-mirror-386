...
class CloudTruthClient:
    ...
    async def set_parameter_value(
        ...
        # Fetch values for this parameter in this specific environment
        values = await self.get_parameter_values(project_id, parameter_id, environment_id)

        # Use internal_value to set the raw value
        payload = {"internal_value": value}

        if values:
            # Update existing value using the value's ID (not environment ID)
            # There should only be one value for this environment
            existing_value = values[0]
            data = await self._request(
                "PATCH",
                f"projects/{project_id}/parameters/{parameter_id}/values/{existing_value.id}/",
                json=payload,
            )
        else:
            # Value doesn't exist for this environment yet, create it
            # Environment must be a full URI for creation
            environment_url = f"{self.base_url}environments/{environment_id}/"
            payload = {"environment": environment_url, "internal_value": value}
            logger.debug(f"Creating value with payload: {payload}")
            data = await self._request(
                "POST",
                f"projects/{project_id}/parameters/{parameter_id}/values/",
                json=payload,
