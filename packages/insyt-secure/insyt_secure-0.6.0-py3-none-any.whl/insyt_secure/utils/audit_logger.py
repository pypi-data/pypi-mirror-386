...
class AuditLogger:
    ...
    def _init_database(self):
        ...
        # Clean up old data with NULL project_id
        self._cleanup_null_project_ids()
    ...
    def _cleanup_null_project_ids(self):
        """Remove old audit logs with NULL project_id."""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                
                # Count records with NULL project_id
                cursor.execute("SELECT COUNT(*) FROM execution_logs WHERE project_id IS NULL")
                null_count = cursor.fetchone()[0]
                
                if null_count > 0:
                    logger.info(f"Cleaning up {null_count} old audit logs with NULL project_id")
                    cursor.execute("DELETE FROM execution_logs WHERE project_id IS NULL")
                    conn.commit()
                    logger.info(f"Cleanup completed: Removed {null_count} old records")
        except Exception as e:
            logger.error(f"Error during NULL project_id cleanup: {str(e)}")
