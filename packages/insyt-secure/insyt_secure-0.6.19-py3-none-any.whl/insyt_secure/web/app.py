...
def _auto_apply_env_preset(env_manager: EnvPresetManager):
    """
    Auto-apply active environment preset on application startup.
    
    Reads configuration from runtime_config.json and applies the preset
    if auto_apply_enabled is True and active_preset_id is set.
    ...
    try:
        ...
        active_preset_id = runtime_config.get('env_presets.active_preset_id')
        if not active_preset_id:
            logger.info("No active environment preset configured for auto-apply")
            return
        
        # Apply the preset
        result = env_manager.apply_preset(active_preset_id)
        
        logger.info("="*60)
        logger.info("🔧 AUTO-APPLIED ENVIRONMENT PRESET")
        logger.info(f"  Preset: {result['preset_name']}")
        logger.info(f"  Variables: {result['applied_count']}")
        logger.info(f"  Keys: {', '.join(result['variables'])}")
        logger.info("  All executed code will have access to these variables")
        logger.info("="*60)
        
    except ValueError as e:
        logger.error(f"Failed to auto-apply preset: {e}")
        logger.error("Check that the preset ID in runtime_config.json is valid")
    except Exception as e:
    ...
    def api_set_auto_apply_config():
        ...
        try:
            ...
            if 'active_preset_id' in data:
                preset_id = data['active_preset_id']
                ...
                runtime_config.set('env_presets.active_preset_id', preset_id)
            
            logger.info(
                f"User '{request.username}' updated auto-apply config: "
                f"enabled={data.get('auto_apply_enabled')}, "
                f"preset_id={data.get('active_preset_id')}"
            )
            
            return jsonify({
                'success': True,
                'message': 'Auto-apply configuration saved. Will take effect on next server restart.',
                'auto_apply_enabled': runtime_config.get('env_presets.auto_apply_enabled', True),
                'active_preset_id': runtime_config.get('env_presets.active_preset_id')
            })
        except Exception as e:
