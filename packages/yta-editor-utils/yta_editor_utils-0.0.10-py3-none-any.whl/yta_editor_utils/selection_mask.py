...
class _NumpySelectionMaskGenerator:
    ...
    *For internal use only*
    ...
    def get_full_mask_for_input(
        ...
        texture.
        """
        width, height, number_of_channels = _SelectionMaskGeneratorUtils.get_width_height_and_number_of_channels_from_input(input)
    ...
    def get_full_mask(
        ...
        dtype = _SelectionMaskGeneratorUtils.get_numpy_dtype_from_moderngl_dtype(dtype)
    ...
    def _numpy_mask_to_texture(
        ...
        *For internal use only*
        ...
        This method will create a completely new texture
        so we have to use it carefully and don't use it
        when we need the best performance. This is very
        useful to make some tests, but the best way to
        handle textures is having it instantiated and
        rewriting it only, not creating it again.

        (!) The only `dtype` we accept, by now, is `f1`.
        """
        import moderngl
        ...
        def prepare_mask(
            ...
            if (
                ...
                raise Exception('The only accepted "dtype" is "f1" for moderngl, and "np.uint8" and "np.float32".')

            return (
                np.clip(mask * 255.0, 0, 255).astype(np.uint8)
                if mask.dtype == np.float32 else
                mask
            )
        ...
        mask_tex = opengl_context.texture(
            ...
            # We force the default value as we want to always
            # use these ones. We are using sampler2d in our
            # shaders so we need this dtype
            dtype = 'f1'
        )
...
class _SelectionMaskGeneratorUtils:
    """
    A class to wrap utils related to the selection
    mask generator.
    """

    @staticmethod
    def get_width_height_and_number_of_channels_from_input(
        input: Union['moderngl.Texture', 'np.ndarray']
    ) -> tuple[int, int, int]:
        """
        *For internal use only*
        ...
        if PythonValidator.is_numpy_array(input):
            return _SelectionMaskGeneratorUtils.get_width_height_and_number_of_channels_from_numpy_array(input)
        elif PythonValidator.is_instance_of(input, 'Texture'):
            return _SelectionMaskGeneratorUtils.get_width_height_and_number_of_channels_from_moderngl_texture(input)
        else:
            raise Exception('The "input" provided is not a numpy array nor a moderngl Texture.')

    @staticmethod
    @requires_dependency('numpy', 'yta_editor_utils', 'numpy')
    def get_width_height_and_number_of_channels_from_numpy_array(
        input: 'np.ndarray'
    ) -> tuple[int, int, int]:
        """
        *For internal use only*
    ...
    @requires_dependency('moderngl', 'yta_editor_utils', 'moderngl')
    def get_width_height_and_number_of_channels_from_moderngl_texture(
    ...
    @staticmethod
    @requires_dependency('numpy', 'yta_editor_utils', 'numpy')
    def get_numpy_dtype_from_moderngl_dtype(
        dtype: str
    ) -> 'np.ndarray':
        """
        *For internal use only*
        ...
        *The "numpy" library is required"
        ...
        if dtype != 'f1':
            raise Exception(f'Unexpected dtype "{dtype}". The accepted value is "f1".')

        # TODO: By now we are accepting 'f1' only so
        # this is just to have an structure to make
        # it work in the next steps (if accepting more
        # types)
        return np.uint8
    ...
    def numpy_to_float32(
        ...
        _SelectionMaskGeneratorUtils._validate_numpy_input(input)
        
        return (
            # np.uint8 has values in [0, 255]
            # np.floate2 has values in [0.0, 1.0]
            (input.astype(np.float32) / 255.0)
            if input.dtype == np.uint8 else
            input
        )
    ...
    def numpy_to_uint8(
        ...
        return (
            ...
            if input.dtype == np.float32 else
            input
        )
    ...
    def _validate_numpy_input(
        ...
        if input.dtype not in [np.uint8, np.float32]:
            raise Exception('The type of the "input" array provided is not "np.uint8" nor "np.float32".')
...
class SelectionMaskGenerator:
    ...
    *Class to be used as a static class*
    ...
    selection masks.
    """
    utils: _SelectionMaskGeneratorUtils = _SelectionMaskGeneratorUtils
    """
