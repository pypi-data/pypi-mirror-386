...
class TextureUtils:
    """
    Class to be used as a static class, including
    some utils related to handling and converting
    OpenGL textures.

    Check the optional libraries that are needed
    to use the different methods included in this
    class.
    """
    ...
    def texture_to_file(
        ...
        *Optional dependency `pillow` (imported as `PIL`) required*
        ...
        return numpy_to_file(
            input = TextureUtils.texture_to_frame(texture),
    ...
    def from_frame(
        ...
        *Optional dependency `moderngl` (imported as `moderngl`) required*
        ...
        # TODO: This is not ok as it is not considering the
        # different types of dtypes we can have and handle
        # and we are doing that in other part of the code...
...
class NumpyTextureUtils:
    """
    A class to wrap utils related to the numpy and
    texture conversion (with the dtypes and values).
    """
    ...
    def numpy_to_float32(
        ...
        NumpyTextureUtils._validate_numpy_input(input)
    ...
    def _validate_numpy_input(
        ...
        if input.dtype not in [np.uint8, np.float32]:
            raise Exception(f'The type of the "input" array provided is not "np.uint8" nor "np.float32" but "{input.dtype}".')