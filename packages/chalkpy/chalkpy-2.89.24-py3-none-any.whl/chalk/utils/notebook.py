...
def _build_script(
    ...
    script_parts.append(f"    {fn_name}()")

    return "\n".join(script_parts)


def parse_notebook_into_script(fn: Callable[[], None]) -> str:
    """
    ...
    if len(sig.parameters) > 0:
        raise ValueError(
            f"Function {fn.__name__} must take no inputs, but has parameters: {list(sig.parameters.keys())}"
        )
