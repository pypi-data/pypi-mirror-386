...
class QueryRequest(BaseModel):
    ...
    limit: int | None = Field(
        None, ge=1, le=constants.API_MAX_PAGE_SIZE, description="Maximum records to return"
    )
...
async def batch_operations(
    ...
    if len(request.operations) > constants.API_MAX_BATCH_SIZE:
        raise HTTPException(
            status_code=400, detail=f"Batch size exceeds maximum of {constants.API_MAX_BATCH_SIZE}"
        )
