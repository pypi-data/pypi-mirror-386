# This file is modified from https://github.com/Dao-AILab/flash-attention/blob/581b68d5a9cabbae959d4a4f99b13c30cdbbf689/flash_attn/ops/triton/rotary.py
# As of 2025-04-23, we require triton >= 3.0
# the main difference is that we want to support [seqlen_ro, nheads, headdim] for cos and sin while original code only supports [seqlen_ro, headdim]
...
def rotary_kernel(
    ...
    COS = COS + (
        rh[:, None, None] * stride_cos_nheads
        + rm_cs[None, :, None] * stride_cos_seqlen
        + rk_half[None, None, :] * stride_cos_headdim
    )
    SIN = SIN + (
        rh[:, None, None] * stride_cos_nheads
        + rm_cs[None, :, None] * stride_cos_seqlen
...
def apply_rotary(
    ...
    Arguments:
        ...
        cos: (seqlen_ro, nheads, rotary_dim / 2)
        sin: (seqlen_ro, nheads, rotary_dim / 2)
    ...
    assert nheads_ro == nheads, "nheads_ro must be equal to nheads"
    ...
    with torch.cuda.device(x.device.index):
        torch.library.wrap_triton(rotary_kernel)[grid](
            ...
            cos.stride(-3),  # seqlen_stride
            cos.stride(-2),  # nheads stride
            cos.stride(-1),  # headdim stride
            rotary_dim,
