"""Reproducible Walk Forward benchmark for independent audit.

This script provides a fully reproducible benchmark for validating Walk Forward
...
Usage:
    python audit_walk_forward_benchmark.py <bundle_name> [options]

Example:
    python audit_walk_forward_benchmark.py quandl --num-runs 10 --num-windows 5 --output-dir profiling-results/audit
...
def run_realistic_backtest(
    ...
    sids_to_use = asset_sids[: min(3, len(asset_sids))]
...
def run_walk_forward_baseline(
    ...
    """Run Walk Forward WITHOUT optimizations (baseline).

    Uses real bundle data with granular data access to measure baseline performance.

    Args:
        bundle_name: Name of bundle to use
        num_windows: Number of walk-forward windows
        trials_per_window: Number of optimization trials per window
    ...
    # Load bundle WITHOUT optimizations
    ...
    # Simulate walk-forward by running multiple backtests
    # In real WF, each window would run multiple parameter combinations
    total_backtests = num_windows * trials_per_window

    grid = GridSearchAlgorithm(parameter_space=param_space)

    backtest_count = 0
    while not grid.is_complete() and backtest_count < total_backtests:
        params = grid.suggest()

        # Run realistic backtest with granular data access
...
def run_walk_forward_optimized(
    ...
    """Run Walk Forward WITH optimizations (optimized).

    Enables all optimization layers:
    - Layer 1: Asset list caching
    - Layer 2: Data caching for repeated window access
...
def run_benchmark_suite(
    ...
    Args:
        bundle_name: Name of bundle to benchmark
        num_runs: Number of runs (≥10 for 95% CI)
        num_windows: Number of walk-forward windows
        trials_per_window: Optimization trials per window
        num_assets: Number of assets to test
        start_date: Start date (YYYY-MM-DD)
        end_date: End date (YYYY-MM-DD)
    ...
    run_walk_forward_baseline(bundle_name, 2, 5, num_assets, start_date, end_date)

    logger.info("warmup", phase="optimized")
    run_walk_forward_optimized(bundle_name, 2, 5, num_assets, start_date, end_date)
    ...
    for i in range(num_runs):
        elapsed = run_walk_forward_baseline(
            bundle_name, num_windows, trials_per_window, num_assets, start_date, end_date
        )
    ...
    for i in range(num_runs):
        elapsed = run_walk_forward_optimized(
            bundle_name, num_windows, trials_per_window, num_assets, start_date, end_date
        )
    ...
    results = {
        "metadata": {
            ...
            "trials_per_window": trials_per_window,
    ...
    raw_data_file = raw_data_dir / f"walk_forward_{timestamp}.json"
...
def generate_report(results: dict[str, Any], output_dir: Path) -> None:
    ...
    """
    report_path = output_dir / "walk_forward_audit_report.md"

    with open(report_path, "w") as f:
        f.write("# Walk Forward Optimization Benchmark - Independent Audit Report\n\n")
        ...
        f.write(f"- **Walk-Forward Windows**: {meta['num_windows']}\n")
        f.write(f"- **Trials per Window**: {meta['trials_per_window']}\n")
        ...
        f.write("### Optimized (All Layers + Parallel Execution)\n\n")
        ...
        f.write("---\n\n")
        f.write("*Report generated by audit_walk_forward_benchmark.py*\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Reproducible Walk Forward benchmark for independent audit"
    )
    parser.add_argument(
        "--num-runs", type=int, default=10, help="Number of runs (≥10 recommended for 95%% CI)"
    )
    parser.add_argument("--num-windows", type=int, default=5, help="Number of walk-forward windows")
    parser.add_argument(
        "--trials-per-window", type=int, default=50, help="Optimization trials per window"
    )
    parser.add_argument(
        "--num-assets", type=int, default=8, help="Number of assets to use from bundle"
    )
    parser.add_argument(
    ...
    parser.add_argument("bundle_name", type=str, help="Bundle name (e.g., mag-7)")
    ...
    logger.info(
        ...
        runs=args.num_runs,
        windows=args.num_windows,
        trials=args.trials_per_window,
        assets=args.num_assets,
    ...
    print("\n" + "=" * 80)
    print("WALK FORWARD OPTIMIZATION AUDIT - SUMMARY")
    print("=" * 80)
    ...
    print("=" * 80)
    print(f"\nFull report: {args.output_dir / 'walk_forward_audit_report.md'}")
