...
from rustybt.utils.functional import instance
...
from rustybt.utils.pandas_utils import ignore_pandas_nan_categorical_warning
...
class MissingValueMismatch(ValueError):
    ...
    def __init__(self, left, right):
        super(MissingValueMismatch, self).__init__(
            "LabelArray missing_values don't match: left={}, right={}".format(left, right)
        )
...
class CategoryMismatch(ValueError):
    ...
    def __init__(self, left, right):
        ...
        if len(mismatches) == 0:
            raise AssertionError("Not actually a mismatch!")
...
class LabelArray(ndarray):
    ...
    def __setitem__(self, indexer, value):
        ...
        if isinstance(value, self.SUPPORTED_SCALAR_TYPES):
            ...
            elif self.missing_value == value.missing_value and set(value.categories) <= set(
                self.categories
            ):
    ...
    def view(self, dtype=_NotPassed, type_=_NotPassed):
        if type_ is _NotPassed and dtype not in (_NotPassed, self.dtype):
