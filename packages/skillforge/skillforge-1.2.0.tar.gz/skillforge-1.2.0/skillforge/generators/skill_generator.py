...
class SkillGenerator:
    ...
    def generate_skill(
        ...
        try:
            ...
            # Check if template is directory-based (with references/) or single file
            is_directory_template = template_path.is_dir()

            if is_directory_template:
                # Load main SKILL.md.template
                main_template_path = template_path / "SKILL.md.template"
                template_content = self._load_template(main_template_path)

                # Load references templates
                references_dir = template_path / "references"
                reference_templates = {}
                if references_dir.exists():
                    for ref_file in references_dir.glob("*.md.template"):
                        # e.g., "routing" from "routing.md.template"
                        ref_name = ref_file.name.replace('.md.template', '')
                        reference_templates[ref_name] = ref_file.read_text(encoding='utf-8')

                print(f"✓ Template loaded: {template_path} (directory with {len(reference_templates)} references)\n")
            else:
                # Single file template (legacy)
            ...
            print("Step 5/7: Processing templates...")
            skill_content = self.process_template(template_content, variables)

            # Process reference templates if any
            processed_references = {}
            if reference_templates:
                for ref_name, ref_template in reference_templates.items():
                    processed_references[ref_name] = self.process_template(ref_template, variables)
                print(f"✓ Templates processed ({len(skill_content)} chars main + {len(reference_templates)} references)\n")
            else:
            ...
            skill_path = self._save_skill(
                ...
                variables=variables,
                references=processed_references if reference_templates else None
            )
    ...
    def get_template_path(self, skill_type: str) -> Path:
        ...
        # Priority: 1) directory/ 2) .template file 3) .hbs file (legacy)
        for subdir in ["tech-stack", "workflow", "integration"]:
            # Check for directory-based template (new format with references/)
            dir_path = self.template_dir / subdir / skill_type
            if dir_path.is_dir() and (dir_path / "SKILL.md.template").exists():
                logger.info(f"Found directory template: {dir_path}")
                return dir_path

            # Check for .template file
            template_file = self.template_dir / subdir / f"{skill_type}.template"
            if template_file.exists():
                logger.info(f"Found template file: {template_file}")
                return template_file

            # Check for .hbs file (legacy)
            hbs_file = self.template_dir / subdir / f"{skill_type}.hbs"
            if hbs_file.exists():
                logger.info(f"Found legacy template: {hbs_file}")
                return hbs_file
    ...
    def _save_skill(
        ...
        Args:
            ...
            references: Optional dict of reference files {name: content}
        """
        ...
        if references:
            ...
            for ref_name, ref_content in references.items():
                # ref_name is like "routing.md", save as "routing.md"
                ref_path = references_dir / f"{ref_name}.md"
                try:
                    ref_path.write_text(ref_content, encoding='utf-8')
                    logger.info(f"Saved reference to {ref_path}")
                except OSError as e:
                    logger.warning(f"Failed to save reference {ref_name}: {e}")
