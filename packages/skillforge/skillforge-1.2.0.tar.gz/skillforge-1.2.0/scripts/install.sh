...
# ============================================================================
print_step "Step 5/8: Installing Core Skill Files"
...
# ============================================================================
print_step "Step 6/8: Installing Slash Commands"
...
# ============================================================================
# STEP 7: Verify/Configure Context7 MCP
# ============================================================================
print_step "Step 7/8: Configuring Context7 MCP Server"

CLAUDE_CONFIG="$HOME/.claude.json"

echo -e "${DIM}Checking Context7 MCP server...${RESET}"
sleep 0.2
echo ""

# Check if Context7 is already configured
if [ -f "$CLAUDE_CONFIG" ]; then
    echo -ne "  ${DIM}Checking ~/.claude.json${RESET} "
    sleep 0.3

    # Check if context7 exists in mcpServers
    if command -v jq &> /dev/null; then
        # Use jq if available
        if jq -e '.mcpServers.context7' "$CLAUDE_CONFIG" &> /dev/null; then
            echo -e "${GREEN}✓${RESET}"
            sleep 0.2
            echo ""
            print_success "Context7 MCP already configured"
            echo -e "  ${DIM}Server: context7${RESET}"
            echo -e "  ${DIM}Command: npx -y @upstash/context7-mcp${RESET}"

            # Show Context7 status
            echo ""
            print_info "Context7 will provide up-to-date documentation for SkillForge"
        else
            echo -e "${YELLOW}!${RESET} ${DIM}Not found${RESET}"
            sleep 0.2
            echo ""

            print_warning "Context7 MCP not configured"
            echo ""
            echo -e "${CYAN}Configuring Context7 MCP server...${RESET}"
            sleep 0.3

            # Configure Context7
            if command -v claude &> /dev/null; then
                echo ""
                echo -e "  ${DIM}Running: claude mcp add context7 -- npx -y @upstash/context7-mcp${RESET}"
                sleep 0.5

                claude mcp add context7 -- npx -y @upstash/context7-mcp 2>&1 | while IFS= read -r line; do
                    echo -e "  ${DIM}$line${RESET}"
                    sleep 0.05
                done || {
                    print_warning "Failed to auto-configure Context7"
                    echo ""
                    echo -e "${YELLOW}Please run manually:${RESET}"
                    echo -e "  ${CYAN}claude mcp add context7 -- npx -y @upstash/context7-mcp${RESET}"
                }

                echo ""
                print_success "Context7 MCP configured successfully"
            else
                print_warning "Claude Code CLI not found. Please configure Context7 manually:"
                echo ""
                echo -e "  ${CYAN}claude mcp add context7 -- npx -y @upstash/context7-mcp${RESET}"
                echo ""
                echo -e "${DIM}Or add to ~/.claude.json manually:${RESET}"
                echo -e '  "context7": {'
                echo -e '    "type": "stdio",'
                echo -e '    "command": "npx",'
                echo -e '    "args": ["-y", "@upstash/context7-mcp"],'
                echo -e '    "env": {}'
                echo -e '  }'
            fi
        fi
        ...
        if grep -q '"context7"' "$CLAUDE_CONFIG"; then
            ...
            sleep 0.2
            echo ""
            print_warning "Context7 MCP not configured (install jq for better detection)"
            echo ""
            echo -e "${CYAN}To configure Context7 manually:${RESET}"
            echo -e "  ${CYAN}claude mcp add context7 -- npx -y @upstash/context7-mcp${RESET}"
        fi
    fi
else
    echo -e "${YELLOW}!${RESET} ${DIM}~/.claude.json not found${RESET}"
    sleep 0.2
    echo ""
    print_warning "Claude Code configuration file not found"
    echo ""
    echo -e "${DIM}Context7 will be configured when you first run Claude Code.${RESET}"
    echo -e "${DIM}Or run manually:${RESET}"
    echo -e "  ${CYAN}claude mcp add context7 -- npx -y @upstash/context7-mcp${RESET}"
fi
