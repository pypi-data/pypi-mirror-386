...
print_banner() {
    ...
    echo ""
    sleep 0.3
    slow_print "              ${BOLD}${WHITE}Meta-Programming Framework for Claude Code${RESET}" $FAST_DELAY
    sleep 0.2
    slow_print "              ${DIM}Transform Claude into YOUR expert developer${RESET}" $FAST_DELAY
    echo ""
    echo -e "${C3}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
    echo ""
    sleep 0.5
...
slow_print "${DIM}Verifying system requirements...${RESET}" $FAST_DELAY
echo ""
sleep 0.3
...
if [ "$1" == "--local" ] || [ -f "pyproject.toml" ]; then
    slow_print "${CYAN}Installing from local source...${RESET}" $MEDIUM_DELAY
    echo ""
    sleep 0.3
    ...
    slow_print "${CYAN}Installing from PyPI...${RESET}" $MEDIUM_DELAY
    echo ""
    sleep 0.3
...
slow_print "${DIM}Setting up directories...${RESET}" $MEDIUM_DELAY
echo ""
sleep 0.3
...
slow_print "${DIM}Copying skill files from package...${RESET}" $MEDIUM_DELAY
echo ""
sleep 0.3
...
slow_print "${DIM}Copying slash commands...${RESET}" $MEDIUM_DELAY
echo ""
sleep 0.3
...
if [ -d "$SKILL_FILES_SOURCE/commands/sf" ]; then
    ...
    slow_print "${DIM}Available commands:${RESET}" $FAST_DELAY
    sleep 0.2
...
slow_print "${DIM}Running installation tests...${RESET}" $MEDIUM_DELAY
echo ""
sleep 0.3
...
slow_print "  ${BOLD}${WHITE}1.${RESET} ${BOLD}Restart Claude Code${RESET}" $FAST_DELAY
slow_print "     ${DIM}Close and reopen Claude to load SkillForge${RESET}" $FAST_DELAY
echo ""
sleep 0.2

slow_print "  ${BOLD}${WHITE}2.${RESET} ${BOLD}Run the Setup Wizard${RESET}" $FAST_DELAY
slow_print "     ${CYAN}claude${RESET} ${DIM}(in any project)${RESET}" $FAST_DELAY
slow_print "     ${CYAN}/sf:wizard${RESET}" $FAST_DELAY
echo ""
sleep 0.2

slow_print "  ${BOLD}${WHITE}3.${RESET} ${BOLD}Generate Your First Skill${RESET}" $FAST_DELAY
slow_print "     ${DIM}Answer a few questions about your preferences${RESET}" $FAST_DELAY
slow_print "     ${DIM}SkillForge will generate personalized skills for your stack${RESET}" $FAST_DELAY
echo ""
sleep 0.2

slow_print "  ${BOLD}${WHITE}4.${RESET} ${BOLD}Start Coding!${RESET}" $FAST_DELAY
slow_print "     ${DIM}SkillForge will automatically orchestrate skills${RESET}" $FAST_DELAY
slow_print "     ${DIM}Claude will code exactly the way YOU do${RESET}" $FAST_DELAY
...
# Final message
slow_print "${BOLD}${WHITE}Welcome to SkillForge!${RESET} 🎨" $FAST_DELAY
slow_print "${DIM}Transform Claude Code into YOUR expert developer.${RESET}" $FAST_DELAY
echo ""
sleep 0.3
