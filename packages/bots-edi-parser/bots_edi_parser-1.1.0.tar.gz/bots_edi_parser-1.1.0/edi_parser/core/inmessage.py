...
# edi_parser modules
from . import global_config as botsglobal
from ..lib import utils as botslib
from . import grammar
from . import message
from . import node
# from . import outmessage  # Not needed for parsing
from .config import (
    OK,
    TRANSLATED,
    SUBTRANSLATION,
...
class Inmessage(message.Message):
    ...
    def initfromfile(self):
        ...
        if leftover:
            ...
            # In validate_only mode, collect error instead of raising
            if self.ta_info.get('validate_only', False):
                self.add2errorlist(str(error_msg))
            else:
                raise error_msg
    ...
    def _formatfield(self, value, field_definition, structure_record, node_instance):
        ...
        """
        # Check validation mode - skip validation if configured
        validation_mode = self.ta_info.get('field_validation_mode', 'lenient')
        if validation_mode == 'skip':
            return value  # Skip all field validation and formatting checks
    ...
    def _parse(self, structure_level, inode):
        ...
        while True:
            ...
            if current_lex_record is None \
                ...
                if current_lex_record is not None and self.ta_info.get('allow_flexible_optional_order', True):
                    # First, look ahead for a match
                    lookahead_index = structure_index

                    while lookahead_index < structure_end:
                        # Stop at required segments - don't skip over them
                        if structure_level[lookahead_index][MIN] > 0 and lookahead_index != structure_index:
                            break

                        # Check if this position matches our segment
                        if structure_level[lookahead_index][ID] == current_lex_record[ID][VALUE]:
                            # Found it! Jump to this position
                            botsglobal.logger.debug(
                                'Flexible ordering: matched segment "%(seg)s" at grammar position %(new)s '
                                '(skipped forward from position %(old)s)',
                                {'seg': current_lex_record[ID][VALUE], 'old': structure_index, 'new': lookahead_index}
                            )
                            structure_index = lookahead_index
                            found_match_ahead = True
                            break

                        lookahead_index += 1

                    # If not found ahead, look backward through skipped optional positions
                    if not found_match_ahead and structure_index > 0:
                        lookback_index = structure_index - 1

                        while lookback_index >= 0:
                            # Only look back at optional segments that were skipped
                            if structure_level[lookback_index][MIN] > 0:
                                break  # Stop at required segments

                            # Check if this skipped position matches our segment
                            if structure_level[lookback_index][ID] == current_lex_record[ID][VALUE]:
                                # Found it! Jump back to this position
                                botsglobal.logger.debug(
                                    'Flexible ordering: matched segment "%(seg)s" at grammar position %(new)s '
                                    '(looked back from position %(old)s)',
                                    {'seg': current_lex_record[ID][VALUE], 'old': structure_index, 'new': lookback_index}
                                )
                                structure_index = lookback_index
                                found_match_ahead = True
                                break

                            lookback_index -= 1

                # If no match found ahead, use original strict validation logic
                if not found_match_ahead:
                    if structure_level[structure_index][MIN] and not countnrofoccurences:
                        ...
                        try:
                            ...
                            # In validate_only mode, collect error and continue
                            if self.ta_info.get('validate_only', False):
                                self.add2errorlist(str(error_msg))
                            else:
                                raise error_msg
    ...
    def _readcontent_edifile(self):
        ...
        # Check if content was passed directly (for standalone parser)
        if '_edi_content' in self.ta_info:
            content = self.ta_info['_edi_content']
            if isinstance(content, bytes):
                self.rawinput = content.decode(
                    self.ta_info['charset'],
                    self.ta_info.get('checkcharsetin', 'strict')
                )
            else:
            ...
            self.rawinput = botslib.readdata(
                ...
                errors=self.ta_info.get('checkcharsetin', 'strict'),
            )
    ...
    def _lex(self):
        ...
        for char in self.rawinput:
            ...
            if char in record_sep:  # end of record
                ...
                # Check for empty segment based on configuration
                # An empty segment has no meaningful content (all values are empty strings)
                is_empty_segment = all(not field[VALUE] or field[VALUE].strip() == '' for field in lex_record)
                if is_empty_segment:
                    empty_handling = self.ta_info.get('empty_segment_handling', 'skip')
                    if empty_handling == 'error':
                        raise InMessageError(
                            _('[A68]: Empty segment found at line %(countline)s, position %(countpos)s.'),
                            {'countline': countline, 'countpos': countpos}
                        )
                    elif empty_handling == 'warn':
                        botsglobal.logger.warning(
                            'Empty segment found at line %s, position %s - skipping',
                            countline, countpos
                        )
                    # For 'skip' or 'warn' mode, skip adding this empty record
                    lex_record = []
                    value = ''
                    sfield = 0
                    mode_inrecord = 0
