...
class UserManager:
    ...
    async def oauth_callback(
        ...
        *,
        ...
        :param oauth_name: OAuth 클라이언트 이름.
        :param token_data: OAuth 토큰 데이터
        :param profile_id: 서비스 공급자 내 사용자의 ID
        :param profile_email: 서비스 공급자 측 사용자의 이메일
        :param profile_image: 프로필 이미지 URL
        :param fullname: 사용자 전체 이름
        :param request: 작업을 트리거한 선택적 FastAPI 요청, 기본값은 None
        :param associate_by_email: 이메일로 기존 계정과 연결할지 여부
        :return: 사용자 객체.
        """
        oauth_account_dict = {
        ...
        try:
            ...
            try:
                ...
                # 4. associate_by_email=False이고 이메일이 이미 존재하는 경우
                # 새 계정 생성 (다른 이메일 필요)
                raise UserAlreadyExists(
                    "이메일이 이미 존재하며 OAuth 계정 연결이 허용되지 않습니다. "
                    "다른 이메일을 사용하거나 기존 계정으로 로그인해 주세요."
                )
