...
def get_reddit_account_by_index(index: int) -> dict:
    """Get Reddit account by index with round-robin cycling."""
    return REDDIT_ACCOUNTS[index % len(REDDIT_ACCOUNTS)]

def get_total_reddit_accounts() -> int:
    """Get total number of available Reddit accounts."""
    return len(REDDIT_ACCOUNTS) 