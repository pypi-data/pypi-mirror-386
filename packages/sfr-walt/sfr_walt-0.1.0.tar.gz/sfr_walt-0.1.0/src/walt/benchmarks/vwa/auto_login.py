...
# Get DATASET from environment with a helpful error message
try:
    DATASET = os.environ["DATASET"]
except KeyError:
    raise RuntimeError(
        "DATASET environment variable is required for benchmark evaluation.\n"
        "Please set DATASET to 'webarena' or 'visualwebarena' in your .env file or environment.\n"
        "If you're not running benchmarks, this module should not be imported."
    ) from None
if DATASET == "webarena":
    ...
    URLS = [
        ...
        # f"{CLASSIFIEDS}/index.php?page=user&action=items",  # debug uses same as classifieds
    ]
...
async def is_expired_async(
    ...
    async with async_playwright() as playwright:
        ...
        await page.goto(url)
        await page.wait_for_timeout(3000)  # Replace time.sleep with async wait
        d_url = page.url
...
def get_reddit_account_for_task(process_id: int = None, task_info: str = None) -> dict:
    ...
    # Generate hash and map to account index
    hash_value = int(hashlib.md5(hash_input.encode()).hexdigest(), 16)
    account_index = hash_value % len(REDDIT_ACCOUNTS)

    account = REDDIT_ACCOUNTS[account_index]
    print(
