...
from playwright.async_api import Playwright  # Keep for type annotations


def get_async_playwright(use_stealth: bool = True):
	"""Get async_playwright function - stealth (patchright) by default, regular playwright when disabled."""
	if use_stealth:
		try:
			from patchright.async_api import async_playwright
			return async_playwright
		except ImportError:
			# Fallback to regular playwright if patchright not available
...
class BrowserConfig:
	...
	Default values:
		...
		use_stealth: True
			Use patchright for bot detection evasion (slower startup, better success rates)
...
class Browser:
	...
	async def _init(self):
		...
		# Get the appropriate async_playwright function based on stealth setting
		async_playwright = get_async_playwright(self.config.use_stealth)
