...
def manual():
    """Deprecated in Xpress 9.8.

    Refer to the online documentation at https://www.fico.com/fico-xpress-optimization/docs/latest/solver/optimizer/python/HTML/
    instead.
    """
    msg = re.sub(r'\s+', ' ', manual.__doc__).strip()
    warnings.warn(msg, _xpress.DeprecationWarning, stacklevel=2)
    return None
    ...
    try:
        if platform.system() == 'Windows':
            ...
            try:
                ...
                # xprsws.dll and xprscuda.dll with LoadLibrary later
                lib_paths.append(solver_libs_dir)
                lib_paths.append(os.path.join(solver_libs_dir, 'xpressdeps'))
                lib_paths.append(xpresslibs.cuda_libs_dir)
            except ModuleNotFoundError:
                # In a Conda package, xprs.dll and the CUDA libraries are in
                # <env>\Library\bin, so we only need to add xpressdeps to the PATH
...
def __getattr__(name):
    if name == 'controls':
        ...
        return controls
    elif name in ['npvar', 'npexpr', 'npconstraint']:
        warnings.warn("Deprecated in Xpress 9.6: use xpress.array(..., dtype='O') instead", _xpress.DeprecationWarning, stacklevel=2)
        return getattr(_xpress, f'_{name}')
...
class ConstraintType(enum.IntEnum):
    ...
    def __call__(self, lhs, rhs):
        with setConstraintOperatorsEnabled(True):
            # Make sure Xpress overloaded operators are used
            if isinstance(lhs, numpy.ndarray):
                lhs = lhs.view(ndarray)
            elif isinstance(rhs, numpy.ndarray):
                rhs = rhs.view(ndarray)
...
try:
    ...
    # Pandas is installed, so we can register our extension array
    from .pandas_ import ObjectDType
    register_extension_dtype(ObjectDType)


# Export public symbols from _xpress and sub-packages
from _xpress import *
from .enums import *
from .numpy_ import ndarray, array

__all__ = [name for name in dir(_xpress) if name[0] != '_'] + dir(enums) + [
    'enums', 'constants', 'leq', 'geq', 'eq', 'rng', 'nonbinding', 'ndarray', 'array'
]
