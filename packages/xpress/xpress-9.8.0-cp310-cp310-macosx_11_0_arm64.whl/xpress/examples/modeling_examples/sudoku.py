...
# sub-grids. These subgrids are lists of tuples with the coordinates of each
# subgrid. In a 9x9 sudoku, for instance, subgrids[0,1] has the 9
# elements in the top square, while subgrids[2,2] has the 9
# elements in the bottom right square. The input is a starting grid
# where the unknown numbers are replaced by zero.
#
...
# We don't need an objective function, we are just looking for a feasible
# solution.
...
for i in N:
    for j in N:
        l = [k for k in N if xsol[i, j, k] >= 0.5]
        assert(len(l) == 1)
