...
def solve_knapsack_problem(n_item_types, profit, resource_consumed, total_resource, demand):
    ...
    # Create a NumPy array of variables
    x = xp.array([p.addVariable(name='use_{}'.format(i), lb=0, ub=demand[i], vartype=xp.integer) for i in N])
...
def cutting_stock_model():
    ...
    v_patterns = xp.array([
        p.addVariable(name='pat_{}'.format(i), lb=0, ub=int(float(demand[i] / patterns[i][i]) + 1))
        for i in N
    ])
    ...
    for npass in range(max_cols):
        ...
        print(x_best)
        cur_pat = p.addVariable(name='pat_{}'.format(n_widths + npass),
