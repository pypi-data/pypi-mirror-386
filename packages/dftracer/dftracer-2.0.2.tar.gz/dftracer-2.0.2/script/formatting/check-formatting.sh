...
find include \( -name "*.c" -o -name "*.cpp" -o -name "*.h" -o -name "*.hpp" \) -print0 | xargs -0 -P "$(nproc)" $clang_format_exe --dry-run -Werror
include_check_rc=$?

echo "Check formatting of C/C++ code in 'test'"
find test \( -name "*.c" -o -name "*.cpp" -o -name "*.h" -o -name "*.hpp" \) -print0 | xargs -0 -P "$(nproc)" $clang_format_exe --dry-run -Werror
test_check_rc=$?

cd $curr_dir

if [ $src_check_rc -ne 0 ] || [ $include_check_rc -ne 0 ] || [ $test_check_rc -ne 0 ]; then
