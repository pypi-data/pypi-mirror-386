...
class PDPConfig:
    ...
    token_url: str
    scope: str = ""
    audience: Optional[str] = None  # RFC 8707 - Resource Indicators for OAuth 2.0
...
class EnhancedPDP:
    ...
    def __init__(
        ...
        if config:
            ...
            # RFC 8707 audience support: prefer explicit parameter, fallback to env var
            resolved_audience = audience or os.getenv("PDP_TOKEN_AUDIENCE")
            self.config = PDPConfig(
                base_url, client_id, client_secret, token_url, scope, resolved_audience
            )
    ...
    async def _get_access_token(self) -> str:
        ...
        # Include audience parameter when specified for strict JWT validation
        if self.config.audience:
            data["audience"] = self.config.audience
            self.logger.debug(f"Requesting token with audience: {self.config.audience}")
