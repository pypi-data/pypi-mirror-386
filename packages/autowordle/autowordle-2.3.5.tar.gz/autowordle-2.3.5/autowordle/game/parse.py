...
def parse_lines(linelist):
    ...
    res['success'] = not re.search('[-X]/[0-9]|X&[0-9]/[0-9]', res['caption'])
    return res
...
class GameParser:
    def __init__(self, game_definitions):
        self.reparse_registry = {i.code: self._make_reparse_function(i)
                                 for i in game_definitions if i.reparse_actions != []}
    ...
    def load_record(self, db_result):
        if db_result.image is not None and db_result.image.startswith(b'HASH:\n'):
            logger.warning('Image in game record has been replaced with hash.')
        if game_text := db_result.text:
            ...
            logger.warning(f'No data for {db_result}; using missing_game')
