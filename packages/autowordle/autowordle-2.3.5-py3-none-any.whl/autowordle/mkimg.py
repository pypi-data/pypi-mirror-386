...
from .res import CHAR_TO_FILE, colours, colr_res, file_res, exe
from . import game as gamelib

def _ensure_imagemagick_loaded():
    global Image
    global Color
    global Drawing
    global Font
...
class Mkimg:
    ...
    def summary_subsequence(self, seq, layout, vertical, recur=0):
        if isinstance(layout, str):
            with Image() as game_image:
                ...
                if game is None:
                    logger.warning(f'Game {layout} not found in collected games; '
                ...
                self.game_image_pixels += game_image.width * game_image.height
    ...
    def make_image(self):
        ...
        with (self.header.make_image(self.activitywatch.time_taken()) as header,
            ...
            efficiency = self.game_image_pixels / (img.width * img.height)
            logger.info(f"Game image {efficiency = :.3f}")
    ...
    def summary_image(self):
        ...
        with (self.make_image() as final,
            ...
            final.format = 'png'
            final.save(f)
            subprocess.run([exe.pngcrush, f.name, outfile])
...
def send_result_to_clipboard(cfg):
    subprocess.run(
        [exe.xclip, '-sel', 'c', '-target', 'text/uri-list'],
