...
def compress_image(image):
    with tempfile.NamedTemporaryFile(prefix="autowordle_", suffix='.png', mode='wb',
        ...
        image.save(tmpfile)
        with subprocess.Popen([exe.pngcrush, tmpfile.name, '/dev/stdout'],
...
def clipboard():
    if re.search(
            "image/",
            subprocess.run(
                [exe.xclip, "-sel", "c", "-target", "TARGETS", "-out"],
                capture_output=True).stdout.decode("UTF-8")):
        return Image.open(
            io.BytesIO(
                subprocess.run(
                    [exe.xclip, "-sel", "c", "-target", "image/png", "-out"],
