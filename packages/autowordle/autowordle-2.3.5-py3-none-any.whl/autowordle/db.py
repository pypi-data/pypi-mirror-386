...
class Db:
    ...
    def __init__(self, cfg):
        if location := cfg.dat['archives'].get('db'):
            self.db_location = os.path.expanduser(location)
        else:
            self.db_location = (
                platformdirs.user_data_dir('autowordle', ensure_exists=True) /
                "archive.db")
    ...
    latest_genimg_query = "SELECT max(gentime) FROM outimgs WHERE julianday(gentime) < julianday(?);"
    def latest_generated_image(self, as_of=None):
        with (self as db, contextlib.closing(db.cursor()) as cur):
            latest = cur.execute(self.latest_genimg_query,
                                 (as_of or now_timestring(),)).fetchone()
