...
from .game import GameParser, Redactle, MISSING_GAME
...
class GameImage:
    ...
    def __init__(self, font=None):
        self.font = font or SimpleNamespace(family='Noto-Sans-CJK-TC', size=18)
...
@GameImage.handler(MISSING_GAME)
...
def redactle_result_grid(font, game: Redactle, result_grid):
    result_grid.read(filename=file_res('redactle-base.png'))
    guess_width = math.log(game.guesses, 10) * 100
    accuracy_width = game.accuracy / 100 * 300
    with Drawing() as ctx:
        ...
        ctx.text(0, 18, str(game.guesses))
        ctx.text(0, 55, str(game.accuracy) + '%')
        ctx(result_grid)
    return result_grid
...
class Mkimg:
    ...
    def load_games(self):
        saved = dict()
        for game in self.config.collection.collection:
            saved[game.short_name] = self.game_parser.load_record(game.result)
        return saved
    ...
    def summary_subsequence(self, seq, layout, keys, vertical, recur=0):
        if isinstance(layout, str):
            with Image() as game_image:
                seq.image_add(
                    self.game_image_maker.make_image(
                        keys.get(layout) or GameParser.missing_game(layout),
                        game_image))
    ...
    def summary_image(self):
        with (self.header.make_image(self.activitywatch.time_taken()) as final,
            ...
            final.gravity = 'north'
            final.image_add(timeline)
            self.summary_subsequence(final, self.layout, self.load_games(), True)
