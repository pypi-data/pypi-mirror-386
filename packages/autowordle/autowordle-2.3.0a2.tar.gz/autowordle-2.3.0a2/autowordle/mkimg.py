...
def redactle_result_grid(font, game, result_grid):
    ...
    guess_width = math.log(int(game.extra_results.guesses), 10) * 100
    accuracy_width = float(game.extra_results.accuracy) / 100 * 300
    with Drawing() as ctx:
        ...
        ctx.text(0, 18, game.extra_results.guesses)
        ctx.text(0, 55, game.extra_results.accuracy + '%')
        ctx(result_grid)
    return result_grid
...
class Mkimg:
    def __init__(self, config):
        ...
        self.game_parser = gamelib.GameParser()
            ...
            with Image() as game_image:
                seq.image_add(
                    self.game_image_maker.make_image(
                        keys.get(layout) or gamelib.GameParser.missing_game(layout),
                        game_image))
