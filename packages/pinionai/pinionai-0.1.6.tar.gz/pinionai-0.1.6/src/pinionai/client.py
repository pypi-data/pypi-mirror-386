_Be="Twilio 'from' number not configured. Cannot send SMS."
_Bd='completedAt'
_Bc='Error: URL template evaluation failed.'
_Bb='contentType'
_Ba='access_token'
_BZ='token_type'
_BY='client_credentials'
_BX='function_call'
_BW='Using environment credentials/IAM role for S3.'
_BV='boto3 library is required for S3 operations.'
_BU='Using Application Default Credentials for GCS.'
_BT='gcs_service_account'
_BS='google-cloud-storage library is required for GCS operations.'
_BR='action_action_key'
_BQ='__builtins__'
_BP='destinationVariable'
_BO='sourceVariable'
_BN='promptName'
_BM='unknown_intent'
_BL='Error: AI response was malformed (function call had no name).'
_BK='resp_mime_type'
_BJ='gemini-2.5-flash-preview-04-17'
_BI='safetySettings'
_BH='mime_type'
_BG='noInputIntentNameList'
_BF='variables'
_BE='DEEPSEEK_API_KEY'
_BD='ANTHROPIC_API_KEY'
_BC='OPENAI_API_KEY'
_BB='EMAIL_FROM_ADDRESS'
_BA='EMAIL_API_KEY'
_B9='GEMINI_API_KEY'
_B8='GCP_REGION'
_B7='GCP_PROJECT'
_B6='WEBSOCKET_URL'
_B5='user_input'
_B4='Journey token not available.'
_B3='Accept-Encoding'
_B2='client_secret'
_B1='client_id'
_B0='grant_type'
...
_Au='function'
_At='header'
_As='gemini-2.0-flash'
_Ar='model_provider'
_Aq='agentDefaultIntent'
_Ap='intentContainsPairsList'
_Ao='intentNameList'
_An='value'
_Am='action_flow'
_Al='action_type'
...
_Ab='privacy'
_Aa='prompt_type'
_AZ='ragStoreVectorDistanceThreshold'
_AY='ragStoreTopK'
_AX='ragStoreResourceId'
_AW='transferPasskeyFlag'
_AV='transferAllowed'
_AU='TWILIO_NUMBER'
_AT='TWILIO_AUTH_TOKEN'
_AS='TWILIO_ACCOUNT_SID'
_AR='parameters'
_AQ='Accept'
_AP='failureResponseMessage'
_AO='gzip'
_AN='response'
_AM='string'
_AL='enum'
...
class AsyncPinionAIClient:
	...
	def __init__(self,create_key,agent_id,host_url,client_id,client_secret,version=_A,initial_engagement_data=_A,grpc_server_address=_A,gcp_project_id=_A,gcp_region=_A,gemini_api_key=_A,twilio_account_sid=_A,twilio_auth_token=_A,twilio_number=_A,email_api_key=_A,email_from_address=_A,openai_api_key=_A,anthropic_api_key=_A,deepseek_api_key=_A):
		...
		self._agent_id=agent_id;self._host_url=host_url.rstrip('/');self._client_id=client_id;self._client_secret=client_secret;self._version=version;self._grpc_server_address=grpc_server_address or os.environ.get(_B6,'localhost:50051');self._gcp_project_id=gcp_project_id or os.environ.get(_B7);self._gcp_region=gcp_region or os.environ.get(_B8);self._gemini_api_key=gemini_api_key or os.environ.get(_B9);self._twilio_account_sid=twilio_account_sid or os.environ.get(_AS);self._twilio_auth_token=twilio_auth_token or os.environ.get(_AT);self._twilio_number=twilio_number or os.environ.get(_AU);self._email_api_key=email_api_key or os.environ.get(_BA);self._email_from_address=email_from_address or os.environ.get(_BB);self._openai_api_key=openai_api_key or os.environ.get(_BC);self._anthropic_api_key=anthropic_api_key or os.environ.get(_BD);self._deepseek_api_key=deepseek_api_key or os.environ.get(_BE);self._http_session=httpx.AsyncClient(base_url=self._host_url,timeout=12e1,follow_redirects=_D);self._token=_A;self._session_id=_A;self._raw_session_data=_A;self.var={};self.engagement=''
	...
	async def _initialize_session_and_vars(self):
		...
		if _C in self._raw_session_data and self._raw_session_data[_C]:
			self.var=self._extract_data_from_session(self._raw_session_data[_C]);agent_data=self._raw_session_data[_C][_E][_C][_J];self.var['sessionId']=self._session_id;self.var['agentTitle']=agent_data.get('title');self.var['agentSubtitle']=agent_data.get('subtitle');self.var[A]=agent_data.get(A);self.var['userImage']=agent_data.get('userImagePath','').strip();self.var['assistImage']=agent_data.get('assistantImagePath','').strip();self.var[_AV]=agent_data.get(_AV);self.var[_AW]=agent_data.get(_AW);self.var[B]=agent_data.get(B);self.var['sessionDateTime']=datetime.datetime.now(zoneinfo.ZoneInfo('UTC')).astimezone().isoformat();self.var[_o]='';startStatement=self._clean_text(agent_data.get('startStatement'));self.var['agentStart']=self._evaluate_f_string(startStatement or'',self.var);stores_data=agent_data.get('stores',[]);self._rag_stores_by_name={}
			if stores_data and isinstance(stores_data,list)and len(stores_data)>0:
				for s in stores_data:
					try:
						name=s.get(_B)
						if not name:continue
						self._rag_stores_by_name[name]={_p:s.get(_p,''),_q:s.get(_q,10),_r:s.get(_r,.5)}
					except Exception:continue
			if self._rag_stores_by_name:first_store_cfg=next(iter(self._rag_stores_by_name.values()));self.var[_AX]=first_store_cfg.get(_p,'');self.var[_AY]=first_store_cfg.get(_q,10);self.var[_AZ]=first_store_cfg.get(_r,.5)
			else:self.var[_AX]='';self.var[_AY]=10;self.var[_AZ]=.5
	...
	async def process_user_input(self,user_input='',sender=_a):
		...
		if not self.transfer_requested:
			...
			if self.next_intent:self.var['prompt_resp_var']=self.next_intent;prompt_type=self.var[_Aa]=_A5;user_input=self.next_intent;intent_data=self._get_intent_details(self.next_intent);self.next_intent=''
			else:
				...
				prompt_type=self.var.get(_Aa);intent_data=self._get_intent_details(self.var[prompt_resp_var_name])
			self.var[_o]=''
			...
			intent_type=intent_data[_F];self.var[B]=intent_data[_B];self._privacy_level=self.var['privacy_resp']=intent_data[_Ab];self.current_pipeline=self.var['pipeline_resp']=intent_data[_s];delivery_method=intent_data[_Ac];language=intent_data[_b];intent_input_vars=self._normalize_intent_vars(intent_data.get(A,{}));context_flow=intent_data.get(E,{});intent_subprompt=context_flow.get(C,{});intent_finalprompt=context_flow.get(D,{});intent_preprocess=context_flow.get(F,{});intent_subprocess=context_flow.get(G,{});intent_finalprocess=context_flow.get(H,{});intent_actionprocess=intent_data.get(_Ad,{})
			if self.var.get(_H):logger.debug(f"Classification | {self.var[B]}");logger.debug(f"Sensitivity | {self._privacy_level}");logger.debug(f"Intent Type | {intent_type}");logger.debug(f"Intent Var (ActionKey) | {self.current_pipeline}")
			if intent_type!=_Ae:
				if intent_input_vars:
					agent_vars_config=self._raw_session_data.get(_C,{}).get(_E,{}).get(_C,{}).get(_J,{}).get(_BF,[])
				...
				if prompt_type==_A5:self._class_message=user_input;self._class_intent_data=intent_data
				elif prompt_type==C:self._sub_message=user_input
				elif prompt_type==D:self._fin_message=user_input
			elif intent_type==_Ae:
				action_key_for_input=intent_data.get(_s,'')
				...
				if self._class_intent_data:intent_data=self._class_intent_data;user_input=self._class_message;intent_type=intent_data[_F];self.var[B]=intent_data[_B];delivery_method=intent_data[_Ac];language=intent_data[_b];intent_input_vars=self._normalize_intent_vars(intent_data.get(A,{}));context_flow=intent_data.get(E,{});intent_subprompt=context_flow.get(C,{});intent_finalprompt=context_flow.get(D,{});intent_preprocess=context_flow.get(F,{});intent_subprocess=context_flow.get(G,{});intent_finalprocess=context_flow.get(H,{});intent_actionprocess=intent_data.get(_Ad,{});self._privacy_level=intent_data[_Ab];self.current_pipeline=intent_data[_s]
				...
				if action_key_for_input not in original_keys and self._class_intent_data:final_response=f"'{user_input}' is a {intent_data[_B]} . Please respond to trigger an intent or input I am listening for such as: {self.var.get(_BG,[])}."
			if intent_input_vars:
				...
				if waiting_for_inputs:
					...
					if final_response:self.chat_messages.append({_R:_Af,_Q:final_response})
					...
					if not self.authorized and'private'in self._privacy_level or I in self._privacy_level:
						...
						if intent_data[J]:
							...
							if customer_id and enrolled:
								pipeline_key=self.var[K]if I in self._privacy_level else self.var['authentication_pipeline'];json_payload={_Ag:pipeline_key,_j:{_d:delivery_method,_e:self.phone_number},_Ah:{_Ai:self.unique_id,_e:self.phone_number},_E:{_Aj:self._session_id},_b:language}
								if self.var.get(_k):json_payload[_Ak]=self.var[_k]
								...
								auth_response_msg=f"You are not enrolled. We are sending a link to {self.phone_number} so that you can enroll your device.";json_payload={_Ag:self.var['enroll_pipeline'],_j:{_d:delivery_method,_e:self.phone_number},_Ah:{_Ai:self.unique_id,_e:self.phone_number},_E:{_Aj:self._session_id},_b:language}
								if self.var.get(_k):json_payload[_Ak]=self.var[_k]
			...
			if not final_response:
				if intent_type=='fixed':fixed_message_template=self._clean_text(intent_data['fixedResponseMessage']);final_response=self._evaluate_f_string(fixed_message_template,self.var)
				elif intent_type==_A6:
					action_details_from_intent=self._map_to_standard_action_details(intent_data);action_type=action_details_from_intent.get(_Al)
					if action_type=='journey':self.var,final_response=await self._run_journey_action(self.var,action_details_from_intent)
						...
						action_flow=action_details_from_intent.get(_Am)
						if action_flow:
					...
					if intent_finalprompt:
						for(_indx,item_config)in sorted(intent_finalprompt.items()):
							if isinstance(item_config,dict):
								if _A7 in item_config:
									target_prompt=item_config[_A7];self.var,resp_var_name=await self._prompt_response(target_prompt,user_input)
									if resp_var_name and self.var.get(resp_var_name):final_response=self.var[resp_var_name]
								elif _l in item_config:
									mcp_name=item_config[_l];self.var,mcp_response=await self._run_mcp_action(self.var,mcp_name,user_input)
	...
	async def update_pinion_session(self):
		...
		data_to_post[_E][_t]=self.var;messages_for_api=[{k:v for(k,v)in msg.items()if k!='avatar'}for msg in self.chat_messages];data_to_post[_E][_u]=messages_for_api;response_obj,response_data=await self._post_session_api(self._host_url,self._token,self._session_id,data_to_post,self.transfer_requested,self.transfer_accepted)
	...
	async def _process_routing(self,current_var,process_items,initial_final_response=_A,user_input=_A):
		...
		for(_key,item_config)in sorted(process_items.items()):
			...
			elif _j in item_config:current_var,step_response=await self._run_delivery_action(current_var,item_config[_j])
			elif _A6 in item_config:current_var,step_response=await self._run_action_action(current_var,item_config[_A6],generated_final_response,user_input=user_input)
			elif _l in item_config:current_var,step_response=await self._run_mcp_action(current_var,item_config[_l],user_input)
	...
	def _extract_data_from_session(self,session_data_root):
		'Extracts variables and agent configuration into the var dictionary.';var_dict={};agent_config=session_data_root.get(_E,{}).get(_C,{}).get(_J,{});global_to_self_attr_map={_B6:'_grpc_server_address',_B7:'_gcp_project_id',_B8:'_gcp_region',_B9:'_gemini_api_key',_AS:'_twilio_account_sid',_AT:'_twilio_auth_token',_AU:'_twilio_number',_BA:'_email_api_key',_BB:'_email_from_address',_BC:'_openai_api_key',_BD:'_anthropic_api_key',_BE:'_deepseek_api_key'}
		for variable_def in agent_config.get(_BF,[]):
			name=variable_def[_B];value=variable_def[_An];var_type=variable_def.get(_F)
			if variable_def.get(_A8)=='global':
		...
		for intent in agent_config.get('intents',[]):
			...
			if intent[_F]=='information'or intent[_F]==_A6:no_input_intent_names.append(intent_name)
			if intent[_F]==_Ae:input_intent_names.append(intent_name)
			intent_privacy_pairs.append((intent_name,intent[_Ab]));intent_type_pairs.append((intent_name,intent[_F]));intent_contains_word_pairs.append((intent_name,intent.get('containsWord','')));intent_action_key_pairs_raw.append((intent_name,intent.get(_s,'')))
		var_dict[_Ao]=intent_names;var_dict[_BG]=no_input_intent_names;var_dict['inputIntentNameList']=input_intent_names;var_dict['intentPrivacyPairsList']=intent_privacy_pairs;var_dict['intentTypePairsList']=intent_type_pairs;var_dict[_Ap]=intent_contains_word_pairs;resolved_action_key_pairs=[]
		...
		var_dict['intentActionPairsList']=resolved_action_key_pairs;var_dict[_A9]=agent_config.get(_A9);var_dict[_Aq]=agent_config.get('defaultIntent');return var_dict
	async def _classify_input(self,user_input):
		if not self.var.get(_A9):logger.error('startPrompt not defined in agent configuration.');return self.var,_A
		_current_var_state,resp_var_name=await self._prompt_response(self.var[_A9],user_input)
	...
	async def _collect_required_inputs(self,current_var,required_vars):
		...
		if not required_vars:current_var[_o]='';return current_var,_A,_G
		for(key_req,prompt_msg)in required_vars:
			val=current_var.get(key_req)
			if val is _A or isinstance(val,str)and not val.strip()or isinstance(val,(list,dict))and not val:current_var[_o]=f"Strongly consider the expected input field should be {key_req}";final_prompt=prompt_msg or f"Please provide a value for {key_req}.";return current_var,final_prompt,_D
		current_var[_o]='';return current_var,_A,_G
	async def _prompt_response(self,target_prompt,user_input=''):
		...
		self.var[_Aa]=prompt_config[_F];prompt_resp_var_name=prompt_config[_AA];prompt_body_template=self._clean_text(prompt_config[_U]);current_prompt_text=self._evaluate_f_string(prompt_body_template,self.var,user_input=user_input);model_provider=prompt_config.get(_Ar,_c).lower();model_name=prompt_config.get(_V);llm_response_text=_A;function_calls=_A;system_instruction=self._evaluate_f_string(self._agent_description or'',self.var)
		if model_provider==_c:
			content_parts=[]
			if prompt_config.get(_AB):
				file_uri_template=self._clean_text(prompt_config[_AB]);file_uri=self._evaluate_f_string(file_uri_template,self.var)
				if file_uri:content_parts.append(Part.from_uri(file_uri=file_uri,mime_type=prompt_config[_BH]))
			content_parts.append(current_prompt_text);gen_config=google_genai_types.GenerateContentConfig(system_instruction=system_instruction if system_instruction else _A,temperature=prompt_config[_AC],top_p=prompt_config[_W],top_k=prompt_config[_AD],candidate_count=prompt_config[_AE],max_output_tokens=prompt_config[_AF],stop_sequences=prompt_config.get('stop')or _A,safety_settings=prompt_config.get(_BI))
			if model_name in[_BJ]and prompt_config.get(_AG)is not _A:gen_config.thinking_config=ThinkingConfig(thinking_budget=prompt_config[_AG])
			if prompt_config.get(_AH):gen_config.response_schema=prompt_config[_AH];gen_config.response_mime_type=prompt_config.get(_BK,_M)
		...
		elif model_provider in[_g,_v,_AI]:
			messages=[]
			if system_instruction and model_provider in[_g,_AI]:messages.append({_R:_w,_Q:system_instruction})
			...
			elif model_provider==_v:anthropic_messages=[m for m in messages if m[_R]!=_w];llm_response_text,function_calls=await self._get_anthropic_response_async(model_name,system_instruction,anthropic_messages,prompt_config,tools_for_provider)
			elif model_provider==_AI:llm_response_text,function_calls=await self._get_deepseek_response_async(model_name,messages,prompt_config,tools_for_provider)
		...
		if function_calls:
			...
			if not function_name:logger.error(f"LLM response contained a function call with no name: {function_call}");return self.var,_BL
		...
		if llm_response_text:
			if prompt_config[_F]==_A5:
				raw_response=llm_response_text.strip().lower()
				if self.var.get(_H):logger.debug(f"Raw Intent Response | {raw_response}")
				matched_intent_key=self._match_intent_from_text(raw_response,self.var.get(_Ap,[]),self.var.get(_Aq,_BM),self.var.get(_Ao,[]));self.var[prompt_resp_var_name]=matched_intent_key
	...
	async def _async_prompt_response(self,target_prompt,user_input='',var_snapshot=_A):
		...
		prompt_resp_var_name=prompt_config[_AA];prompt_body_template=self._clean_text(prompt_config[_U]);current_prompt_text=self._evaluate_f_string(prompt_body_template,current_var,user_input=user_input);model_provider=prompt_config.get(_Ar,_c).lower();model_name=prompt_config.get(_V);llm_response_text=_A;function_calls=_A;system_instruction=self._evaluate_f_string(self._agent_description or'',current_var)
		if model_provider==_c:
			content_parts=[]
			if prompt_config.get(_AB):
				file_uri_template=self._clean_text(prompt_config[_AB]);file_uri=self._evaluate_f_string(file_uri_template,current_var)
				if file_uri:content_parts.append(Part.from_uri(file_uri=file_uri,mime_type=prompt_config[_BH]))
			content_parts.append(current_prompt_text);gen_config=google_genai_types.GenerateContentConfig(system_instruction=system_instruction if system_instruction else _A,temperature=prompt_config[_AC],top_p=prompt_config[_W],top_k=prompt_config[_AD],candidate_count=prompt_config[_AE],max_output_tokens=prompt_config[_AF],stop_sequences=prompt_config.get('stop')or _A,safety_settings=prompt_config.get(_BI))
			if model_name in[_BJ]and prompt_config.get(_AG)is not _A:gen_config.thinking_config=ThinkingConfig(thinking_budget=prompt_config[_AG])
			if prompt_config.get(_AH):gen_config.response_schema=prompt_config[_AH];gen_config.response_mime_type=prompt_config.get(_BK,_M)
			gen_config.tools=self._configure_tools_for_request(direct_tool_configs,processed_fd_dicts,model_name);llm_response_text,_,function_calls=await self._get_gemini_response_async(model_name,content_parts,gen_config)
		elif model_provider in[_g,_v]:
			messages=[]
			if system_instruction and model_provider==_g:messages.append({_R:_w,_Q:system_instruction})
			messages.append({_R:_a,_Q:current_prompt_text});tools_for_provider=self._translate_tools_for_provider(model_provider,direct_tool_configs,processed_fd_dicts)
			if model_provider==_g:llm_response_text,function_calls=await self._get_openai_response_async(model_name,messages,prompt_config,tools_for_provider)
			elif model_provider==_v:anthropic_messages=[m for m in messages if m[_R]!=_w];llm_response_text,function_calls=await self._get_anthropic_response_async(model_name,system_instruction,anthropic_messages,prompt_config,tools_for_provider)
		...
		if function_calls:
			function_call=function_calls[0];function_name=function_call.get(_B);function_args=function_call.get(_I,{})
			if not function_name:logger.error(f"LLM async response contained a function call with no name: {function_call}");return prompt_resp_var_name,_BL
		...
		if llm_response_text:
			if prompt_config[_F]==_A5:
				...
				final_value_for_var=self._match_intent_from_text(raw_response,current_var.get(_Ap,[]),current_var.get(_Aq,_BM),current_var.get(_Ao,[]))
	...
	def _get_mcp_details(self,mcp_name,current_var):
		...
		if mcp_config:
			mcp_config=self._evaluate_vars_in_structure(mcp_config,current_var);mcp_config[_K]=self._clean_text(mcp_config.get(_K,''));mcp_config[_X]=self._evaluate_f_string(mcp_config.get(_X,''),current_var);mcp_config[_x]=self._clean_text(mcp_config.get(_x,''));mcp_config['cwd']=self._clean_text(mcp_config.get('cwd',''))
			...
			if mcp_config.get(_y):mcp_config[_y]={k:self._clean_text(v)for(k,v)in mcp_config[_y].items()}
	...
	async def _run_prompt_action(self,user_input,intent_subprompt):
		...
		for(_indx,current_prompt_item)in sorted(intent_subprompt.items()):
			if _A7 in current_prompt_item:
				prompt_value=current_prompt_item[_A7]
			...
			elif _l in current_prompt_item:
				self.var,step_response=await self._run_mcp_action(self.var,current_prompt_item[_l],user_input)
	...
	async def _select_mcp_config(self,mcp_group_names,current_var,user_input,mcp_group_config):
		...
		select_mcp_declaration=FunctionDeclaration(name=B,description="Select the most appropriate MCP from the list based on the user's request.",parameters={_F:_m,_AJ:{A:{_F:_AM,_K:'The name of the selected MCP.',_AL:[mcp.get(_B)for mcp in mcp_options]}},_AK:[A]});prompt_name=mcp_group_config.get(_BN,'');prompt_config=_A;prompt_resp_var_name=_A;current_prompt_text=user_input or''
		if prompt_name:
			...
			prompt_model_provider=prompt_config.get('modelProvider',_c);prompt_model=_As
			if prompt_config.get(_V):prompt_model=prompt_config.get(_V)
			prompt_resp_var_name=prompt_config.get(_AA,'');prompt_body_template=self._clean_text(prompt_config.get(_U,''));mcp_group_prompt_text=self._evaluate_f_string(prompt_body_template,current_var,user_input=user_input)
	...
	async def _run_mcp_action(self,current_var,mcp_name,user_input=_A):
		...
		try:
			...
			prompt_name=mcp_config.get(_BN,'');prompt_config=_A;prompt_resp_var_name=_A;current_prompt_text=user_input or''
			if prompt_name:
				logger.info(f"MCP '{mcp_name}' is using prompt '{prompt_name}'.");prompt_config,_,_=self._get_prompt_details(prompt_name,current_var)
				if not prompt_config:error_msg=f"Error: Prompt '{prompt_name}' for MCP '{mcp_name}' not found.";logger.error(error_msg);return current_var,error_msg
				prompt_resp_var_name=prompt_config.get(_AA,'');prompt_body_template=self._clean_text(prompt_config.get(_U,''));current_prompt_text=self._evaluate_f_string(prompt_body_template,current_var,user_input=user_input)
			mcp_transport=mcp_config.get(F,'');mcp_url=mcp_config.get(_X,'');mcp_description=mcp_config.get(_K,'');mcp_command=mcp_config.get(_x,'');mcp_args=mcp_config.get(_I,{});mcp_cwd=mcp_config.get('cwd','');mcp_args=mcp_config.get(_I,[]);mcp_params=mcp_config.get('parametersJson',{});mcp_env=mcp_config.get(_y,{});mcp_name=mcp_config.get(_B,'default');required_inputs=[]
			if mcp_params:
			...
			session_context=_A;is_http_call=mcp_transport in[G,L]or not mcp_transport and mcp_url.startswith(('http://','https://'));is_stdio_call=mcp_transport=='stdio'or not mcp_transport and not mcp_url and mcp_config.get(_x)
			if is_http_call:
				eval_mcp_url=mcp_url;http_headers=self._evaluate_vars_in_structure(mcp_config.get(_At,{}),current_var);mcp_connector=mcp_config.get(_T,'')
				if mcp_connector:
				...
				command=self._evaluate_f_string(self._clean_text(mcp_config.get(_x,'')),current_var)
				...
				mcp_env_from_config=self._evaluate_vars_in_structure(mcp_config.get(_y,{}),current_var);final_env=_A
			...
			if session_context:
				async with session_context as session:
					...
					llm_tools=[]
					if mcp_tools:llm_tools.append(FunctionDeclaration(name=H,description='Calls a specific tool available on the MCP server.',parameters={_F:_m,_AJ:{B:{_F:_AM,_AL:[t.name for t in mcp_tools],_K:'The name of the tool to call.'},A:{_F:_m,_K:'The arguments for the tool as a JSON object.'}},_AK:[B,A]}))
					if mcp_resources:llm_tools.append(FunctionDeclaration(name=I,description='Reads a specific resource (e.g., a file or data entry) from the MCP server.',parameters={_F:_m,_AJ:{C:{_F:_AM,_AL:[r.uri for r in mcp_resources],_K:'The URI of the resource to read.'}},_AK:[C]}))
					if mcp_prompts:llm_tools.append(FunctionDeclaration(name=J,description='Gets a pre-defined, rendered prompt template from the MCP server.',parameters={_F:_m,_AJ:{D:{_F:_AM,_AL:[p.name for p in mcp_prompts],_K:'The name of the prompt to get.'},E:{_F:_m,_K:'The arguments for rendering the prompt template.'}},_AK:[D]}))
					if not llm_tools:return current_var,f"Error: MCP '{mcp_name}' has no tools, resources, or prompts to use."
					logger.info(f"Asking LLM to choose a mcp activity: '{current_prompt_text[:100]}...'");model_provider=prompt_config.get(_Ar,_c)if prompt_config else _c;model_to_use=_As
					...
					if hasattr(mcp_result,_z)and getattr(mcp_result,_z)is not _A:final_result_value=getattr(mcp_result,_z)
	...
	async def _call_mcp_tool(session,tool_name,args):
		...
		try:
			...
			if hasattr(result,_z)and result.result is not _A:return result.result
			return result
	...
	async def _execute_tool_calls(function_calls,session):
		...
		for func_call in function_calls:
			...
			try:
				...
				else:tool_result_payload={_z:result_text}
	...
	async def _run_agent_loop(self,prompt,client,session,model_id,max_tool_turns,initial_temperature,tool_call_temperature,mcp_description=_A):
		...
		while has_function_calls and turn_count<max_tool_turns:
			turn_count+=1;function_calls_to_execute=[part.function_call for part in latest_content.parts if part.function_call];tool_response_parts=await self._execute_tool_calls(function_calls_to_execute,session);contents.append(google_genai_types.Content(role=_Au,parts=tool_response_parts));subsequent_config=google_genai_types.GenerateContentConfig(temperature=tool_call_temperature,**base_gen_config_dict);response=await client.aio.models.generate_content(model=model_id,contents=contents,config=subsequent_config)
	...
	async def _run_api_action(self,current_var,api_name):
		...
		headers={}
		if api_config.get(_At):
			try:headers.update(json.loads(api_config[_At]))
	...
	def _run_parser_action(self,current_var,parser_name):
		...
		source_var_name=parser_config.get(_BO)
	...
	def _run_merger_action(self,current_var,merger_name):
		...
		dest_var_name=merger_config.get(_BP)
		...
		try:
			if output_type==_A1:current_var[dest_var_name]=json.loads(merged_output_str)
	...
	async def _run_script_action(self,var,script_name):
		...
		try:
			if script_type=='javascript':
				...
				local_script_vars=var.copy();logger.warning('Executing Python scripts from the database is a security risk and is currently disabled.');exec(script_code,{_BQ:{}},local_script_vars)
	...
	async def _evaluate_rule(self,expr,var):
		...
		try:
			...
			try:return eval(formatted_expr,{_BQ:{}},{_t:var})
	...
	async def _execute_outcome(self,outcome_statement,outcome_type,user_input):
		...
		if outcome_type=='Set a variable'and outcome_statement.get(A):
			set_config=outcome_statement.get(A);variable_to_set=set_config.get('variable');value_to_set=set_config.get(_An)
			...
			response_template=self._clean_text(outcome_statement.get(_AN,''));step_response=self._evaluate_f_string(response_template,self.var)
			...
			auth_value=outcome_statement.get(_An)
	...
	async def _run_journey_action(self,current_var,action_details):
		"Handles a 'journey' type action, from either an intent or an action entity.";B='configuration';A='payment_description';action_key_var_name=action_details.get(_BR);action_key=current_var.get(action_key_var_name)if action_key_var_name else _A
		...
		delivery_method=action_details.get(_Aw);language=action_details.get(_b);_,self.unique_id,self.phone_number=format_phone(self.phone_number);json_payload={_Ag:action_key,_j:{_d:delivery_method,_e:self.phone_number},_Ah:{_Ai:self.unique_id,_e:self.phone_number},_E:{_Aj:self._session_id},_b:language}
		if current_var.get(_k):json_payload[_Ak]=current_var[_k]
		if action_details.get(_Ax)=='credit card payment'and current_var.get(A):json_payload[B]={'credit-card-payment':{'currency':current_var.get('payment_currency'),'lineItems':[{'title':current_var.get(A),'amount':current_var.get('payment_amount'),'quantity':current_var.get('payment_quantity')}]}}
		if action_details.get(_Ax)=='one time passcode':json_payload[B]={'random-code':{'code':'6'}}
		if action_details.get(_Aw)=='voice':json_payload[_j]={_d:delivery_method,_e:self.phone_number,'callOperator':'twilio'}
	...
	def _run_transfer_action(self,current_var,action_details):
		...
		if current_var.get(_AV):
			...
			if current_var.get(_AW):passkey=self._generate_password(6);current_var['transferPasskey']=passkey;sms_message=f"Your agent will validate themselves with the following shared passkey: {passkey}"
		...
		return current_var,final_response
	def _map_to_standard_action_details(self,data):'Maps fields from an intent or action entity to a standard action_details dictionary.';return{_B:data.get(_B),_Al:data.get('actionType'),_BR:data.get(_s),_Aw:data.get(_Ac),_b:data.get(_b),_Ax:data.get('actionEvent'),_f:data.get('actionResponseMessage'),_Am:data.get(_Ad,{})}
	async def _run_action_action(self,current_var,action_name,final_response_from_caller=_A,user_input=_A):
		...
		action_details=self._map_to_standard_action_details(action_entity_data);action_type=action_details.get(_Al,'general')
		...
		action_flow=action_details.get(_Am,{});action_produced_final_response=_A
		if action_flow:
	...
	async def _gcs_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not storage:raise ImportError(_BS)
		gcs_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_A2)==_BT:sa_info=json.loads(conn_config.get(_A3,'{}'));gcs_client=storage.Client.from_service_account_info(sa_info);logger.info(f"Using GCS service account from connector '{connector_name}'.")
		if not gcs_client:gcs_client=storage.Client();logger.info(_BU)
	...
	async def _s3_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not boto3:raise ImportError(_BV)
		s3_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_A2)=='aws_iam':s3_client=boto3.client('s3',aws_access_key_id=conn_config.get(_Ay),aws_secret_access_key=conn_config.get(_A3));logger.info(f"Using AWS IAM credentials from connector '{connector_name}'.")
		if not s3_client:s3_client=boto3.client('s3');logger.info(_BW)
	...
	async def _run_file_action(self,current_var,file_name):
		...
		try:
			...
			if is_write_op:
				...
				if file_format==_A1:content_bytes_to_write=json.dumps(content,indent=2).encode(_N)
			...
			if is_read_op and read_content_bytes is not _A:
				try:
					if file_format==_A1:current_var[dest_var]=json.loads(read_content_bytes)
	...
	def _configure_tools_for_request(self,direct_tool_configs,processed_fd_dicts,model_name):
		...
		RESTRICTED_MODELS_FOR_MIXED_TOOLS={_As};has_functions=bool(function_declarations);has_search=any(t.google_search for t in final_tools_for_config);model_is_restricted=model_name in RESTRICTED_MODELS_FOR_MIXED_TOOLS
	...
	def _resolve_rag_store(self,store_name):
		...
		if hasattr(self,'_rag_stores_by_name')and self._rag_stores_by_name:
			if store_name in self._rag_stores_by_name:cfg=self._rag_stores_by_name[store_name];return cfg.get(_p,default_resource),cfg.get(_q,default_top_k),cfg.get(_r,default_vector_distance_threshold)
			for(k,cfg)in self._rag_stores_by_name.items():
				if k.strip().lower()==normalized:return cfg.get(_p,default_resource),cfg.get(_q,default_top_k),cfg.get(_r,default_vector_distance_threshold)
		...
		try:
			...
			if response.candidates and len(response.candidates)>0:
				...
				if candidate.content and candidate.content.parts:
					fc_parts=[part.function_call for part in candidate.content.parts if hasattr(part,_BX)and part.function_call]
					if fc_parts:
						function_calls=[]
			...
			return response_text,list(response.candidates)if hasattr(response,_AE)else[],function_calls
	...
	async def _get_openai_response_async(self,model_name,messages,prompt_config,tools=_A):
		...
		http_client=httpx.AsyncClient();client=AsyncOpenAI(api_key=self._openai_api_key);openai_params={_V:model_name,_u:messages,_A4:prompt_config.get(_AC),_Az:prompt_config.get(_AF),_W:prompt_config.get(_W),'frequency_penalty':prompt_config.get('frequencyPenalty'),'presence_penalty':prompt_config.get('presencePenalty')}
		if prompt_config.get(A):openai_params[A]=prompt_config.get(A)
		if tools:openai_params[_O]=tools;openai_params[_A_]='auto'
	...
	async def _get_anthropic_response_async(self,model_name,system_prompt,messages,prompt_config,tools=_A):
		...
		client=AsyncAnthropic(api_key=self._anthropic_api_key);anthropic_params={_V:model_name,_w:system_prompt,_u:messages,_A4:prompt_config.get(_AC),_Az:prompt_config.get(_AF,4096),_W:prompt_config.get(_W),_AD:prompt_config.get(_AD)}
		if tools:anthropic_params[_O]=tools;anthropic_params[_A_]={_F:'auto'}
	...
	async def _get_deepseek_response_async(self,model_name,messages,prompt_config,tools):
		...
		request_params={_V:model_name,_u:messages}
		if A in prompt_config:request_params[_Az]=prompt_config[A]
		if _A4 in prompt_config:request_params[_A4]=prompt_config[_A4]
		if _W in prompt_config:request_params[_W]=prompt_config[_W]
		if tools:request_params[_O]=tools;request_params[_A_]='auto'
		try:
	...
	async def _get_token_api(self,host,client_id,client_secret):
		url='/token';headers={_Z:_M};data={_B0:_BY,_B1:client_id,_B2:client_secret}
		try:response=await self._http_session.post(url,headers=headers,json=data);response.raise_for_status();auth_response=response.json();return f"{auth_response[_BZ]} {auth_response[_Ba]}"
	...
	async def _start_session_api(self,host,agent_id_to_start,token):
		url=f"/agent/{agent_id_to_start}";headers={_B3:_AO,_n:_A0,_L:token}
		try:
	...
	async def _start_version_api(self,host,agent_id_to_start,token,version_str):
		url=f"/version/{agent_id_to_start}/{version_str}";headers={_B3:_AO,_n:_A0,_L:token}
		try:
			response=await self._http_session.get(url,headers=headers);response.raise_for_status();data=response.json();session_uid=data.get(_C,{}).get(_E,{}).get('uid')
			if session_uid:return session_uid,data
	...
	async def _get_session_api(self,host,session_uid,token):
		url=f"/session/{session_uid}";headers={_n:_A0,_L:token,_B3:_AO}
		try:
	...
	async def _get_session_lastmodified_api(self,host,session_uid,token):
		url=f"/session/{session_uid}/lastmodified";headers={_n:_A0,_L:token}
	...
	async def _post_session_api(self,host,token,session_uid,data_payload,transfer_req,transfer_acc):
		url=f"/session";headers={_Z:_M,'Content-Encoding':_AO,_L:token};full_payload={'sessionUid':session_uid,_C:data_payload,'transferRequested':transfer_req,'transferAccepted':transfer_acc}
		try:
	...
	async def _get_token_for_connector(self,connector_config):
		'Gets auth token for a given connector configuration.';url=connector_config.get(_X);client_id=connector_config.get(_Ay);client_secret=connector_config.get(_A3);grant_type=connector_config.get(_A2,_BY);content_type=connector_config.get(_Bb,_M);use_header_payload=connector_config.get('headerPayload',_G);scope=connector_config.get(_A8,_A)
		...
		if use_header_payload:client_keys=f"{client_id}:{client_secret}";client_keys_b64=base64.b64encode(client_keys.encode()).decode();headers[_L]=f"Basic {client_keys_b64}";headers[_Z]='application/x-www-form-urlencoded';data_to_send=f"grant_type={grant_type}"
			...
			if _A1 in content_type.lower():
				data_to_send={_B0:grant_type,_B1:client_id,_B2:client_secret}
				if scope:data_to_send[_A8]=scope
			elif'x-www-form-urlencoded'in content_type.lower():
				form_data=[(_B0,grant_type),(_B1,client_id),(_B2,client_secret)]
				if scope:form_data.append((_A8,scope))
		...
		try:
			...
			async with httpx.AsyncClient()as client:response=await client.post(url,headers=headers,content=payload);response.raise_for_status();auth_response=response.json();token_type=auth_response.get(_BZ,'Bearer').capitalize();return f"{token_type} {auth_response[_Ba]}"
	...
	async def _generic_api_post_put(self,current_var,api_config,headers,method):
		...
		if not url:logger.error(f"URL could not be evaluated for API '{api_config.get(_B)}'. Template: {url_template}");return current_var,_A,_A,_Bc
		...
		try:
			...
			if response.status_code>=400 or not parsed_response:
				...
				if api_config.get(_AP):final_response_message=self._evaluate_f_string(api_config[_AP],current_var)
			return current_var,response,parsed_response,final_response_message
		except httpx.RequestError as e:status_code=e.response.status_code if hasattr(e,_AN)and e.response else _A;error_detail=e.response.text if hasattr(e,_AN)and e.response else str(e);raise PinionAIAPIError(f"API {method} request to {url} failed: {error_detail}",status_code=status_code,details=str(e))from e
	async def _generic_api_get(self,current_var,api_config,headers):
		...
		if not url:logger.error(f"URL could not be evaluated for API GET '{api_config.get(_B)}'. Template: {url_template}");return current_var,_A,_A,_Bc
		...
		try:
			...
			if response.status_code>=400:
				...
				if api_config.get(_AP):final_response_message=self._evaluate_f_string(api_config[_AP],current_var)
			return current_var,response,parsed_response,final_response_message
		except httpx.RequestError as e:error_detail=e.response.text if hasattr(e,_AN)and e.response else str(e);raise PinionAIAPIError(f"API GET request to {url} failed: {error_detail}")from e
	...
	async def _journey_lookup(self,unique_id_val,current_var,user_input=_A):
		...
		headers={_AQ:_M,_L:bearer_token};url=f"https://app.journeyid.io/api/system/customers/lookup?account_id={account_id}&unique_id={unique_id_val}"
		try:
	...
	async def _journey_send_pipeline(self,current_var,json_payload,delivery_method,user_input=_A):
		...
		if not bearer_token:return _A,_B4
		post_url='https://app.journeyid.io/api/system/executions';headers={_n:_M,_Z:_M,_L:bearer_token}
		try:
	...
	async def _journey_execution_status_check(self,execution_id,current_var,max_retries=12,delay_seconds=5,user_input=_A):
		...
		if not bearer_token:return _G,_B4
		url=f"https://app.journeyid.io/api/system/executions/{execution_id}";headers={_AQ:_M,_L:bearer_token}
		for attempt in range(max_retries):
			try:
				...
				if status_data.get(_Bd):return _D,status_data.get('outcome',{}).get(_i,'Task completed successfully.')
	...
	async def _journey_session_pipeline_status_check(self,session_id_val,current_var,pipeline_key_to_check,max_retries=12,delay_seconds=5,user_input=_A):
		...
		if not bearer_token:return _G,_B4
		url=f"https://app.journeyid.io/api/system/sessions/lookup?external_ref={session_id_val}";headers={_AQ:_M,_L:bearer_token}
		for attempt in range(max_retries):
			try:
				async with httpx.AsyncClient()as client:response=await client.get(url,headers=headers,timeout=30)
				...
				for exec_item in executions:
					if exec_item.get(A,{}).get('key')==pipeline_key_to_check or exec_item.get(A,{}).get('id')==pipeline_key_to_check:
						if exec_item.get(_Bd):return _D,exec_item.get('outcome',{}).get(_i,'Pipeline completed.')
						break
	...
	async def _twilio_sms_message(self,sms_body,to_phone,from_phone_twilio=_A):
		...
		if not twilio_from_number:logger.error(_Be);return
	...
	async def _infobip_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A,cc_address=_A,bcc_address=_A,reply_to_address=_A):
		...
		infobip_url='https://g9egk8.api.infobip.com/email/4/messages';headers={_L:token,_Z:_M,_AQ:_M};payload={_u:[{'sender':from_address,'destinations':[{'to':[{'destination':to_address}]}],_Q:{_Av:subject,_P:plain_text_body,'html':html_body_content}}]}
		try:
	...
	async def _ms_graph_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A,cc_address=_A,bcc_address=_A,reply_to_address=_A):
		...
		graph_api_endpoint='https://graph.microsoft.com/v1.0';url=f"{graph_api_endpoint}/users/{from_address}/sendMail";headers={_L:token,_Z:_M};email_payload={_i:{_Av:subject,_U:{_Bb:'HTML',_Q:html_body_content}},'saveToSentItems':'true'}
			...
			for(key,value)in list(data_structure.items()):
				if isinstance(value,str):
					if A in value or B in value:
						try:eval_globals={_t:context_vars,C:context_vars,D:context_vars,_B5:user_input};data_structure[key]=eval(f"f'''{value}'''",eval_globals,{})
			...
			for(i,item)in enumerate(data_structure):
				if isinstance(item,str):
					if A in item or B in item or user_input is not _A and'{user_input}'in item:
						try:eval_globals={_t:context_vars,C:context_vars,D:context_vars,_B5:user_input};data_structure[i]=eval(f"f'''{item}'''",eval_globals,{})
	...
	def _evaluate_f_string(self,template_string,context_vars,user_input=_A):
		...
		try:eval_namespace={_t:context_vars,_B5:user_input};return eval(f"f'''{template_string}'''",eval_namespace,{})
	...
	def _create_function_declaration_from_dict(self,declaration_dict):
		...
		name=declaration_dict.get(_B);description=declaration_dict.get(_K);parameters=declaration_dict.get(_AR)
	...
	def _translate_tools_for_provider(self,provider,direct_tool_configs,processed_fd_dicts):
		...
		translated_tools=[]
		if provider==_g:
			for fd in processed_fd_dicts:translated_tools.append({_F:_Au,_Au:{_B:fd.get(_B),_K:fd.get(_K),_AR:fd.get(_AR)}})
		elif provider==_v:
			for fd in processed_fd_dicts:translated_tools.append({_B:fd.get(_B),_K:fd.get(_K),'input_schema':fd.get(_AR)})
...
def twilio_sms_message(sms_body,to_phone,from_phone_twilio=_A):
	'Sends an SMS using Twilio, reading credentials from environment variables.';twilio_account_sid=os.environ.get(_AS);twilio_auth_token=os.environ.get(_AT)
	...
	_national_digits,_unique_id,to_phone_e164=format_phone(to_phone);twilio_from_number=from_phone_twilio or os.environ.get(_AU)
	if not twilio_from_number:logger.error(_Be);return
