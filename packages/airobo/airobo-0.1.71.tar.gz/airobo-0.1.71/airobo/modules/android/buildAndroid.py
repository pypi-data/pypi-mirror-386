    ...
    try:
        ...
        with Image.open(source_icon) as img:
            ...
            for density, size in densities.items():
                ...
                # CRITICAL: Adaptive icon foreground (this is what modern Android actually shows!)
                foreground_output = density_dir / "ic_launcher_foreground.png"
                resized.save(foreground_output, "PNG", optimize=True)
                
                print(f"   📱 {density}: {size}x{size} -> {output_path}")
                print(f"      + foreground: {foreground_output}")
            ...
            if source_splash:
                ...
                with Image.open(source_splash) as splash_img:
                    # Find all splash screen files and replace them
                    for splash_file in res_dir.rglob("*splash*"):
                        if splash_file.is_file() and splash_file.suffix.lower() in ('.png', '.jpg', '.jpeg'):
                            splash_img.save(splash_file, "PNG", optimize=True)
                            print(f"   🖼️ Updated: {splash_file.relative_to(res_dir)}")
                            splash_files_updated += 1
                
                print(f"🎨 Updated {splash_files_updated} splash screen files")
