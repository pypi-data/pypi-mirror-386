...
class ETLDatabase:
    ...
    def _normalize_data_type(self, data_type: str) -> str:
        ...
        # Convert to uppercase and remove size specifications
        ...
        # Extract base type (before parentheses or size)
        base_type = normalized.split("(")[0].strip()

        # Return canonical form
        return type_mappings.get(base_type, normalized)
    ...
    def synchronize_table_schema(
        ...
        Uses global schema cache for performance. Only first thread reads from DB,
        subsequent threads use cached schema.
        ...
        with self._table_operation_lock("schema_sync", f"ALTER TABLE {table_name}"):
            ...
            if not self.db.table_exists(table_name):
                ...
                result = self.db.create_table(table_name, target_schema)
                if result:
                    # Cache empty schema for new table
                    table_key = self._get_table_key(table_name)
                    self._queue_manager.cache_schema(table_key, {})
                return result

            # Try to get from global cache first
            table_key = self._get_table_key(table_name)
            current_columns = self._queue_manager.get_cached_schema(table_key)

            if current_columns is None:
                # Cache MISS - read from database
                msg = f"📦 Schema cache MISS for [{table_name}] - reading from DB"
                self.log.debug(msg)
                ...
                # Diagnostic logging - show actual DB response
                msg = f"🔍 Raw DB schema for [{table_name}] (first 5 columns):"
                self.log.debug(msg)
                for row in current_schema[:5]:
                    col_name = row["COLUMN_NAME"]
                    col_type = row["DATA_TYPE"]
                    max_len = row["CHARACTER_MAXIMUM_LENGTH"]
                    self.log.debug(f"  {col_name}: {col_type} (max_len={max_len})")
                ...
                # Cache the schema globally
                self._queue_manager.cache_schema(table_key, current_columns)

            # Diagnostic logging - show target schema
            self.log.debug("🎯 Target schema from DataFrame (first 5 columns):")
            for col, dtype in list(target_schema.items())[:5]:
                self.log.debug(f"  {col}: {dtype}")
            ...
            for column, target_type in target_schema.items():
                if column not in current_columns:
                    ...
                    # Normalize types for comparison to avoid false positives
                    normalized_current = self._normalize_data_type(current_type)
                    normalized_target = self._normalize_data_type(target_type)
                    ...
                    if normalized_current != normalized_target:
                        ...
                        self.log.warning(
                            f"⚠️ Type mismatch for [{column}]:\n"
                            f"  DB: '{current_type}' → '{normalized_current}'\n"
                            f"  Target: '{target_type}' → '{normalized_target}'"
                        )

            # Summary logging
            if missing_columns or type_updates:
                msg = (
                    f"📋 Schema changes for [{table_name}]: "
                    f"{len(missing_columns)} new columns, "
                    f"{len(type_updates)} type updates"
                )
                self.log.info(msg)
            ...
            if type_updates:
                ...
                self.db.execute_multiple_write_queries(";".join(alter_queries))
                msg = f"Updated {len(type_updates)} column type(s) in [{table_name}]"
                self.log.info(msg)

                # Update cache with new types
                for column, new_type in type_updates.items():
                    current_columns[column]["type"] = new_type
                self._queue_manager.cache_schema(table_key, current_columns)
            else:
                if not missing_columns:
                    self.log.debug(f"✅ Schema for [{table_name}] is already up to date")
