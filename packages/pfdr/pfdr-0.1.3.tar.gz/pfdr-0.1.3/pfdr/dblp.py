        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:  # Not the last attempt
                    print(
                        f"  HTTP {exc.code} error on attempt {attempt + 1}/{max_retries}, retrying in {retry_delay}s..."
                    )
                    ...
                    raise RuntimeError(
                        f"DBLP returned HTTP {exc.code}: {exc.reason} after {max_retries} attempts"
                    ) from exc
            except URLError as exc:
                if attempt < max_retries - 1:  # Not the last attempt
                    print(
                        f"  Network error on attempt {attempt + 1}/{max_retries}, retrying in {retry_delay}s..."
                    )
                    time.sleep(retry_delay)
                    retry_delay *= 2  # Exponential backoff
                    continue
                    ...
                    raise RuntimeError(
                        f"Unable to reach DBLP: {exc.reason} after {max_retries} attempts"
                    ) from exc
