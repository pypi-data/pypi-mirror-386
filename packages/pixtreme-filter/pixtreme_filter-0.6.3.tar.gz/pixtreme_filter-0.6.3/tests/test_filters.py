...
class TestFilters:
    ...
    def test_gaussian_blur_performance(self, sample_image):
        ...
        with px.Device(0):
            ...
            sizes = [50, 100, 500]  # Larger size difference for more reliable comparison
            times = []
            ...
            for size in sizes:
                ...
                start.record()
                blurred = px.gaussian_blur(resized, ksize=9, sigma=2.0)
                end.record()
                end.synchronize()
            ...
            assert times[2] > times[0], f"Expected {sizes[2]}x{sizes[2]} ({times[2]}ms) > {sizes[0]}x{sizes[0]} ({times[0]}ms)"
