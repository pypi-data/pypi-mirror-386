...
@dataclass
class CtimageCreateEcsSystemDiskImageLabelsRequest:
    labelKey: str  # 标签键
    labelValue: str  # 标签值
...
class CtimageCreateEcsSystemDiskImageRequest:
    ...
    description: Optional[str] = None  # 描述信息。注意：长度为 1~128 个字符，不能以空格开头或结尾
    enableImageIntegrityCheck: Optional[bool] = None  # 用于表示是否启用镜像完整性校验的标识。默认 false。注意：仅在传入的 regionID 参数所指定的资源池具备镜像完整性校验功能时生效
    labels: Optional[List[Optional[CtimageCreateEcsSystemDiskImageLabelsRequest]]] = None  # 标签列表。注意：<br />1. 列表中最多 10 个标签<br />2. 标签键不可重复<br />3. 单个标签键或值应满足长度为 1~32 个字符，不能换行，且不能以空格开头或结尾
    maximumRAM: Any = None  # 最大内存。单位为 GiB。取值范围：0（默认值，即不限制）/1/2/4/8/16/32/64/128/256/512。注意：<br />1. 若同时限制最大和最小内存，则最大内存不能小于最小内存<br />2. 仅在传入的 regionID 参数所指定的资源池具备设置最大/最小内存的功能时生效
    minimumRAM: Any = None  # 最小内存。单位为 GiB。取值范围：0（默认值，即不限制）/1/2/4/8/16/32/64/128/256/512。注意：<br />1. 若同时限制最大和最小内存，则最大内存不能小于最小内存<br />2. 仅在传入的 regionID 参数所指定的资源池具备设置最大/最小内存的功能时生效
    projectID: Optional[str] = None  # 企业项目 ID。默认 0（即 default 企业项目）。可使用<a href="https://eop.ctyun.cn/ebp/ctapiDocument/search?sid=77&api=7246&data=114&isNormal=1&vid=107" target="_blank">查询企业项目列表</a>接口来查询您可以使用的企业项目 ID
...
@dataclass
class CtimageCreateEcsSystemDiskImageReturnObjImagesResponse:
    appVersion: Optional[str] = None  # 应用版本
    architecture: Optional[str] = None  # 系统架构。取值范围与对应的请求参数相同
    ...
    @staticmethod
    def from_json(json_data: dict) -> Optional['CtimageCreateEcsSystemDiskImageReturnObjImagesResponse']:
        if not json_data:
            return None
        obj = CtimageCreateEcsSystemDiskImageReturnObjImagesResponse(
...
@dataclass
class CtimageCreateEcsSystemDiskImageReturnObjResponse:
    """成功时返回的数据"""
    images: Optional[List[Optional[CtimageCreateEcsSystemDiskImageReturnObjImagesResponse]]] = None  # 镜像列表。应包含 1 个镜像

    @staticmethod
    def from_json(json_data: dict) -> Optional['CtimageCreateEcsSystemDiskImageReturnObjResponse']:
        if not json_data:
            return None
        ...
        if "images" in json_data:
            ...
            if image_list is not None and isinstance(image_list, list):
                images = [CtimageCreateEcsSystemDiskImageReturnObjImagesResponse.from_json(img) for img in image_list]

        obj = CtimageCreateEcsSystemDiskImageReturnObjResponse(images)
        return obj

@dataclass
class CtimageCreateEcsSystemDiskImageResponse:
    ...
    returnObj: Optional[CtimageCreateEcsSystemDiskImageReturnObjResponse] = None  # 成功时返回的数据

    @staticmethod
    def from_json(json_data: dict) -> Optional['CtimageCreateEcsSystemDiskImageResponse']:
        if not json_data:
            return None
        ...
        if "returnObj" in json_data:
            ...
            if returnObj is not None:
                return_obj = CtimageCreateEcsSystemDiskImageReturnObjResponse.from_json(returnObj)

        obj = CtimageCreateEcsSystemDiskImageResponse(
...
class CtimageCreateEcsSystemDiskImageApi:
    ...
    @staticmethod
    def do(credential: Credential, client: CtyunClient, endpoint: str, request: CtimageCreateEcsSystemDiskImageRequest) -> CtimageCreateEcsSystemDiskImageResponse:
        url = endpoint + "/v4/image/create"
        params = {}
        try:
            header_params = {}
            ...
            return CtimageCreateEcsSystemDiskImageResponse.from_json(response.json())
