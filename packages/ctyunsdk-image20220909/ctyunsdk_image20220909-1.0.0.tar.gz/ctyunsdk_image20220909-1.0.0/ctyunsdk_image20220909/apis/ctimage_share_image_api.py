...
@dataclass
class CtimageShareImageRequest:
...
@dataclass
class CtimageShareImageReturnObjResponse:
    """成功时返回的数据"""
    pass  # 空类占位符
...
@dataclass
class CtimageShareImageResponse:
    ...
    returnObj: Optional[CtimageShareImageReturnObjResponse] = None  # 成功时返回的数据

    @staticmethod
    def from_json(json_data: dict) -> Optional['CtimageShareImageResponse']:
        if not json_data:
            return None
        ...
        if "returnObj" in json_data:
            ...
            if returnObj is not None:
                return_obj = CtimageShareImageReturnObjResponse.from_json(returnObj)

        obj = CtimageShareImageResponse(
...
class CtimageShareImageApi:
    ...
    @staticmethod
    def do(credential: Credential, client: CtyunClient, endpoint: str, request: CtimageShareImageRequest) -> CtimageShareImageResponse:
        url = endpoint + "/v4/image/shared-image/create"
        params = {}
        try:
            header_params = {}
            ...
            return CtimageShareImageResponse.from_json(response.json())
