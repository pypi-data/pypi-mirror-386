...
class Installer:
    ...
    def __init__(self, download_url, install_dir="/usr/local/openssh", ssl_dir=None):
        ...
        # 初始化服务管理和文件管理模块
        self.service_manager = InstallerServiceManager()
    ...
    def check_cls_service(self):
        ...
        """
        # 委托给服务管理模块
        return self.service_manager.check_service_exists('cls')
    
    def stop_cls_service(self):
        """
        停止CLS服务，同时停止xc-ssh.service（属于SSH守护）
        
        Returns:
            bool: 停止是否成功
        """
        # 检查cls服务是否存在，如果存在则停止
        if self.service_manager.check_service_exists('cls'):
            cls_stopped = self.service_manager.stop_service('cls')
            if not cls_stopped:
                print("警告: cls服务停止失败")
        else:
            print("cls服务不存在，无需停止")
            cls_stopped = True  # 服务不存在视为停止成功
        
        # 检查xc-ssh.service是否存在，如果存在则停止
        if self.service_manager.check_service_exists('xc-ssh'):
            xc_ssh_stopped = self.service_manager.stop_service('xc-ssh')
            if not xc_ssh_stopped:
                print("警告: xc-ssh.service停止失败，但继续执行")
        else:
            print("xc-ssh.service不存在，无需停止")
        
        # 只要CLS服务停止成功就返回True，xc-ssh服务停止失败不影响整体结果
        return cls_stopped
    
    def start_cls_service(self):
        """
        启动CLS服务，同时启动xc-ssh.service（属于SSH守护）
        ...
        # 检查cls服务是否存在，如果存在则启动
        if self.service_manager.check_service_exists('cls'):
            cls_started = self.service_manager.start_service('cls')
            if not cls_started:
                print("警告: cls服务启动失败")
        else:
            print("cls服务不存在，无需启动")
            cls_started = True  # 服务不存在视为启动成功
        
        # 检查xc-ssh.service是否存在，如果存在则启动
        if self.service_manager.check_service_exists('xc-ssh'):
            xc_ssh_started = self.service_manager.start_service('xc-ssh')
            if not xc_ssh_started:
                print("警告: xc-ssh.service启动失败，但继续执行")
        else:
            print("xc-ssh.service不存在，无需启动")
        
        # 只要CLS服务启动成功就返回True，xc-ssh服务启动失败不影响整体结果
        return cls_started
    ...
    def check_native_ssh_service(self):
        ...
        """
        # 委托给服务管理模块
        return self.service_manager.check_native_ssh_service()
    ...
    def disable_native_ssh_service(self):
        ...
        """
        # 委托给服务管理模块
        return self.service_manager.disable_native_ssh_service()
    ...
    def install_openssh(self):
        ...
        try:
            ...
            # 第一步：检查并禁用原生SSH服务，避免端口冲突
            print("检查系统原生SSH服务...")
            if not self.disable_native_ssh_service():
                print("警告: 原生SSH服务禁用可能不完整，继续执行安装...")
            
            # 第二步：停止CLS服务，避免升级过程中被误检测执行重置
