...
from ssh_auto_upgrade.argument_validator import validate_arguments_early, validate_arguments
...
def main():
    ...
    # 检查root权限
    if os.geteuid() != 0:
        print("错误: 需要root权限来运行SSH自动升级工具")
        print("请使用sudo或切换到root用户运行此程序")
        return 1
    
    parser = argparse.ArgumentParser(description='OpenSSH自动升级守护进程工具-V1.0.13')
    ...
    # 第一步：参数验证（在systemd检查之前）
    if not validate_arguments_early(parser):
        return 1
    ...
    if args.service:
        try:
            ...
            command_args = []
            if args.mirror:
                command_args.append(f"--mirror {args.mirror}")
            if args.install_dir:
                command_args.append(f"--install-dir {args.install_dir}")
            if args.download_dir:
                command_args.append(f"--download-dir {args.download_dir}")
            if args.log_dir:
                command_args.append(f"--log-dir {args.log_dir}")
            if args.upgrade_time:
                command_args.append(f"--upgrade-time {args.upgrade_time}")
            if args.force:
                command_args.append("--force")
            if args.enable_root_login:
                command_args.append("--enable-root-login")
            if args.disable_root_login:
                command_args.append("--disable-root-login")
