...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow3495(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3494(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3493(__unit: None=None) -> IEnumerable_1[str]:
                    ...
                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow3492))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow3493))

            return append(singleton("{"), delay(_arrow3494))

        return join("\n", to_list(delay(_arrow3495)))
...
ValidationPackage_reflection = _expr3496
