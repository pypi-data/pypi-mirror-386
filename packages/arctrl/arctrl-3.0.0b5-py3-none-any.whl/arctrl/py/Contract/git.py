...
Init_reflection = _expr3645

def _expr3646() -> TypeInfo:
...
def Clone_createCloneContract_5000466F(remote_url: str, merge: bool | None=None, branch: str | None=None, token: tuple[str, str] | None=None, nolfs: bool | None=None) -> Contract:
    ...
    def _arrow3652(__unit: None=None, remote_url: Any=remote_url, merge: Any=merge, branch: Any=branch, token: Any=token, nolfs: Any=nolfs) -> IEnumerable_1[str]:
            ...
            def _arrow3650(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3649(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3648(__unit: None=None) -> IEnumerable_1[str]:
                        ...
                        return append(singleton(remote_url_1), delay(_arrow3647))

                    return append(singleton(value(branch)) if (branch is not None) else empty(), delay(_arrow3648))

                return append(singleton(Clone_get_branchFlag()) if (branch is not None) else empty(), delay(_arrow3649))

            return append(singleton(Clone_get_noLFSConfig()) if nolfs_1 else empty(), delay(_arrow3650))

        return append(singleton(Clone_get_clone()), delay(_arrow3651))

    return create_git_contract(to_array(delay(_arrow3652)))
