...
def _007CRunPath_007C__007C(input: Array[str]) -> str | None:
    ...
    def _arrow3639(x: str, y: str, input: Any=input) -> bool:
        return x == y

    if (len(input) == 3) if (not equals_with(_arrow3639, input, None)) else False:
        if input[0] == "runs":
...
def ARCtrl_ArcRun__ArcRun_ToCreateContract_6FCE9E49(this: ArcRun, WithFolder: bool | None=None) -> Array[Contract]:
    ...
    def _arrow3643(__unit: None=None, this: Any=this, WithFolder: Any=WithFolder) -> IEnumerable_1[Contract]:
        def _arrow3641(__unit: None=None) -> IEnumerable_1[Contract]:
            ...
            def _arrow3640(p: str) -> IEnumerable_1[Contract]:
                return singleton(Contract.create_create(p, DTOType(10))) if ((p != "runs/.gitkeep") if (p != path) else False) else empty()

            return collect(_arrow3640, folder_fs.ToFilePaths(False))

        def _arrow3642(__unit: None=None) -> IEnumerable_1[Contract]:
            return singleton(c)

        return append(_arrow3641() if with_folder else empty(), delay(_arrow3642))
