...
def from_sparse_table(matrix: SparseTable) -> FSharpList[OntologySourceReference]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow1244(__unit: None=None, matrix: Any=matrix) -> OntologySourceReference:
            return_val: OntologySourceReference = OntologySourceReference.create()
            return_val.Comments = comments
            return return_val

        return singleton(_arrow1244())

    else: 
        def _arrow1245(i: int, matrix: Any=matrix) -> OntologySourceReference:
        ...
        return initialize(matrix.ColumnCount, _arrow1245)
...
def to_sparse_table(ontology_sources: FSharpList[OntologySourceReference]) -> SparseTable:
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr1247())), matrix.ColumnCount)
