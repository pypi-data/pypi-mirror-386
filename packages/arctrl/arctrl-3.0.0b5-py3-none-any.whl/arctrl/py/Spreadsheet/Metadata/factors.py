...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Factor]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow911(i: int, matrix: Any=matrix) -> Factor:
        ...
        return initialize(matrix.ColumnCount, _arrow911)
...
def to_sparse_table(factors: FSharpList[Factor]) -> SparseTable:
    ...
    class ObjectExpr916:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow915(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr916())), matrix.ColumnCount)
