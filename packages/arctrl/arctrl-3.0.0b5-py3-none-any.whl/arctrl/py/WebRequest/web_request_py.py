...
def Requests_createHeadersDict_9F3777D(headers: Any | None=None) -> Any | None:
    def mapping(values: Any, headers: Any=headers) -> Any:
        def _arrow3792(__unit: None=None, values: Any=values) -> IEnumerable_1[tuple[str, Any]]:
            def _arrow3791(pair: Any) -> tuple[str, Any]:
                return (pair[0], pair[1])

            return map_1(_arrow3791, values)

        return create_obj(to_list(delay(_arrow3792)))
...
def Requests_mapResponseType_Z730CA7D4(response: InteropResponseType) -> Response:
    ...
    def _arrow3794(__unit: None=None, response: Any=response) -> IEnumerable_1[tuple[str, str]]:
        def _arrow3793(header_name: str) -> IEnumerable_1[tuple[str, str]]:
            return singleton((header_name, response.headers[header_name]))

        return collect(_arrow3793, list(response.headers.keys()))

    class ObjectExpr3795:
        @property
        def Compare(self) -> Callable[[str, str], int]:
            return compare_primitives

    return Response(status_code, text, of_list(to_list(delay(_arrow3794)), ObjectExpr3795()), encoding)
...
def download_file(url: str) -> Async[str]:
    def _arrow3796(__unit: None=None, url: Any=url) -> Async[str]:
        response: Response = Requests_get_43A074E6(url)
        return singleton_1.Return(response.text)

    return singleton_1.Delay(_arrow3796)
