...
def try_int(str_1: str) -> int | None:
    ...
    out_arg: int = 0
    def _arrow3526(__unit: None=None, str_1: Any=str_1) -> int:
        return out_arg

    def _arrow3527(v: int, str_1: Any=str_1) -> None:
        nonlocal out_arg
        out_arg = v or 0

    match_value = (try_parse(str_1, 511, False, 32, FSharpRef(_arrow3526, _arrow3527)), out_arg)
    if match_value[0]:
...
def try_get_parameter_column_index(param_value: ProcessParameterValue) -> int | None:
    def _arrow3530(param: ProtocolParameter, param_value: Any=param_value) -> int | None:
        return try_get_parameter_index(param)

    return bind(_arrow3530, param_value.Category)
...
def try_get_factor_column_index(factor_value: FactorValue) -> int | None:
    def _arrow3531(factor: Factor, factor_value: Any=factor_value) -> int | None:
        return try_get_factor_index(factor)

    return bind(_arrow3531, factor_value.Category)
...
def try_get_characteristic_column_index(characteristic_value: MaterialAttributeValue) -> int | None:
    def _arrow3533(characteristic: MaterialAttribute, characteristic_value: Any=characteristic_value) -> int | None:
        return try_get_characteristic_index(characteristic)

    return bind(_arrow3533, characteristic_value.Category)
