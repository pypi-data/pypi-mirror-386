...
def ISAJson_encoder(id_map: Any | None, oa: MaterialAttributeValue) -> IEncodable:
    def f(oa_1: MaterialAttributeValue, id_map: Any=id_map, oa: Any=oa) -> IEncodable:
        ...
        def _arrow2540(value: str, oa_1: Any=oa_1) -> IEncodable:
        ...
        def _arrow2541(value_2: MaterialAttribute, oa_1: Any=oa_1) -> IEncodable:
            return encoder_1(id_map, value_2)

        def _arrow2542(value_3: Value_1, oa_1: Any=oa_1) -> IEncodable:
            return encoder_2(id_map, value_3)

        def _arrow2543(oa_3: OntologyAnnotation, oa_1: Any=oa_1) -> IEncodable:
            return OntologyAnnotation_ISAJson_encoder(id_map, oa_3)

        values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([try_include("@id", _arrow2540, ISAJson_genID(oa_1)), try_include("category", _arrow2541, oa_1.Category), try_include("value", _arrow2542, oa_1.Value), try_include("unit", _arrow2543, oa_1.Unit)]))
    ...
    if id_map is not None:
        def _arrow2545(oa_4: MaterialAttributeValue, id_map: Any=id_map, oa: Any=oa) -> str:
            return ISAJson_genID(oa_4)

        return encode(_arrow2545, f, oa, id_map)
...
def _arrow2550(get: IGetters) -> MaterialAttributeValue:
    def _arrow2546(__unit: None=None) -> str | None:
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("@id", Decode_uri)

    def _arrow2547(__unit: None=None) -> MaterialAttribute | None:
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("category", decoder_1)

    def _arrow2548(__unit: None=None) -> Value_1 | None:
    ...
    return MaterialAttributeValue(_arrow2546(), _arrow2547(), _arrow2548(), _arrow2549())


ISAJson_decoder: Decoder_1[MaterialAttributeValue] = object(_arrow2550)
