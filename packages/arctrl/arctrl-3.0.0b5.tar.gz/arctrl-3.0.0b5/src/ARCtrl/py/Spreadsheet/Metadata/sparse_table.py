...
def SparseRowModule_fromAllValues(v: IEnumerable_1[str | None]) -> IEnumerable_1[tuple[int, str]]:
    def chooser(tupled_arg: tuple[int, str | None], v: Any=v) -> tuple[int, str] | None:
        def _arrow849(v_1: str, tupled_arg: Any=tupled_arg) -> tuple[int, str]:
            return (tupled_arg[0], v_1)

        return map_1(_arrow849, tupled_arg[1])
...
def SparseRowModule_getAllValues(i: IEnumerable_1[tuple[int, str]]) -> IEnumerable_1[str | None]:
    ...
    def _arrow852(i_1: int, i: Any=i) -> str | None:
        return try_find(i_1, m)

    return initialize(max_by(projection, i, ObjectExpr851())[0] + 1, _arrow852)
...
SparseTable_reflection = _expr873
...
def SparseTable_AddRow(key: str, values: IEnumerable_1[tuple[int, str]], matrix: SparseTable) -> SparseTable:
    ...
    def _arrow878(tuple: tuple[int, str], key: Any=key, values: Any=values, matrix: Any=matrix) -> int:
        return tuple[0]
    ...
    length: int = (0 if is_empty(values) else (1 + max_by(_arrow878, values, ObjectExpr879())[0])) or 0
...
def SparseTable_AddComment(key: str, values: IEnumerable_1[tuple[int, str]], matrix: SparseTable) -> SparseTable:
    if length_1(values) == 0:
        ...
        iterate(action, values)
        def _arrow880(tuple: tuple[int, str], key: Any=key, values: Any=values, matrix: Any=matrix) -> int:
            return tuple[0]
        ...
        length: int = (0 if is_empty(values) else (1 + max_by(_arrow880, values, ObjectExpr881())[0])) or 0
                ...
                if en.System_Collections_IEnumerator_MoveNext():
                    ...
                    if pattern_matching_result == 0:
                        ...
                        if active_pattern_result_1 is not None:
                            ...
                            if vals is not None:
                                def _arrow886(__unit: None=None, matrix: Any=matrix, remarks: Any=remarks, line_number_1: Any=line_number_1) -> bool:
                                    v_2: IEnumerable_1[tuple[int, str]] = vals
                                    k_1: str = key
                                    def _arrow885(label: str) -> bool:
                                        return k_1 == (prefix_1 + label)

                                    return exists(_arrow885, labels)
                        ...
                        if pattern_matching_result_1 == 0:
                            ...
                            def _arrow884(label_1: str, matrix: Any=matrix, remarks: Any=remarks, line_number_1: Any=line_number_1) -> bool:
                                return k_3 == (prefix_1 + label_1)

                            matrix_mut = SparseTable_AddRow(find(_arrow884, labels), v_3, matrix)
                            remarks_mut = remarks
                            line_number_1_mut = line_number_1 + 1
                            continue
...
def SparseTable_ToRows_759CAFC1(matrix: SparseTable, prefix: str | None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    prefix_1: str = "" if (prefix is None) else (prefix + " ")
    def _arrow896(__unit: None=None, matrix: Any=matrix, prefix: Any=prefix) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        def _arrow891(key: str) -> IEnumerable_1[tuple[int, str]]:
            def _arrow889(i: int) -> str:
                return SparseTable__TryGetValueDefault_5BAE6133(matrix, "", (key, i + 1))

            return SparseRowModule_fromValues(cons(prefix_1 + key, initialize_1(matrix.ColumnCount - 1, _arrow889)))

        def _arrow895(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
            def _arrow894(key_1: str) -> IEnumerable_1[tuple[int, str]]:
                def _arrow893(i_1: int) -> str:
                    return SparseTable__TryGetValueDefault_5BAE6133(matrix, "", (key_1, i_1 + 1))

                return SparseRowModule_fromValues(cons(Comment_wrapCommentKey(key_1), initialize_1(matrix.ColumnCount - 1, _arrow893)))

            return map(_arrow894, matrix.CommentKeys)

        return append_1(map(_arrow891, matrix.Keys), delay(_arrow895))

    return delay(_arrow896)
