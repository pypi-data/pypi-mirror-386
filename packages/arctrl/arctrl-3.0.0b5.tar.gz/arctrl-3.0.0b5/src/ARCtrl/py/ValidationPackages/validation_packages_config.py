...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow3502(__unit: None=None) -> IEnumerable_1[str]:
                ...
                def _arrow3500(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3499(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3498(__unit: None=None) -> IEnumerable_1[str]:
                            ...
                            return append(singleton("]"), delay(_arrow3497))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow3498))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow3499))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow3500))

            return append(singleton("{"), delay(_arrow3501))

        return join("\n", to_list(delay(_arrow3502)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow3505(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow3504(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow3504, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow3505))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr3506
