...
def object(builder: Callable[[IGetters], _VALUE]) -> Decoder_1[Any]:
    class ObjectExpr161(Decoder_1[_VALUE_]):
...
def dict_1(decoder: Decoder_1[Any]) -> Decoder_1[Any]:
    def _arrow167(elements: FSharpList[tuple[str, _VALUE]], decoder: Any=decoder) -> Any:
        class ObjectExpr166:
            @property
            def Compare(self) -> Callable[[str, str], int]:
                return compare_primitives

        return of_list(elements, ObjectExpr166())

    return map(_arrow167, key_value_pairs(decoder))


def map_0027(key_decoder: Decoder_1[Any], value_decoder: Decoder_1[Any]) -> Decoder_1[Any]:
    def _arrow169(elements: Array[tuple[_KEY, _VALUE]], key_decoder: Any=key_decoder, value_decoder: Any=value_decoder) -> Any:
        class ObjectExpr168:
            @property
            def Compare(self) -> Callable[[_KEY_, _KEY_], int]:
                return compare

        return of_seq_1(elements, ObjectExpr168())

    return map(_arrow169, array(tuple2(key_decoder, value_decoder)))
