...
class FsCellsCollection:
    ...
    def Add(self, cell: FsCell, row: int | None=None, column: int | None=None) -> None:
        ...
        def _arrow162(__unit: None=None) -> Any:
        ...
        add_to_dict(_arrow162(), column_1, cell)
    ...
    def RemoveCellAt(self, row: int, column: int) -> None:
        ...
        if (row == this._maxRowUsed) if row_removed else False:
            ...
            this._maxRowUsed = (max(this._rowsUsed.keys(), ObjectExpr163()) if (not is_empty(this._rowsUsed.keys())) else 0) or 0
        ...
        if (column == this._maxColumnUsed) if column_removed else False:
            ...
            this._maxColumnUsed = (max(this._columnsUsed.keys(), ObjectExpr164()) if (not is_empty(this._columnsUsed.keys())) else 0) or 0
        ...
        if match_value is None:
            ...
            def _arrow165(__unit: None=None) -> bool:
