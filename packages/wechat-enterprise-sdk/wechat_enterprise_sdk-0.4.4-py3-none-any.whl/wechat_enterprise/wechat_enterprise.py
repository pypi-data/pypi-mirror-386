...
class WechatEnterprise:
    ...
    def get_department_id(self, ID = 0):
        ...
        response = requests.get(url.format(ACCESS_TOKEN = self.get_access_token(), ID=ID))
    ...
    def get_department_userlist(self, department_id = 1):
        ...
        response = requests.get(url.format(ACCESS_TOKEN = self.get_access_token(), DEPARTMENT_ID = department_id))
        return response.json()
    ...
    def get_user_info(self, userid):
        ...
        """
        url = self.GET_USER_URL.format(ACCESS_TOKEN=self.get_access_token(),USERID = userid)
