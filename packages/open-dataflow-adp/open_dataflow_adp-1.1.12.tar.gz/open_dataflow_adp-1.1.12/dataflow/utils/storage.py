...
class MyScaleDBStorage(DataFlowStorage):
    ...
    def write(self, data: Any) -> Any:
        ...
        with pool.get_connection() as client:
            ...
            system_cols = {'pipeline_id', 'task_id', 'raw_data_id', 'min_hashes', 'file_id', 'data'}
            ...
            # 只保留必需字段
            required_cols = ['pipeline_id', 'task_id', 'raw_data_id', 'min_hashes', 'file_id', 'data']
            df = df[required_cols]
            ...
            insert_sql = f"""
            INSERT INTO {self.table} (pipeline_id, task_id, raw_data_id, min_hashes, file_id, data)
            VALUES
            """
