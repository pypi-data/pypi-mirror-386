...
from wagtailmodelsearch.query import Not, PlainText
from wagtailmodelsearch.test.testapp import models
from wagtailmodelsearch.tests.test_backends import BackendTests
...
@override_settings(
    MODELSEARCH_BACKENDS={
        "default": {
            "BACKEND": "wagtailmodelsearch.backends.database.mysql.mysql",
        }
    }
)
class TestMySQLSearchBackend(BackendTests, TransactionTestCase):
    backend_path = "wagtailmodelsearch.backends.database.mysql.mysql"
    ...
    def test_lexeme_validation(self):
        from wagtailmodelsearch.backends.database.mysql.query import Lexeme
