...
from wagtailmodelsearch.backends import (
    InvalidSearchBackendError,
    get_search_backend,
    get_search_backends,
)
from wagtailmodelsearch.backends.base import (
    BaseSearchBackend,
    FieldError,
    FilterFieldError,
)
from wagtailmodelsearch.backends.database.fallback import DatabaseSearchBackend
...
from wagtailmodelsearch.query import (
    MATCH_ALL,
    MATCH_NONE,
    And,
    Boost,
...
from wagtailmodelsearch.test.testapp import models
...
@override_settings(
    MODELSEARCH_BACKENDS={
        "default": {"BACKEND": "wagtailmodelsearch.backends.database"}
    }
)
class TestBackendLoader(TestCase):
    @mock.patch("wagtailmodelsearch.backends.database.connection")
    ...
    def test_import_by_path_unknown_db_vendor(self, connection):
        connection.vendor = "unknown"
        db = get_search_backend(backend="wagtailmodelsearch.backends.database")
        self.assertIsInstance(db, DatabaseSearchBackend)
    ...
    def test_import_by_full_path_unknown_db_vendor(self, connection):
        ...
        db = get_search_backend(
            backend="wagtailmodelsearch.backends.database.SearchBackend"
        )
        self.assertIsInstance(db, DatabaseSearchBackend)
    ...
    def test_import_by_name_postgres_db_vendor(self):
        from wagtailmodelsearch.backends.database.postgres.postgres import (
    ...
    def test_import_by_name_mysql_db_vendor(self):
        from wagtailmodelsearch.backends.database.mysql.mysql import MySQLSearchBackend
    ...
    def test_import_by_name_sqlite_db_vendor(self):
        ...
        if not fts5_available():  # pragma: no cover
            from wagtailmodelsearch.backends.database.fallback import (
            ...
            from wagtailmodelsearch.backends.database.sqlite.sqlite import (
                SQLiteSearchBackend,
            )
    ...
    def test_nonexistent_backend_import(self):
        self.assertRaises(
            InvalidSearchBackendError,
            get_search_backend,
            backend="wagtailmodelsearch.backends.doesntexist",
        )
    ...
    @override_settings(
        MODELSEARCH_BACKENDS={
            "default": {"BACKEND": "wagtailmodelsearch.backends.database"},
            "another-backend": {"BACKEND": "wagtailmodelsearch.backends.database"},
        }
    )
    ...
    @override_settings(
        MODELSEARCH_BACKENDS={
            "default": {
                "BACKEND": "wagtailmodelsearch.backends.database",
                "AUTO_UPDATE": False,
            },
        }
    )
