...
from wagtailmodelsearch import index
...
@mock.patch("wagtailmodelsearch.tests.DummySearchBackend", create=True)
@override_settings(
    MODELSEARCH_BACKENDS={
        "default": {"BACKEND": "wagtailmodelsearch.tests.DummySearchBackend"}
    }
)
class TestInsertOrUpdateObject(TestCase):
    ...
    def test_catches_index_error(self, backend):
        ...
        with self.assertLogs("wagtailmodelsearch.index", level="ERROR") as cm:
            index.insert_or_update_object(obj)
