...
def get_app_config():
    ...
    module path and/or app label, rather than hardcoding "wagtailmodelsearch" - Wagtail makes use of this
    ...
    if len(app_configs) == 0:  # pragma: no cover
        raise ImproperlyConfigured(
            "The wagtailmodelsearch app was not found in INSTALLED_APPS"
        )
    elif len(app_configs) > 1:  # pragma: no cover
        raise ImproperlyConfigured(
            "Multiple instances of the wagtailmodelsearch app were found in INSTALLED_APPS"
        )
