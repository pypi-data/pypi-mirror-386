...
from wagtailmodelsearch.conf import get_app_config
...
def import_backend(dotted_path):
    ...
    eg:
      old: wagtailmodelsearch.backends.elasticsearch.ElasticsearchSearchBackend
      new: wagtailmodelsearch.backends.elasticsearch
...
def get_search_backend(backend="default", **kwargs):
    ...
    - A dotted path to a backend class (in the form wagtailmodelsearch.backends.elasticsearch or wagtailmodelsearch.backends.elasticsearch.ElasticsearchSearchBackend)

    If no name is specified, `default` will be used; this defaults to the `wagtailmodelsearch.backends.database` backend if not specified in MODELSEARCH_BACKENDS.
