...
def process_result(
    *,
    ...
    payload: EventPayload = EventPayload(
        job_id=RemoteID(id=braket_result.task_metadata.id),
        artifacts=[
            Artifact(data=_serialize_result(braket_result), name="result.json"),
        ],
...
def _results_handler(
    ...
    """Generate Function to Handle Results and Generate ExtensionEvent
    ...
    -------
    HandlerFunction
        function that takes the device result and returns an ExtensionEvent
    """
    ...
    def _wrapper(fn, *args, **kwargs):
        ...
        try:
            req: ExtensionEvent = process_result(
