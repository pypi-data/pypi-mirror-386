...
class MuxWavelengthMapForm(NetBoxModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Pre-populate mux from URL parameters
        # todo verify is working as expected
        if not self.instance.pk and "initial" in kwargs:
            initial = kwargs["initial"]
            if "mux_a" in initial and initial["mux_a"]:
                try:
                    mux = Device.objects.get(pk=initial["mux_a"])
                    self.fields["mux"].initial = mux
                except Device.DoesNotExist:
                    pass
            elif "mux_z" in initial and initial["mux_z"]:
                try:
                    mux = Device.objects.get(pk=initial["mux_z"])
                    self.fields["mux"].initial = mux
                except Device.DoesNotExist:
                    pass
