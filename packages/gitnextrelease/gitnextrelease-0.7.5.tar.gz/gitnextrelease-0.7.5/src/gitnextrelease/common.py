...
class CommonFunctions(DefaultValues):
    ...
    def get_project_info(self):
        ...
        if os.path.exists("pyproject.toml"):
            ...
            #     config["version"], config["project"] = self.get_toml_info()
            #     return config
            # # print(self.check_for_pdm())
            try:
                # this parses the old version
...
RESTORE_DATE = ${TODAY}
'''
        help_string = '''@grep -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | sort | awk 'BEGIN {FS = ":.*?## "}; {printf "\033[36m%-20s\033[0m %s\\n", $$1, $$2}' '''
        # fmt: on
        contents = """VERSION := {0}
{2}
clean: ## cleans repo
...
""".format(
            self.config["version"], "	", restore_content, help_string
        )
