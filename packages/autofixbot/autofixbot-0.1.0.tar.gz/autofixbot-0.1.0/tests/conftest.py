...
from autofix_bot import AutofixBot, AsyncAutofixBot, DefaultAioHttpClient
from autofix_bot._utils import is_dict
...
logging.getLogger("autofix_bot").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[AutofixBot]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with AutofixBot(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncAutofixBot]:
