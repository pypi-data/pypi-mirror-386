...
class DiseaseCoder:
    ...
    def __init__(
        ...
        logger.info('Initializing disease code embeddings')
        ...
        self.model_processor = ModelProcessor(
            ...
            parent_to_subcodes=self.parent_to_subcodes,
    ...
    def _initialize_dataset(self):
        ...
        embeddings = np.vstack(self.code_lookup["embeddings"]).astype("float32")
        faiss.normalize_L2(embeddings)
        self.normalized_embeddings = embeddings

        # Build FAISS index (cosine similarity = inner product after normalization)
        dim = embeddings.shape[1]
        index = faiss.IndexFlatIP(dim)
        index.add(embeddings)
        self.faiss_index = index

        # Precompute parent→subcode mapping
        self.parent_to_subcodes = defaultdict(list)
        for i, code in enumerate(self.code_lookup["Code"]):
            parent = code.split(".")[0]
