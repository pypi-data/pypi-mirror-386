...
class TestConnectionLifecycle:
    ...
    def test_start_passes_kwargs_to_nexus(self, mock_nexus_app):
        ...
        result = connection.start(blocking=False)
...
class TestGracefulFailure:
    ...
    def test_connection_without_nexus_graceful(self):
        """Connection should fail gracefully without Nexus installed."""
        from kaizen.integrations.nexus import NEXUS_AVAILABLE

        if NEXUS_AVAILABLE:
            pytest.skip("Test requires Nexus to be unavailable")

        # Should not crash on import
        from kaizen.integrations import nexus

        assert nexus.NEXUS_AVAILABLE is False
