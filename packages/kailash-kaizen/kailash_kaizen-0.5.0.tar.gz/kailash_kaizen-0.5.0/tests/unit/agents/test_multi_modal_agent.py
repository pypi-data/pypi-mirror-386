...
class TestMultiModalAgent:
    ...
    def test_cost_tracking_integration(self, mock_ollama_adapter, test_image_path):
        ...
        result = agent.analyze(image=test_image_path, question="Test question")
    ...
    @pytest.mark.skip(
        reason="Adapter auto-fallback not yet implemented - agent gracefully handles missing adapters"
    )
    ...
    @pytest.mark.skip(
        reason="Modality validation deferred to runtime - graceful degradation preferred over init-time errors"
    )
