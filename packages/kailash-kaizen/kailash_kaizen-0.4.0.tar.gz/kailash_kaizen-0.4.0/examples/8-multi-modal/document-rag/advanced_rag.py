...
class DocumentStore:
    ...
    def add_document(self, doc_id: str, metadata: Dict[str, Any], chunks: List[Dict[str, Any]]):
...
class AdvancedRAGWorkflow:
    ...
    def __init__(self, config: AdvancedRAGConfig):
        ...
        print(f"✅ Advanced RAG workflow initialized")
    ...
    def ingest_document(
        ...
        if cost_estimate > self.config.max_cost_per_document:
            if self.config.prefer_free:
                print(f"   ⚠️  Cost exceeds budget, using free provider...")
                # Switch to Ollama (free)
                self.doc_agent.config.provider = "ollama_vision"
            else:
                raise ValueError(f"Cost ${cost_estimate:.3f} exceeds budget ${self.config.max_cost_per_document:.3f}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_cost_per_document": self.total_cost / store_stats["total_documents"] if store_stats["total_documents"] > 0 else 0,
