...
def estimate_extraction_costs():
    ...
    for doc_path, size in zip(doc_paths, doc_sizes):
        ...
        # Display results
        print(f"   Landing AI:     ${cost_estimates['landing_ai']:.3f} (98% accuracy, bounding boxes)")
        print(f"   OpenAI Vision:  ${cost_estimates['openai_vision']:.3f} (95% accuracy, fastest)")
        print(f"   Ollama Vision:  ${cost_estimates['ollama_vision']:.3f} (85% accuracy, FREE)")

        # Calculate potential savings
        highest_cost = max(cost_estimates['landing_ai'], cost_estimates['openai_vision'])
        ollama_savings = highest_cost - cost_estimates['ollama_vision']

        print(f"\n   💡 Savings with Ollama: ${ollama_savings:.3f} per document")
        print(f"   💡 Best accuracy/cost: Landing AI (98% accuracy, ${cost_estimates['landing_ai']:.3f})")
        print(f"   💡 Best speed: OpenAI Vision (fastest, ${cost_estimates['openai_vision']:.3f})")
...
def demonstrate_budget_decisions():
    ...
    if cost_estimates['landing_ai'] > budget and cost_estimates['openai_vision'] > budget:
    ...
    print(f"   💰 Unlimited free extractions")
