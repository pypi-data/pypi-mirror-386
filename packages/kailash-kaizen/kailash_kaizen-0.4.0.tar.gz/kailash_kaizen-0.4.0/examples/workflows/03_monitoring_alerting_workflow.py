...
class SystemMonitor(BaseAgent):
    ...
    def __init__(self, config: MonitorConfig, tool_registry: ToolRegistry):
        super().__init__(
            config=config, signature=MonitoringSignature(), tool_registry=tool_registry
        )
...
async def main():
    ...
    monitor = SystemMonitor(config, tool_registry=registry)
