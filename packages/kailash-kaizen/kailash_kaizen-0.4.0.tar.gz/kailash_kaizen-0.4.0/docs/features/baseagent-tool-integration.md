...
class ToolAgent(BaseAgent):
    def __init__(self, config: AgentConfig, tool_registry: ToolRegistry):
        super().__init__(
            ...
            tool_registry=tool_registry,  # Enable tools
        )
...
async def main():
    ...
    agent = ToolAgent(config=AgentConfig(), tool_registry=registry)
...
# 1. Create registry
registry = ToolRegistry()
register_builtin_tools(registry)  # 12 tools
...
agent = BaseAgent(
    config=config,
    signature=signature,
    tool_registry=registry  # Add this
)
```
