...
async def get_session(request: Request = None) -> AsyncIterator[AsyncSession]:
    ...
    # Create session manually to avoid automatic close() during exceptions
    session = session_maker()
    try:
        yield session
    except Exception as exc:
        # Defensive rollback - avoid session state conflicts
        try:
        ...
        # Defensive session close - avoid session state conflicts during cleanup
        try:
            await session.close()
        except InvalidRequestError as close_error:
            logger.warning(
                "Session close failed during cleanup: %s. "
                "This is usually due to concurrent database operations.",
                close_error,
            )
        except Exception as close_error:
            logger.warning(
                "Unexpected error during session close: %s",
                close_error,
            )
