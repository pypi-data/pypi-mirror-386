...
def _calculate_metrics(all_matches: List[TextMatch]):
    ...
    translations_applied = sum(
        1
        for m in all_matches
        if m.translated_text is not None and m.translated_text != m.original_text
    )
...
def _log_summary_to_console(metrics: Dict, total_run_time: float):
    ...
    logging.info(
        f"- Translations Applied: {metrics['translations_applied']} ({metrics['total_matches'] - metrics['translations_applied']} skipped)"
    )
...
def _get_report_filepath(start_time: float, end_time: float, export_dir: Path) -> Path:
    ...
    start_ts = datetime.fromtimestamp(start_time, tz=timezone.utc).strftime(
        "%Y-%m-%dT%H-%M-%SZ"
    )
    end_ts = datetime.fromtimestamp(end_time, tz=timezone.utc).strftime(
        "%Y-%m-%dT%H-%M-%SZ"
    )
    return export_dir / f"{start_ts}---{end_ts}.csv"


def _export_summary_to_csv(
    all_matches: List[TextMatch], config: GlocalConfig, filepath: Path
):
...
def generate_summary_report(
    ...
    export_dir = export_dir_override or (
        Path(config.report_options.export_dir)
        if config.report_options.export_dir
        else None
    )
