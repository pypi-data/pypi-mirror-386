...
class ETLDuckDBHandler:
    ...
    def ingest_csv(
        ...
        """
        Minimal CSV ingest into DuckDB via read_csv_auto().
        - No metadata columns
        - No cast map
        - Gzip (.csv.gz) works automatically
        """
        ...
        # Build base SELECT (use literals for HEADER/DELIM; bind only the filename)
        header_sql = "TRUE" if header else "FALSE"
        delim_sql = "'" + str(separator).replace("'", "''") + "'"
        base_sql = (
            f"SELECT * FROM read_csv_auto(?, HEADER={header_sql}, DELIM={delim_sql});"
        )
        params: list[Any] = [str(src)]
        ...
        if create_mode == "replace":
            ...
            if not self.table_exists(tname):
                # Create empty table on first run using LIMIT 0 to capture schema
                self.con.execute(
