...
class MigManExtract:
    ...
    def extract(self) -> None:
        ...
        try:
            ...
            tables_to_extract, fields_to_extract = self.get_tables_and_fields(self.cfg.extract.adapter)

            self.logger.info(f"Tables to extract for {self.cfg.extract.adapter}: {tables_to_extract}")
            self.logger.info(f"Fields to extract for {self.cfg.extract.adapter}: {fields_to_extract}")

            # connect source database
            match self.cfg.extract.adapter:
                case ETLAdapter.INFORCOM.value:
                    conn = dbsrc.connect_odbc(
                        odbc_connstr=self.cfg.extract.inforcom.odbc_connstr,
                        timeout=self.cfg.extract.inforcom.timeout,
                    )
                case ETLAdapter.SAPECC.value:
            ...
            if not conn:
                raise ConnectionError(f"Failed to connect to {self.cfg.extract.adapter} database")

            # extract tables
            for table_name in tables_to_extract:
                self.logger.info(f"Extracting table {table_name} from {self.cfg.extract.adapter} ...")
            ...
            self.logger.error(f"Error during extraction for {self.cfg.extract.adapter}: {e}")
            raise
        finally:
            try:
                if conn:
