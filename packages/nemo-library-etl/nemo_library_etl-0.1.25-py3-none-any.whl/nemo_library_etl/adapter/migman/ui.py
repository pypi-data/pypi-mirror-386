        ...
        def index():
                ...
                with content_container:
                    ...
                    with ui.card().classes("mt-4"):
                        ...
                        adapter_select = (
                            ui.select(
                                label="Adapter",
                                options=["none", "genericodbc", "inforcom", "sapecc"],
                    ...
                    with ui.card().classes("mt-4"):
                        ...
                        ui.label("Configure manual file imports for the extraction process. File paths are automatically validated for existence.").classes("text-sm mb-4")
                            ...
                            with ui.card().classes("mt-2 p-3"):
                                ...
                                with ui.column().classes("gap-2 w-full"):
                                    ...
                                    def on_file_path_change(e, idx=i):
                                        ...
                                        validate_file_path(e.value, file_path_input, file_status_label)
                                    
                                    def validate_file_path(file_path, input_element, status_element):
                                        import os
                                    ...
                                    if file_item.get("file_path"):
                                        validate_file_path(file_item.get("file_path"), file_path_input, file_status_label)
                        ...
                        if extract_active:
                            with ui.card().classes("mt-2 p-3 border-dashed"):
                                ...
                                def on_new_file_path_change(e):
                                    ...
                                    validate_new_file_path(e.value, new_file_path_input, new_file_status_label)
                                ...
                                def validate_new_file_path(file_path, input_element, status_element):
                                    import os
                                    if file_path and file_path.strip():
                                        try:
                                            if os.path.isfile(file_path):
                                ...
                                def add_file():
                                    if new_file_data["project"] and new_file_data["file_path"]:
                                        # Additional check: warn if file doesn't exist but still allow adding
                                        import os
                                        if not os.path.isfile(new_file_data["file_path"]):
                                            ui.notify(f"Warning: File '{new_file_data['file_path']}' does not exist or is not accessible.", type="warning")
                                        file_config.append(new_file_data.copy())
                                        mark_changed()
                                        render_extract()
                                    else:
                                        ui.notify("Please provide both project name and file path.", type="warning")
                                ...
                                with ui.column().classes("gap-2 w-full"):
                                    ...
                                    # File path for new file with validation
                                    new_file_path_input = ui.input(
                                        label="File Path",
                                        placeholder="e.g. /path/to/your/file.csv",
                                        on_change=on_new_file_path_change
                                    ).props("dense").classes("w-full")
                                    
                                    # Status label for new file existence
                                    new_file_status_label = ui.label("").classes("text-sm")
                            ...
                            with ui.card().classes("mt-2 p-3 bg-gray-100"):
                                ui.label("File configurations are disabled because Extract is not active.").classes("text-sm text-gray-500 italic")
            ...
            def render_transform_join_config(transform_config):
                ...
                with ui.column().classes("gap-4"):
                    ...
                    ).props("dense clearable").classes("w-48 mt-3")
                    if not join_active:
                        limit_input.props("disable")

                    ui.label("Set a limit on the number of rows to process during joins (leave empty for no limit)").classes("text-sm text-gray-600 mb-2")
