...
```python
from nexus.sdk import connect, OperationContext

nx = connect(config={"enforce_permissions": True})

# Operations use default context
...
nx.chmod("/workspace/file.txt", "644")
nx.chown("/workspace/file.txt", "alice")
nx.chgrp("/workspace/file.txt", "developers")

# Access control lists (ACL)
acl_entries = nx.getfacl("/workspace/file.txt")
nx.setfacl("/workspace/file.txt", "user:bob:rw-")
```
...
- [Skills API Reference](./api/skills.md)
- [Permissions API Reference](./api/permissions.md)
- [Backends API Reference](./api/backends.md)
