...
def run_current_script_on_remote():
    ...
    file_name_relative = Path(local_file_name).relative_to(python_proj_dir)
    file_name = Path(remote_dir,file_name_relative).as_posix()
    # file_name = re.sub(f'^{python_proj_dir}', remote_dir, local_file_name)  # 远程文件名字。
    print(file_name,python_proj_dir,remote_dir, local_file_name)
