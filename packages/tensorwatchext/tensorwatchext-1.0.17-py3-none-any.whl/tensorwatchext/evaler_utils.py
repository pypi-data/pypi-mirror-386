...
# Define this helper function in your notebook
def safe_process_data(d):
    # This function will be sent to the stream for evaluation
    from tensorwatch.evaler_utils import topk
    
    msg_list = None
    
    # Check for the two possible data structures we've seen
    if isinstance(d.data, dict) and 'data' in d.data:
        # Case 1: d.data is a dictionary like {'data': [messages...]}
        msg_list = d.data['data']
    elif isinstance(d.data, list):
        # Case 2: d.data is a list of messages [messages...]
        msg_list = d.data
        
    # If we found a list of messages, process the first one
    if msg_list and len(msg_list) > 0 and isinstance(msg_list[0], dict):
        labels = msg_list[0].get('labels')
        losses = msg_list[0].get('losses')
        
        # Only call topk if we have the data we need
        if labels and losses:
            return topk(labels=labels, metric=losses, k=3, order='dsc')
            
    # Return None if the data was not in a format we could process
    return None