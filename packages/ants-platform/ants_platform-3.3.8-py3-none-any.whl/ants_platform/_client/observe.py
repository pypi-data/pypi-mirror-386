...
class LangfuseDecorator:
    """Implementation of the @observe decorator for seamless AntsPlatform tracing integration.

    This class provides the core functionality for the @observe decorator, which enables
    automatic tracing of functions and methods in your application with AntsPlatform.
    It handles both synchronous and asynchronous functions, maintains proper trace context,
    and intelligently routes to the correct AntsPlatform client instance.
    ...
    - Thread-safe client resolution when multiple AntsPlatform projects are used
    """
    ...
    def observe(
        ...
        *,
        ...
        """Wrap a function to create and manage AntsPlatform tracing around its execution, supporting both synchronous and asynchronous functions.

        This decorator provides seamless integration of AntsPlatform observability into your codebase. It automatically creates
        ...
        Args:
            ...
            as_type (Optional[Literal]): Set the observation type. Supported values:
                    ...
                    Observation types are highlighted in the AntsPlatform UI for filtering and visualization.
                    The types "generation" and "embedding" create a span on which additional attributes such as model metrics
                    can be set.

        Returns:
            Callable: A wrapped version of the original function that automatically creates and manages AntsPlatform spans.
        ...
        Notes:
            ...
            - Special keyword arguments can be passed to control tracing:
              - ants_platform_trace_id: Explicitly set the trace ID for this function call
              - ants_platform_parent_observation_id: Explicitly set the parent span ID
              - ants_platform_public_key: Use a specific AntsPlatform project (when multiple clients exist)
    ...
    def _async_observe(
        ...
        *,
        ...
        async def async_wrapper(*args: Tuple[Any], **kwargs: Dict[str, Any]) -> Any:
            trace_id = cast(str, kwargs.pop("ants_platform_trace_id", None))
            parent_observation_id = cast(
                str, kwargs.pop("ants_platform_parent_observation_id", None)
            )
            ...
            public_key = cast(str, kwargs.pop("ants_platform_public_key", None))

            # Set public key in execution context for nested decorated functions
            with _set_current_public_key(public_key):
                ants_platform_client = get_client(public_key=public_key)
                context_manager: Optional[
                    ...
                    ants_platform_client.start_as_current_observation(
                        name=final_name,
                ...
                with context_manager as ants_platform_span_or_generation:
                    ...
                    try:
                        ...
                        if capture_output is True:
                            if inspect.isgenerator(result):
                                ...
                                return self._wrap_sync_generator_result(
                                    ants_platform_span_or_generation,
                                    result,
                                    transform_to_string,
                                )
                            ...
                            ants_platform_span_or_generation.update(output=result)

                        return result
                    except Exception as e:
                        ants_platform_span_or_generation.update(
                        ...
                        if not is_return_type_generator:
                            ants_platform_span_or_generation.end()
    ...
    def _sync_observe(
        ...
        *,
        ...
        def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
            trace_id = kwargs.pop("ants_platform_trace_id", None)
            parent_observation_id = kwargs.pop("ants_platform_parent_observation_id", None)
            trace_context: Optional[TraceContext] = (
                {
                    "trace_id": trace_id,
                    "parent_span_id": parent_observation_id,
            ...
            public_key = kwargs.pop("ants_platform_public_key", None)
    ...
    def _wrap_sync_generator_result(
        self,
        ants_platform_span_or_generation: Union[
            AntsPlatformSpan,
            AntsPlatformGeneration,
            AntsPlatformAgent,
            AntsPlatformTool,
        ...
        try:
            ...
            ants_platform_span_or_generation.update(output=output)
            ants_platform_span_or_generation.end()
