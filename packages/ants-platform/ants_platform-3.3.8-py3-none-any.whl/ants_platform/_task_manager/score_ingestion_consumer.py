...
MAX_EVENT_SIZE_BYTES = int(os.environ.get("ANTS_PLATFORM_MAX_EVENT_SIZE_BYTES", 1_000_000))
MAX_BATCH_SIZE_BYTES = int(os.environ.get("ANTS_PLATFORM_MAX_BATCH_SIZE_BYTES", 2_500_000))
...
class ScoreIngestionConsumer(threading.Thread):
    ...
    def _upload_batch(self, batch: List[Any]) -> None:
        self._log.debug(
            f"API: Uploading batch of {len(batch)} score events to AntsPlatform API"
        )

        metadata = ScoreIngestionMetadata(
            batch_size=len(batch),
            sdk_name="python",
            sdk_version=ants_platform_version,
        ...
        self._log.debug(
            f"API: Successfully sent {len(batch)} score events to AntsPlatform API in batch mode"
        )
