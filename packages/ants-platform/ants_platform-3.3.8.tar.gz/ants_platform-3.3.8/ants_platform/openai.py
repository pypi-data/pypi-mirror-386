"""If you use the OpenAI Python SDK, you can use the AntsPlatform drop-in replacement to get full logging by changing only the import.
...
AntsPlatform automatically tracks:
...
class OpenAiArgsExtractor:
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.args["name"] = name
        self.args["ants_platform_public_key"] = ants_platform_public_key
        self.args["ants_platform_prompt"] = ants_platform_prompt
        self.args["trace_id"] = trace_id
        self.args["parent_observation_id"] = parent_observation_id

        self.kwargs = kwargs

    def get_ants_platform_args(self) -> Any:
...
def _ants_platform_wrapper(func: Any) -> Any:
    def _with_ants_platform(open_ai_definitions: Any) -> Any:
        def wrapper(wrapped: Any, instance: Any, args: Any, kwargs: Any) -> Any:
            return func(open_ai_definitions, wrapped, args, kwargs)

        return wrapper

    return _with_ants_platform
...
def _extract_chat_prompt(kwargs: Any) -> Any:
    ...
    if prompt:
        # uf user provided functions, we need to send these together with messages to ants_platform
        prompt.update(
            {
                "messages": [
        ...
        # vanilla case, only send messages in openai format to ants_platform
...
def _get_ants_platform_data_from_kwargs(resource: OpenAiDefinition, kwargs: Any) -> Any:
    ...
    ants_platform_public_key = kwargs.get("ants_platform_public_key", None)
    if ants_platform_public_key is not None and not isinstance(ants_platform_public_key, str):
        raise TypeError("ants_platform_public_key must be a string")
    ...
    ants_platform_prompt = kwargs.get("ants_platform_prompt", None)
    ...
    return {
        ...
        "model": model or None,
        "prompt": ants_platform_prompt,
    }


def _create_ants_platform_update(
    completion: Any,
...
def _get_ants_platform_data_from_default_response(
...
@_ants_platform_wrapper
def _wrap(
    open_ai_resource: OpenAiDefinition, wrapped: Any, args: Any, kwargs: Any
) -> Any:
    arg_extractor = OpenAiArgsExtractor(*args, **kwargs)
    ants_platform_args = arg_extractor.get_ants_platform_args()

    ants_platform_data = _get_ants_platform_data_from_kwargs(open_ai_resource, ants_platform_args)
    ants_platform_client = get_client(public_key=ants_platform_args["ants_platform_public_key"])

    generation = ants_platform_client.start_observation(
        as_type="generation",
        name=ants_platform_data["name"],
        input=ants_platform_data.get("input", None),
        metadata=ants_platform_data.get("metadata", None),
        model_parameters=ants_platform_data.get("model_parameters", None),
        trace_context={
            "trace_id": cast(str, ants_platform_data.get("trace_id", None)),
            "parent_span_id": cast(
                str, ants_platform_data.get("parent_observation_id", None)
            ),
        },
        model=ants_platform_data.get("model", None),
        prompt=ants_platform_data.get("prompt", None),
    )
    ...
    try:
        ...
        if _is_streaming_response(openai_response):
            ...
            model, completion, usage = _get_ants_platform_data_from_default_response(
                open_ai_resource,
