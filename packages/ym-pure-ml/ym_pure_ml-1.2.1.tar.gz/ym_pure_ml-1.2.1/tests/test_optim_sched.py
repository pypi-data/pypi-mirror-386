    ...
    def test_other_opts_reduce_loss_over_few_steps(self):
        for Opt, lr in [(AdaGrad, 0.3), (RMSProp, 0.01), (Adam, 0.01)]:
            with self.subTest(optimizer=Opt.__name__):
                mdl = TinyModel(n=6, m=3, seed=2)
                opt = Opt(mdl.params, lr=lr)
                L0 = float(mdl.loss(B=64).data)
                # do 5 steps with new grads each time
                for _ in range(5):
                    L = mdl.loss(B=64)
                    L.backward()
                    opt.step()
                Lf = float(mdl.loss(B=64).data)
                self.assertLess(Lf, L0, f"{Opt.__name__} failed to reduce loss: {Lf} !< {L0}")
