...
class EasyGUI:
    """
    EasyGUI — Beginner-friendly Python GUI wrapper.
    Supports DearPyGui v2.x or Tkinter fallback.
    Features:
      - Tags for widgets
      - Buttons, labels, sliders, checkboxes, entries
      - Callbacks with static args or dynamic args_tags
      - Fullscreen toggle (DearPyGui only)
      - QOL helpers: get_value / set_value
      - Optional font_size (ignored in DearPyGui default)
    """

    def __init__(self, title="EasyGUI", size=(600,400), bg_color=None, fullscreen_window=False):
        ...
        if GUI_BACKEND == "dpg":
            ...
            self._main_window = dpg.add_window(label=title)
    ...
    def set_value(self, tag, value):
        if GUI_BACKEND == "dpg":
            ...
            if isinstance(w, tk.Entry):
                w.delete(0, "end")
                w.insert(0, value)
            elif isinstance(w, tk.BooleanVar) or isinstance(w, tk.IntVar):
                w.set(value)
