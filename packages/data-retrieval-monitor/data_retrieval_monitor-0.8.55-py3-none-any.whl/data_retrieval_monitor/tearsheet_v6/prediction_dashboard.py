...
class PredictionDashboard:
    ...
    def __init__(
        self,
        *,
        preds_lf: pl.LazyFrame | pl.DataFrame,    # [date, factor1, factor2, ...]
        target_lf: pl.LazyFrame | pl.DataFrame,   # [date, factor1, factor2, ...] (targets)
        title: str = "Prediction Tearsheet",
        output_dir: str = "output/prediction",
        manifest: Optional[PredictionManifest] = None,
        ...
        self.manifest = manifest or PredictionManifest()
        ...
        if self.source_obj is not None and "preds" in data_overrides:
            preds_obj = self._resolve_data_spec(data_overrides["preds"], self.source_obj)
        ...
        if self.source_obj is not None and "target" in data_overrides:
            target_obj = self._resolve_data_spec(data_overrides["target"], self.source_obj)
    ...
    def _render_heatmap_tile(self, df: pl.DataFrame, value_col: str, factor: str, title_prefix: str, cmap: str) -> Optional[str]:
        ...
        ax.set_yticklabels([str(int(r)) for r in pivot.index], fontsize=8)
        ax.set_ylabel('Lag')
        ax.set_title(f"{title_prefix} — {factor}", fontsize=12)
        fig.tight_layout()
