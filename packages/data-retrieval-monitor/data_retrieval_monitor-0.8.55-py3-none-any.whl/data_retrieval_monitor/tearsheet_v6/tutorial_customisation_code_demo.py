...
def residual_quartiles(residual_lf: pl.LazyFrame, dashboard: QuantStatsDashboard) -> str:
...
def build_active_block(_, dashboard: QuantStatsDashboard) -> pd.DataFrame:
...
def replace_risk_block(_, dashboard: QuantStatsDashboard) -> pd.DataFrame:
...
def run_customisation_demo(output_dir: str = "output/tutorial_customisation_suite_v2") -> Dict[str, str]:
    ...
    custom_dash = QuantStatsDashboard(
        returns_df=None,
        benchmark=None,
        manifest=manifest,
    ...
    paths = build_dashboards(
        ...
        custom_backtest_figures=[monthly_heatmap_tiles],
        custom_prediction_tables=[residual_quartiles],
        output_dir=os.path.join(output_dir, "combined"),
    )

    result = {
        "custom_backtest_html": custom_dash.html_path,
        "combined_index_html": paths["index"],
    }
    return result
