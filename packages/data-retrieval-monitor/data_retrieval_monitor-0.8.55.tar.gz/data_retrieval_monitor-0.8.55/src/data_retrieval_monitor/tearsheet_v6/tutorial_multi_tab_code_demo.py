...
tabbed UI using `multi_dashboard.TabbedDashboard`.  It builds three tabs:
    ...
    1. Standard backtest (QuantStats) dashboard
...
def monthly_heatmap_tiles(returns_lf: pl.LazyFrame, dashboard: QuantStatsDashboard):
...
def run_multi_tab_demo(output_dir: str = "output/multi_tab_demo") -> str:
    ...
    standard_backtest = QuantStatsDashboard(
    ...
    pred_manifest = PredictionManifest()
    standard_prediction = PredictionDashboard(
    ...
    custom_manifest = DashboardManifest(
    ...
    custom_backtest = QuantStatsDashboard(
        returns_df=None,
        benchmark=None,
    ...
    TabbedDashboard(
        tabs=[
            TabSpec(title="Backtest", file_path=standard_backtest.html_path),
            TabSpec(title="Prediction", file_path=standard_prediction.html_path),
            TabSpec(title="Custom Backtest", file_path=custom_backtest.html_path),
        ],
