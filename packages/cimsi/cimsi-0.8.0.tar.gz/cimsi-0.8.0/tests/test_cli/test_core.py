...
@pytest.mark.xfail(raises=InvalidSetupConfig)
def test_setup_invalid_runid(config_params, runner, local_test_dir):
    cwd_before = os.getcwd()
    ...
    try:
        result = runner.invoke(
            ...
            ["-f", "setup", f"--runid={bad_runid}",
             f"--repo={target_repo}", f"--model={target_model}", f"--exp={target_exp}"]
             )
    finally:
        os.chdir(cwd_before)
