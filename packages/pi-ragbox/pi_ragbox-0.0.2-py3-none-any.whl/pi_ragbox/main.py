...
@app.command()
def set(
    key: str = typer.Argument(..., help="Configuration key to set (e.g., 'ragbox_repo')"),
    ...
    Common options:
      ragbox_repo - Path to the ragbox repository root directory

    Example:
        pi-ragbox set ragbox_repo /Users/zach/repos/ragbox
    """
    try:
        # Validate and process the value for known keys
        final_value = value

        if key == "ragbox_repo":
            # Expand user home directory (~) and make absolute
            path = Path(value).expanduser().resolve()
            final_value = str(path)

            if not path.exists():
                rprint(f"[bold yellow]⚠[/bold yellow] Warning: Path does not exist: {final_value}")
                confirm = typer.confirm("Do you want to set it anyway?", default=False)
                if not confirm:
                    rprint("[dim]Cancelled.[/dim]")
                    raise typer.Exit(code=0)

        set_config_option(key, final_value)
        rprint(f"[bold green]✓[/bold green] Set [cyan]{key}[/cyan] = [bold]{final_value}[/bold]")
...
@app.command()
def get(
    key: str = typer.Argument(..., help="Configuration key to retrieve (e.g., 'ragbox_repo')")
):
    """
    Get a configuration option value.

    Example:
        pi-ragbox get ragbox_repo
    """
...
def init(
    ...
    """
    # Get ragbox_repo from config
    ragbox_repo = get_config_option("ragbox_repo")
    if not ragbox_repo:
        rprint("[bold red]✗[/bold red] ragbox_repo is not configured")
        rprint("\n[dim]Please set the ragbox_repo configuration option:[/dim]")
        rprint("[dim]  pi-ragbox set ragbox_repo /path/to/ragbox[/dim]")
        raise typer.Exit(code=1)
    ...
    # Validate ragbox_repo and source files
    repo_path = Path(ragbox_repo)
    source_dir = repo_path / "backend" / "search-service" / "src" / "search_service" / "builtin_flows" / "pi_flows"

    if not source_dir.exists():
        rprint(f"[bold red]✗[/bold red] Source directory not found: {source_dir}")
        rprint("\n[dim]Please verify your ragbox_repo is set correctly.[/dim]")
        raise typer.Exit(code=1)

    # Files to copy
    files_to_copy = ["__init__.py", "search_simple.py"]
...
def serve(
    ...
    and custom Python flows directory. Requires RAGBOX_REPO environment variable
    to be set to the root directory of the ragbox repository.
    ...
    # Check for ragbox_repo in config
    ragbox_repo = get_config_option("ragbox_repo")
    if not ragbox_repo:
        rprint("[bold red]✗[/bold red] ragbox_repo is not configured")
        rprint("\n[dim]Please set the ragbox_repo configuration option:[/dim]")
    ...
    if not repo_path.exists():
        rprint(f"[bold red]✗[/bold red] RAGBOX_REPO path does not exist: {ragbox_repo}")
        raise typer.Exit(code=1)

    # Build the working directory path
    working_dir = repo_path / "backend" / "search-service"
    if not working_dir.exists():
        rprint(f"[bold red]✗[/bold red] Search service directory not found: {working_dir}")
        rprint("\n[dim]Expected to find backend/search-service in RAGBOX_REPO[/dim]")
        raise typer.Exit(code=1)
    ...
    # Build the command
    command = ["uv", "run", "modal", "serve", "--env=ragbox", "-m", "search_service.modal_main"]
