...
def highest_bin_center(values, bins=100, value_range=None):
    ...
    if value_range is None:
        ...
        vmin, vmax = value_range
    counts, edges = np.histogram(vals, bins=bins, range=(vmin, vmax), density=False)
    if counts.size == 0 or np.all(counts == 0): return float("nan")
