...
from PySide6.QtCore import QRect, QMargins, QPointF
...
def get_line_svg_text(
    ...
    left, top, width, height = get_rect_from_points(p1, p2)
    points_bounding_area = QRect(left, top, width, height)

    ### offset it to origin, moving the points along with it by the
    ### same amount

    points_bounding_offset = -QPointF(points_bounding_area.topLeft())

    points_bounding_area.translate(
        points_bounding_offset.x(),
        points_bounding_offset.y(),
    )

    v1 = QPointF(*p1) + points_bounding_offset
    v2 = QPointF(*p2) + points_bounding_offset
    ...
    stroke_bounding_area = points_bounding_area.marginsAdded(margins)
    ...
    stroke_bounding_offset = -QPointF(stroke_bounding_area.topLeft())

    stroke_bounding_area.translate(
        stroke_bounding_offset.x(),
        stroke_bounding_offset.y(),
    )
    ...
    width, height = stroke_bounding_area.size().toTuple()
...
def get_pie_chart_svg_text(
    ...
    if remaining_sum:
        ...
        offset = QPointF(cx, cy)

        circle_origin = arc_start = (
             QPointF(fill_radius, 0).rotate(start_degrees)
        )
