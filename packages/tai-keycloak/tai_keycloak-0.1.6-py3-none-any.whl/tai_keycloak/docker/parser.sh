...
main() {
    ...
    # Verificar qué variables están disponibles (con defaults para evitar errores)
    if [[ -n "${MAIN_DATABASE_URL:-}" ]]; then
        has_database_url=true
        log_info "MAIN_DATABASE_URL encontrada"
    else
        log_info "MAIN_DATABASE_URL no encontrada"
    fi
    ...
    if [[ -n "${MAIN_KEYCLOAK_URL:-}" ]]; then
        ...
        log_info "MAIN_KEYCLOAK_URL no encontrada"
    fi
    ...
    if [[ "$has_database_url" == true ]]; then
        ...
        if ! parse_database_url "$MAIN_DATABASE_URL" "$OUTPUT_FILE"; then
            ...
            # Crear archivo básico en lugar de fallar
            cat > "$OUTPUT_FILE" << EOF
# Configuración para Keycloak
# Generado automáticamente (modo desarrollo)
EOF
        fi
    else
        # Crear archivo con encabezado si solo tenemos MAIN_KEYCLOAK_URL o ninguna
        cat > "$OUTPUT_FILE" << EOF
# Configuración para Keycloak
# Generado automáticamente (modo desarrollo)
EOF
    ...
    if [[ "$has_keycloak_url" == true ]]; then
        ...
        log_info "👤 Usando credenciales por defecto"
        append_keycloak_admin_vars "$OUTPUT_FILE" "false"
    fi
