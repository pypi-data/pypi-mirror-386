...
class LockIdentifier(sila.Metadatum, errors=[InvalidLockIdentifier]):
    ...
    def intercept(self) -> None:
        if not isinstance(self._feature, LockController):
            raise RuntimeError

        if self._feature.lock is None or self._feature.lock.identifier != self.lock_identifier:
            raise InvalidLockIdentifier

        self._feature.lock.last_usage = sila.datetime.datetime.now()
...
class LockController(sila.Feature):
    ...
    def lock_server(
        ...
        .. parameter:: The time (in seconds) after a SiLA Server is automatically unlocked when no request with a valid lock identifier
