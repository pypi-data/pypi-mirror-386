...
class LoggerManager:
    ...
    Logging modes:
        The `LoggerManager` supports two logging modes:

        1. Text: Logs in a human-readable format. Used when the `LOG_FORMAT` environment variable is set to "text".
            Output example:
            ```log
        ...
        2. JSON: Logs in a JSON format, recommended for easy parsing due to the structured nature of the log records.
