...
class Cphd03Dataset(BaseSimpleSarDataset):
    ...
    def __init__(self, file_address: str, channel_index: int = 0, c_eff: float = c,
        ...
        self._num_slow_times = self.reader.cphd_meta.Data.ArraySize[self._channel_index].NumVectors
        if self.transmitter_positions.shape[0] < self._num_slow_times:
            self._num_slow_times = self.transmitter_positions.shape[0]
        self._num_freqs = self.reader.cphd_meta.Data.ArraySize[self._channel_index].NumSamples
        sign = self.reader.cphd_meta.Global.PhaseSGN
        if sign > 0:
    ...
    def centre_frequency(self) -> float:
        ...
        """
        f0 = self.reader.read_pvp_variable("Fx0", self._channel_index, 0)[0]
        delta_f = self.reader.read_pvp_variable("Fx_SS", self._channel_index, 0)[0]
    ...
    # Override of pickle state getter, as CPHDTypeReader is not hashable (for obvious reasons). This object
    # needs to be pickable for it to be used in parallel operations by joblib.
    def __getstate__(self):
        state = self.__dict__
        state["_reader"] = None
        return state

    # Override of pickle state setter, which summons a new reader when loading from pickle.
