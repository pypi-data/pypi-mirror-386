...
class AzureDevOpsGitTool(CodeMieTool):
    ...
    def _make_request(self, method: str, url: str, headers: dict[str, str],
                      method_arguments: Union[dict, list]) -> requests.Response:
        """
    ...
    def _process_response(self, response: requests.Response, method: str, url: str) -> AzureDevOpsGitOutput:
        ...
        try:
            ...
            logger.debug(f"HTTP: {method} {url} -> {response.status_code} {response.reason} {json.dumps(response_body, indent=2)}")
    ...
    def execute(self, query: Union[str, dict[str, Any]]) -> AzureDevOpsGitOutput:
        """
        ...
        if not self.config.token or not self.config.url or not self.config.organization:
            raise ValueError(
                "Azure DevOps Git config is not set. Please provide url, organization, and token before using the tool."
            )
    
        try:
            # Parse and validate query
            parsed_query = self._parse_query(query)
            method, relative_url, method_arguments, custom_headers = self._validate_and_extract_query_params(parsed_query)
