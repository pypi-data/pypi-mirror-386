...
def get_codemie_security_policy(
    severity_threshold: SecurityIssueSeverity = SecurityIssueSeverity.LOW
) -> SecurityPolicy:
    """
    ...
    - HTTP write operations (POST, PUT, DELETE, PATCH via requests)
    ...
    This policy allows with low severity:
    - HTTP read operations (GET via requests library)

    Args:
        severity_threshold: Minimum severity level to enforce.
                          SAFE (0): blocks nothing (most permissive).
                          LOW (1): allows read operations like requests.get() (default).
    ...
    policy = SecurityPolicy(
        ...
        restricted_modules=[
            ...
            RestrictedModule(
                name="requests",
                description="HTTP library - read operations (GET) allowed with low severity",
