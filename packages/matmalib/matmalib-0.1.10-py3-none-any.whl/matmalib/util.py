...
class StateBuilder:
    ...
    def gaussian(self):
        ...
        re_normal_term = re.compile(r'Normal termination')
        re_scf_done = re.compile(r'SCF Done')
        re_e_zpe = re.compile(r'Sum of electronic and zero-point Energies')
        re_enthalpy = re.compile(r'Sum of electronic and thermal Enthalpies')
        re_free_e = re.compile(r'Sum of electronic and thermal Free Energies')
        re_std_orient = re.compile(r'Standard orientation:')
        re_coords_header = re.compile(r'Coordinates')
        re_coord_line = re.compile(r'^\s*\d+\s+\d+\s+\d+\s+[+-]?\d+\.\d{6}\s+[+-]?\d+\.\d{6}\s+[+-]?\d+\.\d{6}\s*$')
        re_deg_freedom = re.compile(r' Deg. of freedom')
        re_freq = re.compile(r'^ Frequencies')
        re_ir_inten = re.compile(r'^ IR Inten')
        re_normal_mode_row = re.compile(r'^\s*\d+\s+\d+(?:\s+[+-]?\d+\.\d+){3,}\s*$')
        re_opt_completed = re.compile(r'Optimization completed\.')
        re_total_j = re.compile(r'Total nuclear spin-spin coupling J')
        re_j_values = re.compile(r'-?\d\.\d+[Dd][+\-]\d\d?')
        re_end_minotr = re.compile(r'End of Minotr F.D. properties file')
        ...
        for line in open(self.path, 'r').readlines():
            ...
            if job_type == 'optfreq' or job_type == "freq":
                ...
                if flags['freq_flag'] == False and re_normal_term.search(line): flags['freq_flag'] = True
                # We skip the opt part of optfreq job, all info is in the freq part

                if flags['freq_flag'] == True:

                    if re_scf_done.search(line):
                        self.E = float(line.split()[4])
                    elif re_e_zpe.search(line):
                        self.Ezpe = float(line.split()[6])
                    elif re_enthalpy.search(line):
                        self.H = float(line.split()[6])
                    elif re_free_e.search(line):
                        self.F = float(line.split()[7])

                    elif re_std_orient.search(line):
                        final_geom = True

                    elif re_coords_header.search(line) and len(geom) == 0:
                ...
                if re_scf_done.search(line): E = float(line.split()[4])
                if re_opt_completed.search(line):
                    self.E = E;
                    flags['opt_flag'] = True
                if flags['opt_flag'] == True:
                    if re_std_orient.search(line):
                        flags['read_geom'] = True
                        final_geom = True
                ...
                elif flags['jcoup_flag'] == True and re_end_minotr.search(line):
