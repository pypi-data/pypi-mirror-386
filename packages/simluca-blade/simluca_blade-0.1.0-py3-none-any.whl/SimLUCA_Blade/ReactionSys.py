...
class ReactionSys:
    ...
    def _prepare(self):
        ...
        _fill_diffusion_jac_data(self._n_tri, self._n_bor,
                                 ...
                                 self._mesh_diffusion, self._tri_neighbors, self._tri_neighbors_mask,
                                 self._border_diffusion, self._bor_neighbors, self._bor_neighbors_mask,
    ...
    def _compile_rate_plans_numba(self):
        ...
        for rxn in self.reactions:
            ...
            # Detect whether any membrane species are involved
            mem_involved = False
            for mono in rr.PolyTerms.keys():
                if mono == frozenset([1]):
                    continue
                for var in mono:
...
def _rhs_impl(y, dy,
              n_tri, n_bor,
              cyt_off, mem_off,
              mesh_diff, tri_nb, tri_mask,
              bor_diff, bor_nb, bor_mask,
              adj_tri, to_mem,
              plans_cyt, plans_mem):
    """
        ...
        for i in range(n_tri):
            ...
            for n_idx in range(3):
                if tri_mask[i, n_idx]:  # valid neighbor
                    ...
                    accum += mesh_diff[i, n_idx] * (y[base + neighbor] - y[base + i])
            dy[base + i] += accum
        ...
        for i in range(n_bor):
            ...
            for n_idx in range(2):
                if bor_mask[i, n_idx]:  # valid neighbor
                    ...
                    accum += bor_diff[i, n_idx] * (y[base + neighbor] - y[base + i])
            dy[base + i] += accum
