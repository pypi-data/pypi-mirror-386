...
class TestFilterSimilarHD(unittest.TestCase):
    ...
    def test_hamming_dist_kwds_are_passed(self):
        """
        Test ignore keyword.
        """
        sequences = "abc", "ade"
...
class TestPairwiseHammingDists(unittest.TestCase):
    def test_list_input_returns_list(self):
        collection = ["DAVID", "PAVID", "DAVID"]
        result = ere.pairwise_hamming_dists(collection)
        self.assertIsInstance(result, list)
        self.assertEqual(3, len(result))
    ...
    def test_dict_input_returns_dict(self):
        sequences = {"seq1": "DAVID", "seq2": "PAVID", "seq3": "DAVID"}
        result = ere.pairwise_hamming_dists(sequences)
        self.assertIsInstance(result, dict)
        self.assertEqual(1, result["seq1"]["seq2"])
        self.assertEqual(0, result["seq1"]["seq3"])
        self.assertEqual(1, result["seq2"]["seq3"])

    def test_with_ignore_parameter(self):
        collection = ["DAVID", "PAVID", "DAVID"]
        result = ere.pairwise_hamming_dists(collection, ignore="P")
        self.assertEqual([0, 0, 0], result)

    def test_case_sensitive_parameter_true(self):
        collection = ["DAVID", "david", "DAVID"]
        result_sensitive = ere.pairwise_hamming_dists(collection, case_sensitive=True)
        self.assertEqual([5, 0, 5], result_sensitive)

    def test_case_sensitive_parameter_false(self):
        collection = ["DAVID", "david", "DAVID"]
        result_insensitive = ere.pairwise_hamming_dists(
            collection, case_sensitive=False
        )
        self.assertEqual([0, 0, 0], result_insensitive)

    def test_per_site_parameter(self):
        collection = ["DAVID", "PAVID", "DAVID"]
        result = ere.pairwise_hamming_dists(collection, per_site=True)
        self.assertEqual([0.2, 0, 0.2], result)

    def test_invalid_input_type_raises_error(self):
        with self.assertRaises(ValueError):
            ere.pairwise_hamming_dists("DAVID")
