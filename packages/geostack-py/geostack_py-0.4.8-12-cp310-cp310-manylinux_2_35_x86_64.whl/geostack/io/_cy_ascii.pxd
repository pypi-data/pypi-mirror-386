...
cdef extern from "gs_ascii.h" namespace "Geostack":
    cdef cppclass AsciiHandler[R, C](RasterFileHandler[R, C]):
        AsciiHandler() except +
...
cdef class cyAscii_d_d:
    cdef unique_ptr[AsciiHandler[double, double]] thisptr
...
cdef class cyAscii_f_f:
    cdef unique_ptr[AsciiHandler[float, float]] thisptr
...
cdef class cyAscii_d_byt:
    cdef unique_ptr[AsciiHandler[uint8_t, double]] thisptr
...
cdef class cyAscii_f_byt:
    cdef unique_ptr[AsciiHandler[uint8_t, float]] thisptr
...
cdef class cyAscii_d_i:
    cdef unique_ptr[AsciiHandler[uint32_t, double]] thisptr
...
cdef class cyAscii_f_i:
    cdef unique_ptr[AsciiHandler[uint32_t, float]] thisptr
