...
class Byneuron:
    ...
    def get_numberevent_keys(self, keys, indexset=None):
        ...
        data = self._numberevents(indexsets=indexset, filters=[f], size=1, deleted=False)
        if isinstance(data, list):
            if len(data) != len(keys):
                data_deleted = self._numberevents(indexsets=indexset, filters=[f], size=1, deleted=True)
                if isinstance(data_deleted, list):
                    data.extend(data_deleted)
            return [Numberevent(datamodel=i) for i in data]
