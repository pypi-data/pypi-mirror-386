...
def init(path, dry_run, create_repo, interactive, backend_heavy):
    ...
    if already_initialized:
        ...
        # Initialize MCP registry if it doesn't exist (even on re-init)
        import importlib_resources
        templates_root = importlib_resources.files("multiagent_core") / "templates"
        _initialize_mcp_registry(templates_root, console)
    ...
    # Bashrc organization DISABLED - script needs fixes
...
def _initialize_mcp_registry(templates_root, console):
    """Initialize the global MCP servers registry if it doesn't exist.

    Creates ~/.multiagent/config/mcp-servers-registry.json from template.
    This registry is the central source of truth for all available MCP servers.
    Projects can pull from this registry using /mcp:manage commands.
    """
    import importlib_resources

    # Target location for MCP registry
    config_dir = Path.home() / ".multiagent" / "config"
    registry_file = config_dir / "mcp-servers-registry.json"

    # Create config directory if it doesn't exist
    config_dir.mkdir(parents=True, exist_ok=True)

    # Skip if registry already exists (preserve user customizations)
    if registry_file.exists():
        console.print("[dim]Skipped existing MCP registry (preserving customizations)[/dim]")
        return

    # Deploy template registry
    try:
        template_path = templates_root.joinpath('mcp-servers-registry.json')
        with importlib_resources.as_file(template_path) as template_src:
            if template_src.exists():
                shutil.copy(template_src, registry_file)
                console.print(f"[green]Initialized MCP registry: {registry_file}[/green]")
                console.print("[dim]   Registry contains standard MCP servers (github, postman, memory, playwright, etc.)[/dim]")
                console.print("[dim]   Add custom servers using /mcp:manage registry add[/dim]")
            else:
                console.print("[yellow]Warning: MCP registry template not found[/yellow]")
    except Exception as e:
        console.print(f"[yellow]Warning: Could not initialize MCP registry: {e}[/yellow]")
...
def _generate_project_structure(cwd, backend_heavy=False, interactive=True):
    ...
    console.print(" Initializing MCP registry...")
    _initialize_mcp_registry(templates_root, console)
