...
def bashrc_organize(dry_run, force):
    """[DISABLED] Organize ~/.bashrc into clean three-tier structure.

    THIS COMMAND IS TEMPORARILY DISABLED DUE TO BUGS.

    The bashrc organizer script has issues that can damage your bashrc.
    We're working on fixes. Please do NOT use this command.

    If you accidentally ran this and it broke your bashrc, restore from backup:
      cp ~/.bashrc.backup-YYYYMMDD-HHMMSS ~/.bashrc
      source ~/.bashrc
    """
    console.print("[bold red]⚠️  COMMAND DISABLED[/bold red]")
    console.print("")
    console.print("[yellow]The bashrc-organize command has been temporarily disabled")
    console.print("due to bugs that can damage your ~/.bashrc file.[/yellow]")
    console.print("")
    console.print("If your bashrc was damaged, restore from backup:")
    console.print("  1. List backups: ls -lt ~/.bashrc.backup-*")
    console.print("  2. Restore: cp ~/.bashrc.backup-YYYYMMDD-HHMMSS ~/.bashrc")
    console.print("  3. Reload: source ~/.bashrc")
    console.print("")
    console.print("[dim]We're working on fixes and will re-enable once safe.[/dim]")
    return
