...
from bear_dereth.operations import increment
...
def test_table_update_with_callable(tmp_path: Path):
    """Test updating records with a callable function."""
    with BearBase(file=tmp_path / "test.json", storage="json") as db:
        db.create_table(
            "counters",
            columns=[
        ...
        q = query("mapping")()
        updated_count: int = table.update(increment("count"), cond=q.name == "views")
        assert updated_count == 1

        views: Records = table.get(name="views")
        assert views is not None
        assert views.first()["count"] == 11

        likes: Records = table.get(name="likes")
        assert likes is not None
        assert likes.first()["count"] == 5  # Unchanged
...
def test_table_update_all_records(tmp_path: Path):
    ...
    assert all(rec["archived"] for rec in table.all())
...
def test_update_callable_with_complex_logic(tmp_path: Path) -> None:
    """Test callable updates with more complex logic."""
    with BearBase(file=tmp_path / "test.json", storage="json") as db:
        db.create_table(
            "posts",
            columns=[
        ...
        q = query("mapping")()
        updated: int = table.update(boost_popular, cond=q.views > 20)
        assert updated == 1
        post2: Record = table.get(id=2).first()
        assert isinstance(post2, Record)
        assert post2["likes"] == 20
        assert post2["title"] == "⭐ World"

        post1: Record = table.get(id=1).first()
        assert isinstance(post1, Record)
        assert post1["likes"] == 2  # Unchanged
        assert post1["title"] == "Hello"  # Unchanged
